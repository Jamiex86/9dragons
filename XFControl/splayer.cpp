...
SPlayer::SPlayer()
{
	...
	// View Control
	scaleMode = DisplayList::showAll | DisplayList::exactFit;
...
void SPlayer::Zoom100()
{
	if ( scaleMode & DisplayList::zoom100Scroll ) {
		...
		RectSetEmpty(&zoomRect);
		scaleMode = DisplayList::noScale | DisplayList::exactFit;
	}
	SetCamera(updateLazy);
}
				...
				if ( skipCount > 0 ) {
					...
					switch ( actionCode ) {
						...
						case sactionSetTarget: {
							...
							if ( *targetName ) {
								//strcpy(m_strtargetname, targetName);
								m_strtargetname = (char*)targetName;
								// Set a new target
...
{
	{
		...
		int i = 0 ; for ( i=0; i<iNumButtons; i++) {
...
void SPlayer::UpdateFullTexture( void )
{
	NativePlayerWnd* native = (NativePlayerWnd*)( this );
	bits.updateFullTexture( native );
}
