...
void CFlashControlInterfaceImpl::initialize( HWND hwnd, HINSTANCE hinst, LPDIRECT3DTEXTURE9 pOffScreenTexture )
{
	m_ctrPlayer.initialize(hwnd, hinst, pOffScreenTexture);
}

void CFlashControlInterfaceImpl::SetOffScreenTextureObject( LPDIRECT3DTEXTURE9	pOffScreenTexture )
{
	m_ctrPlayer.SetOffScreenTextureObject( pOffScreenTexture );
	m_ctrPlayer.UpdateFullTexture();	
}

void CFlashControlInterfaceImpl::SetResourceFilePtr( FILE* pResourcePointer, long iResourceSize )
{
	m_ctrPlayer.SetResourceFilePtr( pResourcePointer, iResourceSize );
}
...
void CFlashControlInterfaceImpl::UpdateFullTexture() 
{
	m_ctrPlayer.UpdateFullTexture();
}
...
__declspec(dllexport) CFlashControlInterface* GetFlashControl()
{ 
...
BOOL CFlashControlInterfaceImpl::IsPlaying()
{
	return m_ctrPlayer.IsPlaying();
}
