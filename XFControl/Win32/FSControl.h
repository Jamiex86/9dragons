...
class NativePlayerWnd : public SPlayer 
{
  public:	
	...
	void initialize( HWND hwnd, HINSTANCE hinst, LPDIRECT3DTEXTURE9 pOffScreenTexture );
...
public:
	...
	void SetOffScreenTextureObject( LPDIRECT3DTEXTURE9	pOffScreenTexture )
	{
		m_pOffScreenTexture = pOffScreenTexture;
	}

	void SetResourceFilePtr( FILE* pResourcePointer, long iResourceSize )
	{
		m_pResourcePointer = pResourcePointer;
		m_iResourceSize = iResourceSize;
	}
	...
	FILE*				m_pResourcePointer;
	long				m_iResourceSize;
};

DWORD WINAPI StreamThread(  LPVOID lpParameter );

#define MSG_STREAM_NEW     (WM_USER+2072)
#define MSG_STREAM_WRITE   (WM_USER+2073)
#define MSG_STREAM_DESTROY (WM_USER+2074)
