...
void CPackingView::setItem( int SectionIndex )
{
	...
	int i = 0 ; for(  i = 0; i < selectedsectionlist->m_itemcount; i++ )
	{
		tfileitem = NULL;
...
void CPackingView::OnInsertfilesmenuitem() 
{
	...
	{		 
		...
		int i = 0 ; for(  i = 0; i<BFlistIndex; i++ )
		{
			int j = 0 ; for( j = 0; j<BFlistIndex-1; j++ )
			{
...
BOOL CPackingView::savePackStructFile( LPCSTR filename )
{
	...
	int i = 0 ; for(  i = 0; i < packfilecount; i++ )
	{
		if( childItem != NULL )
		{
			...
			if( sectioncount > 0 )
			{
				...
				int j = 0 ; for( j = 0; j < sectioncount; j++ )	
				{
...
BOOL CPackingView::loadXPP(LPCSTR filename)
{
	...
	int i = 0 ; for(  i = 0; i < packfilecount; i ++ )
	{
		...
		for( int scount = 0; scount < sectioncount; scount++ )
		{
			...
			int icount = 0 ; for( icount  = 0; icount <  sectionitemcount; icount++ )
			{
				loadStringandSize( treeitemstring, _MAX_PATH, fp );
...
{	
	...
	int i = 0 ; for(  i = StartPackageIndex; i < EndPackageIndex; i++ )
	{
	...
	for( i = 0; i < packagecount; i++ )
	{
		...
		{		
			...
			if( sectioncount > 0 )
			{
				...
				int j = 0 ; for( j = 0; j < sectioncount && checkData; j++ )
				{
...
void CPackingView::OnSortfilelist() 
{
	...
	int i = 0 ; for(  i = 0; i < selectedsectionlist->m_itemcount; i++ )
	{
		...
		int j = 0 ; for( j = 0; j < selectedsectionlist->m_itemcount-1; j++ )
		{
			tfileitem = NULL;
...
{	
	...
	int i = 0 ; for(  i = strlen(pathstring)-1; i > 0; i-- )
	{
		if(  pathstring[i] == '\\' )
		{
			pathstring[i] = NULL;
