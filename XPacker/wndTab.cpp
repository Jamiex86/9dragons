...
void CwndTab::DrawSelectedTab( CDC* pDC )
{
   ...
   {
	   UINT uCounter = 0 ; for( uCounter = 0; uCounter < m_uSelectedViewIndex; uCounter++ )
	   {
...
void CwndTab::UpdateTabWidth( CDC* pDC )
{
   ...
   int iCounter = 0 ; for( iCounter  = 0; iCounter < m_sarrayViewName.GetSize(); iCounter++ )
   {
      // get text size
...
void CwndTab::OnLButtonDown(UINT nFlags, CPoint point) 
{
   ...
   UINT uCounter = 0 ; for( uCounter = 0; uCounter < (UINT)m_dwarrayTabWidth.GetSize(); uCounter++ )
   {
      iOffsetToLeft += uDistanceToNextTab;
