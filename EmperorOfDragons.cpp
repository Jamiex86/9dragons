...
BOOL CEmperorOfDragonsApp::CheckData( void )
{
	...
	else
	{
		...
		int idx = 0 ; for(  idx = 0; idx < 63; ++idx )
		{
...
//#endif
	...
	int i = 0 ; for(  i = 16; i < _COUNT_MAX_GROUP_; ++i )
	...
	// 한글
	int i = 0 ; for(; i < _XDEF_TOTALLEVELNAME; ++i ) memset( g_LevelName[i], 0, sizeof(TCHAR) * 16 );
...
{	
	...
	if( g_UseHardwareCursor )	
	{
		...
		int i = 0 ; for(; i < mouseimagearchive.GetResourceCount(); ++i )
		{
	...
	case AHNHS_ENGINE_DETECT_GENERAL_HACK:
		{
			...
			sprintf(szMsg, _XGETINTERFACETEXT(ID_STRING_EMPEROROFDRAGONS_2597), szFilename );
...
int CALLBACK NPGameMonCallback(DWORD dwMsg, DWORD dwArg)
{
	...
	case NPGAMEMON_COMM_ERROR:
		sprintf(msg, _T("Communication error"), dwArg);
		lpszMsg = msg;
		bAppExit = true;
		break;
	case NPGAMEMON_COMM_CLOSE:
		sprintf(msg, _T("Communication closing"), dwArg);
		lpszMsg = msg;
		bAppExit = true;
		break;
	case NPGAMEMON_INIT_ERROR:
		bAppExit = true; // 종료 코드
		sprintf(msg, _XGETINTERFACETEXT(ID_STRING_EMPEROROFDRAGONS_2592), dwArg);
		lpszMsg = msg;
		break;
	...
	case NPGAMEMON_CHECK_CSAUTH2:
		...
		memcpy(&g_GameGuardData.data, (PVOID)dwArg, sizeof(GG_AUTH_DATA));
