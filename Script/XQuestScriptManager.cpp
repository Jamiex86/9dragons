...
BOOL _XQuestScriptManager::LoadQuestScript(FILE* fileptr)
{
	...
	int i = 0 ; for( ; i < m_nQuestScriptCount ; ++i)
	{
									...
									{	
										for( i = 0; i < 8; ++i )
										{
											if( currentquest->m_ItemCounterInformation[i].CounterTargetType == -1 )
											{
												...
												memset(currentquest->m_ItemCounterInformation[i].CounterTargetName, 0, sizeof(TCHAR)*256);
												strcpy(currentquest->m_ItemCounterInformation[i].CounterTargetName, _XGameItem::GetItemName(cType, sID) );
								...
								else
								{
									...
									if( currentaction->m_lpszMessageString )
									{
										...
										if(strlength > 0)
										{
											...
											strcpy( currentquest->m_CounterInformation[countertype].CounterTargetName, currentaction->m_lpszMessageString );
										}		
										else
										{
		...
		int itemcounterindex = 0 ; for(  itemcounterindex = 0; itemcounterindex < 8; ++itemcounterindex )
		{
			if( currentquest->m_ItemCounterInformation[itemcounterindex].CounterTargetType != -1 )
			{
					...
					&& currentquest->m_ItemCounterInformation[itemcounterindex].CounterTargetID == itemid )
				{
					...
					{
						if(g_pLocalUser->m_UserItemList[i].m_cType == itemtype
