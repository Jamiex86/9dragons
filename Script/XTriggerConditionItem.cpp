...
void _XTriggerConditionItem::ResetData(void)
{
	...
	memset(m_szConditionString, 0, sizeof(TCHAR) * 64);
...
void _XTriggerConditionItem::SetCompareString(LPCTSTR comparestring)
{
	...
	if(strlength > 0)
	{
		...
		strcpy(m_lpszCompareString, comparestring);
	}
#endif
}
...
void _XTriggerConditionItem::SetConditionString(LPCTSTR conditionstring)
{
	...
	if(strlength < 64)
	{
		strcpy(m_szConditionString, conditionstring);
	}
	else
	{
...
_XQUESTCONDITION_RESULT _XTriggerConditionItem::ProcessCheckMessage(void)
{
	...
	{
		memset( queststring, 0, sizeof(TCHAR)*1024 );
		memset( comparestring, 0, sizeof(TCHAR)*1024 );
		memset( wordstring, 0, sizeof(TCHAR)*ID_MAX_CHECKMESSAGE*1024 );
		strcpy( queststring, pNPCScript_Window->m_QuestString );
		strcpy( comparestring, m_lpszCompareString );
		
		int i = 0 ; for( ; i < ID_MAX_CHECKMESSAGE ; ++i)
		{
	...
	else
	{
		...
		for(i = 0 ; i < ID_MAX_CHECKMESSAGE ; ++i)
		{
			...
			int j = 0 ; for( j = location1 + 1 ; j < location2 ; ++j)
							...
							if((g_ServerTimeCode - g_pLocalUser->m_CharacterLog.killTime) < 10)
							{
								...
								else if( m_TDC.m_compareclass2 == 8 || m_TDC.m_compareclass2 == 9)// 아이템 타입일때 || 아이템 타입(디스플레이 안함)일때
								{
									...
									if( !currentquest->GetPartyQuest() )
									{
										...
										int i = 0 ; for( ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; ++i)
										{
											if(g_pLocalUser->m_UserItemList[i].m_cType == m_TDC.m_compareclass3 )
											{
...
_XQUESTCONDITION_RESULT _XTriggerConditionItem::ProcessCheckWearItem(void)
{
	...
	{
		if(  g_pLocalUser->m_UserItemList[ m_TDC.m_numericvalue2 ].m_cType == m_TDC.m_compareclass1 
