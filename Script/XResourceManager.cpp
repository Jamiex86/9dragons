...
{		
	...
	// 자원 리스트 속의 수집 아이템 삭제
	int i = 0 ; for(; i < m_TotalCountResourceItem; ++i)
	{
		...
		if((strlen(buffer) > 0) && buffer[0] != _T(';') && buffer[0] != _T('\n') && buffer[0] != _T('\r'))
		{
			...
			{		
				...
				else if( collectionlistlinecount == 4 )		// 수집해야 할 아이템
				{
					...
					int itemid;					
					sscanf(buffer, "%d %d %d %d", &newneeditem->m_Index, &newneeditem->m_cType, &itemid, &newneeditem->m_ucCount );
				...
				{
					int itemid;					
					sscanf(buffer, "%d %d %d", &m_arrayResourceItem[index].m_RewardItem.m_cType, &itemid, &m_arrayResourceItem[index].m_RewardItem.m_ucCount );
					m_arrayResourceItem[index].m_RewardItem.Set_m_sID(itemid);

					m_arrayResourceItem[index].m_RewardItem.m_cSecond = _XGameItem::GetItemSecond( m_arrayResourceItem[index].m_RewardItem.m_cType, m_arrayResourceItem[index].m_RewardItem.Get_m_sID() );
					index++;
