...
//					explainlinecount = 0;
				...
				{
					sscanf_s(buffer, "%d", &newcollection->m_CollectionIndex );
					index++;
					collectionlistlinecount++;					
				}
				else if( collectionlistlinecount == 2 )		// 선행으로 완료해야 할 명품 수집품 고유 인덱스 5개
				{
					//sscanf_s(buffer, "%d", &newcollection->m_CollectionVersion );
					sscanf_s(buffer, "%d %d %d %d %d", &newcollection->m_PrecedeCollectionIndex[0], &newcollection->m_PrecedeCollectionIndex[1], 
				...
				{
					sscanf_s(buffer, "%d", &newcollection->m_CollectionType );
					collectionlistlinecount++;
				}
				else if( collectionlistlinecount == 4 )		// 수집품 가능 최소레벨
				{
					sscanf_s(buffer, "%d", &newcollection->m_CollectionMinimumLevel );
					collectionlistlinecount++;
				}
...
#endif
					sscanf_s(buffer, "%d", &newcollection->m_CollectionExplainLineCount);
					collectionlistlinecount++;
				}
				...
				else if( collectionlistlinecount == 8 )		// 수집해야 할 아이템 갯수
				{
                        ...
                        int strlength = 0;
                        int i = 0 ; for( ; i < _XDEF_MAX_COLLECTION_LINE ; i++)
					    {
    ...
    //					_XDWINPRINT("%s", newcollection->m_CollectionExplain);
...
#endif
					sscanf_s(buffer, "%d", &newcollection->m_NeedItemTotalCount );
					collectionlistlinecount++;
				}
				else if( collectionlistlinecount == 9 )		// 수집해야 할 아이템
				{
					...
					int itemid;
					sscanf_s(buffer, "%d %d %d %d", &newneeditem->m_Index, &newneeditem->m_cType, &itemid, &newneeditem->m_ucCount );					
					newneeditem->Set_m_sID(itemid);

					newneeditem->m_cSecond = _XGameItem::GetItemSecond( newneeditem->m_cType, newneeditem->Get_m_sID() );
				...
				else if( collectionlistlinecount == 13 )		// 보상 아이템
				{
					...
					int itemid;					
					sscanf_s(buffer, "%d %d %d %d", &newrewarditem->m_Index, &newrewarditem->m_cType,  &itemid, &newrewarditem->m_ucCount );
					newrewarditem->Set_m_sID( itemid );

					newrewarditem->m_cSecond = _XGameItem::GetItemSecond( newrewarditem->m_cType, newrewarditem->Get_m_sID() );
	...
	do 
	{
		...
		if((strlen(buffer) > 0) && buffer[0] != _T(';') && buffer[0] != _T('\n') && buffer[0] != _T('\r'))
		{
			sscanf_s(buffer, "%s %d", tempstr, &param1);
