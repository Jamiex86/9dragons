...
void Sound::Clear()
{
	...
	if(m_pResourceBuffer) { delete m_pResourceBuffer; m_pResourceBuffer=NULL; }	
	m_ResourceSize	= 0;
}
...
bool Sound::Init(const SoundInit& init)
{
	...
	{
		m_pLoader = MemoryWave::CreateObject();
...
void Sound::Term()
{
	...
	_XASAFE_DELETE( m_pResourceBuffer );
...
bool Sound::LoadSource(uint32& nBufferSize)
{
	...
	{
		int dataindex = m_Init.m_pPackage->FindResource( (LPSTR)m_Init.m_sFileName.c_str() );
		...
		_XSetDefaultPath("./Data");
		m_Init.m_pPackage->ReOpen();
		m_pResourceBuffer = m_Init.m_pPackage->LoadPackedFileToMemory( (DWORD)dataindex, &m_ResourceSize  );
		m_Init.m_pPackage->Close( FALSE );		

		if( !m_pResourceBuffer ) return false;

		if(!m_pLoader->Open( m_pResourceBuffer, m_ResourceSize ))
			return false;
	}
	else
	{
...
bool Sound::SetVolume(float fVolume)
{
	...
	if( fVolume > VOLUME_MAX ) m_Init.m_Prop.m_fVolume = VOLUME_MAX;
	else if( fVolume < VOLUME_MIN ) m_Init.m_Prop.m_fVolume = VOLUME_MIN;
	else m_Init.m_Prop.m_fVolume = fVolume;
	...
	else if( iVolume < DSBVOLUME_MIN ) iVolume = DSBVOLUME_MIN;
	//Clamp<int32>(iVolume, DSBVOLUME_MIN, DSBVOLUME_MAX);
...
bool Sound::GetVolume(float& fVolume) const
{
	...
	if( m_Init.m_Prop.m_fVolume > VOLUME_MAX )
	{
		fVolume = VOLUME_MAX;
	}
	else
