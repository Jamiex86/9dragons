...
typedef struct{
	LPDIRECTSOUNDBUFFER	SBuffer[MAXVOICE];
	int					bufferPos;
}SOUNDBUFFER;
...
public:
	...
	HRESULT Create( CSound** ppSound, _XPackageArchive& package, LPTSTR strWaveFileName, DWORD dwCreationFlags = 0, GUID guid3DAlgorithm = GUID_NULL, DWORD dwNumBuffers = 1 );
	HRESULT Create( CSound** ppSound, _XPackageArchive& package, int PackedFileID, DWORD dwCreationFlags = 0, GUID guid3DAlgorithm = GUID_NULL, DWORD dwNumBuffers = 1 );
...
public:
    ...
    HRESULT Reset();
    BOOL    IsSoundPlaying();
	inline	DWORD Getm_dwNumBuffers(){ return m_dwNumBuffers;}
	inline  VOID  Setm_dwNumBuffers(DWORD dwNumBuffers){ m_dwNumBuffers = dwNumBuffers; }		
};
...
public:
	...
	HRESULT Open( _XPackageArchive& package, LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags );
	HRESULT Open( _XPackageArchive& package, int PackedFileID, WAVEFORMATEX* pwfx, DWORD dwFlags );
