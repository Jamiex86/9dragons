...
HRESULT _XSetDeviceCursor( POINT hotspot,  _XBitmap* bmp )
{
	if( !bmp ) return E_FAIL;

	LPDIRECT3DSURFACE9 surf = NULL;    // Temp surface
	D3DLOCKED_RECT data;               // Structure to get the temp surface data ptr

    // Create temp D3D Surface
	HRESULT hr = gpDev->CreateOffscreenPlainSurface(bmp->w, bmp->h, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &surf, NULL );
	if(FAILED(hr))
	{
		LOG_DXERROR();
		_XLogError("_XSetDeviceCursor( _XBitmap* ) : Could not create temporary D3D surface");
		goto error;
	}
		
	// Lock the surface and copy the data into.
	hr = surf->LockRect(&data, NULL, 0);
	if(FAILED(hr))
	{
		LOG_DXERROR();
		_XLogError("_XSetDeviceCursor( _XBitmap* ) : Can't locking D3D surface");
		goto error;
	}

	memcpy(data.pBits, bmp->idat, bmp->size);
    ...
    // Set the device cursor
    if( FAILED( hr = gpDev->SetCursorProperties( hotspot.x, hotspot.y, surf ) ) )
    {
        goto error;
    }
