...
#include "dxutil.h"
/*
void PlayVoice(SOUNDBUFFER *sb);
{
	IDirectSoundBuffer_Play(sb->SBuffer[sb->bufferPos], 0, 0, 0);

	if( sb->bufferPos < MAXVOICE-1 ) sb->bufferPos++;
	else sb->bufferPos = 0;
}
*/
							  ...
							  DWORD dwNumBuffers )
{
    ...
    pWaveFile->Open( package, strWaveFileName, NULL, WAVEFILE_READ );
							  ...
							  DWORD dwNumBuffers )
{
    ...
    pWaveFile->Open( package, PackedFileID, NULL, WAVEFILE_READ );
...
LPDIRECTSOUNDBUFFER CSound::GetFreeBuffer()
{
    ...
    DWORD i=0;for( ; i<m_dwNumBuffers; i++ )
    {
        if( m_apDSBuffer[i] )
        {  
            DWORD dwStatus = 0;
...
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags )
{
	...
	//return pDSBSetCurrentPosition(0);		//소리 연속 출력
...
HRESULT CWaveFile::Open( _XPackageArchive& package, LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
	HRESULT hr;
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
		...
		int		PackedFileID = package.FindResource( strFileName );
		if( PackedFileID == -1 )
		{
			return DXTRACE_ERR_NOMSGBOX( TEXT("FindResource"), E_FAIL );
		}		

		FILE*	pWaveFile = package.GetPackedFile( PackedFileID );

		if( !pWaveFile )
		{
			return DXTRACE_ERR_NOMSGBOX( TEXT("FindResource"), E_FAIL );
		}
		dwSize = package.GetPackedFileSize( PackedFileID );
		
		if( dwSize == -1 )
		{
			return DXTRACE_ERR_NOMSGBOX( TEXT("SizeofResource"), E_FAIL );
		}

		pWaveFile = package.GetPackedFile( PackedFileID );

        m_pResourceBuffer = new CHAR[ dwSize ];        
		if( fread( m_pResourceBuffer, sizeof(CHAR) * dwSize, 1, pWaveFile ) < 1 )
		{
			return DXTRACE_ERR_NOMSGBOX( TEXT("ReadResource"), E_FAIL );
		}
...
HRESULT CWaveFile::Open( _XPackageArchive& package, int PackedFileID, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
	HRESULT hr;
    ...
    {        
        ...
        m_pResourceBuffer = new TCHAR[ dwSize ];        
		if( fread( m_pResourceBuffer, dwSize, 1, pWaveFile ) < 1 )
		{
			return DXTRACE_ERR_NOMSGBOX( TEXT("ReadResource"), E_FAIL );
		}
