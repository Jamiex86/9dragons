...
// Desc: Settings class
//-----------------------------------------------------------------------------
#ifndef _X3DSETTINGS_H_
#define _X3DSETTINGS_H_


//-----------------------------------------------------------------------------
// Name: class _X3DSettings 
// Desc: Current X3D settings: adapter, device, mode, formats, etc.
//-----------------------------------------------------------------------------
class _X3DSettings 
{
public:
...
//-----------------------------------------------------------------------------
// Name: class _X3DSettingsDialog
// Desc: Dialog box to allow the user to change the D3D settings
//-----------------------------------------------------------------------------
class _X3DSettingsDialog
{
private:
    HWND m_hDlg;
    _XDeviceEnum* m_pEnumeration;
...
public:
    _X3DSettingsDialog( _XDeviceEnum* pEnumeration, _X3DSettings* pSettings);
    INT_PTR ShowDialog( HWND hwndParent );
    INT_PTR DialogProc( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam );
    void GetFinalSettings( _X3DSettings* pSettings ) { *pSettings = m_d3dSettings; }
};
