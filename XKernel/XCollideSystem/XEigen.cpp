...
//---------------------------------------------------------------------------
SuEigen::SuEigen (int iSize)
{
   // assert( iSize >= 2 );
    m_iSize = iSize;

    m_aafMat = new float*[m_iSize];
    int i = 0 ; for ( i = 0; i < m_iSize; i++)
...
//---------------------------------------------------------------------------
void SuEigen::Tridiagonal2 (float** m_aafMat, float* m_afDiag,
    float* m_afSubd)
{
...
//---------------------------------------------------------------------------
void SuEigen::Tridiagonal3 (float** m_aafMat, float* m_afDiag,
    float* m_afSubd)
{
    ...
    if ( fM02 != 0.0f )
    {
        float fLength = sqrtf(fM01*fM01 + fM02*fM02);//MgcMath::Sqrt(fM01*fM01+fM02*fM02);
...
//---------------------------------------------------------------------------
void SuEigen::Tridiagonal4 (float** m_aafMat, float* m_afDiag,
    float* m_afSubd)
{
    // save matrix M
    float fM00 = m_aafMat[0][0];
    ...
    if ( fM02 != 0.0f || fM03 != 0.0f )
    {
        ...
        // build column Q1
        fLength = sqrtf(fM01*fM01+fM02*fM02+fM03*fM03);
        ...
        fLength = sqrtf(fQ13*fQ13+fQ23*fQ23+fQ33*fQ33);
        if ( fLength > 0.0f )
        {
            fInvLength = 1.0f/fLength;
            fQ13 *= fInvLength;
    ...
    else
    {
        ...
        if ( fM13 != 0.0f )
        {
            fLength = sqrtf(fM12*fM12+fM13*fM13);
            fInvLength = 1.0f/fLength;
            fM12 *= fInvLength;
            fM13 *= fInvLength;
...
//---------------------------------------------------------------------------
void SuEigen::TridiagonalN (int iSize, float** m_aafMat,
    float* m_afDiag, float* m_afSubd)
{
    ...
    for (i0 = iSize-1, i3 = iSize-2; i0 >= 1; i0--, i3--)
    {
        ...
        {
            for (i2 = 0; i2 <= i3; i2++)
				fScale += (float)fabs( m_aafMat[i0][i2] );//MgcMath::Abs(m_aafMat[i0][i2]);
            if ( fScale == 0 )
            {
            ...
            else
            {
                ...
                float fG = sqrtf(fH);//MgcMath::Sqrt(fH);
                if ( fF > 0.0f )
                    fG = -fG;
    ...
    // re-ordering if SuEigen::QLAlgorithm is used subsequently
    for (i0 = 1, i3 = 0; i0 < iSize; i0++, i3++)
        m_afSubd[i3] = m_afSubd[i0];
    m_afSubd[iSize-1] = 0;
}
//---------------------------------------------------------------------------
bool SuEigen::QLAlgorithm (int iSize, float* m_afDiag, float* m_afSubd,
    float** m_aafMat)
{
    ...
    for (int i0 = 0; i0 < iSize; i0++)
    {
        ...
        for (i1 = 0; i1 < iMaxIter; i1++)
        {
            ...
            {
                float fTmp = (float)fabs( m_afDiag[i2] ) + (float)fabs( m_afDiag[i2+1] ) ; //MgcMath::Abs(m_afDiag[i2])+MgcMath::Abs(m_afDiag[i2+1]);
                if ( fabs(m_afSubd[i2]) + fTmp == fTmp)//MgcMath::Abs(m_afSubd[i2]) + fTmp == fTmp )
                    break;
            }
            if ( i2 == i0 )
                break;
            ...
            float fR = sqrtf(fG*fG+1.0f);//MgcMath::Sqrt(fG*fG+1.0);
            if ( fG < 0.0f )
            ...
            for (int i3 = i2-1; i3 >= i0; i3--)
            {
                ...
                if( fabs(fF) >= fabs(fG) )//if ( MgcMath::Abs(fF) >= MgcMath::Abs(fG) )
                {
                    fCos = fG/fF;
                    fR = sqrtf(fCos*fCos + 1.0f) ;//MgcMath::Sqrt(fCos*fCos+1.0);
                    m_afSubd[i3+1] = fF*fR;
                    fSin = 1.0f/fR;
                    fCos *= fSin;
                }
                ...
                {
                    fSin = fF/fG;
                    fR = sqrtf(fSin*fSin + 1.0f); //MgcMath::Sqrt(fSin*fSin+1.0);
                    m_afSubd[i3+1] = fG*fR;
                    fCos = 1.0f/fR;
                    fSin *= fCos;
                }
...
//---------------------------------------------------------------------------
void SuEigen::DecreasingSort (int iSize, float* afEigval,
    float** aafEigvec)
{
...
//---------------------------------------------------------------------------
void SuEigen::IncreasingSort (int iSize, float* afEigval,
    float** aafEigvec)
{
...
//---------------------------------------------------------------------------
void SuEigen::SetMatrix (float** aafMat)
{
...
//---------------------------------------------------------------------------
void SuEigen::DecrSortEigenStuff2 ()
{
    Tridiagonal2(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::DecrSortEigenStuff3 ()
{
    Tridiagonal3(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    DecreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::DecrSortEigenStuff4 ()
{
    Tridiagonal4(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    DecreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::DecrSortEigenStuffN ()
{
    TridiagonalN(m_iSize,m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    DecreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::DecrSortEigenStuff ()
{
    switch ( m_iSize )
    {
        case 2:
...
//---------------------------------------------------------------------------
void SuEigen::IncrSortEigenStuff2 ()
{
    Tridiagonal2(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::IncrSortEigenStuff3 ()
{
    Tridiagonal3(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    IncreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::IncrSortEigenStuff4 ()
{
    Tridiagonal4(m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    IncreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::IncrSortEigenStuffN ()
{
    TridiagonalN(m_iSize,m_aafMat,m_afDiag,m_afSubd);
    QLAlgorithm(m_iSize,m_afDiag,m_afSubd,m_aafMat);
    IncreasingSort(m_iSize,m_afDiag,m_aafMat);
...
//---------------------------------------------------------------------------
void SuEigen::IncrSortEigenStuff ()
{
    switch ( m_iSize )
    {
        case 2:
