...
__forceinline void _XFC_FloatToInt(int *int_pointer, float f) 
{
 __asm  fld f
...
// is rounded to (int + 1).
__forceinline int _XFC_RoundFloatToInt (float f)
{
 int i;
 __asm fld [f]
 __asm fistp [i]
...
__forceinline int __stdcall _XFC_FloatToIntRet(float  x)
{
 int    t;
 __asm  fld   x  
 __asm  fistp t
...
__forceinline BYTE __stdcall _XFC_FloatToByte(float  x) 
{ 
...
// floor()
__forceinline int __stdcall _XFC_ifloor(float  x)
{
...
__forceinline int _XFC_asmifloor(float f)
{
...
// Asm version of fabs()
__forceinline float __fastcall _XFC_Abs(float a) 
{
 __asm
 {
  fld  DWORD PTR [esp+4] 
...
// Asm version of sinf()
__forceinline float __fastcall _XFC_Sin(float a) 
{
 __asm 
 {
  fld  DWORD PTR [esp+4] 
...
// Asm version of cosf()
__forceinline float __fastcall _XFC_Cos(float a) 
{
 __asm 
 {
  fld  DWORD PTR [esp+4] 
...
__inline DWORD _XFC_NextPowerOfTwo(DWORD N)
{
 if (N<=0L  ) return 0L;
 if (N<=1L  ) return 1L;
 if (N<=2L  ) return 2L;
...
// Fast logarithm
__inline DWORD _XFC_Log2(DWORD val)
{
 DWORD answer = 0;
...
__inline void _XFC_NormVect3(float *v)
{
 float ilength;

 ilength = _XFC_RSqrt(_XFC_DotProduct(v, v));
...
// Does not test if the vector has 0 length
__inline void _XFC_NormVect2(float *v)
{
 float ilength;

 ilength = _XFC_RSqrt(v[0] * v[0] + v[1] * v[1]);
...
__forceinline float __cdecl _XFC_DotProduct(const float v1[3], const float v2[3]) 
{
 FLOAT dotret;
...
__inline float _XFC_RSqrt(float number)
{
 long i;
 float x2, y;
...
__inline float __fastcall _XFC_InverseSqrt(float a) 
{
 __asm 
 {
  mov  eax, 0be6eb508h
...
__inline double __fastcall _XFC_Inv_Sqrt(double x) 
{
 __asm 
 {
...
__inline int _XFC_isqrt0 (unsigned long r)
{
 double x, y;
 float rr;
 float tempf;
...
__forceinline float _XFC_sqrt(float n)
{
...
// Fast memory copy
__inline void _XFC_MemCpy(void *Dest, const void *Src, int Count)
{ 
 __asm
 {
  mov  ecx, Count
...
// Fast memory zero
__inline void _XFC_MemZero(void *Dest, int Count)
{ 
...
__inline int _XFC_FixMul(int a, int b) 
{
 _asm 
 {
  xor edx, edx
