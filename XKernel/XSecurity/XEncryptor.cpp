...
#include "XEncryptdata.h"
#include "XKernel.h"

_XEncryptor::_XEncryptor()
{
	Reset();	
}
...
void _XEncryptor::setKey( UINT shift, UINT parityvalue,  LPCSTR encryptionkey )
{

	if( shift == 0 || shift >= _ENCRYPTION_FIELD_LENGTH )
	{
		_XFatalError( "ERROR : Invalid shift value" );
		return;
	}
...
void _XEncryptor::leftshiftString ( LPSTR encryptSerial, UINT repeat )
{
	BYTE taildata;
...
void _XEncryptor::rightshiftString( LPSTR encryptSerial, UINT repeat )
{
	BYTE taildata;
...
int _XEncryptor::enciperSerial( LPCSTR orgSerial, LPSTR encryptSerial )
{
...
int _XEncryptor::deciperSerial( LPCSTR encryptSerial, LPSTR decryptSerial )
{
	if( !encryptSerial || !decryptSerial ) return -1;

	int length = strlen( encryptSerial );
	if( length < _ENCRYPTION_FIELD_LENGTH ) return -1;
...
BOOL _XEncryptor::checksumSerial( LPCSTR encryptSerial )
{
	if( strlen( encryptSerial ) < _ENCRYPTION_FIELD_LENGTH ) return FALSE;

	int i = 0;for( ; i < _ENCRYPTION_FIELD_LENGTH; i++ )
	{
...
BOOL _XEncryptor::xorEncyper( LPCSTR encryptSerial, LPSTR xorencryptSerial )
{
	if( strlen( encryptSerial ) < _ENCRYPTION_FIELD_LENGTH ) return FALSE;

	int i = 0 ; for(  i = 0; i < _ENCRYPTION_FIELD_LENGTH; i++ )
	{
...
DWORD _XEncryptor::getDiskSerial(int drv)
{
...
BOOL _XEncryptor::mixDiskSerial( LPCSTR orgserialnumber, LPSTR mixedserialnumber )
{
	...
	memset( mixedserialnumber, 0, 257 );
...
BOOL _XEncryptor::SplitSerial( LPCSTR orgserialnumber, LPSTR firstserialnumber, LPSTR secondserialnumber )
{
...
BOOL  _XEncryptor::JointSerial( LPCSTR firstserialnumber, LPCSTR secondserialnumber, LPSTR jointserialnumber )
{
...
DWORD _XEncryptor::GetDiskserialFromSerialString( LPCSTR serialstring )
{
