...
void _X3PCamera::AddYaw(FLOAT a)
{
	mp_fYaw += a;
	if( mp_fYaw > 360.0f )
		mp_fYaw = (FLOAT)fmod((float)mp_fYaw, (float)360.0);
...
void _X3PCamera::Advance( void )
{
	...
	if( abscamvel_x > EPSILON3 || abscamvel_y > EPSILON3 || abscamvel_z > EPSILON3 || abscamvel_dist > EPSILON3 )
	{
		if( abscamvel_x > EPSILON3 )
		{
			...
			if( mp_fYaw > 360.0f ) mp_fYaw = (FLOAT)fmod((float)mp_fYaw, (float)360.0);
		}
	...
	if( m_QuaterViewChanging )
	{
		...
		fyaw = _X_DEG(fyaw);
		if( fyaw > 360.0f ) fyaw = (FLOAT)fmod((float)fyaw, (float)360.0);	
