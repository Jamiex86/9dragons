...
void _XServerAddressManager::XOREncrypt( void )
{
	...
	int i = 0 ; for(  i = 0; i < sizeof(_XTESTSERVERINFOSTRUCT); i++ )
	{
...
BOOL _XServerAddressManager::GetServerInfo( void )
{
	...
	if( m_TestServerSettingMode )
		retval = g_RegManager._XRegReadBinary( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, _T("ts ndc info"), (LPBYTE)&m_ServerInfo, (LPBYTE)&m_ServerInfo, sizeof(_XTESTSERVERINFOSTRUCT), TRUE );	
	else
	{
		_XSERVERINFOSTRUCT serverinfo;
		retval = g_RegManager._XRegReadBinary( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, _T("ndc info"), (LPBYTE)&serverinfo, (LPBYTE)&serverinfo, sizeof(_XSERVERINFOSTRUCT), TRUE );
...
void _XServerAddressManager::SetServerAddress( LPTSTR address )
{
	...
	if( m_TestServerSettingMode )
		g_RegManager._XRegWriteBinary( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, _T("ts ndc info"), (LPBYTE)&m_ServerInfo, sizeof(_XTESTSERVERINFOSTRUCT) );
	else
	{
		_XSERVERINFOSTRUCT serverinfo;
		...
		g_RegManager._XRegWriteBinary( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, _T("ndc info"), (LPBYTE)(&serverinfo), sizeof(_XSERVERINFOSTRUCT) );
	}
...
BOOL CSetRegistryDlg::OnInitDialog()
{
	...
	if( !_XCheckExistFile( "ndreg.xrg" ) )
	{
		//g_RegManager.ImportWindowsRegistry();
		AfxMessageBox( _T("ERROR : Could not found ndre.xrg file") );
		EndDialog(IDOK);
		return FALSE;
	}
	else
	...
	int i = 0 ; for(  i = _MAX_PATH-1; i > 0; i-- )
	{
		if( currentPath[i] == '\\' )
		{
			currentPath[i] = NULL;
	...
	else
	{
		...
		{				
			...
			int i = 0 ; for(  i = 0; i < servercount; i++ )
			{
	...
	BOOL windowmode = g_RegManager._XRegReadInt( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_MANAGERWINDOW, 1 );
	...
	_XServerAddressManager	MainServerAddressManager;
	MainServerAddressManager.m_TestServerSettingMode = FALSE;
	MainServerAddressManager.GetServerInfo();

	CString message;
	message.Format( _T("[ MAIN ] Server IP : %s, Patch server IP : %s, Download URL : %s\n\n[ TEST ] Server IP : %s, Patch server IP : %s, Download URL : %s"), 
					MainServerAddressManager.m_ServerInfo.serveraddress,
					MainServerAddressManager.m_ServerInfo.patchserveraddress,
					MainServerAddressManager.m_ServerInfo.cdnserverurl,

					g_ServerAddressManager.m_ServerInfo.serveraddress,
					g_ServerAddressManager.m_ServerInfo.patchserveraddress,
					g_ServerAddressManager.m_ServerInfo.cdnserverurl );
		...
		if ( pos )
		{
			...
			if( !m_ServerInfoList.listEmpty() )
			{
				...
				if( psi )
				{
					...
					//g_RegManager._XRegWriteString( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_SERVERADDRESS, psi->si.serveraddress );
					//g_RegManager._XRegWriteString( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_VERSIONSERVERADDRESS, psi->si.patchserveraddress );
...
void CSetRegistryDlg::OnManagerwindow() 
{
	// TODO: Add your control notification handler code here
	BOOL windowmode = g_RegManager._XRegReadInt( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_MANAGERWINDOW, 1 );
	g_RegManager._XRegWriteInt( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_MANAGERWINDOW, !windowmode );
