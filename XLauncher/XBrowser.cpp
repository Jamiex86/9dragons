/////////////////////////////////////////////////////////////////////////////
// XBrowser.cpp: Web browser control
/////////////////////////////////////////////////////////////////////////////
...
BOOL _XBrowser::Create(DWORD dwStyle, 
                           const RECT& rect, 
                           CWnd* pParentWnd, 
						   UINT nID)
{
...
BOOL _XBrowser::CreateFromControl(CWnd *pParentWnd,UINT nID)
{
	BOOL result = FALSE;
...
void _XBrowser::Navigate(LPCTSTR URL)
{
	if (_Browser != NULL) {
...
void _XBrowser::NavigateResource(int resource_ID)
{
	if (_Browser != NULL) {
...
void _XBrowser::Write(LPCTSTR string)
{
	if (_Browser != NULL) {
...
void _XBrowser::Print(LPCTSTR header,LPCTSTR footer)
{
	if (_Browser != NULL) {
...
CString _XBrowser::GetLocationName()
{
...
CString _XBrowser::GetLocationURL()
{
...
READYSTATE _XBrowser::GetReadyState()
{
...
void _XBrowser::PutSilent(bool silent)
{
	if (_Browser != NULL) {
...
IHTMLDocument2 *_XBrowser::GetDocument()
{
	IHTMLDocument2 *document = NULL;
...
BEGIN_MESSAGE_MAP(_XBrowser,CWnd)
    //{{AFX_MSG_MAP(_XBrowser)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void _XBrowser::OnSize(UINT nType, int cx, int cy)
{
...
BEGIN_EVENTSINK_MAP(_XBrowser,CWnd)
    ON_EVENT(_XBrowser,AFX_IDW_PANE_FIRST,
...
_XBrowser::Notification::Notification(HWND hwnd,UINT ID,NotificationType type)
{
	hdr.hwndFrom	= hwnd;
	hdr.idFrom		= ID;
	hdr.code		= type;
...
void _XBrowser::_OnBeforeNavigate2(LPDISPATCH lpDisp,
                                       VARIANT FAR *URL,
                                       VARIANT FAR *Flags,
...
bool _XBrowser::OnBeforeNavigate2(CString URL,
                                      CString frame,
...
void _XBrowser::_OnDocumentComplete(LPDISPATCH lpDisp,VARIANT *URL)
{
    if (lpDisp == _BrowserDispatch) {
...
void _XBrowser::OnDocumentComplete(CString URL)
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnDownloadBegin()
{
    OnDownloadBegin();
}

void _XBrowser::OnDownloadBegin()
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnProgressChange(long progress,long progress_max)
{
    OnProgressChange((int)progress,(int)progress_max);
}

void _XBrowser::OnProgressChange(int progress,int progress_max)
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnDownloadComplete()
{
    OnDownloadComplete();
}

void _XBrowser::OnDownloadComplete()
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnNavigateComplete2(LPDISPATCH lpDisp,VARIANT *URL)
{
    if (lpDisp == _BrowserDispatch) {
...
void _XBrowser::OnNavigateComplete2(CString URL)
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnStatusTextChange(BSTR bstrText)
{
    CString text;
...
void _XBrowser::OnStatusTextChange(CString text)
{
	CWnd *parent = GetParent();
...
void _XBrowser::_OnTitleChange(BSTR bstrText)
{
    CString text;
...
void _XBrowser::OnTitleChange(CString text)
{
	CWnd *parent = GetParent();
