...
BOOL BCMenu::LoadToolbar(UINT nToolBar)
{
	...
	if(bar.LoadToolBar(nToolBar)){
		...
		if(AddBitmapToImageList(&imglist,nToolBar)){
			returnflag=TRUE;
			int i = 0  ; for( i=0;i<bar.GetCount();++i){
				nID = bar.GetItemID(i); 
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	int i = 0  ; for( i=0;i<(int)GetMenuItemCount();++i){
...
{
	int i = 0  ; for( i=0;i<=m_MenuList.GetUpperBound();++i){
...
void BCMenu::GetFadedBitmap(CBitmap &bmp)
{
	...
	int i = 0  ; for( i=0;i<BitMap.bmWidth;++i){
		int j = 0 ; for( j=0;j<BitMap.bmHeight;++j){
...
{
	int i = 0 ; for ( i = 0; i < pToolBar->GetCount(); i++) {
	...
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			if(NULL==pSubMenu){
				...
				int i = 0  ; for( i=0;i<m_MenuList.GetSize(); i++){
				...
				int i = 0  ; for( i=num-1;i>=0;--i)pSubMenu->RemoveMenu(i,MF_BYPOSITION);
	...
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			if(NULL==pSubMenu){
				...
				int num = pSubMenu->GetMenuItemCount();
				int i = 0  ; for( i=num-1;i>=0;--i)pSubMenu->DeleteMenu(i,MF_BYPOSITION);
				for(i=m_MenuList.GetUpperBound();i>=0;i--){
					if(m_MenuList[i]->nID==(UINT)pSubMenu->m_hMenu){
						delete m_MenuList.GetAt(i);
						m_MenuList.RemoveAt(i);
...
int BCMenu::GlobalImageListOffset(int nID)
{
	...
	int existsloc = -1;
	int i = 0  ; for( i=0;i<numcurrent;++i){
...
BOOL BCMenuToolBar::LoadToolBar(LPCTSTR lpszResourceName)
{
	...
	int i = 0 ; for ( i = 0; i < pData->wItemCount; i++)
