								   ...
								   LPCTSTR lpszFilename, CBitmap *pBitmap)
{
	...
	m_brushHollow = (HBRUSH) GetStockObject (WHITE_BRUSH);
...
BOOL CBitmapDialog :: LoadBitmap (UINT nIDResource, DWORD nRegionID )
{
	// Release the bitmap if it was created
	ReleaseBitmap ();
	...
	// Make the window transparent if needed
	if( nRegionID > 0 )
		MakeWindowRgn( FALSE, nRegionID );
	else
		MakeWindowRgn( TRUE, 0, "window.rgn" );
...
void CBitmapDialog :: MakeWindowRgn ( BOOL rebuild, DWORD resourceid, LPTSTR szSaveName )
{
	if (!m_bTransparent || ( !m_bBitmapCreated && !m_bBitmapExists ) )
	{
	...
	else
	{
		if( !rebuild )
		{
			...
			if ((hrsrc=FindResource(NULL, (LPCSTR)resourceid, RT_RCDATA)) == NULL) 
			{ 			
				return; 
			} 
			...
			//get size of region-data structure (= header + data) 
			if ((dwSize=SizeofResource (NULL,hrsrc)) == 0) 
			{ 			
				return; 
			} 
			//lock resource to get a pointer for it 
			if ((rd=(RGNDATA *)LockResource (hmem)) == NULL) 
			{
				return; 
			} 
			
			// Create region object locally on heap
			// because this method may be called a lot
			// and CRgn objects can cause memory leaks
			CRgn region;
			
			//create a region from our data
			if (! region.CreateFromData(NULL, dwSize, rd) )
			{			
				return;
			}
			
			// Let's dynamically resize static region data
			// We call "GetRgnBox" for bounding rect
			RECT rcSkin;
			region.GetRgnBox(&rcSkin);
		...
		else
		{
			...
			for (int y=0; y<height; y++)
			{
				...
				for (int x=0; x<width; x++)
				{
					...
					if (dcImage.GetPixel(x, y) == dcImage.GetPixel(0, 0))
					{
			...
			if( szSaveName )
			{
				...
				if( f.Open(szSaveName, CFile::modeCreate | CFile::modeWrite, &e ) )
				{
					nSize = GetRegionData(rgn, NULL, 0);
					ASSERT (nSize);
