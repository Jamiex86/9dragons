...
public:
	...
	DIBLoadResult LoadPPM ( FILE* fp );
    DIBSaveResult SavePPM ( LPSTR Filename , UINT uDescID = 0);
    
	DIBSaveResult SaveBMP (  LPSTR Filename );

	DIBLoadResult LoadXI( LPSTR Filename );	
	DIBLoadResult LoadXI( FILE* fp );	
    DIBSaveResult SaveXI( LPSTR Filename );	
	...
	BOOL CreateFromText(CDC *pDC, LPCTSTR szString, COLORREF fontcolor, int width = -1, LPCTSTR szSecondString = NULL, int secondoffset = 0 );
	void FillAlphaChannel( COLORREF maskcolor, BOOL checkcolor = FALSE );
	...
	void BlendRect ( CDIB *Dib, int x, int y );
	void BlendRectTransparently( CDIB *Dib, int x, int y, int A );
