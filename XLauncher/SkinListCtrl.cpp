...
void CSkinListCtrl::OnPaint() 
{
	...
	memDC.FillSolidRect(clip, RGB(0,0,0));
...
BOOL CSkinListCtrl::ShowWindow( int nCmdShow )
{
	if( IsWindow( m_SkinHorizontalScrollbar.GetSafeHwnd() ) )
		m_SkinHorizontalScrollbar.ShowWindow(nCmdShow);

	if( IsWindow( m_SkinVerticleScrollbar.GetSafeHwnd() ) )
		m_SkinVerticleScrollbar.ShowWindow(nCmdShow);
