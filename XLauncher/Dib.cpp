...
BOOL CDIB::CreateFromText(CDC *pDC, LPCTSTR szString, COLORREF fontcolor, int width, LPCTSTR szSecondString, int secondoffset )
{
	...
	SIZE  size;
	GetTextExtentPoint32( BufferDC.GetSafeHdc(), szString, strlen(szString), &size );
	...
	{
		SIZE secondsize;
		GetTextExtentPoint32( BufferDC.GetSafeHdc(), szSecondString, strlen(szSecondString), &secondsize );
...
DIBLoadResult CDIB::LoadPPM ( LPSTR Filename )
{
    ...
    {
		AfxMessageBox( "PPM File open Faild !" );
...
{    
	if( !fp )
	{
		AfxMessageBox( "LoadPPM : Invalide file pointer..." );
		return DIB_LOAD_NOFILE_OR_CORRUPT;
	}
...
DIBLoadResult CDIB::LoadXI( FILE* fp )
{
	...
	TCHAR header[3];
	memset( header, 0, sizeof(TCHAR) * 3 );	    
	fread( header, sizeof(TCHAR) * 2, 1, fp );
...
DIBSaveResult CDIB::SaveXI( LPSTR Filename )
{
	...
	memset( header, 0, sizeof(TCHAR) * 3 );
	strcpy( header, "XI" );
    fwrite( header, sizeof(TCHAR) * 2, 1, fp );
...
{    
	...
	int i = 0 ; for(  i=0; i<m_Info.bmiHeader.biHeight; i++ )
	{
		int j = 0 ; for( j=0; i<m_Info.bmiHeader.biWidth; j++ )	 
		{
