...
#ifdef  _LOGFILE_
extern void _XLog( BOOL insertlistview, char* msg, ... );
#endif

extern char	g_Installedpath[_REGSTRING_MAXLENGTH+1];

void _XMessage( char* argv, ... )
{
	char szBuff[1024];
	int retValue;
	va_list argptr;
...
BOOL CPackage::Open( LPCTSTR filename, BOOL skipcreateindextable )
{
	...
	pIDXFilePtr = fopen( indexfilename, "rb" );
	
	if( !pIDXFilePtr )
	{
		_XMessage( "ERROR : Package openning [%s]", indexfilename );
		return FALSE;
	}
	
	_XLog( FALSE, "Package Message : Package opened [%s]\n", indexfilename );
	...
	int i = 0 ; for(  i = 0; i < this->m_PackedFileCount; i++ )
	{
		length = 0;
	...
	{
		_XMessage( "ERROR : Package header loading [%s]", filename );
		Close();
		return FALSE;
	}
	...
	if( packagefileheader.HeaderIndicator != _HEADERINDICATOR_PACKAGEFILE )
	{
		_XMessage( "ERROR : Package header check [%s]", filename );
		Close();
		return FALSE;
	}
	...
	if( packagefileheader.FileVersion != _HEADERINDICATOR_PACKAGEFILEVERSION )
	{
		_XMessage( "ERROR : Package version [%s]", filename );
		Close();
		return FALSE;
	}
	...
	if( !m_PackageFilePositionArray )
	{
		_XMessage( "ERROR : Allocate position array [%s]", filename );
		Close();
		return FALSE;
	}
	...
	if( !m_PackageFilenameArray )
	{
		_XMessage( "ERROR : Allocate resource description array [%s]", filename );
		Close();
		return FALSE;
	}
	...
	int i = 0 ; for(  i = 0; i < this->m_PackedFileCount; i++ )
	{
		...
		{
			_XMessage( "ERROR : Read file description array size [%s]", filename );
			Close();
			return FALSE;
		}
		...
		{
			_XMessage( "ERROR : Read file description array [%s]", filename );
			Close();
			return FALSE;
		}
	...
	if( this->m_PackedFileCount > 0 )
	{
		...
		{
			_XMessage( "ERROR : Read position array [%s]", filename );
			Close();
			return FALSE;
		}
	}
...
FILE* CPackage::GetPackedFile( int file_id )
{
	if( !m_PackageFileptr )		
	{
		_XMessage( "ERROR : Not ready package" );
		return NULL;
	}

	if( m_PackedFileCount <= file_id )
	{
		_XMessage( "ERROR : Invalid Package ID" );
		return NULL;
	}
	...
	{
		_XMessage( "ERROR : Package not found" );
		return NULL;
	}
...
FILE* CPackage::GetPackedFile( LPSTR file_name )
{
	ASSERT( file_name );
	...
	if( resourceindex == -1 )
	{		
		_XMessage( "ERROR : Can't find resource [%s]", file_name );
		return NULL;
	}
...
int  CPackage::FindResource( LPSTR resourcename )
{
	ASSERT( resourcename );
	...
	int i = 0 ; for(  i = 0; i < m_PackedFileCount; i++ )
	{
...
LPSTR CPackage::GetResourceName( int file_id )
{
	if( !m_PackageFileptr )		
	{
		_XMessage( "ERROR : Not ready package" );
...
{	
	...
	_XLog( FALSE, "Package Message : Export file [%d:%s]\n", file_id, filename );
	...
	_XLog( FALSE, "Package Message : Check folder and create [%d:%s]\n", file_id, filename );
	if(FilePath[0]) MakeSureDirectoryPathExists( FilePath );
	
	CString newfilename = filename;
	newfilename += _T("_new");
	
	pExportFile = fopen( newfilename.GetBuffer(512), "wb" );

	if( !pExportFile )
	{
		_XMessage( "ERROR : Export file open [%s]", newfilename );
		return FALSE;
	}
	...
	_XLog( FALSE, "Package Message : Exporting [%d:%s]\n", file_id, filename );
	...
	do
	{
		...
		{
			_XMessage( "ERROR : Exporting packed data [%s]", newfilename );
	...
	if( !pBuffer )
	{
		_XMessage( "ERROR : Allocation copy buffer [%s]", newfilename );
		fclose( pExportFile );
		if( CheckExistFile( filename, TRUE ) ) ::DeleteFile( newfilename );		
		return FALSE;
	}
	...
	{
		_XMessage( "ERROR : Read exporting data [%s]", newfilename );
		fclose( pExportFile );
		if( CheckExistFile( filename, TRUE ) ) ::DeleteFile( newfilename );		
	...
	{
		_XMessage( "ERROR : Write exporting data [%s]", newfilename );
		fclose( pExportFile );
		if( CheckExistFile( filename, TRUE ) ) ::DeleteFile( newfilename );		
		if( pBuffer ) delete[] pBuffer;
		return FALSE;
	...
	_XLog( FALSE, "Package Message : Check old file [%d:%s]\n", file_id, filename );
	...
	_XLog( FALSE, "Package Message : Rename new file [%s->%s]\n", newfilename, filename );
...
BOOL CPackage::ExportFile( int file_id, FILE* pExportFile )
{
	...
	_XLog( FALSE, "Package Message : Search Export file [%d]\n", file_id );
	...
	_XLog( FALSE, "Package Message : Exporting [%d]\n", file_id );
	...
	if( !pBuffer )
	{
		_XMessage( "ERROR : Allocation copy buffer" );
		return FALSE;
	}

	_XLog( FALSE, "Package Message : Read org file [%d]\n", file_id );

	if( fread( pBuffer, sizeof( BYTE ) * filesize, 1, pPackageFile ) < 1 )
	{
		_XMessage( "ERROR : Read exporting data" );
		return FALSE;
	}

	_XLog( FALSE, "Package Message : write new file [%d]\n", file_id );

	if( fwrite( pBuffer, sizeof( BYTE ) * filesize, 1, pExportFile ) < 1 )
	{
		_XMessage( "ERROR : Write exporting data" );
		return FALSE;
	}
...
BOOL CPackage::Patch( LPCTSTR packagefilename, LPCTSTR patchinfofilename, CXLauncherDlg* pMainWindow )
{
	...
	while( !feof( pPatchinfoFile ) )
	{
		...
		{
			strcpy( szPatchFileString[patchfilecount], parsingString );
			patchfilecount++;
		}
	}	
		...
		{				
			...
			if( strcmpi( GetFileExtension(tFilenamestring), "XP" ) == 0 )
			{
				...
				if( CheckExistFile(tFilenamestring, TRUE) )
				{
					...
					{
						tMessagestring.Format( "Export failed. [%s_Merge]\n", szPatchFileString[packageindexcount] );
						if( pMainWindow ) 
							pMainWindow->InsertStringToProgressMessageListCtrl(tMessagestring);
						_XLog( TRUE, tMessagestring.GetBuffer(256) );
						break;
			...
			tMessagestring.Format( "Upgrade complete.[%s]\n", szPatchFileString[packageindexcount] );
			if( pMainWindow ) 
				pMainWindow->InsertStringToProgressMessageListCtrl(tMessagestring);
			_XLog(  TRUE, tMessagestring.GetBuffer(256) );
		}
...
BOOL CPackage::PatchXPToXP( LPCTSTR packagefilename )
{
	...
	CString tMessagestring;
	tMessagestring.Format( "Modifing package file.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
	tMessagestring.Format( "    %s\n", packagefilename );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
	...
	tMessagestring.Format( "Calculate package informations.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
	...
	int i = 0 ; for(  i = 0; i < MergePackage.GetResourceCount(); i++ )
	{
		BOOL find = FALSE;
		int j = 0 ; for( j = 0; j < OrgPackage.GetResourceCount(); j++ )
		{
			if( strcmpi( OrgPackage.m_PackageFilenameArray[j].m_filename,
	...
	newpackagefile	= fopen( newpackagename, "wb" );

	if( !newpackagefile )
	{
		AfxMessageBox( "ERROR : New package file open" );
		MergePackage.Close();
		OrgPackage.Close();
		return FALSE;
	}
	...
	{
		BOOL find = FALSE;
		int j = 0 ; for( j = 0; j < MergePackage.GetResourceCount(); j++ )
		{
			if( strcmpi( OrgPackage.m_PackageFilenameArray[i].m_filename,
	...
	tMessagestring.Format( "Rebuild package\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
	...
	{					
		...
		{
			_XMessage( "ERROR : Save file description array size [%s]", packagefilename );			
		...
		if( fwrite( fileinfo[i].m_filename , fileinfo[i].m_length, 1, newpackagefile ) < 1 )
		{
			_XMessage( "ERROR : Save file description array [%s]", packagefilename );			
			fclose( newpackagefile );
			delete[] PackageFilePositionArray;
			delete[] fileinfo;
			MergePackage.Close();
	...
	{
		_XMessage( "ERROR : Read position array [%s]", packagefilename );
		fclose( newpackagefile );
		delete[] PackageFilePositionArray;
		delete[] fileinfo;
		MergePackage.Close();
	...
	tMessagestring.Format( "Delete temporary patch files\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
...
//#define _TEST_EXPORT_
	...
	tMessagestring.Format( "Complete rebuild\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  TRUE, tMessagestring.GetBuffer(256) );
...
BOOL CPackage::SaveIndexTable( LPCTSTR filename )
{
	...
	{
		_XMessage( "ERROR : Org pack file open [%s]", filename );
		return FALSE;
	}

	FILE* pFilePtr = fopen( indexfilename, "wb" );
	
	if( !pFilePtr )
	{
		_XMessage( "ERROR : IDXTBL File open [%s]", indexfilename );
		return FALSE;
	}
	...
	{
		_XMessage( "ERROR : IDXTBL Package header write [%s]", indexfilename );
		fclose( pFilePtr );
		return FALSE;
	}
	...
	int i = 0 ; for(  i = 0; i < this->m_PackedFileCount; i++ )
	{
		...
		{
			_XMessage( "ERROR : IDXTBL Write file description array size [%s]", indexfilename );
			fclose( pFilePtr );
			return FALSE;
		}
		
		memset( tempbuffer, 0, sizeof(TCHAR) * 512 );
		strcpy( tempbuffer, m_PackageFilenameArray[i].m_filename );
		_EncodeData( tempbuffer, _ENCODEKEY );

		if( fwrite( &tempbuffer, m_PackageFilenameArray[i].m_length, 1, pFilePtr ) < 1 )
		{
			_XMessage( "ERROR : IDXTBL Write file description array [%s]", indexfilename );
			fclose( pFilePtr );
			return FALSE;
		}
	}
	...
	if( this->m_PackedFileCount > 0 )
	{
		...
		{
			_XMessage( "ERROR : IDXTBL Write position array [%s]", indexfilename );
			fclose( pFilePtr );
			return FALSE;
		}
	}	
