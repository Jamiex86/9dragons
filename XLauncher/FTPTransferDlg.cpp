...
BOOL CFTPTransferDlg::OnInitDialog() 
{
  ...
  m_ProgressMessageList.InsertColumn(0, "Progress Status", LVCFMT_LEFT, rect.right - rect.left);
...
void CFTPTransferDlg::TransferThread()
{
  ...
  _XLog( FALSE, "Download ftp file [%s]\n", m_sRemoteFile );
  ...
  else _XLog( FALSE, "Downloaded ftp file [%s]\n", m_sRemoteFile );
