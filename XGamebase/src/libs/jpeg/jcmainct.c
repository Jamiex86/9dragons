...
typedef struct {
  ...
  _boolean suspended;		/* remember if we suspended output */
			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  while (mymain->cur_iMCU_row < cinfo->total_iMCU_rows) {
     ...
     * call suspended; then mark the mymain buffer empty.
     */
    if (mymain->suspended) {
      (*in_row_ctr)++;
      mymain->suspended = FALSE;
			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  _boolean writing = (mymain->pass_mode != JBUF_CRANK_DEST);
...
GLOBAL(void)
jinit_c_main_controller (j_compress_ptr cinfo, _boolean need_full_buffer)
{
  my_main_ptr mymain;
  int ci;
  jpeg_component_info *compptr;
