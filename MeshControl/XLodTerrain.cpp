...
{	
	...
	int side=0;for (;side<TOTAL_SIDES;++side)
	{				
...
BOOL _XLodTerrain::CreateTerrainVertex( void )
{
	...
	{
		int j = 0 ; for( j = 0; j < m_TileCount; j++ )
		{
...
void _XLodTerrain::ReleaseTerrainTileInfo( void )
{
	...
	{
		int i = 0 ; for ( i=0;i<(m_TileCount<<4);++i)
		{
...
void _XLodTerrain::ReleaseTileRenderingCache( void )
{
	...
	{
		int layer = 0 ; for(  ; layer < m_UsedLayerCount+3; layer++ )
		{
...
BOOL _XLodTerrain::PreCachingMonsterModel( void )
{
	...
	int progresspoint = 0;
	int i = 0 ; for(  i = 1; i < _XDEF_MODELCLASSCOUNT_MOB; i++ )
	{
...
{
	int i = 0 ; for ( i=0;i<m_TileCount;++i)
	{
...
BOOL _XLodTerrain::LoadTerrainTileDetailInfo(FILE *fileptr)
{
	...
	m_UsedLayerCount = 0;
	int layer = 0 ; for( ; layer < _XDEF_MAXSPLATCOUNT; layer++ )
	{
	...
	int i = 0 ; for(; i < m_UsedLayerCount; i++ )
	{
	...
	int layerindex = 0 ; for( ; layerindex < _XDEF_MAXSPLATCOUNT; layerindex++ ) TileUseCount[layerindex] = 0;
	...
	for (i=0;i<m_TileCount;++i)
	{
		for (int j=0;j<m_TileCount;++j)
		{
			...
			int layer = 0 ; for( ; layer < m_UsedLayerCount; layer++ )
			{
...
BOOL _XTileSplat::ApplyBlendLevelToBlendMap( LPBYTE pBlendMap, FILE* pTerrainFilePtr )
{
	...
	for ( z = 0; z < _XDEF_SPLATBLENDTEXTURESIZE; z++ )
	{
		for ( int x = 0; x < _XDEF_SPLATBLENDTEXTURESIZE; x++ )
		{
			...
			{	
				BOOL looping = TRUE;
				int i = 0 ; for(  i = -1; i <= 1; i++ )
				{
					int j = 0 ; for( j = -1; j <= 1; j++ )
					{
...
*/
	...
	int i = 0 ; for(  i = stz; i <= endz; i++ )
	{
		int j = 0 ; for( j = stx; j <= endx; j++ )
		{
...
void _XLodTerrain::RebuildLevel( BOOL check )
{
	...
	int layerindex = 0 ; for(  ; layerindex < m_UsedLayerCount+3; layerindex++ )
		 m_TerrainTileRenderingCacheLength[layerindex] = 0;	
	...
	int tilerenderingcacheindex = 0 ; for( ; tilerenderingcacheindex < m_TerrainTileRenderingCacheLength[m_UsedLayerCount]; tilerenderingcacheindex++ )
	{
...
void _XLodTerrain::DrawReplectTerrain( void )
{
	...
	if( CWater::m_UseObjectReflection && CWater::m_UseTerrainReflection )
	{
		...
		g_WorldObjectTextureArchive.SetTexture_NoRenderStyle( m_LayerInfo[splatlayer].m_DetailTextureIndex, 0 );
		  ...
		  // render each tile
		  int tilerenderingcacheindex = 0 ; for( ; tilerenderingcacheindex < m_TerrainTileRenderingCacheLength[splatlayer]; tilerenderingcacheindex++ )
		  {
		...
		int tilerenderingcacheindex = 0 ; for( ; tilerenderingcacheindex < m_TerrainTileRenderingCacheLength[layerindex]; tilerenderingcacheindex++ )
		{
...
void _XLodTerrain::DrawTerrain()
{
	...
	{	
		...
		{
			int i = 0 ; for(; i < m_FunctionalObjectManager.m_countFunctionalObject; i++ )
			{
