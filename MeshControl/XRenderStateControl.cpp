...
void _XRenderStateControl::ProcCommand( LPSTR commandstring )
{
	...
	else if( strcmpi( ParseCommandString[1], "gotonpc" ) == 0 )
	{
		...
		int i = 0 ; for(; i < g_Npclist.m_NPCObjectCount; i++ )
		{
...
*/
			 ...
			 (strcmpi(ParseCommandString[1], "ms") == 0 ) )
	{
		...
		if(strlen(ParseCommandString[5]) > 0)
			sprintf(name, "%s %s %s", ParseCommandString[3], ParseCommandString[4], ParseCommandString[5]);
		else if(strlen(ParseCommandString[4]) > 0)
			sprintf(name, "%s %s", ParseCommandString[3], ParseCommandString[4]);
		else if(strlen(ParseCommandString[3]) > 0)
			sprintf(name, "%s", ParseCommandString[3]);
...
//			g_LastMobSpawnTime = 0;
			/*
			int mobindex = -1;
			int i = 0 ; for(; i < _XDEF_MODELCLASSCOUNT_MOB; i++ )
			{
	...
	{			
		...
		int i = 0 ; for(; i < _XDEF_COMMADCOUNT; i++ )
		{				
	...
	else if( strcmpi( ParseCommandString[1], "moblist" ) == 0 )
	{
		...
		sprintf( messagestring, "Total mob count : %d", g_Moblist.m_smMobList.size()  );
		...
		if( !g_Moblist.m_smMobList.empty() )
		{
			...
			{
				curmob = (*it).second;
				sprintf( messagestring, "%4d [%d:%s] [ %4.4f %4.4f %4.4f ]", i, curmob->m_MobID, g_MonsterName[curmob->m_MobType], curmob->m_Position.x,curmob->m_Position.y,curmob->m_Position.z );
				_XDebugOutputString( messagestring, 1, 3 );				
			}
		}
	...
	else if( strcmpi( ParseCommandString[1], "speedtreelod" ) == 0 )		
	{
		...
		int i = 0 ; for (; i < g_XST_Wrapper.m_ObjectContainerCount; ++i)
		{
	...
	{		
		...
		memset( tempStr, 0, sizeof(TCHAR)*256 );
				...
				sprintf( postdata, "AL_ID=%s&AL_PWD=%s&AL_Server=%d", g_pLocalUser->m_ItemMallWebLoginID, g_pLocalUser->m_UserPW, selectedserver );
	...
	if( checkvalidcommand == _XDEF_VALIDCOMMAND )
	{
		sprintf( comandresultstr, "Command :" );				
		g_RegManager._XRegWriteString( ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_LASTCOMMAND, commandstring );
	}
	else if( checkvalidcommand == _XDEF_INVALIDCOMMAND )
	{
		sprintf( comandresultstr, "Invalid command :" );			
	}
	else if( checkvalidcommand == _XDEF_NOTREADYCOMMAND )
	{
		sprintf( comandresultstr, "Not ready this command :" );			
	}
	...
	case WM_INITDIALOG:		
			...
			if( hconsolewindowhistorylist )
			{
				...
				int i = 0 ; for(; i < 10; i++ )
				{
					...
					sprintf( mrucommandlistid, "Last command %d", i );
					...
					case LBN_SELCHANGE :
						{
							...
							case 5:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Range );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
							...
							case 6:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Falloff );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 7:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Attenuation0 );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 8:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Attenuation1 );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 9:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Attenuation2 );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 10:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Theta );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 11:
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", pLightObject->Phi );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							case 12 :
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", g_EnvironmentManager.m_FogNear );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);									
								}								
							...
							case 13 :
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", g_EnvironmentManager.m_FogFar );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);									
								}
							...
							case 14 :
								{
									TCHAR data[20];
									sprintf( data, "%3.5f", g_EnvironmentManager.m_FogDensity );
									HWND valueedit = GetDlgItem( hDlg, IDC_VALUEEDIT );
									Edit_SetSel(valueedit, 0, -1);
									Edit_ReplaceSel(valueedit, (LPARAM)data);
								}
							...
							{
								TCHAR data[20];
								sprintf( data, "%3.3f", red/255.0f );
								HWND rededit = GetDlgItem( hDlg, IDC_REDEDIT );				
								Edit_SetSel(rededit, 0, -1);
								Edit_ReplaceSel(rededit, (LPARAM)data);
								
								sprintf( data, "%3.3f", green/255.0f );
								HWND greenedit = GetDlgItem( hDlg, IDC_GREENEDIT );				
								Edit_SetSel(greenedit, 0, -1);
								Edit_ReplaceSel(greenedit, (LPARAM)data);

								sprintf( data, "%3.3f", blue/255.0f );
								HWND blueedit = GetDlgItem( hDlg, IDC_BLUEEDIT );				
								Edit_SetSel(blueedit, 0, -1);
								Edit_ReplaceSel(blueedit, (LPARAM)data);

								sprintf( data, "%3.3f", alpha/255.0f );
								HWND alphaedit = GetDlgItem( hDlg, IDC_ALPHAEDIT );				
								Edit_SetSel(alphaedit, 0, -1);
								Edit_ReplaceSel(alphaedit, (LPARAM)data);
							}
