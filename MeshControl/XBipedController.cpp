...
#ifdef _XDWDEBUG		
		TCHAR messagestr[64];
		sprintf( messagestr, "MDEffectManager restore : %p\n", m_EffectManager );
...
_XModelDescriptor& _XModelDescriptor::operator =(_XModelDescriptor& modeldescriptor)
{
	...
	int i = 0 ; for( ; i < m_ModelStackSize ; i++)
	{
...
void _XModelDescriptor::Reset( void )
{
	...
	int i = 0 ; for (; i < _XParticleLinkBone_MaxCount; ++ i)
	{
...
#ifdef _XDEF_MODELTHREADLOADING
	int i = 0 ; for(; i < _XDEF_MAXMODELSTACKSIZE; i++ )
#else
	int i = 0 ; for(; i < modelstacksize; i++ )
#endif
	{
		m_ppModelList[i] = NULL;
	}
...
#ifdef _XDWDEBUG		
		TCHAR messagestr[64];
		sprintf( messagestr, "MDEffectManager allocated : %p\n", m_EffectManager );
		OutputDebugString( messagestr );
#endif
...
#ifdef _XDEF_MODELTHREADLOADING	
	int i = 0 ; for(; i < m_SwordEffectCount; i++ )
	{
...
void _XModelDescriptor::RenderObjectShadow( void )
{
	...
	int j = 0 ; for( j = 0; j < m_ModelStackSize; j++ )
	{			
		if( m_ppModelList[j] )
		{
...
BOOL _XBipedController::ReserveAnimationStack( int motionstacksize )
{
	...
	int i = 0 ; for(; i < m_ReservedMotionStackCount; i++ ) m_pMotionStack[i].ResetMotionData();
...
{		
	...
	int i = 0 ; for(; i < m_ReservedMotionStackCount; i++ ) 
	{
		if( m_pMotionStack )
...
int	_XBipedController::FindObject( LPSTR objectname )
{
	...
	int i = 0 ; for(; i < m_BipedNodeCnt; i++ )
	{
...
bool _XBipedController::AddMotion(int index, int SFrame, int EFrame, int FrameSpeed)
{
	...
	int i = 0  ; for( i = m_MotionCnt; i > index; i--)
	{		
...
void _XBipedController::Advance( _XModelDescriptor* pAniData, bool& playsound, int eventlevel, FLOAT particlesize )
{
	...
	int i = 0 ; for(; i < _XDEF_MAXFRAMEEVENT; i++ )
	{
		// Check command frame
		...
		{				
			...
			if( modelcontainner )
			{
				...
				else
				{
					...
					int ilod = 0 ; for(  ilod = 0; ilod < lodcount; ilod++ )
					{
#ifdef _XDEF_MODELTHREADLOADING						
