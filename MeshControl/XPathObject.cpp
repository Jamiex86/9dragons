...
{
	int i = 0 ; for( ; i < DEF_MAXNUM_CURVE ; ++i)
	{
		int j = 0 ; for( j = 0 ; j < DEF_NUM_CONTROLPOINT ; ++j)
		{
...
/*	if(m_bPointDrawing)
	{
		...
		D3DCOLOR		color;
		int i = 0 ; for( ; i <= m_iCurrentCurve ; i++)
		{
			...
			else
			{
				...
				int j = 0 ; for( j = 0 ; j < pointindex; j++)
				{
					point = m_Curve[i].vControlPoint[j];
					rectsize = 0.11f;
...
void CPathObject::SetCurveSelect(int curveindex, BOOL selected)
{
	...
	int i = 0 ; for( ; i < DEF_NUM_CONTROLPOINT ; ++i)
...
void CPathObject::GeneratePathIterate(int selectedcurve, int steps)
{
	...
	int i = 1 ; for( ; i < steps ; i++, t+=stepsize)
	{
