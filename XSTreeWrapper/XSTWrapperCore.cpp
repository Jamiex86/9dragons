...
{
	unsigned int i = 0 ; for (; i < m_ObjectContainerCount; ++i)
	{
...
{	
	...
	int i = 0 ; for(; i < m_ObjectContainerCount; ++i )
	{
...
BOOL _XST_WrapperCore::LoadSpeedTreeInstanceData(  FILE* Fileptr )
{
	...
	int i = 0 ; for(; i < speedtreetemplatecount; ++i )
	{	
...
{
	unsigned int i = 0 ; for (; i < m_TemplateDataCount; ++i)
	{
...
void _XST_WrapperCore::SetupWindMatrices( void )
{
	...
	{
		unsigned int i = 0 ; for (; i < c_nNumWindMatrices; ++i)
	...
	int i = 0 ; for (; i < c_nNumWindMatrices; ++i)
...
void _XST_WrapperCore::UpdateWindStrengths( FLOAT strength )
{
	...
	int i = 0 ; for (; i < m_ObjectContainerCount; ++i)
	{
		...
		m_ObjectContainerArray[i].GetSpeedTree()->SetWindStrength(m_fWindStrength);//m_ObjectContainerArray[i].GetSpeedTree()->SetWindStrengthAndLeafAngles(m_fWindStrength, m_cSpeedWind.GetLeafAngles(_XSpeedWind::ROCK), m_cSpeedWind.GetLeafAngles(_XSpeedWind::RUSTLE), m_cSpeedWind.GetNumLeafAngles( ));
	}
}
