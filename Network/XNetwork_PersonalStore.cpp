				...
				if( pPersonalStore_Window )
				{
					...
					int i = 0 ; for( ; i < _XDEF_PERSONALSTORE_TRADEITEM_MAX_SLOTNUMBER ; ++i)
					{
						if( pPersonalStore_Window->m_TradeItem[i].Get_m_sID() != 0 )
						{
							...
							psstartreq.ucItemCount[i]	= (unsigned char)pPersonalStore_Window->m_TradeItem[i].m_ucCount;// 아이템 각각의 팔려는 갯수
				...
				int i = 0 ; for( ; i < _XDEF_PERSONALSTORE_TRADEITEM_MAX_SLOTNUMBER ; ++i)
				{
					if( pPersonalStore_Window->m_TradeItem[i].Get_m_sID() != 0)
					{
						...
						if( pPersonalStore_Window->m_TradeItem[i].IsStackable() )
						{
							...
							{
								g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount += pPersonalStore_Window->m_TradeItem[i].m_ucCount;
							}
						}
						else
						{
	...
	default: // 성공
		{
			...
			if( pPersonalStore_Window )
			{
				...
				{
					pPersonalStore_Window->m_TradeItem[i].m_cType = psshoplistresult->ItemList[i].cItemType;
					pPersonalStore_Window->m_TradeItem[i].m_cSecond = _XGameItem::GetItemSecond(psshoplistresult->ItemList[i].cItemType, psshoplistresult->ItemList[i].sItem_Id);
					pPersonalStore_Window->m_TradeItem[i].Set_m_sID( psshoplistresult->ItemList[i].sItem_Id );
					pPersonalStore_Window->m_TradeItem[i].m_ucCount = psshoplistresult->ItemList[i].ucItemCount;
					pPersonalStore_Window->m_TradeItem[i].Set_m_usMaxDur( _XGameItem::GetMaxDurability( pPersonalStore_Window->m_TradeItem[i].m_cType, pPersonalStore_Window->m_TradeItem[i].Get_m_sID() ) );
					...
					case _XGI_FC_WEAPON3 :
#endif
						{
							...
							//강도 // 사용안함
							pPersonalStore_Window->m_TradeItem[i].Set_m_ucWeaponStrength( _XGameItem::GetNature( pPersonalStore_Window->m_TradeItem[i].m_cType, pPersonalStore_Window->m_TradeItem[i].Get_m_sID() ) );
#endif
						}
						break;
#ifdef _XTS_ITEM_OPTIMIZATION
					...
					case _XGI_FC_CLOTHES3 :
#endif
						{
							...
							if( pPersonalStore_Window->m_TradeItem[i].m_cSecond != _XGI_SC_CLOTHES_BACKPACK )
							{
								int j = 0 ; for( j = 0; j < 4; ++j)
								{
									pPersonalStore_Window->m_TradeItem[i].Set_m_sSocket(j, psshoplistresult->ItemList[i].sSocket[j] );
	...
	case 251: // 사는 사람의 인벤이 부족해서 물건을 사서 옮길 수가 없다.
		{
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_NOTENOUGHINV));
	...
	default: // 
		{
			...
			{
				g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_cType = psbuyresult->BuyItem.cType;
				g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_cSecond = psbuyresult->BuyItem.cSecond;
				...
				g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_cThird = _XGameItem::GetThird(psbuyresult->BuyItem.cType, psbuyresult->BuyItem.sID);
				g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucCount = psbuyresult->BuyItem.Item_Money.S_NormalItem.ucCount;
				...
				if( psbuyresult->BuyItem.ucIncludeLimit == 1 )
				{
					...
					{
						g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucYear		= psbuyresult->BuyItem.uselimit.ucYear;	
						g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucMonth	= psbuyresult->BuyItem.uselimit.ucMonth;	
						g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucDay		= psbuyresult->BuyItem.uselimit.ucDay;	
						g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucHour		= psbuyresult->BuyItem.uselimit.ucHour;	
						g_pLocalUser->m_UserItemList[psbuyresult->cReturn].m_ucMinute	= psbuyresult->BuyItem.uselimit.ucMinute;
					}				
				}
			}
			...
			if( pPersonalStore_Window )
			{
				...
				pPersonalStore_Window->m_TradeItem[pPersonalStore_Window->m_SelectedSlotNumber].m_cType = -1;
	...
	else if( g_pLocalUser->m_PersonalStoreState == _XUSER_PERSONALSTORE_OWNER_OPEN )
	{
		if( g_pLocalUser->m_Uniqueid == pstraderesultbroadcasting->OwnerUID )
		{
			...
			if( pPersonalStore_Window )
			{
				//if( pPersonalStore_Window->GetShowStatus() )
				{			
					int type = pPersonalStore_Window->m_TradeItem[ pstraderesultbroadcasting->cSoldSlot ].m_cType;
	...
	if( pstraderesulttoowner->cReturn == 1 )
	{
		...
		{			
			...
			if( pPersonalStore_Window )
			{
				//if( pPersonalStore_Window->GetShowStatus() )
				{
					int type = pPersonalStore_Window->m_TradeItem[ pstraderesulttoowner->cPSWIndex ].m_cType;
					...
					{
						if( g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucCount != 0)
						{
							int buynumber = pstraderesulttoowner->Item.Item_Money.S_NormalItem.ucCount - g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucCount;
							
							g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_cType = pstraderesulttoowner->Item.cType;
							g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_cSecond = pstraderesulttoowner->Item.cSecond;
							...
							g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_cThird = _XGameItem::GetThird(pstraderesulttoowner->Item.cType, pstraderesulttoowner->Item.sID);						
							...
							{								
								g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucYear		= pstraderesulttoowner->Item.uselimit.ucYear;	
								g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucMonth	= pstraderesulttoowner->Item.uselimit.ucMonth;	
								g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucDay		= pstraderesulttoowner->Item.uselimit.ucDay;	
								g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucHour		= pstraderesulttoowner->Item.uselimit.ucHour;	
								g_pLocalUser->m_UserItemList[pstraderesulttoowner->cIndex].m_ucMinute	= pstraderesulttoowner->Item.uselimit.ucMinute;
							}
						}
						else
						{
		...
		else
		{
			...
			{					
				...
				{				
					...
					g_pLocalUser->m_UserItemList[slotnumber].m_cType	= Item.cType;
					g_pLocalUser->m_UserItemList[slotnumber].m_cSecond	= Item.cSecond;
					g_pLocalUser->m_UserItemList[slotnumber].Set_m_sID( Item.sID );
					
					g_pLocalUser->m_UserItemList[slotnumber].m_cThird	= _XGameItem::GetThird(Item.cType, Item.sID);  
					g_pLocalUser->m_UserItemList[slotnumber].m_ucCount	= Item.Item_Money.S_NormalItem.ucCount;
					...
					{						
						g_pLocalUser->m_UserItemList[slotnumber].m_ucYear		= Item.uselimit.ucYear;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucMonth		= Item.uselimit.ucMonth;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucDay		= Item.uselimit.ucDay;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucHour		= Item.uselimit.ucHour;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucMinute	= Item.uselimit.ucMinute;
					}
				}
				else
				{
