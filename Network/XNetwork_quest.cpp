...
void _XNetwork::MSGQuestData(MSG_QUEST_DATA* questdata)
{
	...
	int i = 0 ; for( ; i < _MAX_CURQUEST_NUM_ ; ++i)
	{
	...
	for( i = 0 ; i < questcompletedata->sCountCQ ; ++i)
	{
		...
		else if(questcompletedata->QuestData[i].ucNodeID != 0)
		{
			...
			{				
				...
				int j = 0 ; for( j = 0; j < ID_QUEST_SYNC-1; ++j ) // 제룡행 퀘스트는 진행중인 퀘스트에서 제외 -> 5개 
				{
	...
	case 0 :	// 초기화
		{
			...
			{
				if(questresult->ItemDelete[i].slot >= 0)
				{
					...
					{
						g_pLocalUser->m_UserItemList[questresult->ItemDelete[i].slot].m_ucCount = questresult->ItemDelete[i].number;
					}
					...
					if(questresult->ItemDelete[i].slot >= _XINVENTORY_SLOTTYPE_AMOR && questresult->ItemDelete[i].slot <= _XINVENTORY_SLOTTYPE_WEAPON_2)
					{
						...
						ChangeModel(lparam, g_pLocalUser->m_UserItemList[questresult->ItemDelete[i].slot].m_cType);
					}
...
void _XNetwork::QuestItemReceive(MSG_QUESTITEM_RECEIVE* questitem)
{
	...
	if( questitem->userItem.cSlot != 0 )
	{
			...
			if( questitem->userItem.Item_Money.S_MoneyItem.uiCount>= 0 )
			{
				if( g_pLocalUser->m_Money <= questitem->userItem.Item_Money.S_MoneyItem.uiCount )
				{
					...
					if( g_QuestScriptManager.m_bReceiveQuestData )
					{
						...
						sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORKQUEST_1767), additemcount);
				...
				else
				{
					...
					{
						TCHAR noticestring[256];
						sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORKQUEST_1768), additemcount);
						InsertChatString( noticestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET);		
					}		
				}
				return;
		...
		if( questitem->userItem.sID != 0 )
		{
			...
			int currentitemcount = g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount;
			g_pLocalUser->m_UserItemList[inventoryindex].Reset();

			if(g_pLocalUser->m_UserItemList[inventoryindex].m_cType == questitem->userItem.cType && 
				g_pLocalUser->m_UserItemList[inventoryindex].Get_m_sID() == questitem->userItem.sID)
			{
				if(g_pLocalUser->m_UserItemList[inventoryindex].IsStackable())
				{
					...
					g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount = questitem->userItem.Item_Money.S_NormalItem.ucCount;
				}
				else
				{
					additemcount = questitem->userItem.Item_Money.S_NormalItem.ucCount - currentitemcount;
					
					g_pLocalUser->m_UserItemList[inventoryindex].m_cType = questitem->userItem.cType;
					...
					g_pLocalUser->m_UserItemList[inventoryindex].m_cSecond = 
						_XGameItem::GetItemSecond(questitem->userItem.cType, questitem->userItem.sID);
...
//Author : 양희왕 //breif : 2007년 10월 24일 추가					
					...
					//breif : 08/03/04 수정..
					if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[inventoryindex].m_cType) )
					{
						
						_XGI_WeaponItem_Property* tempWeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[inventoryindex].m_cType);
...
// 여기까지
					...
					{						
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucYear	= questitem->userItem.uselimit.ucYear;	
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucMonth	= questitem->userItem.uselimit.ucMonth;	
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucDay	= questitem->userItem.uselimit.ucDay;	
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucHour	= questitem->userItem.uselimit.ucHour;	
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucMinute	= questitem->userItem.uselimit.ucMinute;
					}
				}
			}
			else
...
void _XNetwork::QuestItemRemove(MSG_QUESTITEM_REMOVE* questitem)
{
	...
	int i = 0 ; for(; i < questitem->cSlotNum; ++i )
	{
...
//					g_pLocalUser->m_UserItemList[questitem->ResItem[i].slot].m_sID, questitem->ResItem[i].slot);
				...
				g_QuestScriptManager.RecheckWearItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[questitem->ResItem[i].slot].m_cType, g_pLocalUser->m_UserItemList[questitem->ResItem[i].slot].Get_m_sID() );
			...
			g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[questitem->ResItem[i].slot].m_cType, g_pLocalUser->m_UserItemList[questitem->ResItem[i].slot].Get_m_sID() );
		...
		{
			g_pLocalUser->m_UserItemList[ questitem->ResItem[i].slot ].m_ucCount = questitem->ResItem[i].number;
		}		
	}
...
void _XNetwork::QuestLearnSkill(MSG_LEARN_SKILL* questmartial)
{
	...
	TCHAR noticestring[256];
	sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_LEARNMARTIAL), g_SkillProperty[questmartial->sSkillID]->skillName); // // 무공 %s(을)를 배웠습니다.
...
void _XNetwork::QuestForgetSkill(MSG_FORGET_SKILL* questmartial)
{
	int i = 0 ; for( ; i < _XDEF_MAX_LEARNSKILL ; ++i)
	{
		if(g_pLocalUser->m_SkillList[i].m_sSkillID == questmartial->sSkillID)
		{
			...
			TCHAR noticestring[256];
			sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REMOVEMARTIAL), g_SkillProperty[questmartial->sSkillID]->skillName);
	...
	case ID_JINEXP :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_NETWORKQUEST_1769));
	...
	case ID_VITALITY :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_CINFO_HEALTHENERGY));
		}
		break;
	case ID_ZEN :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_CINFO_INNERENERGY));
		}
		break;
	case ID_SOUL :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_CINFO_HEART));
		}
		break;
	case ID_HEART :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_CINFO_DEX));
		}
		break;
	case ID_STAMINA :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_CINFO_STR));
		}
		break;
	case ID_VITALPOWER :
		{
			g_pLocalUser->m_CharacterInfo.Set_current_lifepower( queststatus->uiUpdatedValue );
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_NETWORK_HEALTHENERGY));
		}
		break;
	case ID_INNERPOWER :
		{
			g_pLocalUser->m_CharacterInfo.Set_current_forcepower( queststatus->uiUpdatedValue );
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_NETWORK_INNERENERGY));
		}
		break;
	case ID_SOULPOWER :
		{
			g_pLocalUser->m_CharacterInfo.Set_current_concentrationpower( queststatus->uiUpdatedValue );
			sprintf(statusname,_XGETINTERFACETEXT(ID_STRING_NETWORK_SOULENERGY));
		}
		break;
	case ID_HEARTPOWER :
		{
	...
	case ID_HONOR :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_NETWORK_REPUTATION));
		}
		break;
	case ID_FAME :
		{
			...
			sprintf(statusname, _XGETINTERFACETEXT(ID_STRING_NEW_2636));//_T("악명")
#endif
...
void _XNetwork::QuestJoinGroup(MSG_QUEST_JOIN_GROUP* questjoingroup)
{
	...
	case _XGROUP_NANGIN :
		{
			...
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_00));
		}
		break;
	case _XGROUP_GAEBANG :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_GAEBANG;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_01));
		}
		break;
	case _XGROUP_BEEGOONG :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_BEEGOONG;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_02));
		}
		break;
	case _XGROUP_SORIM :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_SORIM;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_03));
	...
	case _XGROUP_NOCKRIM :
		{
			...
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_04));
		}
		break;
	case _XGROUP_MOODANG :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_MOODANG;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_05));
		}
		break;
	case _XGROUP_MAKYO :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_MAKYO;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_06));
		}
		break;
	case _XGROUP_SEGA :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_SEGA;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_07));
		}
		break;
	case _XGROUP_NOFIXED :
		{
			g_pLocalUser->m_CharacterInfo.groupindex = _XGROUP_NOFIXED;
			sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_GROUPNAME_08));
		}
		break;
	...
	{
		TCHAR noticestring[256];
		sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_JOINGROUP), groupname);
...
void _XNetwork::QuestInventoryFullProcess(MSG_QUEST_AUTH_RESULT* questresult)
{
	...
	if( invenfull )
	{
		...
		{		
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKQUEST_1770),questresult->cNeedInven);
...
void _XNetwork::MSGQuestNodeBackItemResult(MSG_QUEST_NODEBACK_ITEM_RESULT* questcheck)
{
	...
	int i = 0 ; for( ; i < 5 ; ++i)
	{
		if( questcheck->removeskill[i].sSkillID != 0 )
		{
			...
			{
				int j = 0 ; for( j = 0 ; j < _XDEF_MAX_LEARNSKILL ; ++i)
				{
					if(g_pLocalUser->m_SkillList[j].m_sSkillID == questcheck->removeskill[i].sSkillID)
					{
						...
						TCHAR noticestring[256];
						sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REMOVEMARTIAL), g_SkillProperty[questcheck->removeskill[i].sSkillID]->skillName);
						InsertChatString( noticestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
...
//	}
	...
	for( i = 0; i < 95; ++i)
	{
		if( questcheck->removeitem[i].slot != -1 )
		{
			...
			{
				g_pLocalUser->m_UserItemList[ questcheck->removeitem[i].slot ].m_ucCount = questcheck->removeitem[i].number;
			}
		}
	}
	...
	for( i = 0; i < 10; ++i)
	{
		if( questcheck->additem[i].cSlot != 0 )
		{
			if( questcheck->additem[i].sID != 0 )
			{
				...
				if(g_pLocalUser->m_UserItemList[inventoryindex].m_cType == questcheck->additem[i].cType && 
					g_pLocalUser->m_UserItemList[inventoryindex].Get_m_sID() == questcheck->additem[i].sID)
				{
					if(g_pLocalUser->m_UserItemList[inventoryindex].IsStackable())
					{
						g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount += questcheck->additem[i].Item_Money.S_NormalItem.ucCount;
					}
					else
					{
						g_pLocalUser->m_UserItemList[inventoryindex].m_cType = questcheck->additem[i].cType;
						...
						{							
							g_pLocalUser->m_UserItemList[inventoryindex].m_ucYear	= questcheck->additem[i].uselimit.ucYear;	
							g_pLocalUser->m_UserItemList[inventoryindex].m_ucMonth	= questcheck->additem[i].uselimit.ucMonth;	
							g_pLocalUser->m_UserItemList[inventoryindex].m_ucDay	= questcheck->additem[i].uselimit.ucDay;	
							g_pLocalUser->m_UserItemList[inventoryindex].m_ucHour	= questcheck->additem[i].uselimit.ucHour;	
							g_pLocalUser->m_UserItemList[inventoryindex].m_ucMinute	= questcheck->additem[i].uselimit.ucMinute;
						}
					}
				}
				else
