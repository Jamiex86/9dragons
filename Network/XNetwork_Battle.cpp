...
void _XNetwork::MSGRegionDamage(MSG_REGION_DAMAGE* regiondamage)
{
	...
	if(regiondamage->ucAttackerType == 1)	// attacker가 pc
	{
		if(regiondamage->usAttackerID == g_pLocalUser->m_Uniqueid)
		{
			...
			int i = 0 ; for( ; i < regiondamage->ucObjectCount ; ++i)
			{
				...
				if(g_ViewBattleDetail)
				{
					if(regiondamage->usAttackerID == g_pLocalUser->m_Uniqueid)
					{
						...
						if(regiondamage->Result[i].ucResult == 1)
							sprintf(messagestring, "RegionDamage To monster CRITICAL damage[%d]", pMob->m_HealthPercentage-regiondamage->Result[i].cMobVitalRate);
						else if(regiondamage->Result[i].ucResult == 2)
							sprintf(messagestring, "RegionDamage To monster MISS damage[%d]", pMob->m_HealthPercentage-regiondamage->Result[i].cMobVitalRate);
						else
							sprintf(messagestring, "RegionDamage To monster HIT damage[%d]", pMob->m_HealthPercentage-regiondamage->Result[i].cMobVitalRate);
				...
				if(pUser)
				{
					...
					if(g_ViewBattleDetail)
					{
						if(regiondamage->usAttackerID == g_pLocalUser->m_Uniqueid)
						{
							...
							if(regiondamage->Result[i].ucResult == 1)
								sprintf(messagestring, "RegionDamage To User CRITICAL");
							else if(regiondamage->Result[i].ucResult == 2)
								sprintf(messagestring, "RegionDamage To User MISS");
							else
								sprintf(messagestring, "RegionDamage To User HIT");
	...
	if(minigamectrl->sMiniGameID == 1)
	{
		...
		if(minigamectrl->usID == g_pLocalUser->m_Uniqueid)
		{
			...
			if(pMeditation_Window)
			{
				...
				{
					_XDWINBATTLELOG("==== MiniGameCtrl [운기조식] CancelFromMonster LocalUser");
	...
	case en_eff_ctrl_rs_del_skill :
		{
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2366), g_SkillProperty[effectctrl->id]->skillName);
	...
	case en_eff_ctrl_rs_del_effect :
		{
			...
			if(iter_table != g_CharacterStateTable.end())
			{
				...
				TCHAR messagestring[256];
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2366), g_CharacterStateTable[effectctrl->id]->name);
...
{
	int i = 0 ; for( ; i < effectmybhinfo->cCount ; ++i)
	{
		...
		int j = 0 ; for( j = 0 ; j < effectcount ; ++j)
		{
...
{
	int i = 0 ; for( ; i < effectmyentryinfo->cCount; ++i)
	{
...
#endif
		int i = 0 ; for( ; i < effectotherinfo->cCount ; ++i)
		{
...
{
	int i = 0 ; for(; i < pEffectSpecialInfo->cCount; i++ )
	{
		...
		if(g_pState_Window)
		{
			g_pState_Window->InsertStateListFromOther(_XST_SPECIALBUFF, pEffectSpecialInfo->entry[i].skillID, pEffectSpecialInfo->entry[i].slot);
		}
	}
}
	...
	case en_eff_ctrl_rs_del_extra :
		{
			...
			else
			{
				...
				if(iter_table != g_CharacterStateTable.end())
				{
					...
					TCHAR messagestring[256];
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2366), g_CharacterStateTable[effectextra->id]->name);
					InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGENERAL);
				}
			}
		}
...
{
	int i = 0 ; for( ; i < effectextra->cCount; i++)
	{
...
//Author : 양희왕 //breif : 수정
	...
	if(g_SkillProperty[addspecial->id]->detailInfo[0].effect)
	{
		...
		if(g_pState_Window)
		{
			g_pState_Window->InsertStateListFromOther(_XST_SPECIALBUFF, addspecial->id, addspecial->slot);
		}
	}
}
...
//Author : 양희왕 //breif : 수정
	...
	{
		int index = 0 ; for(  index = 8054; index <= 8057; index++ )
		{
	...
	if(g_pState_Window)
	{
		g_pState_Window->InsertStateListFromOther(_XST_SPECIALBUFF, pUpdateSpecial->id, pUpdateSpecial->slot);
	}
}
...
//author : 양희왕
		...
		if(pNPCBuff_Window)
		{
			...
			if(pNPCBuff)
			{
				...
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2367), pNPCBuff->name);
				...
				if(needMoney > 0)
				{
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2368), needMoney);
		...
		if(g_pLocalUser->m_SelectedSkillID > 0)
		{
			...
			else
			{
				if(g_pLocalUser->m_CurrentBattleState != _XUSER_STATE_NONE && g_pLocalUser->m_CurrentBattleState != _XUSER_STATE_LAST)
				{
					...
					TCHAR messagestring[256];
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBATTLE_2603), g_SkillProperty[g_pLocalUser->m_SelectedSkillID]->skillName);	
...
void _XNetwork::MSGPKWinLoseRecord(MSG_PK_WIN_LOSE_RECORD* record)
{
	if(g_pPKRecord_Window)
	{
		...
		int i = 0 ; for( ; i < record->ucWinCount ; ++i)
		{
