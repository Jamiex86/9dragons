...
void _XNetwork::MSGNicknameListReceive(MSG_NIC_LIST* nicknamelistreceive)// 별호 리스트를 반환
{
	...
	if( g_pLocalUser )
	{
		...
		int i = 0 ; for(; i < _NIC_COUNT_TOTAL ; ++i )
		{
	...
	if( g_pCInfo_Window )
		memset( g_pCInfo_Window->m_FavoritesNickNameIndexList, 0, sizeof(int)*10 );
}
...
void _XNetwork::MSGRankHiding(MSG_RANK_HIDING* hidingresult)
{
	...
	{
		sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1642), g_pLocalUser->m_CharacterInfo.charactername );				
	}
	else if( hidingresult->cMode == MSG_RANK_HIDING::_HIDE_OFF )
	{
		sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1643), g_pLocalUser->m_CharacterInfo.charactername );				
	}
		...
		case MSG_HONOR_PLAYER::_TAKE_FROM_PLAYER :			// PC로 부터 명성을 부여받았습니다.
			{
				if( honorresult->iHohorValue > 0 )	// 선행
				{
					TCHAR tempstr[256];
					memset(tempstr, 0, sizeof(TCHAR)*256);
					sprintf( tempstr, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1647), honorresult->cToCharacName );
		...
		case MSG_HONOR_PLAYER::_TAKE_FROM_SERVER :
			{
				if( honorresult->iHohorValue > 0 )	// 선행
				{
					CHAR tempstr[256];
					memset(tempstr, 0, sizeof(TCHAR)*256);
					sprintf( tempstr, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1649), honorresult->iHohorValue );
				...
				else	// 악행
				{
					CHAR tempstr[256];
					memset(tempstr, 0, sizeof(TCHAR)*256);
					sprintf( tempstr, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1650), honorresult->iHohorValue );
