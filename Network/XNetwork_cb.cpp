...
void _XNetwork::MSGCBOnwerScheduleList(MSG_CB_OWNER_SCHEDULE_LIST* schedulelist)
{
	...
	int i = 0 ; for( ; i < max_count_of_challenge_list ; ++i)
	{
...
void _XNetwork::MSGCBChangeTimeFail(MSG_CB_CHANGE_TIME_FAIL* changetime)
{
	...
	if(changetime->errorCode == 0)
	{
		...
		sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3209)); // 도전 시간을 지정할 수 있는 기간이 지났습니다.
	...
	else
	{
		...
		sprintf(msg, "Unknown Error [ErrorCode : %d]", changetime->errorCode);
		InsertChatString(msg, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
...
void _XNetwork::MSGCBChallengeFail(MSG_CB_CHALLENGE_FAIL* challengeresult)
{
	...
	case 0 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1761));	// 금액이 부족합니다.
		}
		break;
	case 1 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3210));	// 더 이상 도전할 수 없습니다.
		}
		break;
	case 2 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3222));	// 단주나 회주가 아닙니다.
		}
		break;
	case 3 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3223));	// 현재 장주 이므로 도전할 수 없습니다. 
		}
		break;
	case 4 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NETWORK_REQUESTLATER));	// 잠시 후에 다시 시도하십시오.
		}
		break;
	case 6 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3268));	//장원 도전 시간이 아닙니다.
		}
		break;
	case 7 :
		{
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NEW_3396));		//"다른 장원에 이미 도전 신청 하였습니다."
		}
		break;
	default :
	case 5 :
		{
#ifdef _XDWDEBUG
			sprintf(msg, "system error [ErrorCode : %d]", challengeresult->errorCode);
#endif
		}
		break;
	}
...
void _XNetwork::MSGCBProgReadyEvent(MSG_CB_PROG_READY_EVENT* readyevent)
{
	...
	int i = 0 ; for( ; i < readyevent->ucAttackerCount ; ++i)
	{
...
void _XNetwork::MSGCBProgStartEvent(MSG_CB_PROG_START_EVENT* startevent)
{
	...
	int i = 0 ; for( ; i < startevent->ucAttackerCount ; ++i)
	{
...
*/
	...
	if(!g_pLocalUser->m_bIsBossBattle)	// "결"안에서는 메시지 보여주지 않음
	{
		...
		if(pCBResultNotice_Window)
		{
			...
			if(bMonsterWin)
			{
				sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3295), _XGETINTERFACETEXT(g_CBManager.GetCastleNameTextId(g_CurrentZoneInfoIndex)));//%s을 함락시키지 못하였습니다.
			}
			else
			{
				TCHAR groupname[128];
				...
				if(endevent->ucGroupType == 0)
				{
					sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_ORGANIZATION_ORGANIZATIONINFO2), endevent->cGroupName);
				}
				else if(endevent->ucGroupType == 1)
				{
					sprintf(groupname, _XGETINTERFACETEXT(ID_STRING_NEW_3125), endevent->cGroupName);
				}

				sprintf(messagestr, g_StringDataBase.CompositeString(_XSTRINGDB_SN_EODSTRING_XSR,ID_STRING_NEW_3255, 
...
void _XNetwork::MSGCBThreatStartOK(MSG_CB_THREAT_START_OK* threatok)
{
	...
	TCHAR messagestr[256];
	sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3256), g_pLocalUser->m_CharacterInfo.charactername);	//%s님이 장주를 협박하고 있습니다.
...
void _XNetwork::MSGCBThreatStartNotice(MSG_CB_THREAT_START_NOTICE* notice)
{
	...
	if(notice->ucGroupType == 0)
	{
		...
		if(orginfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3363), orginfo->m_cName);	//"%s단이 장주를 협박하고 있습니다."
			bMsg = TRUE;
		}
	}
	else if(notice->ucGroupType == 1)
	{
		...
		if(allyinfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3364), allyinfo->m_cName);	//"%s회가 장주를 협박하고 있습니다."
			bMsg = TRUE;
		}
	}
	...
	if(!bMsg)
	{
		...
		{	
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3257));	//방어측이 장주를 협박하고 있습니다.
		}
		else
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3258));	//공격측이 장주를 협박하고 있습니다.
		}
	}
...
void _XNetwork::MSGCBThreatCancelOK(MSG_CB_THREAT_CANCEL_OK* cancel)
{
	...
	if(cancel->ucGroupType == 0)
	{
		...
		if(orginfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3365), orginfo->m_cName);	//"장주가 %s단의 협박에 항복하지 않았습니다."
			bMsg = TRUE;
		}
	}
	else if(cancel->ucGroupType == 1)
	{
		...
		if(allyinfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3366), allyinfo->m_cName);	//"장주가 %s회의 협박에 항복하지 않았습니다."
			bMsg = TRUE;
		}
	}
	...
	if(!bMsg)
	{
		sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3259));	//장주가 협박에 항복하지 않았습니다.
	}
...
void _XNetwork::MSGCBThreatCompleteNotice(MSG_CB_THREAT_COMPLETE_NOTICE* complete)
{
	...
	if(complete->ucGroupType == 0)
	{
		...
		if(orginfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3367), orginfo->m_cName);	//"장주가 %s단의 협박에 항복하였습니다."
			bMsg = TRUE;
		}
	}
	else if(complete->ucGroupType == 1)
	{
		...
		if(allyinfo)
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3368), allyinfo->m_cName);	//"장주가 %s회의 협박에 항복하였습니다."
			bMsg = TRUE;
		}
	}
	...
	if(!bMsg)
	{
		...
		{
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3261));		//장주가 방어측의 협박에 항복하였습니다.
		}
		else
		{	
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3262));		//장주가 공격측의 협박에 항복하였습니다.
		}
	}
...
void _XNetwork::MSGCBInfoCurrentInfo(MSG_CB_INFO_CURRENT_INFO* info)
{
	...
	int i = 0 ; for( ; i < info->ucAttackerCount ; ++i)
	{
...
void _XNetwork::MSGCBEtcLoudChat(MSG_CB_ETC_LOUD_CHAT* chat)
{
	...
	sprintf(chatmsg, "%s : ", chat->cCharName);
...
//			g_CBManager.SetCastleTaxMoney(g_pLocalUser->m_nCastleMapId, answertake->uiLeftTotalMoney);
			...
			TCHAR msg[128];
			sprintf(msg, _XGETINTERFACETEXT(ID_STRING_NETWORK_GETMONEY), getmoney);		//%d금을 획득했습니다
...
void _XNetwork::MSGCBRecordInfoRes(MSG_CB_RECORD_INFO_RES* res)
{
	...
	if(g_pGroup_Window)
	{
		...
		sprintf(g_pGroup_Window->m_strLastWarDate, g_StringDataBase.CompositeString(_XSTRINGDB_SN_EODSTRING_XSR, ID_STRING_ORGANIZATION_2102,
			&year, &month, &day));
