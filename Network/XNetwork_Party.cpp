...
void _XNetwork::MSGPartyJoinReq(MSG_PARTY_JOIN_REQ* joinparty)
{
	...
	if( !g_bCommunityOptionRejectParty )
	{
		...
		else
		{
			...
			else
			{
				...
				{
					TCHAR tostring[256];
					sprintf(tostring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REQUESTPARTY), joinparty->cTo);
										
					_XWindow_RequestParty* pRequestParty_Window = (_XWindow_RequestParty*)g_MainWindowManager.FindWindow(_XDEF_WTITLE_REQUESTPARTY);
					if(pRequestParty_Window)
					{
						if(joinparty->cTo[0] != 0)
							strcpy(pRequestParty_Window->m_RequestCharacterName, joinparty->cTo);
					}
	...
	else// 동행신청 거부 옵션에 켜 있을 때 
	{
		...
		strcpy(partyresponse.cToName, joinparty->cTo);
...
void _XNetwork::MSGPartyResponse(MSG_PARTY_JOIN_RESPONSE* partyresponse)
{
	...
	{		
		...
		case -1 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_FAILPARTY));
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
		...
		case 1 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REJECTPARTY), partyresponse->cToName);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
		case 2 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_NOTARGET));
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
		case 3 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKPARTY_1673));
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_OTHERPARTY), partyresponse->cToName);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
		case 4 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_FULLPARTY));
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
		...
		case 6 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_OTHERWORK), partyresponse->cToName);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
		case 7 :
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKPARTY_1674));
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			break;
	...
	{		
		if( partyresponse->cCmd ==1 )
		{
			...
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REJECTPARTY), _XGETINTERFACETEXT(ID_STRING_NEW_2659) );	// _T("복면인(覆面人)")
			}
			else
			{
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REJECTPARTY), partyresponse->cToName);
	...
	case 4 : 
		{
			...
			else
			{
				...
				sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_EXPULTIONOTHER), g_pLocalUser->m_PartyInfo[partyetc->cInfo].playername);
				InsertChatString(noticestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
				
				memset(g_pLocalUser->m_PartyInfo[partyetc->cInfo].playername, 0, sizeof(TCHAR)*13);
	...
	case 5 :	// S->C cInfo = 멤버번호 : 탈퇴 하였음.
		{
			...
			else
			{
				...
				if( g_pLocalUser->m_PartyInfo[partyetc->cInfo].isLeader )
				{
					...
					TCHAR noticestring[256];
					sprintf(noticestring,_XGETINTERFACETEXT(ID_STRING_NETWORKPARTY_1677), g_pLocalUser->m_PartyInfo[partyetc->cInfo].playername);
					InsertChatString(noticestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
				...
				else
				{
					...
					TCHAR noticestring[256];
					sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_SECESSIONOTHER), g_pLocalUser->m_PartyInfo[partyetc->cInfo].playername);
					InsertChatString(noticestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
	...
	case 6 : // S->C cInfo = 멤버번호. 파티장이 권한을 위임
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKPARTY_1678), g_pLocalUser->m_PartyInfo[partyetc->cInfo].playername );			
				...
				{					
					int i = 0 ; for(; i < _XDEF_MAXPARTYMEMBERCOUNT ; ++i)
					{
						if( pEnterBossRoom_Window->m_EnterPartyMember[i].PartyMemberNo == -1)
						{
							...
							int j = 0 ; for( j = 0; j < _XDEF_MAXPARTYMEMBERCOUNT ; ++j )
							{
...
#ifdef _XDEF_INTERFACE_RENEWAL_071002_KUKURI	
							#ifdef _ACCLAIM_RUBICONADSYSTEM	
							pMemberMiniBar_Window->MoveWindow( 50, 170 + ypos + ((g_ViewRubiconAD)?g_RubiconADWindowSize.cy:0));
							#else
							pMemberMiniBar_Window->MoveWindow( 50, 170 + ypos);
							#endif
#else
							#ifdef _ACCLAIM_RUBICONADSYSTEM	
							pMemberMiniBar_Window->MoveWindow( 14, 170 + ypos + ((g_ViewRubiconAD)?g_RubiconADWindowSize.cy:0));
							#else
