...
void _XNetwork::MSGMatchListModeListUpdate(MSG_MATCH_LIST_INFO* matchlistinfo)
{
	...
	int i = 0 ; for( ; i < matchlistinfo->ucCount ; ++i)
	{
...
void _XNetwork::MSGMatchReadyChallengeFromOther(MSG_MATCH_READY_CHALLENGE_FROM* matchreadyfrom)
{
	...
	case en_matchtype_pvp :
		{
			sprintf(matchtype, "%s", _XGETINTERFACETEXT(ID_STRING_MATCH_2494));//"개인전"
		}
		break;
	case en_matchtype_group_deathmatch :
		{
			sprintf(matchtype, "%s", _XGETINTERFACETEXT(ID_STRING_MATCH_2495));//"집단전"
		}
		break;
	}

	TCHAR messagestring[256];
	sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2924), matchreadyfrom->cName, matchtype); //"%s님이 %s를 요청하고 있습니다. 승락하시겠습니까?"
...
void _XNetwork::MSGMatchReadyStartMakeTable(MSG_MATCH_READY_START_MAKE_TABLE* matchreadymaketable)
{
	...
	int i = 0 ; for( ; i < 2 ; ++i)
	{
		...
		else
		{
			...
			strcpy(g_pLocalUser->m_myMatchMemberInfo.cName, matchreadymaketable->MemberInfo[i].cName);
		}
	}
	...
	case en_match_ready_update_recordtype :
		{
			...
			if(pMatchTablePVP_Window)
			{
				if(g_pLocalUser->m_myMatchTableInfo.cRecordType == 0)
					sprintf(pMatchTablePVP_Window->m_strRecordType, _XGETINTERFACETEXT(ID_STRING_NEW_2939));//_T("공식비무")
				else if(g_pLocalUser->m_myMatchTableInfo.cRecordType == 1)
					sprintf(pMatchTablePVP_Window->m_strRecordType, _XGETINTERFACETEXT(ID_STRING_NEW_2940));	//_T("비공식비무")
			}
	...
	case en_match_ready_update_matchtype :
		{
			...
			if(g_pLocalUser->m_myMatchTableInfo.cMatchType == 1)
			{
				if(pMatchTablePVP_Window)
				{
					sprintf(pMatchTablePVP_Window->m_strMatchType, _XGETINTERFACETEXT(ID_STRING_MATCH_2486));//_T("개인전")
				}
			}
	...
	case en_match_ready_update_matchtime :
		{
			...
			if(pMatchTablePVP_Window)
			{
				if(g_pLocalUser->m_myMatchTableInfo.cMatchTime == 2)
					sprintf(pMatchTablePVP_Window->m_strMatchTime, _XGETINTERFACETEXT(ID_STRING_MATCH_2511));//_T("2분")
				else if(g_pLocalUser->m_myMatchTableInfo.cMatchTime == 3)
					sprintf(pMatchTablePVP_Window->m_strMatchTime, _XGETINTERFACETEXT(ID_STRING_MATCH_2512));//_T("3분")
			}
	...
	case en_match_ready_update_disable_hyperrun :
		{
			...
			if(pMatchTablePVP_Window)
			{
				if(g_pLocalUser->m_myMatchTableInfo.cUseHyperrun == 0)
					sprintf(pMatchTablePVP_Window->m_strUseHyperrun, _XGETINTERFACETEXT(ID_STRING_ADDSAVEITEM_USE));//_T("사용")
				else if(g_pLocalUser->m_myMatchTableInfo.cUseHyperrun == 1)
					sprintf(pMatchTablePVP_Window->m_strUseHyperrun, _XGETINTERFACETEXT(ID_STRING_MATCH_2513));//_T("금지")
				else if(g_pLocalUser->m_myMatchTableInfo.cUseHyperrun == 2)
					sprintf(pMatchTablePVP_Window->m_strUseHyperrun, _XGETINTERFACETEXT(ID_STRING_NEW_2941));//_T("피격시 중지")
			}
	...
	case en_match_ready_update_keep_buff :
		{
			...
			if(pMatchTablePVP_Window)
			{
				if(g_pLocalUser->m_myMatchTableInfo.cUseBuff == 0)
					sprintf(pMatchTablePVP_Window->m_strUseBuff, _XGETINTERFACETEXT(ID_STRING_MATCH_2514));//_T("현 상태 유지")
				else if(g_pLocalUser->m_myMatchTableInfo.cUseBuff == 1)
					sprintf(pMatchTablePVP_Window->m_strUseBuff, _XGETINTERFACETEXT(ID_STRING_GMCOMMAND_2074));	//_T("초기화")
			}
...
void _XNetwork::MSGMatchCtrlStart(MSG_MATCH_CTRL_START* matchctrlstart)
{
	if(matchctrlstart->ucStartWith == 0)
	{
		...
		TCHAR messagestring[256];
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2943), g_AreaManager.GetAreaName(_XDEF_AREACATEGORY_PVPAREA, g_pLocalUser->m_MatchFieldIndex));//"'%s'에서 비무가 시작됩니다."
...
//	_XDWINPRINT("Stop special BGM ==");
	...
	{
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2944), g_pLocalUser->m_CharacterInfo.charactername, g_pLocalUser->m_myMatchMemberInfo.cName);//"%s님과 %s님의 비무가 무승부로 종료되었습니다."
		g_pLocalUser->m_MatchResult = 2;
	}	
	else if(g_pLocalUser->m_myMatchTeamID == matchctrlend->ucWinTeam)
	{
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2945), g_pLocalUser->m_CharacterInfo.charactername);//"%s님이 승리하셨습니다."
	...
	{
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2945), g_pLocalUser->m_myMatchMemberInfo.cName);//"%s님이 승리하셨습니다."
...
void _XNetwork::MSGMatchNoticeInform(MSG_MATCH_NOTICE_INFORM* matchnoticeinform)
{
	...
	if(pUser)
	{
		...
		strcpy( pUser->m_stringMatchPR, matchnoticeinform->cInform );
	}
}
	...
	if(pNoticeLarge_Window)
	{
		...
		if(iter_notice != pNoticeLarge_Window->m_mapMatchNoticeInfo.end())
		{
			...
			if(currentnotice)
			{
				...
				strcpy(currentnotice->cChiefName0, matchnoticesbinfo->cChiefName0);
				strcpy(currentnotice->cChiefName1, matchnoticesbinfo->cChiefName1);
				...
				for(i = 0 ; i < 2 ; ++i)
				{
					...
					strcpy(currentnotice->Player[i].cName, matchnoticesbinfo->Player[i].cName);
		...
		else
		{
			...
			strcpy(newnotice->cChiefName0, matchnoticesbinfo->cChiefName0);
			strcpy(newnotice->cChiefName1, matchnoticesbinfo->cChiefName1);
			...
			for(i = 0 ; i < 2 ; ++i)
			{
				...
				strcpy(newnotice->Player[i].cName, matchnoticesbinfo->Player[i].cName);
			...
			sprintf(newnotice->strLevelNamePlayer0, "%s %d%s",
			...
			sprintf(newnotice->strLevelNamePlayer1, "%s %d%s",
...
void _XNetwork::MSGFMNotice(MSG_FM_NOTICE* fmnotice)
{
	if(fmnotice->usTextArg != 0)
	{
		...
		sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3729), fmnotice->usTextArg);//%d분 후에 비무대회가 시작됩니다.
		...
		{
			TCHAR messagestr[256];
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3740), g_LevelName[fromlevel]);//%s 성취단계  분들은 합비 비무장으로 모여주시길  바랍니다.
			InsertChatString(messagestr, _XDEF_CHATMESSAGECOLOR_SYSTEMGENERAL);
		}
		else
		{
			TCHAR messagestr[256];
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3741), g_LevelName[fromlevel], g_LevelName[tolevel]);	//%s-%s 성취단계 분들은 합비 비무장으로 모여주시길 바랍니다.
			InsertChatString(messagestr, _XDEF_CHATMESSAGECOLOR_SYSTEMGENERAL);
		}
...
{
	TCHAR messagestr[256];
	sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NEW_3731), packet->cWinnerName);//%s님이 비무대회에서 우승하였습니다.
...
void _XNetwork::MSGFMSetNick(MSG_FM_SET_NICK* packet)
{
	...
	if( pNickName_Window )
	{
		if(packet->usNick == 0)//제거
		{
			...
			TCHAR tempNIck[256];	
			memset(tempNIck, 0, sizeof(TCHAR)*256 );
			...
			int i = 0 ; for(; i < g_MaxNickNameCount; ++i)
			{
				if(g_NickNameInfoTable[i].usIndex == g_pLocalUser->m_nWinnerNickName)
				{
					...
					{
						//현재 달고 있는 별호였음
						sprintf(tempNIck, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1629), g_NickNameInfoTable[i].cNickName);
						...
						sprintf(tempNIck, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1630), g_NickNameInfoTable[i].cNickName);
		...
		else//부여
		{
			...
			int i = 0 ; for(; i < g_MaxNickNameCount; ++i)
			{
				...
				{
					sprintf(tempNIck, _XGETINTERFACETEXT(ID_STRING_NETWORKNICKNAME_1631), g_NickNameInfoTable[i].cNickName);
	...
	case 1:
		{
			...
			if(pFMInfo_Window && pFMInfo_Window->GetShowStatus())
			{
				...
				sprintf(pFMInfo_Window->m_szDate, g_StringDataBase.CompositeString(_XSTRINGDB_SN_EODSTRING_XSR,ID_STRING_NEW_2654, 
				...
				if(fromlevel == tolevel)
				{
					sprintf(pFMInfo_Window->m_szLevel, "%s", g_LevelName[fromlevel]);
				}
				else
				{
					sprintf(pFMInfo_Window->m_szLevel, "%s-%s", g_LevelName[fromlevel], g_LevelName[tolevel]);
				}
			}
		}
		break;
