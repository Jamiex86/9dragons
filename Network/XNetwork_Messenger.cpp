		...
		if( pMessengerDialog_Window )
		{
			...
			if(strcmpi(chattingmessage->cFromCharacName, pMessengerDialog_Window->m_ToCharactername) == 0)
			{
				if(strcmpi(chattingmessage->cToCharacName, g_pLocalUser->m_CharacterInfo.charactername) == 0)
				{
					...
					memset(speaker, 0, sizeof(TCHAR)*256);
					sprintf( speaker, "%s :", chattingmessage->cFromCharacName );
...
void _XNetwork::MMSGMessengerRequestFriend(MMSG_MAKE_FRIEND_REQUEST*joinfriend)
{
	if( !g_bCommunityOptionRejectFriend )
	{
		...
		{
			TCHAR tostring[256];
			sprintf(tostring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_REQUESTFRIEND), joinfriend->cCharacName);
			
			_XWindow_RequestParty* pRequestParty_Window = (_XWindow_RequestParty*)g_MainWindowManager.FindWindow(_XDEF_WTITLE_REQUESTPARTY);
			if(pRequestParty_Window)
			{
				memset( pRequestParty_Window->m_RequestCharacterName, 0, sizeof(TCHAR)*13 );
				strcpy( pRequestParty_Window->m_RequestCharacterName , joinfriend->cCharacName );
			}
...
void _XNetwork::MMSGMessengerMemberInfo(MMSG_FRIEND_LIST_RES* messengermemberinfo)
{
	...
	int i = 0 ; for(; i < messengermemberinfo->cCount; ++i ) 
	{
...
void _XNetwork::MMSGMessengerResponseMakeFriend(MMSG_MAKE_FRIEND_RES *responsemakefriend)
{
	...
	case 0:// 성공	(양쪽다 통보)
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_REGISTFRIEND), responsemakefriend->cCharacName);
	...
	case 1:// 상대가 거부 (거부당한 캐릭터(cFromCharacName)에게 통보)
		{			
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_REJECTFRIEND), responsemakefriend->cCharacName );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
	...
	case 2:// 2 : 상대 캐릭터가 없거나.. 로그오프중..
		{			
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_FAILFRIEND) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
	...
	case 3:// 3 : 상대 캐릭터의 친구 허용 인원 초과..
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_EXCESSFRIEND) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	...
	case 5:// 5 : 이미 등록된 캐릭터
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_ALREADYFRIEND) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	case 6:// 6 : GM 캐릭터 입니다
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2775));	// "GM에게는 친구 신청을 할 수 없습니다."
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	default:
...
void _XNetwork::SendMessengerChatMessage( LPSTR chatmessage, int windowid, int messagemode   )
{
	...
	if( !pMessengerDialog_Window->m_bGMChatting ) // 대화 상대가 GM이면 메신져 친구 리스트를 검사하지 않는다.
	{
		...
		if( ChatUserStatus == 0 )
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_LOGOFFTARGET));
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
	...
	memset(speaker, 0, sizeof(TCHAR)*256);
	sprintf( speaker, "%s :", g_pLocalUser->m_CharacterInfo.charactername );
						...
						do 
						{
							...
							if(currentfriend)
							{
								if( strcmp( currentfriend->m_FriendName, resultcutfriend->cCharacName ) == 0 )
								{
									...
									TCHAR messagestring[256];
									memset( messagestring, 0, sizeof(TCHAR)*256 );
									sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_DELETEFRIEND), resultcutfriend->cCharacName);
									pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		...
		{			
			...
			if( !pMessenger_Window->m_MessengerGroupList.listEmpty() )
			{
				...
				if(messengergroup && responsemakegroup->cGroupName[0] != 0)
				{
					...
					TCHAR messagestring[256];
					memset( messagestring, 0, sizeof(TCHAR)*256 );
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_CREATEGROUP), responsemakegroup->cGroupName);
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
				}				
			}	
			else // 그룹이 없을 때 
	...
	case 1:		// 실패
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_FAILCREATE) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	}
			...
			do 
			{
				...
				if(currentgroup)
				{
					if( strcmp( currentgroup->m_GroupName, responsedeletegroup->cGroupName ) == 0 )
					{
						...
						TCHAR messagestring[256];
						memset( messagestring, 0, sizeof(TCHAR)*256 );
						sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_DELETEGROUP), responsedeletegroup->cGroupName);
						pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
						break;	
					}				
	...
	case 1:		// 실패
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_FAILMOVEGROUP) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	}
...
{	
	...
	case 1:		// 실패
		{			
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_FAILRENAME) );
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
		}
		break;
	}
							...
							if(currentfriend)
							{
								if( strcmp( currentfriend->m_FriendName, userstateupdate->cCharacName ) == 0 )
								{
									...
									if( currentfriend->m_FriendState == 0 )// 로그오프 상태
									{
										TCHAR messagestring[256];
										memset( messagestring, 0, sizeof(TCHAR)*256 );
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKMESSENGER_1627), currentfriend->m_FriendName );
									...
									else if( currentfriend->m_FriendState == 1 )// 로그인 상태
									{
										TCHAR messagestring[256];
										memset( messagestring, 0, sizeof(TCHAR)*256 );
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKMESSENGER_1628), currentfriend->m_FriendName );
...
void _XNetwork::MMSGEmailListResponse(MMSG_MAIL_LIST_RES* responsemaillist)
{
	...
	if( notreadcount != 0 )//새로 도착한 편지가 있을 경우 -> 수신함
	{
		...
		TCHAR messagestring[256];
		memset( messagestring, 0, sizeof(TCHAR)*256 );
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKMESSENGER_2370), notreadcount);
		pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);	
...
void _XNetwork::MMSGEmailTextResponse(MMSG_MAIL_TEXT_RES* responsemailtext)
{
	...
	if( tempmailindex != -1 )
	{
		...
		{
			memset(pEmail_Window->m_TempEmailInfo[tempmailindex].cFrom, 0, sizeof(TCHAR) * 13);			
			strncpy( pEmail_Window->m_TempEmailInfo[tempmailindex].cFrom, _XGETINTERFACETEXT(ID_STRING_ORGANIZATION_ORGANIZATION), 12 );
			//%s 영웅단에서 강제추방 되셨습니다.
			sprintf( pEmail_Window->m_TempEmailInfo[tempmailindex].cText, _XGETINTERFACETEXT(ID_STRING_ORGANIZATION_2092), responsemailtext->mailInfo.cText );
		}
		else
		{
			strncpy( pEmail_Window->m_TempEmailInfo[tempmailindex].cFrom, responsemailtext->mailInfo.cFrom, 13 );
	...
	if( responsemailtext->mailInfo.cMode == _MailInfo::_SYSTEM_MSG_OR_KICK)
	{
		strcpy( pEmail_Window->m_EmailSender, _XGETINTERFACETEXT(ID_STRING_ORGANIZATION_ORGANIZATION) );
	}
	else
	{
		strcpy( pEmail_Window->m_EmailSender, responsemailtext->mailInfo.cFrom );
	}
...
void _XNetwork::MMSGWhisperChatMessage( MMSG_CHAT* chattingmessage )
{
	if( !g_bCommunityOptionRejectWhisper )
	{
		if(strcmpi(chattingmessage->cToCharacName, g_pLocalUser->m_CharacterInfo.charactername) == 0)
		{
			...
			TCHAR	chatbuffer[256];
			memset( chatbuffer, 0, sizeof(TCHAR)*256 );		 /* header size */	
			...
			TCHAR	tempbuffer[256];
			memset(tempbuffer, 0, sizeof(TCHAR)*256);
			sprintf( tempbuffer, "%s:(%s) ", chattingmessage->cFromCharacName, chattingmessage->cToCharacName );
	...
	case _NOT_FIND_USER_:		// 귓속말 상대 없음
		{
			...
			memset(targetname, 0, sizeof( TCHAR ) * 256 );
			sprintf( targetname, _XGETINTERFACETEXT(ID_STRING_NETWORKMESSENGER_2381), whisperchatmsg->cToCharacName );
	...
	case en_rf_recv_request_recall :			// 친구 소환 요청을 서버에서 받음.
		{
			...
			{
				sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3562), tempresult->cFromName, _XGETINTERFACETEXT(ID_STRING_NEW_3489) ); //상태이상으로 %s님의 %s요청을 거절 합니다.   , 호출
			...
			{
				sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3563), tempresult->cFromName, _XGETINTERFACETEXT(ID_STRING_NEW_3489) ); // %s님이 친구목록에 없으므로 %s요청을 거절 합니다.   , 호출
				g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
			...
			sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3540), //%s님께서 자신의 위치(%s)로 호출하였습니다. 이동하시겠습니까?
				tempresult->cFromName, g_ZoneInfoTable[tempresult->ucFromZoneIndex-1].zonename2 );
	...
	case en_rf_recv_answer_yes : //요청 수락 패킷을 받음 //상대방이 요청을 수락 했으므로 마법진을 만든다 //서버에서 딜레이후 타겟이 이동됨
		{
			...
			g_pLocalUser->m_UserItemList[tempresult->ucItemSlot].m_ucCount = tempresult->ucLeftCount; //지정된 슬롯에.. 남은개수를 저장
			...
			//0개
			if( g_pLocalUser->m_UserItemList[tempresult->ucItemSlot].m_ucCount < 1 )
	...
	case en_rf_recv_answer_no : // 요청 거부 패킷을 받음.
		{
			...
			case 0 : //거절을 선택했을 때.
				{
					sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3543), tempresult->cTargetName ); //%s님께서 호출을 거부 하였습니다.
					//메세지 박스 
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			...
			case 1 : //( 요청받았을 때, 상태검사룰 수행하여, 못 받는 상태이면 이 값으로 보낸다. )
				{
					sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3544), _XGETINTERFACETEXT(ID_STRING_NEW_3489) ); //상대방의 상태 이상으로 %s할 수 없습니다., 호출
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), tempString, TRUE );
				}
				break;
	...
	case en_rf_fail_to_request : // 요청이 실패했다
		{
			...
			case en_rf_answer_no : // no 선택.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3549) ); //선택된 상대가 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_no_target :	// 0 타겟이 없다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3550), tempresult->cTargetName);  //%s님께서 로그인 중이 아닙니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_no_item :  // 1 소환 아이템 없다. 
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3551) );//소환령이 존재하지 않습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_wrong_point :	// 소환 지점의 위치가 이상하다. 
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3552) );//소환 지점의 위치가 잘못되어 있습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_enemy : // 적대 세력이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3799) );//적대관계에 있어 호출/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_my_con_trade : // (자신은) 거래중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3553) );//거래 중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_my_con_combat : // (자신은) 장원전 참여중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3554) );//장원전 참여중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_my_con_zone :	// (자신은) 패왕동/호출할 수 없는 지역이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3555) );//패왕동 지역이므로 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_my_con_exbattle : // (자신은) 결 진행중이다. 
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3556) );//결 진행 중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_target_con_trade :	// 거래중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3557) );//상대방이 거래 중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_target_con_combat : // 장원전 참여중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3558) );//상대방이 장원전 참여중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_target_con_zone : // 패왕동/호출할 수 없는 지역이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3559) );//상대방이 패왕동 지역이므로 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_target_con_exbattle : // 결 진행중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3560) );//상대방이 결 진행 중에는 소환/이동을 할 수 없습니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_my_busy : 	// (자신은) 다른 캐릭터와 친구 소환 중이다.             
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3548) );//다른 대상과 친구 소환 중 입니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			case en_rf_fail_target_busy : // 4 다른 대상과 친구 소환 중이다.
				{
					sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3561) );//상대방이 다른 대상과 친구 소환 중 입니다.
					g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
				break;
			}
	...
	case en_rf_recv_request_goto : //이동 요청이 들어옴
		{
			...
			if( !pRecallItemWindow->CharRecallCondition() ) 
			{
				sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3562), tempresult->cFromName, _XGETINTERFACETEXT(ID_STRING_NEW_3490) ); //상태이상으로 %s님의 %s요청을 거절 합니다.   , 이동
				g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
			...
			if(!pRecallItemWindow->CheckCharacterName( tempresult->cFromName ))
			{
				sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3563), tempresult->cFromName, _XGETINTERFACETEXT(ID_STRING_NEW_3490) ); // %s님이 친구목록에 없으므로 %s요청을 거절 합니다.   , 이동
				g_NetworkKernel.InsertChatString( tempString, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
			...
			sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3545), tempresult->cFromName ); //%s님께서 이동을 요청 하였습니다. 수락하시겠습니까?
	...
	case en_rf_recv_goto_answer_no : //이동 거절 당했다..
		{
			...
			case 0 : //거절을 선택했을 때.
				{
					sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3547), tempresult->cTargetName ); //%s님께서 이동요청을 거부 하였습니다.
					//메세지 박스 
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), tempString, TRUE );
				}
			...
			case 1 : //( 요청받았을 때, 상태검사룰 수행하여, 못 받는 상태이면 이 값으로 보낸다. )
				{
					sprintf( tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3544), _XGETINTERFACETEXT(ID_STRING_NEW_3490) );//상대방의 상태 이상으로 %s할 수 없습니다. , 이동
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), tempString, TRUE );
				}
				break;
