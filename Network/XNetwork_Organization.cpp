...
void _XNetwork::MSGOrganizationInfoResult(MSG_ORGANIZATION_INFO_RES* inforesult)
{
	if( inforesult->bCreateAble )	// 새로 창설
	{
		...
		else
		{
			...
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1652), inforesult->_organizationBase.m_cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_INVITE_ :					// 타조직 짱이 자기네 조직에 들어오라고 권유
		{
			...
			else
			{
				TCHAR message[256];
				memset( message, 0, 256 );
				sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1661), clientresult->cName );
				if( g_pGroup_Window )
	...
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_ENTRUST_ :					// 니가 짱 먹으라고 권유..
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1662), g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName );
			if( g_pGroup_Window )
				g_pGroup_Window->SetSendTargetName(clientresult->cName);
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_INVITE_OK_ :					// 조직에 가입하겠다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1663), clientresult->cName, g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_INVITE_NO_ :					// 조직에 가입하지 않겠다.		
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1664), clientresult->cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_ENTRUST_OK_ :				// 짱 하겠다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1665), clientresult->cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_ENTRUST_NO_ :				// 짱 안하겠다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1666), clientresult->cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_LOGOFF_ :					// 상대방은 로그오프중..
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1667), clientresult->cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
	case MSG_ORGANIZATION_CLIENT_::_OR_REQ_ALREADY_INNER_ :				// 이미 다른조직에 몸담고 있다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKORGANIZATION_1668), clientresult->cName );
			_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
			pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), message, TRUE );
		}
		break;
...
//	char            cName[_XDEF_ORGANIZATIONNAMELENGTH+1];
	if( alience_request_from->ucType == 0 )
	{
		TCHAR message[256];
		memset( message, 0, 256 );
		sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3110), alience_request_from->cName );
	...
	else if( alience_request_from->ucType == 1 )
	{
		TCHAR message[256];
		memset( message, 0, 256 );
		sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3114), alience_request_from->cName );
...
void _XNetwork::MSGAlienceRequestNo(MSG_ALLY_MODE_RESPONSE_NO* alience_request_no)
{
	TCHAR message[256];
	memset( message, 0, 256 );
	sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3417)); //"동맹 요청을 거부하였습니다."
...
void _XNetwork::MSGAlienceMade(MSG_ALLY_MODE_IS_MADE* alience_made)
{
	...
	if( g_pGroup_Window )
	{
		...
		if( g_pGroup_Window->m_OrganizationBaseInfo.m_iIndex == alience_made->iOr2Index )
		{
			...
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3115), alience_made->iOr1Name );			
		...
		else // 영웅단 동맹원일때 
		{
			...
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3113), alience_made->iOr2Name );			
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMGENERAL, TRUE);
		}
	}
...
void _XNetwork::MSGAlienceEnter(MSG_ALLY_MODE_ORG_ENTER_ALLY* alience_enter)
{
	...
	{		
		...
		int i = 0  ; for( i =0; i < alience_enter->ucNoOrg; ++i) 
		{
...
//		
	if( g_pGroup_Window )
	{
		...
		int i = 0  ; for( i =0; i < alience_allinfo->ucNoOrg; ++i) 
		{
			__ally_client_info* pAlienceMember = new __ally_client_info;
...
void _XNetwork::MSGAlienceLeaveMember(MSG_ALLY_MODE_NOTICE_LEAVE_ORG* alience_leavemember)
{
	...
	if( g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_iIndex == alience_leavemember->iLeaveOrgIndex )
	{
		...
		TCHAR message[256];
		memset( message, 0, 256 );
		sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3122), g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName );			
		InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		...
		for(iter_alience = g_pGroup_Window->m_listAlienceMember.begin() ; iter_alience != g_pGroup_Window->m_listAlienceMember.end() ; )
		{
			...
			if( aliencemember )
			{
				...
				{				
					...
					TCHAR message[256];
					memset( message, 0, 256 );
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3121), aliencemember->iOrName );			
					InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
...
void _XNetwork::MSGAlienceScattered(MSG_ALLY_MODE_SCATTERED* alience_scattered)
{
	...
	if( g_pGroup_Window )
	{
		...
		TCHAR message[256];
		memset( message, 0, 256 );
		sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3123), g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName );			
		InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
...
{	
	...
	TCHAR message[256];
	memset( message, 0, 256 );
	sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3175) );			
	InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
}
	...
	case en_ally_mode_error_we_already_ally :		
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3176) );			//이미 방회에 속해 있습니다
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_they_already_ally :		// 대상 영웅단은 이미 동맹을 맺고 있다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3177) );			//대상 영웅단은 이미 방회에 속해 있습니다.
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_cannot_ally :			// 흑/백도 의 이유로 동맹을 맺을 수 없다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3178) );			//흑도와 백도는 동맹을 맺을 수 없습니다
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_not_found_info :		// 요청한 동맹을 찾을 수 없습니다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3179) );			//요청한 방회를 찾을 수 없습니다.
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;		
	case en_ally_mode_error_cannot_find_user :		// 유저를 찾을 수 없다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3180) );			//유저를 찾을 수 없습니다.
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_i_am_busy :				// 현재 요청중..
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message,_XGETINTERFACETEXT(ID_STRING_NEW_3181)  );			//_T("현재 방회 요청 중 입니다.")
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_busy :					// 대상이 다른 일 중.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3182) );			//_T("대상이 다른 일을 하고 있습니다.")
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	...
	case en_ally_mode_error_not_hero :				// 대상은 영웅단 단주가 아니다.
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message,_XGETINTERFACETEXT(ID_STRING_NEW_3184) );			//_T("영웅단 단주가 아닙니다.")
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_in_combat :
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message,_XGETINTERFACETEXT(ID_STRING_NEW_3394) );	//"현재 장원전 신청 상태이므로 방회를 결성할 수 없습니다."
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
	case en_ally_mode_error_target_in_combat :
		{
			TCHAR message[256];
			memset( message, 0, 256 );
			sprintf( message,_XGETINTERFACETEXT(ID_STRING_NEW_3395) );	//"상대 영웅단은 현재 장원전 신청 상태이므로 방회를 결성할 수 없습니다."
			InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
		}
		break;
