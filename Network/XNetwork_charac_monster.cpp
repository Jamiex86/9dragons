...
void _XNetwork::MSGResSpellEffect(MSG_RES_SPELLEFFECT* spelleffect)
{
	...
	if(pMob)
	{
		int i = 0 ; for( ; i < MAX_SPELLEFFECTDATA_PER_PACKET ; ++i)
		{
...
void _XNetwork::MSGSetLevel(MSG_SET_LEVEL* setlevel)
{
	...
	if(((CEmperorOfDragonsApp*)gApp)->m_proc_maingame.m_Initialized && (prevjinlevel != g_pLocalUser->m_CharacterInfo.Get_jinlevel()))
	{
		...
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_GETEXP));
...
void _XNetwork::MSGLevelUpReq(MSG_LEVEL_UP_RESULT* levelupresult)
{
	...
	{
		TCHAR messagestring[256];
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_WARNLEVELUP));
	...
	else
	{
		...
		// Item Check
		int i = 0  ; for( i = _XINVENTORY_SLOTTYPE_AMOR ; i <= _XINVENTORY_SLOTTYPE_WEAPON_2 ; ++i)
			CheckValidItem(i);
	}
}
		...
		case 5 :
			{
				...
				TCHAR messagestring[256];
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_WARNGETPOINT));
...
void _XNetwork::MSGCharacCtrl(MSG_CHARAC_CTRL* characctrl)
{
	...
	VIEWPACKETLOG
		_XDWPACKETLOG("[CharacCtrl] uid [%d], flag [%d]", characctrl->uid, characctrl->cFlag);
	...
	case charac_state_en_wound_rate :
		{
			...
			else
			{
				...
				if(g_pLocalUser->m_CharacterInfo.Get_externalwoundrate() <= 0)
				{
					if(g_pState_Window)
					{
						FLOAT percentage = (((FLOAT)characsetstate->state.sWoundRate)/(FLOAT)g_pLocalUser->m_CharacterInfo.Get_max_lifepower())*100.0f;
						if(percentage >= 40.0f)
...
//			g_pLocalUser->m_CharacterInfo.internalwoundrate = characsetstate->state.sInsideWoundRate;
			...
			else
			{
				...
				if(g_pLocalUser->m_CharacterInfo.Get_internalwoundrate() <= 0)
				{
					if(g_pState_Window)
					{
						FLOAT percentage = (((FLOAT)characsetstate->state.sInsideWoundRate)/(FLOAT)g_pLocalUser->m_CharacterInfo.Get_max_forcepower())*100.0f;
						if(percentage >= 40.0f)
...
void _XNetwork::MSGSkillInfo(MSG_SKILL_INFO* skillinfolist)
{
	...
	int i = 0 ; for( ; i < skillinfolist->ucLearnSkillCount ; ++i)
	{
	...
	case en_skill_ctrl_mode_add :
		{
			...
			TCHAR messagestring[256];
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_LEARNMARTIAL), g_SkillProperty[skillctrl->ctrl.S_skill_add.sAddSkillID]->skillName); // 무공 %s(을)를 배웠습니다.
	...
	case en_skill_ctrl_mode_delete :
		{
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_REMOVEMARTIAL), g_SkillProperty[skillid]->skillName); // 무공 %s이(가) 삭제되었습니다.
	...
	case en_skill_ctrl_mode_del_bonus_hyperrun :
		{
			...
			{
				TCHAR messagestring[256];
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3811), g_SkillProperty[g_pLocalUser->m_EventSkillList.m_sSkillID]->skillName); //%s의 사용 시간이 종료되었습니다.
...
{
	int i = 0 ; for( ; i < cooltime->ucCoolTimeListCount ; ++i)
	{
		// cooldown time 처리
