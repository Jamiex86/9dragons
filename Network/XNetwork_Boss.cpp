			...
			case _RESULT_CODE_SUCCESS:
				{
					...
					int i = 0 ; for( ; i < 3 ; ++i)
					{
						if(bossresult->_sUsedItem[i].itemSlotNum >= _XINVENTORY_SLOTTYPE_POCKETSTART)
						{
							...
							{
								g_pLocalUser->m_UserItemList[bossresult->_sUsedItem[i].itemSlotNum].m_ucCount = bossresult->_sUsedItem[i].itemCount;
							}
						}
					}
#endif
...
void _XNetwork::MSGBossRoomType(MSG_EX_BATTLE_TYPE_INFO* bossroomtpye)
{
	...
	if( pEnterBossRoom_Window )
	{
		int i = 0 ; for(; i < _XDEF_BOSSROOM_NEEDITEMTYPE ; ++i )
		{
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKBOSS_1621), _XGameItem::GetItemName(bossroomtpye->_sItem[i].cType, bossroomtpye->_sItem[i].sID) );
