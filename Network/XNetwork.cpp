...
//#define _XDEF_NPROTECT_GUARD_TIME_LOG
	...
	long _ApexNetSendToGameServer(const char * pBuffer,int nLen)
	{
		...
		if( !g_NetworkKernel.SendPacket( MSG_NO_SECURITY, (int)pBuffer, nLen ) ) return -1;
...
void _XServerAddressManager::XOREncrypt( void )
{
	...
	int i = 0 ; for(; i < sizeof(_XSERVERINFOSTRUCT); ++i )
	{
...
void _XPatchVersionManager::XOREncrypt( void )
{
	...
	int i = 0 ; for(; i < sizeof(_XPATCHVERSIONINFOSTRUCT); ++i )
	{
...
BOOL _XCheckLoginServerIP( LPCTSTR szHostName ) // check virtual server
{
	...
	else
	{
		if( ( pHostEntry = gethostbyname( szName ) ) != NULL )
		{
			...
			sprintf( sAddress, _T("%s"), inet_ntoa( inAddr ) );
		}
		else
		{
			return FALSE;
	...
	default :
		{
			...
			sprintf( messagebuffer, "Network error : invalid login packet number [%d]", pPacket[2] );
			g_MessageBox.SetMessageColor( D3DCOLOR_ARGB( 255, 255, 0, 0 ) );
...
//Last updated: 07/01/22
	...
	if( pChattingWindowDefault )	
	{
		/*if( pChattingWindowDefault->m_DockingSystemMessageButton.GetCheck() )	// 함께
		{
			...
			int i = 0 ; for(; i < splitinfo.splitcount; i++ )
			{			
...
//Last updated: 07/01/22
	...
	if( g_pLocalUser->m_ChatMuteTime > 0 )
	{
		...
		if( minutes > 0 )
		{
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CHATTINGDEFAULT_2009), minutes );
		}
		else
		{
			int seconds = (g_pLocalUser->m_ChatMuteTime % 60000) / 1000;
			sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CHATTINGDEFAULT_2010), seconds );
		}
	...
	if( /*pChattingWindowDefault->m_ChatMode == _XCHATMODE_WHISPER ||*/ messagemode == _XDEF_CHATMESSAGECOLOR_WHISPER )
	{
	...
	strcpy( tempbuffer, g_pLocalUser->m_CharacterInfo.charactername );	
	strcat( tempbuffer, _T(":") );
	if( /*pChattingWindowDefault->m_ChatMode == _XCHATMODE_WHISPER || */messagemode == _XDEF_CHATMESSAGECOLOR_WHISPER )
	{
		TCHAR tempidbuffer[256];
		sprintf(tempidbuffer, "(%s) ", pChattingWindowDefault->m_WhisperTargetName );
		strcat( tempbuffer, tempidbuffer );
	}
	tabwidth = g_XBaseFont->GetWidth(tempbuffer);
	strcat( tempbuffer, tcChatData );	
...
//			}
	...
	*/
	...
	if( /*pChattingWindowDefault->m_ChatMode == _XCHATMODE_WHISPER || */messagemode == _XDEF_CHATMESSAGECOLOR_WHISPER )
	{
		...
		strcpy( packet.cToCharacName, pChattingWindowDefault->m_WhisperTargetName );
	...
	else
	{
		...
		if( messagemode == _XDEF_CHATMESSAGECOLOR_PARTY )
		{
			...
			strcpy( packet.cName, g_pLocalUser->m_CharacterInfo.charactername );
		}
		else if( messagemode == _XDEF_CHATMESSAGECOLOR_USERCLAN )
		{
			...
			case en_present_mode_notice :
				{
					...
					sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3800), //"%d레벨 선물이 도착 했습니다. %d개의 행낭 공간이 필요합니다.", 
			...
			case en_present_mode_recv_present :	// s -> c : 선물을 준다.
				{
					...
					g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_cType = tempresult->UserInven.cType;
					g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_cSecond = tempresult->UserInven.cSecond;
					...
					g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_cThird = _XGameItem::GetThird(tempresult->UserInven.cType, tempresult->UserInven.sID);
					g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucCount = tempresult->UserInven.Item_Money.S_NormalItem.ucCount;
					...
					{				
						g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucYear		= tempresult->UserInven.uselimit.ucYear;	
						g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucMonth	= tempresult->UserInven.uselimit.ucMonth;	
						g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucDay		= tempresult->UserInven.uselimit.ucDay;	
						g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucHour		= tempresult->UserInven.uselimit.ucHour;	
						g_pLocalUser->m_UserItemList[tempresult->UserInven.cSlot].m_ucMinute	= tempresult->UserInven.uselimit.ucMinute;
					}

					//sprintf( messagestring, "%d레벨 선물을 받으셨습니다.", tempresult->iInfo );
					sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3052)); //축하드립니다. 이벤트상품을 받으셨습니다. 행낭을 확인하세요.
					InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET, TRUE);
			...
			case en_present_mode_error :	// 에러 메시지 : req_present 의 응답으로 사용된다. 
				{
					...
					case en_present_error_no_event :	// 그런 이벤트 없다. 
						{
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_1599)); //이미 끝난 이벤트 입니다.
							InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
						}
						break;
					case en_present_error_already_recv :	// 이미 받아간 선물.
						{
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3801), tempresult->iInfo ); //"%d레벨 선물은 이미 받으셨습니다."
							InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
						}
						break;
					case en_present_error_need_inven :	// 인벤 공간이 부족. 다시 시도할 것.
						{
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1991) ); //행낭에 공간이 부족합니다. 비우고 다시 시도하세요.
							InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
						}
						break;
					}
	...
	case MSG_NO_US_AD_REWARD_ACK :
		{
			...
			if( pCurrentProcess == &(pApp->m_proc_maingame) )
			{
				...
				int i = 0 ; for(; i < us_ad_reward_ack->ucItemCnt; i++ )
				{
	...
	case MSG_NO_NATIONAL_DIST :
		{
			...
			if(ndist->ucMode == en_national_vina_sds_ctrl)
			{
			...
			case PKTMODE_PRESENT_CASH_INVEN_ACK :
				{
					...
					if( pCashBox_Window )
					{
						...
						int i = 0  ; for( i =0; i < ack.ucRowCount; ++i) 
						{
			...
			case PKTMODE_MOVE_PRESENT_PACKAGE_ACK :
				{
					...
					{						
						...
						{
							int i = 0  ; for( i =0; i < ack.ucPkgCnt; ++i) 
							{
								memcpy(&ack.iOrderNo, pPacket + iOffset, sizeof(int));  iOffset += sizeof(int);
								memcpy(&ack.iProductNo, pPacket + iOffset, sizeof(int));  iOffset += sizeof(int);
								memcpy(&ack.iPkgIdx, pPacket + iOffset, sizeof(int));  iOffset += sizeof(int);
								...
								int j = 0 ; for( j =0; j < ack.ucItemCnt; ++j) 
								{
									...
									if( ack.cSlot != -1 )
									{
										...
										if(g_pLocalUser->m_UserItemList[ack.cSlot].m_cType == ack.cType&& 
											g_pLocalUser->m_UserItemList[ack.cSlot].Get_m_sID() == ack.sId)
										{
											if(g_pLocalUser->m_UserItemList[ack.cSlot].IsStackable())
											{
												g_pLocalUser->m_UserItemList[ack.cSlot].m_ucCount = ack.ucNum;
											}
											else
											{									
												g_pLocalUser->m_UserItemList[ack.cSlot].m_cType = ack.cType;
												g_pLocalUser->m_UserItemList[ack.cSlot].Set_m_sID( ack.sId );
												g_pLocalUser->m_UserItemList[ack.cSlot].m_ucCount = ack.ucNum;
												
												// Second Type setting 
												g_pLocalUser->m_UserItemList[ack.cSlot].m_cSecond = 
													_XGameItem::GetItemSecond(ack.cType, ack.sId);
							...
							}// end int i = 0  ; for( i =0; i < ack.ucPkgCnt; ++i) 
							pCashBox_Window->SetListItem();
						}
			...
			case en_pitem_add :
				{
					...
					{				
						...
						else
						{
							...
							int i = 0  ; for( i =0; i < addperioditem.ucPkgCnt; ++i) 
							{
								...
								int j = 0 ; for( j =0; j < addperioditem.ucItemCnt; ++j) 
								{
									...
									{					
										...
										g_pLocalUser->m_UserItemList[slotnumber].m_cType	= addperioditem.client_item.cType;
										g_pLocalUser->m_UserItemList[slotnumber].m_cSecond	= addperioditem.client_item.cSecond;
										g_pLocalUser->m_UserItemList[slotnumber].Set_m_sID(addperioditem.client_item.sID);
										
										g_pLocalUser->m_UserItemList[slotnumber].m_cThird	= _XGameItem::GetThird(addperioditem.client_item.cType, addperioditem.client_item.sID);  
										g_pLocalUser->m_UserItemList[slotnumber].m_ucCount	= addperioditem.client_item.Item_Money.S_NormalItem.ucCount;
										...
										{
											g_pLocalUser->m_UserItemList[slotnumber].m_ucYear		= addperioditem.client_item.uselimit.ucYear;	
											g_pLocalUser->m_UserItemList[slotnumber].m_ucMonth		= addperioditem.client_item.uselimit.ucMonth;	
											g_pLocalUser->m_UserItemList[slotnumber].m_ucDay		= addperioditem.client_item.uselimit.ucDay;	
											g_pLocalUser->m_UserItemList[slotnumber].m_ucHour		= addperioditem.client_item.uselimit.ucHour;	
											g_pLocalUser->m_UserItemList[slotnumber].m_ucMinute	= addperioditem.client_item.uselimit.ucMinute;
										}
									}
									else
									{
			...
			case en_pitem_set_period :
				{
					...
					{
						g_pLocalUser->m_UserItemList[slotnumber].m_ucYear		= setperiod.period.ucYear;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucMonth		= setperiod.period.ucMonth;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucDay		= setperiod.period.ucDay;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucHour		= setperiod.period.ucHour;	
						g_pLocalUser->m_UserItemList[slotnumber].m_ucMinute		= setperiod.period.ucMinute;
					}
			...
			case en_pitem_remove :
				{
					...
					char itemtype	= g_pLocalUser->m_UserItemList[removeperioditem.slot].m_cType;
					...
					TCHAR	messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);					
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3160),_XGameItem::GetItemName(itemtype, itemid));//%s의 사용기간이 만료되어 삭제되었습니다.
			...
			case en_storage_pitem_remove :
				{
					...
					TCHAR	messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);					
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3160),_XGameItem::GetItemName(strremove->cType, strremove->sID));//%s의 사용기간이 만료되어 삭제되었습니다.
					g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS); 
				}
				break;
#endif
...
//	_XLog("Send packet : %d", type );
	...
	case MSG_NO_ZONE_MOVE :
		{	
			...
			sprintf( _messagestring, "Packet send time : %d : %d", m_MovePacket_lastSendedTime, g_LocalSystemTime - m_MovePacket_lastSendedTime );
	...
	case MSG_NO_INVEN_MOVE :
		{
			...
			if(g_pInventory_Window)
			{
				...
				invenmovepacket.ucSendCount = (unsigned char)g_pInventory_Window->m_MouseSlotItem.m_ucCount;
				...
				if(param1 == 0)
				{
					...
					for(iter_item = pWarehouse_Window->m_TempItemList.begin() ; iter_item != pWarehouse_Window->m_TempItemList.end() ; ++iter_item)
					{
						...
						if(pWarehouse_Window)
						{
							...
							{
								if( g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType == _XGI_FC_QUEST )
								{
								...
								if( _XGameItem::CheckItemTempField4( pWarehouseItem->m_cType ,
								...
								stritemsave.Storage[i].usNumber = (u_char)pWarehouseItem->m_ucCount;
				...
				if(param1 == 0)
				{
					...
					for(iter_item = pWarehouse_Window->m_TempItemList.begin() ; iter_item != pWarehouse_Window->m_TempItemList.end() ; ++iter_item)
					{
						...
						if(pWarehouse_Window)
						{
							...
							stritemeject.Storage[i].usNumber = (u_char)pWarehouseItem->m_ucCount;
			...
			case en_socket_gamble_mode_boxgamble:
				{
					...
					int i = 0 ; for(  i = startpos; i < socketitem.cAddonCounter+startpos; ++i )
					{
	...
	case MSG_NO_NPC_TRADE_REQ :
		{
			...
			if(pNPCTrade_Window && pInventory_Window)
			{
				...
				case en_npc_trade_mode_sell :
					{
						// 팔기
						npctradereq.ucCount = (unsigned char)pInventory_Window->m_MouseSlotItem.m_ucCount;
						itemNo = pInventory_Window->m_MouseSlotItem.m_InventoryNumber;
						cType = pInventory_Window->m_MouseSlotItem.m_cType;
				...
				case en_npc_trade_mode_buy :
					{
						// 사기
						npctradereq.ucCount = (unsigned char)pNPCTrade_Window->m_MouseSlotItem.m_ucCount;
	...
	case MSG_NO_PVP_TRADE_CANCEL :
		{
			...
			if(pPCTrade_Window)
			{
				...
				int i = 0 ; for( ; i < _XDEF_PCTRADE_MAX_SLOTNUMBER ; ++i)
				{
					pvptradecancel.result[i].slot = (char)pPCTrade_Window->m_SelfTradeItem[i].m_InventoryNumber;
					pvptradecancel.result[i].number = pPCTrade_Window->m_SelfTradeItem[i].m_ucCount;
				}
	...
	case MSG_NO_SECURITY :
		{
			...
			apexclientdata.header.sLength = size - (APEX_MAX_LENGTH-param2);
			apexclientdata.ucMessage = MSG_NO_SECURITY;
			apexclientdata.ucMode = en_apex_client_data;
			
			apexclientdata.data_length = param2;
			char* pData = (char*)param1;

			memcpy( apexclientdata.data, pData, param2 );

			#ifdef _XAPEXMODULE_LOG
					_XLog( "Apex send client data, data length = %d", param2 );
			#endif
...
void _XNetwork::MSGChangeServerGroup(MSG_SHIFT_STATE* changeservergroup)
{
	...
	// 퀘스트를 취소하는 코드를 넣는다.
//	int i = 0 ; for(; i < ID_QUEST_SYNC-1; i++ ) // 제룡행 퀘스트는 진행중인 퀘스트에서 제외 -> 5개 
//	{
...
void _XNetwork::MSGCharacList(MSG_CHARAC_LIST* characterlist)
{
	...
	{
		int i = 0 ; for( ; i < characterlist->nCharac ; ++i)
		{
...
void _XNetwork::MSGCharacInfo(MSG_CHARAC_INFO* characterinfo)
{
	...
	TCHAR	tstr[256];
	sprintf(tstr, _XGETINTERFACETEXT(ID_STRING_NETWORK_MOVEZONE), g_ZoneInfoTable[g_CurrentZoneInfoIndex - 1].zonename);
...
void _XNetwork::MSGZoneMove(MSG_ZONE_MOVE* charactermove)
{
	...
	if(pUser)
	{
		...
		if( g_EnableUserPacketFiltering )
		{
			...
			{
				TCHAR _messagestring[64];
				sprintf( _messagestring, "User move packet : X:%4.2f Y:%4.2f AnimIndex:%d Action:%d", charactermove->fgotoX, charactermove->fgotoZ, charactermove->sAnimationNumber, pUser->m_CharacterInfo.animationdescript.motionclass );
				_XDWSENDPACKETLOG( _messagestring );
			}
		}
#endif
...
void _XNetwork::MSGZoneCharacInfo(MSG_ZONE_CHARAC_INFO* characterinfo)
{
	...
	if(pUser)
	{
		...
		{
			int i = 0 ; for(; i < g_MaxNickNameCount; ++i)
			{
...
{	
	...
	if( chatnormal->cMode == 50 )
	{
		...
		else
		{
			...
			sprintf( szBuffer, _XGETINTERFACETEXT(ID_STRING_NEW_3146), g_PlayLeftTime );
...
void _XNetwork::MSGTargetInfo(MSG_CHARAC_MORE_INFO* targetinfo)
{
	...
	if( targetinfo_window )
	{
		...
		{		
			int i = 0 ; for(  i = MSG_CHARAC_MORE_INFO::pos_amor; i < MSG_CHARAC_MORE_INFO::get_on_max ; ++i )
			{
				targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_cType = targetinfo->getOnItems[i].cType ;
				targetinfo_window->m_SelectedTargetInfo.getOnItems[i].Set_m_sID( targetinfo->getOnItems[i].sID );
				targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_cSecond = targetinfo->getOnItems[i].cSecond ;
				...
				{
					targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_ucYear		= targetinfo->getOnItems[i].uselimit.ucYear;	
					targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_ucMonth		= targetinfo->getOnItems[i].uselimit.ucMonth;	
					targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_ucDay		= targetinfo->getOnItems[i].uselimit.ucDay;	
					targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_ucHour		= targetinfo->getOnItems[i].uselimit.ucHour;	
					targetinfo_window->m_SelectedTargetInfo.getOnItems[i].m_ucMinute	= targetinfo->getOnItems[i].uselimit.ucMinute;
				}
	...
	case _PACKET_ALERT_REMAIN_TIME		:	// 사용자의 남은 시간
		{
			...
			if( remainhour > 0 )
			{
				strcpy( szBuffer,  g_StringDataBase.CompositeString( _XSTRINGDB_SN_EODSTRING_XSR,ID_STRING_NETWORK_2354, &remainhour, &remainminute ) );
			}
			else if( remainminute > 0 )
			{
				sprintf( szBuffer, _XGETINTERFACETEXT(ID_STRING_NETWORK_2355), remainminute );
			}
			else
			{
				sprintf( szBuffer, _XGETINTERFACETEXT(ID_STRING_NETWORK_2356), g_LocalUserBillingInformations.BillRemain / 1000 );
			}
...
void _XNetwork::MSGQuickSlot( MSG_QUICKSLOT_ALLINFO* quickslotinfo )
{
	...
	int i = 0 ; for(; i < _QUICKSLOT_COUNT_; i++ )
	{
