...
int _XNetwork::CheckEmptyFameItem(void)
{
	...
	int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_AMOR; i < _XINVENTORY_SLOTTYPE_POCKETSTART; ++i )
	{
		id = g_pLocalUser->m_UserItemList[i].Get_m_sID();
		if( id > 0 )
		{
			switch(g_pLocalUser->m_UserItemList[i].m_cType) 
			{
			case _XGI_FC_WEAPON:
				{
...
void _XNetwork::InvenInfoSetting(MSG_INVEN_INFO* templist)
{
	...
	int i = 0 ; for( ; i < inventorylist->ucItemCount ; ++i)
	{
...
//					CheckFlag = TRUE;
			...
			if( tempInvenItem->sID > 0 )
			{
...
#endif
					{
						if( tempInvenItem->ucIncludeLimit == 1 )
						{
							...
							if( tempInvenItem )
							{
								...
								g_pLocalUser->m_UserItemList[slotnumber].m_cType	= tempInvenItem->cType;
								g_pLocalUser->m_UserItemList[slotnumber].m_cSecond	= tempInvenItem->cSecond;
								g_pLocalUser->m_UserItemList[slotnumber].Set_m_sID( tempInvenItem->sID );
								
								g_pLocalUser->m_UserItemList[slotnumber].m_cThird	= _XGameItem::GetThird(tempInvenItem->cType, tempInvenItem->sID);  
								g_pLocalUser->m_UserItemList[slotnumber].m_ucCount	= tempInvenItem->Item_Money.S_NormalItem.ucCount;
								...
								{									
									g_pLocalUser->m_UserItemList[slotnumber].m_ucYear	= tempInvenItem->uselimit.ucYear;	
									g_pLocalUser->m_UserItemList[slotnumber].m_ucMonth	= tempInvenItem->uselimit.ucMonth;	
									g_pLocalUser->m_UserItemList[slotnumber].m_ucDay	= tempInvenItem->uselimit.ucDay;	
									g_pLocalUser->m_UserItemList[slotnumber].m_ucHour	= tempInvenItem->uselimit.ucHour;	
									g_pLocalUser->m_UserItemList[slotnumber].m_ucMinute	= tempInvenItem->uselimit.ucMinute;
								}
							}							
						}											
				...
				if( slotnumber == _XINVENTORY_SLOTTYPE_WEAPON_1)
				{
					if( g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID() > 0)
					{
						...
						int	effectid = _XGameItem::GetInchantEffectId(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType,
	...
	if( tempInvenItem )
	{
		...
		_XLog( "Idx : %d : Type %d, Second %d, ID %d, Third %d, Count %d, Dur %d, MaxDur %d", i, tempInvenItem->cType, tempInvenItem->cSecond, tempInvenItem->sID,
				g_pLocalUser->m_UserItemList[slotnumber].m_cThird, tempInvenItem->Item_Money.S_NormalItem.ucCount, tempInvenItem->Item_Money.S_NormalItem.usCurDur,
	...
	for(i = _XINVENTORY_SLOTTYPE_AMOR ; i <= _XINVENTORY_SLOTTYPE_WEAPON_2 ; ++i)
	{
		...
		ChangeModel(lparam, g_pLocalUser->m_UserItemList[i].m_cType );
	}
}
...
void _XNetwork::MSGStorageInfo(MSG_STR_ITEM_INFO* storagelist)
{
	...
	if( storagelist->ucSavedItem )
	{
		...
		int i = 0 ; for( ; i < _XDEF_WAREHOUSE_TOTALCOUNT; ++i)
		{
			_sUser_Inven_v1_all* tempInvenItem = (_sUser_Inven_v1_all*)currentptr;
			if( tempInvenItem )
			{
				...
				{					
					...
					g_pLocalUser->m_WarehouseItemList[slotnumber].m_cType	= tempInvenItem->cType;
					g_pLocalUser->m_WarehouseItemList[slotnumber].m_cSecond	= tempInvenItem->cSecond;
					g_pLocalUser->m_WarehouseItemList[slotnumber].Set_m_sID(tempInvenItem->sID);
					
					g_pLocalUser->m_WarehouseItemList[slotnumber].m_cThird	= _XGameItem::GetThird(tempInvenItem->cType, tempInvenItem->sID);  
					g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucCount	= tempInvenItem->Item_Money.S_NormalItem.ucCount;
					...
					{						
						g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucYear		= tempInvenItem->uselimit.ucYear;	
						g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucMonth		= tempInvenItem->uselimit.ucMonth;	
						g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucDay		= tempInvenItem->uselimit.ucDay;	
						g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucHour		= tempInvenItem->uselimit.ucHour;	
						g_pLocalUser->m_WarehouseItemList[slotnumber].m_ucMinute	= tempInvenItem->uselimit.ucMinute;
					}
				}
				else
				{
...
void _XNetwork::MSGInvenMove(MSG_INVEN_MOVE* invenmove)
{
	...
	{
		TCHAR messagestring[256];
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_CANNOTCHANGE));
	...
	else
	{
		...
		{
			// merge
			g_pLocalUser->m_UserItemList[invenmove->cTo].m_ucCount += invenmove->ucSendCount;
			g_pLocalUser->m_UserItemList[invenmove->cFrom].m_ucCount -= invenmove->ucSendCount;
			if(g_pLocalUser->m_UserItemList[invenmove->cFrom].m_ucCount <= 0)
			{
				
				// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
				g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType, g_pLocalUser->m_UserItemList[invenmove->cFrom].Get_m_sID()  );
			...
			PlayItemEquipSound( g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType, 
		...
		else if(invenmove->cFlag == 0)
		{
			...
			PlayItemEquipSound( g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType, 
			...
			{
#ifdef _XTS_ITEM_OPTIMIZATION
				if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType) ||
					_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType) )
#else
				if( g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_WEAPON || 
					g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_WEAPON2 ||
					g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_WEAPON3 ||
					g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_CLOTHES ||
					g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_CLOTHES2 ||
					g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType == _XGI_FC_CLOTHES3 )
#endif
				{										
					if( invenmove->cFrom == _XINVENTORY_SLOTTYPE_WEAPON_1 )
					{						
						int effectid = _XGameItem::GetInchantEffectId(g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType,
					...
					ChangeModel(lparam, g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType );
					...
					{
						char tempPocket = _XGameItem::GetPocketNum(g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType, g_pLocalUser->m_UserItemList[invenmove->cFrom].Get_m_sID());
				...
				if(g_pState_Window)
				{
					g_pState_Window->DeleteStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[invenmove->cTo].Get_m_sID(), g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType, invenmove->cFrom);
				}
				// 퀘스트 시작조건에 아이템 장착 조건이 있으면 시작조건의 아이템인지 체크를 한다.
				g_QuestScriptManager.RecheckWearItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType, g_pLocalUser->m_UserItemList[invenmove->cTo].Get_m_sID() );
			}
			...
			if(invenmove->cTo >= _XINVENTORY_SLOTTYPE_AMOR && invenmove->cTo < _XINVENTORY_SLOTTYPE_POCKETSTART)
			{
...
#endif
				{
					...
					{
						int effectid = _XGameItem::GetInchantEffectId(g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType,
					...
					ChangeModel(lparam,g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType);
					...
					{
						char tempPocket = _XGameItem::GetPocketNum(g_pLocalUser->m_UserItemList[invenmove->cTo].m_cType, g_pLocalUser->m_UserItemList[invenmove->cTo].Get_m_sID());
						if( invenmove->cAmorPocket == tempPocket )
							g_pLocalUser->m_cAmorPocketNum = invenmove->cAmorPocket;
					}
				...
				if(g_pState_Window)
				{
					g_pState_Window->DeleteStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[invenmove->cFrom].Get_m_sID(), g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType, invenmove->cTo);
				}
			...
			if( g_pInventory_Window )
			{
				if( g_pInventory_Window->m_pGamble_Window )
				{
					if( g_pInventory_Window->m_pGamble_Window->GetShowStatus() )
					{
						...
						{
							if( g_pLocalUser->m_UserItemList[invenmove->cFrom].m_cType == _XGI_FC_BOX )
							{
		...
		else if(invenmove->cFlag == 2)
		{
...
#endif
			{
				...
				if(g_pState_Window)
				{
					g_pState_Window->UpdatePlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].Get_m_sID(), g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType,
						_XINVENTORY_SLOTTYPE_WEAPON_1, _XINVENTORY_SLOTTYPE_WEAPON_2);

					g_pState_Window->UpdatePlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_2].Get_m_sID(), g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_2].m_cType,
				...
				int effectid = _XGameItem::GetInchantEffectId(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_2].m_cType,
...
void _XNetwork::MSGItemGrabSuccess(MSG_ITEM_GRAB_SUCCESS* successgrabitem)
{
	...
	int itemcountbeforetrade = g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_ucCount;

   	if(successgrabitem->cCount == 0)
	{ 
		success = true;
		// 인벤토리 한 슬롯만 채운다.
		g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cType = successgrabitem->ItemInfo.cType;
		g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cSecond = successgrabitem->ItemInfo.cSecond;
		...
		g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cThird = _XGameItem::GetThird(successgrabitem->ItemInfo.cType, successgrabitem->ItemInfo.sID);
		g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_ucCount = successgrabitem->ItemInfo.Item_Money.S_NormalItem.ucCount;
...
// 기간제 아이템 : 행낭에 바로 들어가기 처리하기.
		...
		// 무기, 의상일때만 소켓값을 읽어들인다.. 
	#ifdef _XTS_ITEM_OPTIMIZATION
		switch( GetEqualItemType(g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cType) ) 
	#else
		switch(g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cType) 
	#endif
		{
	#ifdef _XTS_ITEM_OPTIMIZATION
		case _XGI_FC_WEAPON :
		...
		PlayItemEquipSound( g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].m_cType, 
							g_pLocalUser->m_UserItemList[successgrabitem->cInvenSlot].Get_m_sID() );
	}
	else if(successgrabitem->cCount > 0)
	{
		success = true;
		int i = 0 ; for( ; i < successgrabitem->cCount ; ++i)
		{
			g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_cType = successgrabitem->ItemInfo.cType;
			g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_cSecond = successgrabitem->ItemInfo.cSecond;
			...
			g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_cThird = _XGameItem::GetThird(successgrabitem->ItemInfo.cType, successgrabitem->ItemInfo.sID);
			g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucCount = successgrabitem->result[i].number;
			...
			if( successgrabitem->ItemInfo.uselimit.ucYear > 0 )
			{				
				g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucYear		= successgrabitem->ItemInfo.uselimit.ucYear;	
				g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucMonth		= successgrabitem->ItemInfo.uselimit.ucMonth;	
				g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucDay		= successgrabitem->ItemInfo.uselimit.ucDay;	
				g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucHour		= successgrabitem->ItemInfo.uselimit.ucHour;	
				g_pLocalUser->m_UserItemList[successgrabitem->result[i].slot].m_ucMinute	= successgrabitem->ItemInfo.uselimit.ucMinute;
			}
	...
	if( success )
	{
		...
		int i = 0 ; for(; i < ID_QUEST_SYNC; ++i ) // 제룡행 퀘스트는 진행중인 퀘스트에서 제외 -> 5개 
		{
			if( checkitem ) break;
			...
			if( currentquest )
			{
				int itemcounterindex = 0 ; for(  itemcounterindex = 0; itemcounterindex < 8; ++itemcounterindex )
				{
					if( currentquest->m_ItemCounterInformation[itemcounterindex].CounterTargetType != -1 )
					{
							...
							&& currentquest->m_ItemCounterInformation[itemcounterindex].CounterTargetID == successgrabitem->ItemInfo.sID )
						{
							...
							int i = 0 ; for( ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; ++i)
							{
								if(g_pLocalUser->m_UserItemList[i].m_cType == successgrabitem->ItemInfo.cType
		...
		}//int i = 0 ; for(; i < ID_QUEST_SYNC-1; i++ )
...
{
	TCHAR messagestring[256];
	sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_GETMONEY), successgrabmoney->uiMoney - g_pLocalUser->m_Money);
...
void _XNetwork::MSGInvenDrop(MSG_INVEN_DROP* inventoryitemdrop)
{
	...
	{
		TCHAR messagestring[256];
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_WARNDROPITEM));
		InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
	}
	...
	{	
		PlayItemEquipSound( g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_cType, 
							g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].Get_m_sID() );

		int invenitemtype	= g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_cType;
		...
		{
			g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_ucCount -= inventoryitemdrop->uiCount;
			if(g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_ucCount <= 0)
			{
				// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
				g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_cType, g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].Get_m_sID() );
			...
			if(inventoryitemdrop->cInvenIndex >= _XINVENTORY_SLOTTYPE_AMOR && inventoryitemdrop->cInvenIndex <= _XINVENTORY_SLOTTYPE_WEAPON_2)
			{
				if(g_pState_Window)
				{
					if(g_pState_Window->FindPlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].Get_m_sID(), 
						g_pLocalUser->m_UserItemList[inventoryitemdrop->cInvenIndex].m_cType, inventoryitemdrop->cInvenIndex))
					{
		...
		{
			TCHAR messagestring[256];
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORK_DROPITEM));
			InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
		}
	}
}
...
void _XNetwork::MSGItemDrop(MSG_ITEM_DROP* itemdrop)
{
	if(itemdrop->cNoItems > 0)
	{
		...
		int i = 0 ; for( ; i < itemdrop->cNoItems ; ++i)
		{
...
void _XNetwork::MSGTempInven(MSG_TEMP_INVEN* tempinven)
{
	...
	if( tempinven->cTo )	// 템프 인벤
	{
		...
		int i = 0 ; for(; i < _TEMP_INVEN_MAX_COUNT_; ++i )
		{
			if( pPocket_Window )
			{
	...
	int i = 0 ; for( ; i < _TEMP_INVEN_MAX_COUNT_ ; ++i)
	{
		if(inveninfo->TInven[i].sID > 0)
		{
			...
			if(pPocket_Window)
			{
				...
				pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_cType = inveninfo->TInven[i].cType;
				...
				pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucCount = inveninfo->TInven[i].Item_Money.S_NormalItem.ucCount;
				...
				{						
					pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucYear	= inveninfo->TInven[i].uselimit.ucYear;	
					pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucMonth	= inveninfo->TInven[i].uselimit.ucMonth;	
					pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucDay	= inveninfo->TInven[i].uselimit.ucDay;	
					pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucHour	= inveninfo->TInven[i].uselimit.ucHour;	
					pPocket_Window->m_AccessoryItemList[inveninfo->TInven[i].cSlot - _XINVENTORY_SLOTTYPE_TOTALCOUNT].m_ucMinute	= inveninfo->TInven[i].uselimit.ucMinute;
				}
			}
		}
	}
...
void _XNetwork::MSGMoneyDrop(MSG_MONEY_DROP* moneydrop)
{
	if(moneydrop->bSuccess)
	{
		...
		TCHAR messagestring[256];
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1699), moneydrop->uiCount);
		InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
	...
	if( g_pInventory_Window )
	{
		...
		case 1 :
			{
				if( g_pLocalUser->m_bKeyLPSProcess ) 
				{
					...
					//key LPS에 대한 처리 					
					TCHAR messagestring[256];
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1717), 
						_XGameItem::GetItemName(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, 
				...
				else
				{
					...
					if( !(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType == _XGI_FC_POTION &&
						g_PotionItemProperty[g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID()].cSecondType == _XGI_SC_POTION_MASK) )
					{
						PlayItemUseSound( g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, 
											g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID() );
					}					
#else
					PlayItemUseSound( g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, 
				...
				if( useitemresult->ucCount == 0 )
				{
							...
							g_PotionItemProperty[g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID()].cSecondType == _XGI_SC_POTION_HERT_OUT)
						{
							if(g_pState_Window)
								g_pState_Window->InsertStateListFromOther(_XST_ITEM, g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID(),
																 g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, useitemresult->ucInvenIndex);
						}
					}
					else if(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType == _XGI_FC_QUEST)
					{
					...
					else if(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType == _XGI_FC_BOOK)
					{
						if( g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cSecond == 1 )	// 무공서
						{
							TCHAR messagestring[256];
							sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1718), 
								_XGameItem::GetItemName(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, 
								g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID()));
					...
					// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
					g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID()  );
				...
				else
				{
					...
					g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_ucCount = useitemresult->ucCount;
				}
		...
		case 2 :	// 복용 후 성공
			{				
				...
				// Item 사용
				if(g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType == _XGI_FC_ELIXIR)
				{
					if( g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cSecond == _XGI_SC_ELIXIR_SUPPORT )
					{
						if(g_pState_Window)
							g_pState_Window->InsertStateListFromOther(_XST_ITEM, g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].Get_m_sID(),
							g_pLocalUser->m_UserItemList[useitemresult->ucInvenIndex].m_cType, useitemresult->ucInvenIndex);
...
void _XNetwork::MSGElixirUseItem(MSG_ELIXIR_USE_RESULT* useitemresult)
{
	...
	{
		TCHAR message[256];
		memset( message, 0, sizeof(TCHAR)*256 );
		sprintf( message, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1722), g_pLocalUser->m_ElixirInfo._sIntoxication ); 
	...
	default:	// 수리 OK
		{
			...
			int i = 0 ; for(; i < repairitemresult->ucRepairCount; ++i )
			{
				...
				strcpy(namestring, _XGameItem::GetItemName(g_pLocalUser->m_UserItemList[slotnumber].m_cType, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID()));
				...
				if(pNPCScript_Window)
				{
					...
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1728), namestring);
					pNPCScript_Window->InsertNPCChatString( messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET );
				}
			}			
		}
	...
	default:	// 수리 OK
		{
			...
			int i = 0 ; for(; i < repairitemresult->cRepairCount; ++i )
			{
				int slotnumber = repairitemresult->_S_Repair_Result[i].ucInvenIndex;
				g_pLocalUser->m_UserItemList[slotnumber].Set_m_usCurDur( repairitemresult->_S_Repair_Result[i].usCurDurability );
				g_pLocalUser->m_UserItemList[slotnumber].Set_m_usMaxDur( repairitemresult->_S_Repair_Result[i].usMaxDurability );
...
void _XNetwork::MSGWorldItemInfo(MSG_WORLD_ITEM* worlditeminfo)
{
	...
	if(worlditeminfo->sNearItemNo > 0)
	{
		...
		int i = 0 ; for( ; i < worlditeminfo->sNearItemNo ; ++i)
		{
	...
	case inven_ctrl_item_dur_set :
		{
			...
			if( percent <= 0.0f)// 2004.06.19->oneway48 insert
			{
				...
				if(g_pState_Window)
				{
					...
					g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType, invenctrlresult->_S_Dur_Set.cIdx);
				}
				
				// 아이템 파괴에 대한 로그 처리 
				g_pLocalUser->m_CharacterLog.isDestroyItem		= TRUE;
				g_pLocalUser->m_CharacterLog.DestroyItemType	= g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType;
				...
				// 수정 hotblood
				sprintf(noticestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1729), _XGameItem::GetItemName(g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType, g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].Get_m_sID() ) );
				...
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_1; i <= _XINVENTORY_SLOTTYPE_ACCESSORY_4; ++i )
					{
						// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
						g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[i].m_cType, g_pLocalUser->m_UserItemList[i].Get_m_sID()  );
						
						// 퀘스트 시작조건에 아이템 장착 조건이 있으면 시작조건의 아이템인지 체크를 한다.
						g_QuestScriptManager.RecheckWearItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[i].m_cType, g_pLocalUser->m_UserItemList[i].Get_m_sID()  );
				...
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_5; i <= _XINVENTORY_SLOTTYPE_ACCESSORY_8; ++i )
					{
						// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
						g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[i].m_cType, g_pLocalUser->m_UserItemList[i].Get_m_sID()  );
				...
				// 퀘스트 시작 조건에 아이템 체크가 있으면 시작조건의 아이템인지 체크를 한다.
				g_QuestScriptManager.RecheckHaveItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType, g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].Get_m_sID()  );
				
				// 퀘스트 시작조건에 아이템 장착 조건이 있으면 시작조건의 아이템인지 체크를 한다.
				g_QuestScriptManager.RecheckWearItemTypeQuestStartCondition( g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType, g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].Get_m_sID()  );
				...
				ChangeModel(lparam, g_pLocalUser->m_UserItemList[invenctrlresult->_S_Dur_Set.cIdx].m_cType);
...
{	
	...
	if(pWarehouse_Window)
	{
	...
	#endif
		{
			...
			// 창고 사용 만료일은 %d년 %d월 %d일 입니다.
			TCHAR messagestring[256];
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1731),	pWarehouse_Window->m_UseSlotCount);
			pNPCScript_Window->InsertNPCChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET);
	...
	if(pWarehouse_Window)
	{
		if(stritemeject->cItemNum == 0)
		{
			...
			if(pNPCScript_Window)
			{
				TCHAR messagestring[256];
				memset(messagestring, 0, sizeof(TCHAR)*256);				
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1732), pWarehouse_Window->m_TempMoney);
				pNPCScript_Window->InsertNPCChatString( messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET );
			}
		...
		else
		{
			...
			if( stritemeject->cItemNum > 0 )	// 서버에서 보내준 갯수 만큼만 꺼낸다.
			{
				...
				if( g_pLocalUser->m_Money != stritemeject->uiMoney )		// 인벤돈과 서버에서 보내준 돈이 틀릴 경우만 처리
				{
					TCHAR messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);
					sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1733), g_pLocalUser->m_Money-stritemeject->uiMoney);
...
void _XNetwork::MSGPvPTradeCancel(MSG_PVP_TRADE_CANCEL* pvptradecancel)
{
	...
	if(pPCTrade_Window && g_pInventory_Window)
	{
		...
		{	
			...
			{
				// 인벤토리가 비어있지 않음
				g_pLocalUser->m_UserItemList[pvptradecancel->result[slotnumber].slot].m_ucCount += pvptradecancel->result[slotnumber].number;
			}
			else
			{
				// 인벤토리가 비어있음
				...
				if(pUser)
				{
					...
					if( !npcdialogmode && !npcwideviewmode && !functionalwideviewmode)
					{
						...
						else if( g_pLocalUser->m_bInternalUse || g_pLocalUser->m_bSocketPlugIn || g_pLocalUser->m_bGambleBoxOpen ) // 영약을 복용중인지 검사
						{
							...
							SendPacket(MSG_NO_PVP_TRADE_REQ, SUB_MSG_TRADE_REJECT);
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1734), pUser->m_CharacterInfo.charactername );
						...
						else if(g_pLocalUser->m_UserState >= _XDEF_USERSTATE_MATCH_MAKETABLE && g_pLocalUser->m_UserState <= _XDEF_USERSTATE_MATCH_WAITEXIT)
						{
							...
							SendPacket(MSG_NO_PVP_TRADE_REQ, SUB_MSG_TRADE_REJECT);
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1735), pUser->m_CharacterInfo.charactername );
						...
						{
							// Pk중 거절
							sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2745), pUser->m_CharacterInfo.charactername);	// "대전 중이므로 %s님의 교환 요청을 거절하였습니다."
							InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
							_XPlayInterfaceSound(ID_SR_INTERFACE_ERROR01_WAV);
						}
#endif
						else
						{
							...
							if(pPCTrade_Window)
							{
								if( !pPCTrade_Window->m_bTrading )
								{
									...
									sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1736), pUser->m_CharacterInfo.charactername );
	...
	case SUB_MSG_TRADE_REJECT :
		{
			...
			if( !strlen(pPCTrade_Window->m_TargetName) )
			{
				...
				if(pUser)
				{
					...
					sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1737), pUser->m_CharacterInfo.charactername );
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	case SUB_MSG_TRADE_BUSY :
		{
			...
			if(pUser)
			{		
				sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1738), pUser->m_CharacterInfo.charactername );
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
				pMessageBox->StartTimer(10000, TRUE);				
			}
	...
	case SUB_MSG_TRADE_SELFREJECT :
		{
			...
			if( !strlen(pPCTrade_Window->m_TargetName) )
			{
				...
				if(pUser)
				{
					...
					sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1741), pUser->m_CharacterInfo.charactername );	
					_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
		...
		if(stritemsave->cItemNum == 0)
		{
			...
			if( g_pLocalUser->m_WarehouseMoney != stritemsave->uiMoney )	// 창고돈과 서버에서 보내준 돈이 틀릴 경우만 처리
			{
				...
				if(pNPCScript_Window)
				{
					TCHAR messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1742), pWarehouse_Window->m_TempMoney);
...
{	
	...
	if(pPCTrade_Window && g_pInventory_Window)
	{
		...
		if(pvptradedelete->ucSubjectID == g_pLocalUser->m_Uniqueid)
		{
			...
			{
				// 빈 인벤토리가 아님, 더해준다.
				g_pLocalUser->m_UserItemList[pvptradedelete->Inven_Index].m_ucCount += pvptradedelete->number;
			}
			else
			{
				// 인벤토리 슬롯이 비었음
			...
			{
				pPCTrade_Window->m_SelfTradeItem[pvptradedelete->TWindowIndex].m_ucCount -= pvptradedelete->number;
				if(pPCTrade_Window->m_SelfTradeItem[pvptradedelete->TWindowIndex].m_ucCount <= 0)
		...
		else
		{
			...
			{
				pPCTrade_Window->m_TargetTradeItem[pvptradedelete->TWindowIndex].m_ucCount -= pvptradedelete->number;
				if(pPCTrade_Window->m_TargetTradeItem[pvptradedelete->TWindowIndex].m_ucCount <= 0)
...
void _XNetwork::MSGPvPTradeUpdateInfo(MSG_PVP_TRADE_UPDATE_INFO* pvptradeupdate)
{
	...
	if(pPCTrade_Window)
	{
		...
		{
			pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_cType = pvptradeupdate->item.cType;
			pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_cSecond = pvptradeupdate->item.cSecond;
			...
			pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_cThird = _XGameItem::GetThird(pvptradeupdate->item.cType, pvptradeupdate->item.sID);
			pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucCount = pvptradeupdate->item.Item_Money.S_NormalItem.ucCount;	
			...
			{				
				pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucYear		= pvptradeupdate->item.uselimit.ucYear;	
				pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucMonth		= pvptradeupdate->item.uselimit.ucMonth;	
				pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucDay		= pvptradeupdate->item.uselimit.ucDay;	
				pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucHour		= pvptradeupdate->item.uselimit.ucHour;	
				pPCTrade_Window->m_TargetTradeItem[pvptradeupdate->TradeWindowSlotNo].m_ucMinute	= pvptradeupdate->item.uselimit.ucMinute;
			}
	...
	case -2 :
		{
			...
			if(pUser)
			{
				TCHAR	messagestring[256];
				memset(messagestring, 0, sizeof(TCHAR)*256);
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_NOTENOUGHOPP), pUser->m_CharacterInfo.charactername);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			else
	...
	case -7 :
		{
			...
			if(pUser)
			{
				TCHAR	messagestring[256];
				memset(messagestring, 0, sizeof(TCHAR)*256);
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKTWO_CANCELTRADELOGOUT), pUser->m_CharacterInfo.charactername);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
			}
			else
	...
	case 1 :
		{
			...
			if(pUser)
			{
				TCHAR messagestring[256];
				memset(messagestring, 0, sizeof(TCHAR)*256);
				sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1747), pUser->m_CharacterInfo.charactername );
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), messagestring, TRUE);
				pMessageBox->StartTimer( 100000, TRUE );
			}
	...
	case 0 :
		{
			...
			if( pNPCTrade_Window )
			{
				if( pNPCTrade_Window->m_bIsBuy ) //true = NPC에게서 사기의 결과
				{
					...
					// 퀘스트 아이템 카운트 표시할때 2개이상 들어와도 표시하기 위해서 더하기 이전 값을 저장해야 한다.
					int itemcountbeforetrade = g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucCount;

					int buynumber = traderesult->Item.Item_Money.S_NormalItem.ucCount - g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucCount;
					
					g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_cType = traderesult->Item.cType;
					g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_cSecond = traderesult->Item.cSecond;
					...
					g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_cThird = _XGameItem::GetThird(traderesult->Item.cType, traderesult->Item.sID);
					g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucCount = traderesult->Item.Item_Money.S_NormalItem.ucCount;
					...
					{						
						g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucYear		= traderesult->Item.uselimit.ucYear;	
						g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucMonth	= traderesult->Item.uselimit.ucMonth;	
						g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucDay		= traderesult->Item.uselimit.ucDay;	
						g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucHour		= traderesult->Item.uselimit.ucHour;	
						g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucMinute	= traderesult->Item.uselimit.ucMinute;
					}
				...
				{	
					TCHAR		namestring[256];
					memset(namestring, 0, sizeof(TCHAR)*256);
					strcpy(namestring, _XGameItem::GetItemName(g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_cType, g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].Get_m_sID()));
					...
					if(pNPCScript_Window)
					{
						TCHAR messagestring[256];
						memset(messagestring, 0, sizeof(TCHAR)*256);
						int sellnumber = g_pLocalUser->m_UserItemList[traderesult->cInvenIndex].m_ucCount - traderesult->Item.Item_Money.S_NormalItem.ucCount;
	...
	case en_socket_gamble_mode_equip_result :
		{
    ...
    #endif
			{
				...
				int i = 0 ; for(; i < _XDEF_SOCKETITEM_MAXCOUNT; ++i )
				{
	...
	case en_socket_gamble_mode_boxgamble_result :
		{
			...
			{		
				if( pGamble_Window )
				{
					...
					g_pInventory_Window->m_iSocketIndexList[0] = 0;
					int i = 0 ; for(; i < _XDEF_SOCKETITEM_MAXCOUNT-1; ++i )
					{
						if( g_pInventory_Window->m_iSocketIndexList[i+1] > 0 )
						{
		#ifdef VN_RB_GAMBLEBOX_070607_KUKURI
		...
		case en_socket_gamble_mode_separate_success : //소켓 분리 성공
			{
				...
				//타겟아이템, 무기 또는 의상의 처리
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_cType = pSocketDeatchResult->TargetItem.cType;
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_cSecond = pSocketDeatchResult->TargetItem.cSecond;
				...
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_cThird = _XGameItem::GetThird(pSocketDeatchResult->TargetItem.cType, pSocketDeatchResult->TargetItem.sID);
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucCount = pSocketDeatchResult->TargetItem.Item_Money.S_NormalItem.ucCount;
				...
				{					
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucYear		= pSocketDeatchResult->TargetItem.uselimit.ucYear;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucMonth	= pSocketDeatchResult->TargetItem.uselimit.ucMonth;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucDay		= pSocketDeatchResult->TargetItem.uselimit.ucDay;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucHour		= pSocketDeatchResult->TargetItem.uselimit.ucHour;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucTargetSlot].m_ucMinute	= pSocketDeatchResult->TargetItem.uselimit.ucMinute;
				}
				//타겟아이템, 무기 또는 의상의 처리 끝

				//분리된 소켓 아이템을 빈슬롯에 만들기
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_cType = pSocketDeatchResult->ResultSocket.cType;
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_cSecond = pSocketDeatchResult->ResultSocket.cSecond;
				...
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_cThird = _XGameItem::GetThird(pSocketDeatchResult->ResultSocket.cType, pSocketDeatchResult->ResultSocket.sID);
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucCount = pSocketDeatchResult->ResultSocket.Item_Money.S_NormalItem.ucCount;
				...
				{					
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucYear		= pSocketDeatchResult->ResultSocket.uselimit.ucYear;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucMonth	= pSocketDeatchResult->ResultSocket.uselimit.ucMonth;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucDay		= pSocketDeatchResult->ResultSocket.uselimit.ucDay;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucHour		= pSocketDeatchResult->ResultSocket.uselimit.ucHour;	
					g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucResultSocketSlot].m_ucMinute	= pSocketDeatchResult->ResultSocket.uselimit.ucMinute;
				}
				//분리된 소켓 아이템을 빈슬롯에 만들기 끝

				//장식도구는 삭제
				g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucToolSlot].m_ucCount = pSocketDeatchResult->ucLeftToolCount; // 남은 갯수
			
				// 0이면 삭제
				if( g_pLocalUser->m_UserItemList[pSocketDeatchResult->ucToolSlot].m_ucCount < 1 )
				...
				sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3751) ); //장식물이 성공적으로 분리되었습니다.
				...
				case 1 :
					{
						...
						_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_SOCKETPLUGINMESSAGEWINDOW);
						sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3752) ); //장식물이 없거나 장착 공간이 없는 물품입니다.
						pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  NULL, tempString, TRUE);
					}
					break;
				case 2 :
					{
						sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3753) ); //평화상태에서만 장식물을 분리할 수 있습니다.
				...
				case 3 :
					{
						...
						_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_SOCKETPLUGINMESSAGEWINDOW);
						sprintf(tempString, _XGETINTERFACETEXT(ID_STRING_NEW_3754));//행낭에 여유 공간이 없어서 장식물 분리에 실패하였습니다.
						pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  NULL, tempString, TRUE);
					}
					break;
				}
...
void _XNetwork::CheckValidItem(int slotnumber)
{
	...
	if(percent > 0.0f)
	{
		...
		if(percent <= 15.0f)
		{
			if(g_pState_Window)
			{
				g_pState_Window->InsertPlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID(), g_pLocalUser->m_UserItemList[slotnumber].m_cType, slotnumber, 1);
				g_pState_Window->SetRedLayer(_XST_ITEM, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID(), g_pLocalUser->m_UserItemList[slotnumber].m_cType, slotnumber);
				g_pState_Window->SetIconWarning(_XST_ITEM, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID(), g_pLocalUser->m_UserItemList[slotnumber].m_cType);
				return;
			}
		}
		...
		else
		{
			...
			if(g_pState_Window)
			{
				if(g_pState_Window->FindPlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID(), g_pLocalUser->m_UserItemList[slotnumber].m_cType, slotnumber))
				{
					g_pState_Window->DeletePlayerStateList(_XST_ITEM, g_pLocalUser->m_UserItemList[slotnumber].Get_m_sID(), g_pLocalUser->m_UserItemList[slotnumber].m_cType, slotnumber);
				}
			}
		}
	} // if(percent > 0.0f)	
	...
	// 능력치 검사
	char cType = g_pLocalUser->m_UserItemList[slotnumber].m_cType;
			...
			if( tempresult )
			{
				...
				if( pInchant_Window )
				{
					...
					case 0 :	// 성공
						{
							TCHAR tempStr[256];
							sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1764), tempresult->ucInchantGrade );
							InsertChatString( tempStr, _XDEF_CHATMESSAGECOLOR_SYSTEMGET );
							_XPlayInterfaceSound(ID_SR_INTERFACE_ELIXIR_YES_WAV);

							g_pLocalUser->m_UserItemList[g_pInventory_Window->m_iSocketIndexList[0]].m_ucCount -= g_ItemInchantTable[g_pLocalUser->m_UserItemList[g_pInventory_Window->m_UsedLifeItemIndex].Get_m_ucStrength()].ucCount;
							if( g_pLocalUser->m_UserItemList[g_pInventory_Window->m_iSocketIndexList[0]].m_ucCount < 1 )
								g_pLocalUser->m_UserItemList[g_pInventory_Window->m_iSocketIndexList[0]].Reset();
					...
					case 1 :	// 실패 1
						{
							TCHAR tempStr[256];
							char type = g_pLocalUser->m_UserItemList[g_pInventory_Window->m_UsedLifeItemIndex].m_cType;
							...
							strcpy( tempStr, g_StringDataBase.CompositeString( _XSTRINGDB_SN_EODSTRING_XSR,ID_STRING_NETWORKITEMTRADE_1765, &param1, param2 ) );
							...
							{
								sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1766), tempresult->ucLeftItemCount );
							...
							{
								g_pLocalUser->m_UserItemList[tempresult->cSlot].m_cType		= _XGI_FC_LIFE;
								g_pLocalUser->m_UserItemList[tempresult->cSlot].Set_m_sID(19);	// 혈정
								g_pLocalUser->m_UserItemList[tempresult->cSlot].m_cSecond	= _XGameItem::GetItemSecond(_XGI_FC_LIFE, 19 );
								g_pLocalUser->m_UserItemList[tempresult->cSlot].m_ucCount	= tempresult->ucLeftItemCount;
							}
							g_pLocalUser->m_Money = tempresult->uiLeftMoney;
					...
					case 2 :	// 실패 2 (완전 실패)
						{
							...
							char type = g_pLocalUser->m_UserItemList[g_pInventory_Window->m_UsedLifeItemIndex].m_cType;
							short sid = g_pLocalUser->m_UserItemList[g_pInventory_Window->m_UsedLifeItemIndex].Get_m_sID();
							sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1765), tempresult->ucInchantGrade+1, _XGameItem::GetItemName(type, sid) );
							InsertChatString( tempStr, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
							_XPlayInterfaceSound(ID_SR_INTERFACE_ELIXIR_NO_WAV);
					...
					case 3 : // 실패 3 ( 부분 실패 ) 용린이 사용되어 무기가 파괴되지않음
#ifdef _XDEF_SMELT_ITEM // Author : 양희왕 
						{
							...
							{
								sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NEW_3453), param1 ); //%d차 제련에 실패하였습니다.
								InsertChatString( tempStr, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
								_XPlayInterfaceSound(ID_SR_INTERFACE_ELIXIR_NO_WAV);
							...
							{
								wsprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NETWORKITEMTRADE_1765), tempresult->ucInchantGrade+1, _XGameItem::GetItemName(type, sid) );
								InsertChatString( tempStr, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
								_XPlayInterfaceSound(ID_SR_INTERFACE_ELIXIR_NO_WAV);
					...
					case 4 : // 실패 3 ( 완전 실패 ) 용린이 사용되어 무기가 파괴되지않음
#ifdef _XDEF_SMELT_ITEM // Author : 양희왕 
						{
							...
							if(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_UsedLifeItemIndex].m_usCurProtect > 0) 
							{
								sprintf( tempStr, _XGETINTERFACETEXT(ID_STRING_NEW_3453), tempresult->ucInchantGrade+1 );
								InsertChatString( tempStr, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
								_XPlayInterfaceSound(ID_SR_INTERFACE_ELIXIR_NO_WAV);
	...
	case inchant_mode_protect_add_count_success :	// 용린 바르기 성공 s->c
		{
			...
			g_pLocalUser->m_UserItemList[tempresult->ucProtectItemSlot].m_ucCount = tempresult->ucLeftProtectItem; // 남은 용린수
			
			// 0이면 삭제
			if( g_pLocalUser->m_UserItemList[tempresult->ucProtectItemSlot].m_ucCount < 1 )
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3454) ); // 용린을 발랐습니다.
	...
	case inchant_mode_protect_inc_str_success :	// 용루 바르기 성공 s->c
		{
			...
			//남은 갯수 저장
			g_pLocalUser->m_UserItemList[tempresult->ucIncStrItemSlot].m_ucCount = tempresult->ucLeftIncStrItemCount; 

			// 0개 일때 없애자
			if(g_pLocalUser->m_UserItemList[tempresult->ucIncStrItemSlot].m_ucCount <1 )
			...
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3455) ); //용루를 발랐습니다.
			InsertChatString( messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMGET );
		}
		break;
	...
	case inchant_mode_combine_success :
		{
			...
			g_pLocalUser->m_UserItemList[inventoryindex].m_cType = combinesuccess->WeaponItem.cType;
			g_pLocalUser->m_UserItemList[inventoryindex].Set_m_sID( combinesuccess->WeaponItem.sID );
			g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount = combinesuccess->WeaponItem.Item_Money.S_NormalItem.ucCount;
			
			// Second Type setting 
			g_pLocalUser->m_UserItemList[inventoryindex].m_cSecond = 
					_XGameItem::GetItemSecond(combinesuccess->WeaponItem.cType, combinesuccess->WeaponItem.sID);
			...
			{					
				g_pLocalUser->m_UserItemList[inventoryindex].m_ucYear	= combinesuccess->WeaponItem.uselimit.ucYear;	
				g_pLocalUser->m_UserItemList[inventoryindex].m_ucMonth	= combinesuccess->WeaponItem.uselimit.ucMonth;	
				g_pLocalUser->m_UserItemList[inventoryindex].m_ucDay	= combinesuccess->WeaponItem.uselimit.ucDay;	
				g_pLocalUser->m_UserItemList[inventoryindex].m_ucHour	= combinesuccess->WeaponItem.uselimit.ucHour;	
				g_pLocalUser->m_UserItemList[inventoryindex].m_ucMinute	= combinesuccess->WeaponItem.uselimit.ucMinute;
			}

			if(combinesuccess->ucLeftAddItem1 > 0)
				g_pLocalUser->m_UserItemList[combinesuccess->ucAddItem1Slot].m_ucCount = combinesuccess->ucLeftAddItem1;
			else
				g_pLocalUser->m_UserItemList[combinesuccess->ucAddItem1Slot].Reset();

			if(combinesuccess->ucLeftAddItem2 > 0)
				g_pLocalUser->m_UserItemList[combinesuccess->ucAddItem2Slot].m_ucCount = combinesuccess->ucLeftAddItem2;
			else
			...
			sprintf(messagestr, _XGETINTERFACETEXT(ID_STRING_NETWORK_GETSOMETHING), _XGameItem::GetItemName(combinesuccess->WeaponItem.cType, combinesuccess->WeaponItem.sID));
...
#ifdef _XDWDEBUG
					TCHAR messagestr[256];
					sprintf(messagestr, "Combine Fail reason [%d]", combinefail->ucFailReason);
...
void _XNetwork::MSGBloodPointGiveItem(MSG_BLOODPOINT_GIVE_ITEM* giveitem)
{
	...
	if(g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_cType == _XGI_FC_LIFE &&
		g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].Get_m_sID() == 19)
	{
		// 개수 업데이트
		getcount = giveitem->ucCount - g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_ucCount;
		g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_ucCount = giveitem->ucCount;
	}
	else
	{
		...
		g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_cType = _XGI_FC_LIFE;
		g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_cSecond = _XGI_SC_LIFE_INCHANTSTUFF;
		g_pLocalUser->m_UserItemList[giveitem->cInsertedSlot].m_cThird = _XGameItem::GetThird(_XGI_FC_LIFE, 19);
	...
	TCHAR messagestring[256];
	sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2747), getcount);	// "혈정을 %d개 획득했습니다. 행낭을 확인 해 주세요."
	InsertChatString(messagestring,  _XDEF_CHATMESSAGECOLOR_SYSTEMGET, TRUE);
