...
BOOL XProc_SelectCharacter::InitializeProcess(void)
{
	...
	if( this->m_SelectedCharacter > -1 )
	{
		_XLocalUser::GetUserLevel( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_innerlevel(), m_CurrentCharacterLevelIndex, m_CurrentCharacterSubLevel );
	}
	...
	if( strlen( m_NewCreatedCharacterName ) ) // 새로 생성된 캐릭터를 선택함.
	{
		if( g_pLocalUser->m_CharacterCount > 0 )
		{
			int i = 0 ; for(; i < g_pLocalUser->m_CharacterCount; i++ )
			{
	...
	{
		_XLog( "Can't initialize login process." );
		_XMsgClose = TRUE;
		return FALSE;
	}
#endif
		...
		case 0 :
			{
				...
				if( strlen( g_CharacterRollNameTable[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].groupindex]
													[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_cClass()]
													[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_cClassGrade() ] ) > 0 )
				{
					strcpy(pSelectCharacter_Window->m_RollClass, 
						g_CharacterRollNameTable[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].groupindex]
												[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_cClass()]
												[g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_cClassGrade() ] );
				}
				else
				{
				...
				pSelectCharacter_Window->m_constitution	= g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_constitution();
				pSelectCharacter_Window->m_zen			= g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_zen();
				pSelectCharacter_Window->m_intelligence	= g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_intelligence();
				pSelectCharacter_Window->m_dexterity	= g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_dexterity();
				pSelectCharacter_Window->m_strength		= g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_strength();
...
void XProc_SelectCharacter::Draw(void)
{
	...
	if( !g_pTitleFlashObject->IsPlaying() )
	{
		...
		*/
		...
		if( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_constitution() == 8 &&
			g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_zen() == 8 &&
			g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_intelligence() == 8 &&
			g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_dexterity() == 8 &&
			g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_strength() == 8 )
		{
			...
			if( !g_pTitleFlashObject->IsPlaying() && !m_CharacterDeleteMode && !m_FirstRotateAnimation )
			{
				...
				else
				{
					...
					else
					{
						...
						// parameter setting
						g_pLocalUser->m_Strength.Set_org( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_strength() );
						g_pLocalUser->m_Zen.Set_org( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_zen() );
						g_pLocalUser->m_Intelligence.Set_org( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_intelligence() );
						g_pLocalUser->m_Dex.Set_org( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_dexterity() );
						g_pLocalUser->m_Constitution.Set_org( g_pLocalUser->m_AllCharacterInfo[this->m_SelectedCharacter].Get_constitution() );
...
/*#ifdef _XTS_WARNPLAYTIME
		...
		case _XDEF_CS_TYPEPASSWORD :			
			...
			{
				g_MessageBox.m_IMEControl.SetMaxLength(8);
			}
