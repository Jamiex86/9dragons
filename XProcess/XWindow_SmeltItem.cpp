	...
	case ITEMSTEP_READY :
		{
			...
			if( m_pUserItem )
			{
				...
				g_XBaseFont->SetColor(_XGameItem::GetGradeColor(m_pUserItem->m_cType, m_pUserItem->Get_m_sID()));
		
				if( _XGameItem::GetItemNick(m_pUserItem->m_cType, m_pUserItem->Get_m_sID() ) )
				{

					g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+32, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemNick(m_pUserItem->m_cType, m_pUserItem->Get_m_sID() ) );
					g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+47, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_pUserItem->m_cType, m_pUserItem->Get_m_sID() ) );
				}
				else
				{
					g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+40, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_pUserItem->m_cType, m_pUserItem->Get_m_sID() ) );
				}
	...
	case ITEMSTEP_CONFIRM :
		{// ITEMSTEP_CONFIRM 과 ITEMSTEP_PROGRESS 같은 소스
			...
			if( m_pUserItem )
			{
				...
				g_XBaseFont->PutsAlign(m_WindowPosition.x+50, m_WindowPosition.y + nCurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, _XGETINTERFACETEXT(ID_STRING_NEW_3432) );// [용린 바르기]
				...
				{
					memset(tempString, 0, sizeof(tempString));
					sprintf( tempString, "%s %s", _XGameItem::GetItemNick(m_pUserItem->m_cType, m_pUserItem->Get_m_sID()) , 
														 _XGameItem::GetItemName(m_pUserItem->m_cType, m_pUserItem->Get_m_sID()) );
				...
				{
					memset(tempString, 0, sizeof(tempString));
					sprintf( tempString, "%s", _XGameItem::GetItemName(m_pUserItem->m_cType, m_pUserItem->Get_m_sID())); 
				...
				int i = 0 ; for(; i < m_pUserItem->m_usCurMaxProtect; i++)
				{
		...
		if( mousestate->bButton[0])
		{
			...
			if(CheckMousePosition())
			{
					...
					g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART )
				{
					...
					{
						_XGI_WeaponItem_Property* tempWeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType);
					...
					if(CheckMousePosition())
					{
						if(scrnpos->x > m_WindowPosition.x+12 && scrnpos->x < m_WindowPosition.x+44)
						{
							if(scrnpos->z > m_WindowPosition.y+172 && scrnpos->z < m_WindowPosition.y+203)
							{
								if( g_pDefaultTooltip_Window )
								{							
									if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_INCHANT, m_nUserIndex, m_pUserItem->m_cType, m_pUserItem->Get_m_sID() ) )
									{
...
void XWindow_SmeltItem::DrawItem()
{
	...
	if( _XGI_FC_WEAPON == GetEqualItemType(m_pUserItem->m_cType) )
	{
		D3DXMATRIX			zrotmat;
...
int XWindow_SmeltItem::GetItemModelIndex()
{
	...
	if( _XGI_FC_WEAPON == GetEqualItemType( m_pUserItem->m_cType) )
	{
		_XGI_WeaponItem_Property* tempWeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_pUserItem->m_cType);
		if( tempWeaponItem )
				...
				case SCALES :
					{
						...
						if( g_pLocalUser->m_UserItemList[selectedslot].Get_m_sID() > 0 &&
							g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_LIFE &&
			...
			case SCALES :
				{		
					if( g_pLocalUser->m_UserItemList[count].Get_m_sID() > 0 &&
						g_pLocalUser->m_UserItemList[count].m_cType == _XGI_FC_LIFE &&
						g_LifeItemProperty[g_pLocalUser->m_UserItemList[count].Get_m_sID()].cSecondType == _XGI_SC_LIFE_SMLETITEM_SCALES )
					{
						//용린일 때, 용린의 개수를 합쳐준다
						nMaxCount += (int)(g_pLocalUser->m_UserItemList[count].m_ucCount);
					}
				}
				break;
			}
