...
void _XWindow_Rebirth::SetButton()
{
	...
	else if(m_RebirthType == _XREBIRTH_HONSU)
	{
		...
		m_FieldButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23+23);
		m_UseItemButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23+23+23);
#ifdef _ACCLAIM_IGAADSYSTEM
		m_AcclaimAdVideoButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23+23+23+23);
#endif
		// 0.
		if(g_pLocalUser->m_CharacterInfo.groupindex == _XGROUP_NANGIN)
		...
		case 3 :
			{
				m_SavePointButton->ShowWindow(FALSE);
				m_FieldButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23);
				m_UseItemButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23+23);
#ifdef _ACCLAIM_IGAADSYSTEM
				m_AcclaimAdVideoButton->MoveWindow(m_WindowPosition.x+5, m_WindowPosition.y+20+23+23+23);
#endif
			}
			break;
		}
		...
		case 1 :
			{
				m_UseItemButton->ShowWindow(FALSE);
#ifdef _ACCLAIM_IGAADSYSTEM
				POINT pos = m_UseItemButton->GetWindowPos();
				m_AcclaimAdVideoButton->MoveWindow(pos.x, pos.y);
#endif
			}
			break;
		}
