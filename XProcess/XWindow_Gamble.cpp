...
{	
	...
	if( m_GambleBoxItem )
	{
		...
		g_XBaseFont->SetColor(_XSC_DEFAULT);
		int tempLength = strlen( _XGameItem::GetItemName(m_GambleBoxItem->m_cType, m_GambleBoxItem->Get_m_sID() ));
		if( tempLength > 16 )
		{
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+40, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_GambleBoxItem->m_cType, m_GambleBoxItem->Get_m_sID() ) );
		}
		else
		{
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+40, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_GambleBoxItem->m_cType, m_GambleBoxItem->Get_m_sID() ) );
				...
				if(CheckMousePosition())
				{
					...
					if(g_pInventory_Window)	// 인벤토리에서 드래그 했을 경우
					{
						if(g_pInventory_Window->GetShowStatus() && !g_pLocalUser->m_bGambleBoxOpenLock)
						{
							...
							{
								if( g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_BOXKEY )
								{
									if( m_GambleBoxItem )
									{
										int insertindex = -1;
										...
										else
										{
											...
											{
												if( g_pLocalUser->m_UserItemList[ivindex].m_ucCount > 0 )
												{
													m_GambleKeyList[insertindex].Copy(g_pLocalUser->m_UserItemList[ivindex]);
													m_GambleKeyList[insertindex].m_ucCount = 1;
													m_GambleKeyFromInvenNumber[insertindex] = ivindex;
#ifdef VN_RB_GAMBLEBOX_070607_KUKURI
													// inven 건드리지 않는다
#else
													g_pLocalUser->m_UserItemList[ivindex].m_ucCount--;									
#endif
												}
																			
												PlayItemEquipSound( m_GambleKeyList[insertindex].m_cType, m_GambleKeyList[insertindex].Get_m_sID() );
			...
			if(m_WindowMouseState == _XW_STATE_RIGHTCLICK)
			{
				...
				if( !m_bHaveMouseSlot && !g_pLocalUser->m_bGambleBoxOpenLock )
				{
					...
					if( m_LastDraggingSlot != -1)
					{
						...
						// inven 건드리지 않음
#else
						g_pLocalUser->m_UserItemList[g_pInventory_Window->m_iSocketIndexList[m_LastDraggingSlot+1]].m_ucCount++;
#endif
						g_pInventory_Window->m_iSocketIndexList[m_LastDraggingSlot+1] = 0;

						// 소켓 중간이 빠졌을 경우
						int i = 0 ; for(  i = m_LastDraggingSlot+1; i < _XDEF_GAMBLEBOX_MAXCOUNT; i++ )
						{
							if( m_GambleKeyList[i].Get_m_sID() > 0 )
							{
...
void _XWindow_Gamble::DrawSocketPlugInionItem( void )
{
	...
	if( _XGI_FC_WEAPON == GetEqualItemType(m_GambleBoxItem->m_cType) )
#else
	if( m_GambleBoxItem->m_cType == _XGI_FC_WEAPON || m_GambleBoxItem->m_cType == _XGI_FC_WEAPON2 || m_GambleBoxItem->m_cType == _XGI_FC_WEAPON3 )
#endif
	{
		D3DXMATRIX			zrotmat;
	...
	FLOAT scalefactor;
	switch(m_GambleBoxItem->m_cType) 
	{
	case _XGI_FC_BOX :
		{
