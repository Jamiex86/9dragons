		...
		case _XDEF_CC_CREATEBUTTON  :
			...
			{	
				...
				{	
					...
					int idx = 0 ; for(  idx = 0; idx < 25; idx++ )
					{					
					...
					if( g_LanguageType == _XLANGUAGE_TYPE_KOREAN )
					{
						...
						{
							TCHAR messagestring[128];
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CREATECHARACTER_1794), _T("NEXON") );	
							g_MessageBox.SetMessage( _XDEF_DLGBTNTYPE_OK, warnstring, messagestring, TRUE );
							free( tempcharacternamestring );
							return;
						}
						...
						{
							TCHAR messagestring[128];
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CREATECHARACTER_1794), _T("넥슨") );	
							g_MessageBox.SetMessage( _XDEF_DLGBTNTYPE_OK, warnstring, messagestring, TRUE );
							free( tempcharacternamestring );
							return;
						}
						else 
						{
								...
								(pFourth = strstr( tempcharacternamestring, _T("선") ) ) != NULL ) // 캐릭터 이름내에  현공구선 사용 불가
							{
								...
								{
									TCHAR messagestring[128];
									sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CREATECHARACTER_1794), _T("현공구선") );	
									g_MessageBox.SetMessage( _XDEF_DLGBTNTYPE_OK, warnstring, messagestring, TRUE );
									free( tempcharacternamestring );
									return;
								}
						...
						if( strlength > 1 )
						{
							int i = 0 ; for(; i < strlength - 1; i++ )
							{
								...
								{
									TCHAR messagestring[128];
									sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_CREATECHARACTER_1794), _T("GM") );	
									g_MessageBox.SetMessage( _XDEF_DLGBTNTYPE_OK, warnstring, messagestring, TRUE );
									free( tempcharacternamestring );
									return;
								}
				...
				case _XCREATECHARACTERRESULT_NOVALIDASCIICHARACTER :
					{
						...
						if(g_LanguageType == _XLANGUAGE_TYPE_TAIWANESE)
						{
							...
							sprintf( message, _XGETINTERFACETEXT(ID_STRING_CREATECHARACTER_ERRORASCII), tempstring2 );	// "[%s]와(과) 같은 ASCII 문자는 사용하실 수 없습니다. (기호문자는 -,_,[,]만 가능합니다)"
							g_MessageBox.SetMessage( _XDEF_DLGBTNTYPE_OK, warnstring, message, TRUE );
							pCreateCharacter_Window->m_NameEdit.SetFocus();
						}
						else
...
//#endif
						{
							g_WordFilteringManager.Filtering( resultstring, _XDEF_WORDFILETERINGTYPE_ALWAYS, TRUE ); // 욕설만으로 필터링을 한번 더 체크 한다.
		...
		case _XDEF_CC_REQUESTCHARACTERLIST:
			{
				...
				memset( ((CEmperorOfDragonsApp*)gApp)->m_proc_selectcharacter.m_NewCreatedCharacterName, 0, sizeof(TCHAR) * 13 );
				strcpy( ((CEmperorOfDragonsApp*)gApp)->m_proc_selectcharacter.m_NewCreatedCharacterName, m_CharacterNameString );
				((CEmperorOfDragonsApp*)gApp)->m_ProcessManager.ChangeProcess( &((CEmperorOfDragonsApp*)gApp)->m_proc_selectcharacter );
			}
			break;
		// 성별
