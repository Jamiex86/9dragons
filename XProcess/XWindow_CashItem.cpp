			...
			if(!mousestate->bButton[0])
			{
				...
				if( curslot != -1 )
				{
					...
					if( m_CashItemMode == _XDEF_CASHITEM_BUYITEM_BTN )
					{
						...
						if( tempBuyItem )
						{
							if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_CASHITEM, 0, tempBuyItem->Item.m_cType, tempBuyItem->Item.Get_m_sID()) )
							{
					...
					else if( m_CashItemMode == _XDEF_CASHITEM_PRESENTITEM_BTN )
					{
						...
						if( tempPresentItem )
						{
							if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_CASHITEM, 0, tempPresentItem->Item.m_cType, tempPresentItem->Item.Get_m_sID()) )
							{
