...
BOOL _XWindow_Warehouse::Initialize(void)
{
	...
	int i = 0 ; for( ; i < _XDEF_WAREHOUSE_MAX_SENDCOUNT ; ++i)
	{
					...
					if(g_pInventory_Window)
					{
						if(g_pInventory_Window->GetShowStatus())
						{
							...
							if(g_pInventory_Window->m_bHaveMouseSlot && g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)
							{
								...
								else if( _XGameItem::CheckItemTempField4( g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType ,
					...
					if(m_bDragState)
					{
						if(scrnpos->x == m_IconDragStartPos.x && scrnpos->z == m_IconDragStartPos.y)
						{
							...
							if(g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].Get_m_sID() > 0)
							{
								...
								{
									SetTempItemFromWarehouse(m_LastDraggingSlot, g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].m_ucCount);									
								}
					...
					if(m_bDragState)
					{
						if(scrnpos->x == m_IconDragStartPos.x && scrnpos->z == m_IconDragStartPos.y)
						{
							...
							if(g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].Get_m_sID() > 0)
							{
								...
								m_MouseSlotItem.Copy(g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot]);
								m_MouseSlotItem.m_InventoryNumber = m_LastDraggingSlot;
								SetTempItemFromWarehouse(m_LastDraggingSlot, m_MouseSlotItem.m_ucCount);								
								if( g_pDefaultTooltip_Window )
								{
									...
									if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_WAREHOUSE_WAREHOUSEITEM, m_LastDraggingSlot,
										g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].m_cType, g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].Get_m_sID() ) )
									{
			...
			if(m_WindowMouseState == _XW_STATE_RIGHTCLICK)
			{
				...
				if(!m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot].Get_m_sID() > 0)
					{
						m_MouseSlotItem.Copy(g_pLocalUser->m_WarehouseItemList[m_LastDraggingSlot]);
						m_MouseSlotItem.m_InventoryNumber = m_LastDraggingSlot;
						SetTempItemFromWarehouse(m_MouseSlotItem.m_InventoryNumber, m_MouseSlotItem.m_ucCount);
...
void _XWindow_Warehouse::SetTempItemFromInven(int invenindex, int count)
{
	...
	if( g_pLocalUser->m_UserItemList[invenindex].m_cType == _XGI_FC_QUEST )
		return;
	...
	if(m_bProcessOneClickItem)
	{
		...
		for(iter_item = m_TempItemList.begin() ; iter_item != m_TempItemList.end() ; ++iter_item)
		{
			...
			if(pWarehouseItem)
			{
				...
				{
					pWarehouseItem->m_ucCount = count;
					m_bProcessOneClickItem = FALSE;
					break;
				}
			}
	...
	for(iter_item = m_TempItemList.begin() ; iter_item != m_TempItemList.end() ; ++iter_item)
	{
		...
		if(pWarehouseItem)
		{
			...
			if(pWarehouseItem->IsStackable())
			{
				...
				g_pLocalUser->m_WarehouseItemList[pWarehouseItem->m_WarehouseNumber].m_ucCount = pWarehouseItem->m_ReceiveCount;
				g_pLocalUser->m_UserItemList[pWarehouseItem->m_InventoryNumber].m_ucCount -= pWarehouseItem->m_ucCount;
				if(g_pLocalUser->m_UserItemList[pWarehouseItem->m_InventoryNumber].m_ucCount <= 0)
				{
			...
			strcpy(namestring, _XGameItem::GetItemName(g_pLocalUser->m_WarehouseItemList[pWarehouseItem->m_WarehouseNumber].m_cType, 
	...
	for(iter_item = m_TempItemList.begin() ; iter_item != m_TempItemList.end() ; ++iter_item)
	{
		...
		if(pWarehouseItem)
		{
			...
			if(pWarehouseItem->IsStackable())
			{
				...
				g_pLocalUser->m_UserItemList[pWarehouseItem->m_InventoryNumber].m_ucCount = pWarehouseItem->m_ReceiveCount;
				g_pLocalUser->m_WarehouseItemList[pWarehouseItem->m_WarehouseNumber].m_ucCount -= pWarehouseItem->m_ucCount;
				if(g_pLocalUser->m_WarehouseItemList[pWarehouseItem->m_WarehouseNumber].m_ucCount <= 0)
				{
			...
			TCHAR		namestring[128];
			memset(namestring, 0, sizeof(TCHAR)*128);
			strcpy(namestring, _XGameItem::GetItemName(g_pLocalUser->m_UserItemList[pWarehouseItem->m_InventoryNumber].m_cType,
