				...
				if(CheckMousePosition())
				{
					if(g_pInventory_Window)	// 인벤토리에서 드래그 했을 경우
					{
						if(g_pInventory_Window->GetShowStatus())
						{
							...
							{
								if(CheckItem(g_pInventory_Window->m_MouseSlotItem.m_cType,g_pInventory_Window->m_MouseSlotItem.Get_m_sID(), g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber))
								{
									SetItemSlot(g_pInventory_Window->m_MouseSlotItem.m_cType, g_pInventory_Window->m_MouseSlotItem.Get_m_sID(), g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber);
...
BOOL _XWindow_MixWeapon::CheckItem(int cType, int sID, int nInvenNumber)
{
	// 인벤 체크
	BOOL bValid = FALSE;
	if(g_pLocalUser->m_UserItemList[nInvenNumber].m_cType == cType &&
		g_pLocalUser->m_UserItemList[nInvenNumber].Get_m_sID() == sID)
	{
		if(g_pLocalUser->m_UserItemList[nInvenNumber].m_ucCount > 0)
		{
			bValid = TRUE;
		}
	}
