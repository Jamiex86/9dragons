...
void _XWindow_Inchant::Draw( _XGUIObject*& pfocusobject )
{
	...
	if( m_InchantItem )
	{
		...
		g_XBaseFont->PutsAlign(m_WindowPosition.x + 58, m_WindowPosition.y + nCurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, _XGETINTERFACETEXT(ID_STRING_NEW_3443) ); //[제련 정보]
		...
		g_XBaseFont->SetColor(_XGameItem::GetGradeColor(m_InchantItem->m_cType, m_InchantItem->Get_m_sID()));
		if( _XGameItem::GetItemNick(m_InchantItem->m_cType, m_InchantItem->Get_m_sID() ) )
		{
			memset(tempString, 0, sizeof(tempString));
			sprintf( tempString, "%s %s", _XGameItem::GetItemNick(m_InchantItem->m_cType, m_InchantItem->Get_m_sID()) , 
												 _XGameItem::GetItemName(m_InchantItem->m_cType, m_InchantItem->Get_m_sID()) );
		...
		{
			memset(tempString, 0, sizeof(tempString));
			sprintf( tempString, "%s", _XGameItem::GetItemName(m_InchantItem->m_cType, m_InchantItem->Get_m_sID())); 
			g_XBaseFont->PutsAlign(m_WindowPosition.x + 50, m_WindowPosition.y + nCurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, tempString );
		}
		...
		memset(tempString, 0, sizeof(tempString));
		sprintf( tempString, ":" ); 
		g_XBaseFont->PutsAlign(m_WindowPosition.x + 45, m_WindowPosition.y + nCurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, tempString );

		int nSmallFontDraw = 0;
		int i = 0 ; for(; i < m_InchantItem->m_usCurMaxProtect; i++)
		{
			if( m_InchantItem->m_usCurProtect > i )
			{
				_XDrawSolidBar( m_WindowPosition.x+ 60 +(i*3), m_WindowPosition.y + nCurrentDrawPosY,
					m_WindowPosition.x+60+2+(i*3), m_WindowPosition.y + nCurrentDrawPosY+11, D3DCOLOR_ARGB(255,0,240,100));
			}
			else
			{
				_XDrawSolidBar( m_WindowPosition.x+ 60 +(i*3), m_WindowPosition.y + nCurrentDrawPosY,
					m_WindowPosition.x+60+2+(i*3), m_WindowPosition.y + nCurrentDrawPosY+11, D3DCOLOR_ARGB(255,255,255,255));
			}
			nSmallFontDraw = (i*3);
		}
		...
		g_XSmallFont->PutsAlign(m_WindowPosition.x + 70 + nSmallFontDraw, m_WindowPosition.y + nCurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, tempString );
		g_XSmallFont->Flush();	
	...
	if( m_InchantItem )
	{
...
#endif
		{
			...
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+32, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemNick(m_InchantItem->m_cType, m_InchantItem->Get_m_sID() ) );
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+47, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_InchantItem->m_cType, m_InchantItem->Get_m_sID() ) );
		}
		else
		{
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+40, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_InchantItem->m_cType, m_InchantItem->Get_m_sID() ) );
		}
						...
						if(g_pInventory_Window->GetShowStatus())
						{
							if(g_pInventory_Window->m_bHaveMouseSlot && g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)
							{
								   ...
								   g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)
								{
									...
									if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType) )
#else
									if( g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType == _XGI_FC_WEAPON ||
										g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType == _XGI_FC_WEAPON2 ||
										g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType == _XGI_FC_WEAPON3 )
#endif
									{
										// -----------------------------------------------------------------------------------------------=
									...
									else if( g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType == _XGI_FC_LIFE )
									{
										if( m_InchantItem )
										{							
											...
											else
											{
												...
												if( check == 1 )
												{
													...
													m_InchantStuff[1].m_ucCount = m_InchantStuff[0].m_ucCount;
													...
													g_pInventory_Window->m_bHaveMouseSlot = FALSE;
													g_pInventory_Window->m_MouseSlotItem.ResetMouseSlotItem();
													PlayItemEquipSound( m_InchantItem->m_cType, m_InchantItem->Get_m_sID() );
												}
												else if( check == -1 )
												{
													MessagePrint(7);									
					...
					if(CheckMousePosition())
					{
						if(scrnpos->x > m_WindowPosition.x+12 && scrnpos->x < m_WindowPosition.x+44)
						{
							if(scrnpos->z > m_WindowPosition.y+172 && scrnpos->z < m_WindowPosition.y+203)
							{
								if( g_pDefaultTooltip_Window )
								{							
									if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_INCHANT, 0, m_InchantStuff[0].m_cType, m_InchantStuff[0].Get_m_sID() ) )
									{
...
#ifdef _XTS_ITEM_OPTIMIZATION
	if( _XGI_FC_WEAPON == GetEqualItemType(m_InchantItem->m_cType) )
#else
...
int _XWindow_Inchant::GetInchantItemModelIndex(void)
{
	...
	if( _XGI_FC_WEAPON == GetEqualItemType(m_InchantItem->m_cType) )
	{
		_XGI_WeaponItem_Property* tempWeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_InchantItem->m_cType);
		if( tempWeaponItem )
	...
	case US_STEP2 :	// 제련버튼 클릭시
		{
			...
			LPSTR	param3 = _XGameItem::GetItemName(m_InchantItem->m_cType, m_InchantItem->Get_m_sID());
	...
	case US_STEP1 :	// 재료를 올린 상태
		{
			if( m_InchantItem )
			{
				...
				m_InchantStuff[0].Reset();
				m_InchantStuff[0].m_cType	= g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].cType;
				m_InchantStuff[0].Set_m_sID( g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].sID );
				m_InchantStuff[0].m_ucCount = g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].ucCount;
				m_InchantStuff[0].m_cSecond = _XGI_SC_LIFE_CONSUMING;	// 임시

#ifdef _XDEF_EXPAND_INCHANT_TABLE_080425_KUKURI
				m_InchantStuff[2].Reset();
				m_InchantStuff[2].m_cType	= g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].cType;
				m_InchantStuff[2].Set_m_sID( g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].sID_2 );
				m_InchantStuff[2].m_ucCount = g_ItemInchantTable[m_InchantItem->Get_m_ucStrength()].ucCount;
				m_InchantStuff[2].m_cSecond = _XGI_SC_LIFE_CONSUMING;	// 임시
#endif
			}
		}
		break;
...
int _XWindow_Inchant::CheckStuffCount(int selectslot)
{
		...
		g_pLocalUser->m_UserItemList[selectslot].m_cType == m_InchantStuff[0].m_cType )
	{
		...
		g_pLocalUser->m_UserItemList[selectslot].m_cType == m_InchantStuff[2].m_cType )
	{
		if( g_pLocalUser->m_UserItemList[selectslot].m_ucCount >= m_InchantStuff[2].m_ucCount )
			return 1;
		else
			return -1;
	}
...
{
	int grade = _XGameItem::GetItemGrade(g_pLocalUser->m_UserItemList[searchslot].m_cType, g_pLocalUser->m_UserItemList[searchslot].Get_m_sID());
	switch(grade) 
	{
	case 0:
	case 10:
	...
	case 30 :	// 무기 조합으로 추가된 무기
		{
			int inchantgrade = 0;
			if(g_LanguageType == _XLANGUAGE_TYPE_KOREAN || g_LanguageType == _XLANGUAGE_TYPE_TAIWANESE || g_LanguageType == _XLANGUAGE_TYPE_VIETNAMESE)
				inchantgrade = 11;	// 제련 12단계로 확장
			else
...
{
#ifdef _XDEF_PERIODITEM_20061027
	if(CheckCashItem(g_pLocalUser->m_UserItemList[slotindex].m_cType, g_pLocalUser->m_UserItemList[slotindex].Get_m_sID()) == 2 || 
		CheckCashItem(g_pLocalUser->m_UserItemList[slotindex].m_cType, g_pLocalUser->m_UserItemList[slotindex].Get_m_sID()) == 3)
	{
