		...
		if(pfocusobject)
		{
			...
			if(mousestate->bButton[0])
			{
				...
				if( selectedindex > -1 )
				{
					...
					{
						char cType = m_SelectedTargetInfo.getOnItems[m_SelectedSlotIndex].m_cType;
...
{
	int iter = 0 ; for(  iter = _TARGETINFO::pos_amor; iter < _TARGETINFO::get_on_max ; iter++)
	{
...
int	_XWindow_TargetInfo::SearchSlotRegion(  RECT* pRect  )
{
	...
	// 장착 슬롯 검사
	int iter = 0 ; for(  iter = _XINVENTORY_SLOTTYPE_AMOR; iter <= _XINVENTORY_SLOTTYPE_WEAPON_2; iter++ )
	{
