...
*/
	...
	if( resourceid >= 0 )
	{
		...
		FILE* pFile = package.GetPackedFile( resourceid );
		if(pFile){
			g_pTitleFlashObject->SetResourceFilePtr( pFile , filesize );	
	...
	if( pQSlot_Window )
	{	
		int i = 0 ; for( i = 0; i < _QUICKSLOT_COUNT_; i++ )
		{
...
//			strcpy( registrybuffer,  _T("85.17.144.237") );
	...
	{
		int index = 0;
		int i = 0 ; for( i = 0; i < _COUNT_MAX_GROUP_; ++i )
		{
