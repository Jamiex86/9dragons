...
{		
	...
	int i = 0 ; for( ; i < 5 ; i++) //Author : 양희왕 //breif : 행낭 확장 
	{
...
//	m_ExpandDockingBorder->SetClipRect( 0, 79, 183, 100);	
	...
	_XImageStatic* pImageWasteBasket= new _XImageStatic;
	...
	InsertChildObject( pImageWasteBasket );
#endif
...
int	_XWindow_Inventory::SearchSlotRegion(  RECT* pRect  )
{
	...
	if( selectedslot == -1 )
	{
		...
		int row = 0 ; for( row = 0; row < _XDEF_INVENTORYSLOT_HEIGHT; row++ )
		{
			int column = 0 ; for( column = 0; column < _XDEF_INVENTORYSLOT_WIDTH; column++ )
			{
				if( count >= _XINVENTORY_SLOTTYPE_TOTALCOUNT ) break;
	...
	if( m_pGamble_Window )
	{
		...
		{
			int i = 0 ; for(; i < _XDEF_GAMBLEBOX_MAXCOUNT+1; i++ )
			{
				if( m_iSocketIndexList[i] == selectedslot )
				{
					if( i == 0 || g_pLocalUser->m_UserItemList[selectedslot].m_ucCount == 0 )
						return -1;
				}
			}			
		}
...
void SingleModeChangeItem( char cFrom, char cTo )
{
	...
	if(pInventory_Window)
	{
		...
		if( cTo >= _XINVENTORY_SLOTTYPE_AMOR && cTo <= _XINVENTORY_SLOTTYPE_ACCESSORY_8 )
		{
			...
			ChangeModel( lparam, g_pLocalUser->m_UserItemList[cTo].m_cType );
		}
					...
					if(scrnpos->x == m_IconDragStartPos.x && scrnpos->z == m_IconDragStartPos.y)
					{
						if( m_pWarehouseWindow )
						{
							if( m_pWarehouseWindow->GetShowStatus() )
							{
								...
								//Author : 양희왕 //breif : 수정
								if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_QUEST ) // 퀘스트 아이템 선택 X
								{
								...
								else if( _XGameItem::CheckItemTempField4( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType ,
						...
						if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0)
						{
							...
							if( g_pDefaultTooltip_Window )
							{
								if(m_pNPCTradeWindow )
								{
									...
									{
										int type = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType;
...
//	if(g_MessageBox.m_ShowWindow) return;
	...
	{
		ScrnPos* scrnpos = gpInput->GetMouseVirScreenPos();
		DrawIcon(TRUE, scrnpos->x - 16 , scrnpos->z - 16);
...
void _XWindow_Inventory::DrawIcon(BOOL smallicon, int X, int Y, _XUserItem* pItem, BOOL bMarkStackble,  BOOL busingitem)// 2004.06.10->oneway48 insert
{
	...
	if(!pItem)
	{
		cType = m_MouseSlotItem.m_cType;
		sID = m_MouseSlotItem.Get_m_sID();
		ucCount = m_MouseSlotItem.m_ucCount;
				...
				case _XGI_SC_CLOTHES_MASK :
					{
						...
						if( sIconNo < 32 )
						{
							imageindex = g_MainInterfaceTextureArchive.FindResource("Item_Clo_Mask_01.tga");
						}
						else if( sIconNo < 64 )
						{
							sIconNo -= 32;
							iconclipcolumn = sIconNo % 8;
							iconcliprow    = sIconNo / 8;
							imageindex = g_MainInterfaceTextureArchive.FindResource("Item_Clo_Mask_02.tga");
						}
						else if( sIconNo < 96 )
						{
							sIconNo -= 64;
							iconclipcolumn = sIconNo % 8;
							iconcliprow    = sIconNo / 8;
							imageindex = g_MainInterfaceTextureArchive.FindResource("Item_Clo_Mask_03.tga");
						}
#endif
					}
					break;
...
{		
	...
	case _XINVENTORY_SLOTTYPE_AMOR :			// 0	상의
		{
			BOOL accCount = FALSE;
			if( _XGI_FC_CLOTHES != GetEqualItemType(m_MouseSlotItem.m_cType) )
			{
				return _XEQUIPERROR_TYPE;
			}

			_XGI_ClothesItem_Property* tempClothesitem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_MouseSlotItem.m_cType);
			if( tempClothesitem )
			{
				...
				//if( accCount && (g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_AMOR].Get_m_sID() > 0) )
				{
					_XGI_ClothesItem_Property* tempClothesitem2 = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_AMOR].m_cType);
					if( tempClothesitem2 )
					{
						//if( tempClothesitem[m_MouseSlotItem.Get_m_sID()].cPocket < tempClothesitem2[g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_AMOR].Get_m_sID()].cPocket )
						if( tempClothesitem[m_MouseSlotItem.Get_m_sID()].cPocket < nAccCount )
	...
	case _XINVENTORY_SLOTTYPE_PANTS :			// 1	하의
		{
			...
			if( tempClothesitem )
			{
				...
				//if( accCount && (g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_PANTS].Get_m_sID() > 0) )
				{
					_XGI_ClothesItem_Property* tempClothesitem2 = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_PANTS].m_cType);
					if( tempClothesitem2 )
					{
						//if( tempClothesitem[m_MouseSlotItem.Get_m_sID()].cPocket < tempClothesitem2[g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_PANTS].Get_m_sID()].cPocket )
						if( tempClothesitem[m_MouseSlotItem.Get_m_sID()].cPocket < accCount)
							return _XEQUIPERROR_POCKET;						
					}
				}			
#ifdef _XTS_FAME
	...
	case _XINVENTORY_SLOTTYPE_GAP_LOWER :		// 6	갑주 하
		{			
			...
			if( m_MouseSlotItem.m_cType != _XGI_FC_ACCESSORY )
			{
				return _XEQUIPERROR_TYPE;
			}
			...
			if( m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART )
			{
				...
				if( g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cSecond == 0 && g_AccessoryItemProperty[m_MouseSlotItem.Get_m_sID()].cClan == 20 )
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_GAP_UPPER ; i <= _XINVENTORY_SLOTTYPE_GAP_LOWER; i++ )
					{
						if( i != slotindex )
						{
...
#ifdef _XTS_ITEM_V22
				int retvalue = _XGameItem::CheckAccessoryReq(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID());
				if(retvalue != 0)
				{
					switch(retvalue)
					{
	...
	case _XINVENTORY_SLOTTYPE_WEAPON_2 :		// 9	무기 2
		{				
#ifdef _XTS_FAME
			if( _XGI_FC_WEAPON == GetEqualItemType(m_MouseSlotItem.m_cType) )
			{	
				_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_MouseSlotItem.m_cType);
				if( WeaponItem )
				{
	...
	case _XINVENTORY_SLOTTYPE_ACCESSORY_8 :
		{
			...
			if( m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART )
			{
				...
				if( g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cSecond == 0 && g_AccessoryItemProperty[m_MouseSlotItem.Get_m_sID()].cClan == 20 )
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_1 ; i <= _XINVENTORY_SLOTTYPE_ACCESSORY_8; i++ )
					{
						if( i != slotindex )
						{
							if( m_MouseSlotItem.Get_m_sID() == g_pLocalUser->m_UserItemList[i].Get_m_sID() )
...
/*			if( g_AccessoryItemProperty[m_MouseSlotItem.Get_m_sID()].ucFame > 0 )
	...
	for(iter_holditem = g_QuestScriptManager.m_listHoldItem.begin() ; iter_holditem != g_QuestScriptManager.m_listHoldItem.end() ; )
	{
		...
		if( questholditem.m_cHoldItemType == m_MouseSlotItem.m_cType && questholditem.m_sHoldItemID == m_MouseSlotItem.Get_m_sID() )
		{
			bexist = true;
...
void _XWindow_Inventory::ProcessDblClick(void)
{
	...
	m_LastDraggingSlot = SearchSlotRegion();	
	if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].Get_m_sID() <= 0 ) return;	
	...
	if(m_pNPCTradeWindow)
	{
		if(m_pNPCTradeWindow->GetShowStatus())
		{
			if(CheckMousePosition())
			{
				if( m_LastDraggingSlot != -1 && g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && 
					m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART )
				{
					if( _XGameItem::CheckNPCTrade( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType, g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() ) )
					{						
						_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
					...
					int resultcashcheck = CheckCashItem(m_MouseSlotItem.m_cType,m_MouseSlotItem.Get_m_sID());
					...
					else 
					{
						...
						strcpy(namestring, _XGameItem::GetItemName(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID()));
	...
	if(m_pPCTradeWindow)
	{
		if(m_pPCTradeWindow->GetShowStatus())
		{
			if(CheckMousePosition())
			{
				if(m_LastDraggingSlot != -1 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
				{
					if( _XGameItem::CheckUserTrade( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType, g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].Get_m_sID() ) )
					{
						_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	if(m_pRepairItem_Window)
	{
		if(m_pRepairItem_Window->GetShowStatus())
		{
			if( m_pRepairItem_Window->m_RepairMode != RM_UNDERREPAIR )
			{
				if(CheckMousePosition())
				{
					if( m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART && g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 )
					{
						if( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType == _XGI_FC_WEAPON ||
							g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType ==  _XGI_FC_CLOTHES || 
							g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType == _XGI_FC_WEAPON2 ||
							g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType ==  _XGI_FC_CLOTHES2 ||
							g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType == _XGI_FC_WEAPON3 ||
							g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType ==  _XGI_FC_CLOTHES3 )
						{
		...
		if(g_pLocalUser->m_UserItemList[selectedslot].IsStackable())
		{
			...
			else
			{
				...
				if(pTradeConfirm_Window)
				{
					pTradeConfirm_Window->ShowWindow(TRUE);									
					pTradeConfirm_Window->m_IMEControl.SetFocus();
					pTradeConfirm_Window->m_SelectedCategory1 = g_pLocalUser->m_UserItemList[selectedslot].m_cType;
			...
			if(m_pNPCTradeWindow->m_bHaveMouseSlot)
			{
				...
				if(m_pNPCTradeWindow->m_MouseSlotItem.IsStackable())
				{
					...
					if(pTradeConfirm_Window)
					{
						pTradeConfirm_Window->ShowWindow(TRUE);
						pTradeConfirm_Window->m_IMEControl.SetFocus();
						pTradeConfirm_Window->m_SelectedCategory1 = m_pNPCTradeWindow->m_MouseSlotItem.m_cType;
				...
				else
				{
					...
					TCHAR		namestring[128];
					memset(namestring, 0, sizeof(TCHAR)*128);
					strcpy(namestring, _XGameItem::GetItemName(m_pNPCTradeWindow->m_MouseSlotItem.m_cType, m_pNPCTradeWindow->m_MouseSlotItem.Get_m_sID()));
			...
			if(m_pWarehouseWindow->m_bHaveMouseSlot)
			{
				...
				if(m_pWarehouseWindow->m_MouseSlotItem.IsStackable())
				{
					...
					if(pTradeConfirm_Window)
					{
						pTradeConfirm_Window->ShowWindow(TRUE);
						pTradeConfirm_Window->m_IMEControl.SetFocus();
						pTradeConfirm_Window->m_SelectedCategory1 = m_pWarehouseWindow->m_MouseSlotItem.m_cType;
				...
				{
					m_pWarehouseWindow->SetTempItemFromWarehouse(m_pWarehouseWindow->m_MouseSlotItem.m_InventoryNumber, m_MouseSlotItem.m_ucCount);
			...
			if(CheckMousePosition())
			{
				if(m_pPCTradeWindow->m_bHaveMouseSlot)
				{
					g_NetworkKernel.SendPacket(MSG_NO_PVP_TRADE_DELETE, m_pPCTradeWindow->m_MouseSlotItem.m_InventoryNumber,
												m_pPCTradeWindow->m_MouseSlotItem.m_ucCount);
					return TRUE;
				}
			}
		}
		...
		if( m_pSocketPlugIn_Window->GetShowStatus() && !g_pLocalUser->m_bSocketPlugInLock )
		{
			...
			{		
				...
				else
				{
					...
					// 소켓 중간이 빠졌을 경우
					int i = 0 ; for(  i = tempslotindex+1; i < m_pSocketPlugIn_Window->m_IndependentItem->Get_m_ucSocketCount(); i++ )
					{
						...
						{
							int j = 0 ; for( j = 0; j < m_pSocketPlugIn_Window->m_IndependentItem->Get_m_ucSocketCount(); j++ )
							{
					...
					if( tempflag )
					{
						...
						BOOL tempflag2 = TRUE;
						int i = 0 ; for(; i < m_pSocketPlugIn_Window->m_IndependentItem->Get_m_ucSocketCount()+1; i++ )
						{
							if( m_pSocketPlugIn_Window->m_SocketItemList[i].Get_m_sID() > 0 )
								tempflag2 = FALSE;
						}
			...
			{
				{
					...
					// inven 건드리지 않음
#else
					g_pLocalUser->m_UserItemList[m_iSocketIndexList[tempslotindex+1]].m_ucCount++;
#endif
					m_iSocketIndexList[tempslotindex+1] = 0;
					
					// 소켓 중간이 빠졌을 경우
					int i = 0 ; for(  i = tempslotindex+1; i < _XDEF_GAMBLEBOX_MAXCOUNT; i++ )
					{
...
//		if( pTradeConfirm_Window )
		{
			TCHAR messagestring[256];
			memset(messagestring, 0, sizeof(TCHAR)*256);
			if( g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cType == _XGI_FC_QUEST )
			{
			...
			int resultcashcheck = CheckCashItem(g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cType,g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].Get_m_sID());
			if( resultcashcheck == 1 || resultcashcheck == 2 )
			{
...
{
	if( _XGameItem::CheckDump( m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID()) )
	{
		_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	{
		int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_1; i < _XINVENTORY_SLOTTYPE_ACCESSORY_1+g_pLocalUser->m_cAmorPocketNum; i++ )
		{
			if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
			{
	...
	{
		int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_5; i < _XINVENTORY_SLOTTYPE_ACCESSORY_5+g_pLocalUser->m_cPantsPocketNum; i++ )
		{
			if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
			{
				g_NetworkKernel.InsertChatString(_XGETINTERFACETEXT(ID_STRING_CINFO_POCKET), _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS); // 주머니를 비워야 옷을 갈아 입을 수 있습니다.
	...
	else if( m_MouseSlotItem.m_InventoryNumber == _XINVENTORY_SLOTTYPE_BACKPACK1 )
	{
		...
		{
		#ifdef _XDEF_CASHBACKPACK_080404_MAGVIPER //Author : 양희왕 //breif : 행낭 확장
			int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART; i < _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART; i++ )
		#else
			int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend; i++ )
		#endif
			{
				if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
				{
	...
	{
	#ifdef _XDEF_CASHBACKPACK_080404_MAGVIPER //Author : 양희왕 //breif : 행낭 확장
		int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART; i < _XINVENTORY_SLOTTYPE_FIRST_CASHBACKSTART; i++ )		
	#else
		int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; i++ )
	#endif
		{
			if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
			{
...
void _XWindow_Inventory::ProcessInvenMove(void)
{
	if(m_LastDraggingSlot != -1)
	{
		...
		else if ( m_MouseSlotItem.m_InventoryNumber == _XINVENTORY_SLOTTYPE_BACKPACK2 )
		{
			...
			int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend; i++ )
			{
				if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
				{
					g_NetworkKernel.InsertChatString(_XGETINTERFACETEXT(ID_STRING_CINFO_EMPTYIBACKPACK), _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);	// "행낭의 물건을 비운 후 시도 하십시오."
		...
		else if ( m_MouseSlotItem.m_InventoryNumber == _XINVENTORY_SLOTTYPE_CASHBACKPACK1 )
		{
			...
			{
				//1캐시행낭 안에 물품이있다.				
				int i = _XINVENTORY_SLOTTYPE_FIRST_CASHBACKSTART ; for( ; i < _XINVENTORY_SLOTTYPE_SECOND_CASHBACKSTART; i++ )
				{
					if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
					{
						g_NetworkKernel.InsertChatString(_XGETINTERFACETEXT(ID_STRING_CINFO_EMPTYIBACKPACK), _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);	// "행낭의 물건을 비운 후 시도 하십시오."
		...
		else if ( m_MouseSlotItem.m_InventoryNumber == _XINVENTORY_SLOTTYPE_CASHBACKPACK2 )
		{
			...
			//2캐시행낭 안에 물품이있다.
			int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_SECOND_CASHBACKSTART; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; i++ )
			{
				if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
				{
					g_NetworkKernel.InsertChatString(_XGETINTERFACETEXT(ID_STRING_CINFO_EMPTYIBACKPACK), _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);	// "행낭의 물건을 비운 후 시도 하십시오."
		...
		else
		{
			if(!m_bLockInventory)
			{
				...
				if(m_MouseSlotItem.IsStackable())
				{
					if(m_MouseSlotItem.m_cType == g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType 
						&& m_MouseSlotItem.Get_m_sID() == g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID()
						&& m_MouseSlotItem.m_cType != _XGI_FC_CONSUMING )
					{
						if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount >= 255)
						{
							g_NetworkKernel.SendPacket(MSG_NO_INVEN_MOVE, 0, m_LastDraggingSlot );
						}
						else if((m_MouseSlotItem.m_ucCount+g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount) > 255)
						{
							m_MouseSlotItem.m_ucCount = 255 - g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount;
		...
		if(m_pNPCTradeWindow->GetShowStatus())
		{
			...
			{				
				...
				if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
				{
					...
					TCHAR messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);
					int resultcashcheck = CheckCashItem(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType,g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID());
					if( resultcashcheck == 1 || resultcashcheck == 2 )
					{
						sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2981) );	// 캐시아이템을 버리게 되면 영원히 삭제되며 환불/복구 되지 않습니다.판매하시겠습니까?
					}
					else
					{
						TCHAR		namestring[128];
						memset(namestring, 0, sizeof(TCHAR)*128);
						strcpy(namestring, _XGameItem::GetItemName(g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType, g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].Get_m_sID()));
...
{
	int i = _XINVENTORY_SLOTTYPE_POCKETSTART ; for( ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; i++)
	{
		if(g_pLocalUser->m_UserItemList[i].Get_m_sID() == 0)
			return i;
	}
...
BOOL _XWindow_Inventory::ProcessToSocketPlugIn(void)
{
	if( m_pSocketPlugIn_Window )
	{
		if( m_pSocketPlugIn_Window->GetShowStatus() && !g_pLocalUser->m_bSocketPlugInLock )
		{
			if( !m_pSocketPlugIn_Window->m_bHaveMouseSlot && !this->m_bHaveMouseSlot )
			{
				...
				{
#ifdef _XTS_ITEM_OPTIMIZATION
					if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType) ||
						_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType) )
#else
					if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_WEAPON ||
						g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_CLOTHES ||
						g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_WEAPON2 ||
					...
					{
						// error 처리 ------------------------------------------------------------------------------------=
						if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cSecond == _XGI_SC_CLOTHES_BACKPACK )
						{
						...
						if(_XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType))
						{
							_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType);
							if( WeaponItem )
							{
								socketcheck = WeaponItem[g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID()].ucSocketCheck;
							}
						}
						else if(_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType))
						{
							_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType);
							if( ClothesItem )
							{
						...
						else		// independent item 이 없을 경우
						{
							...
							PlayItemEquipSound( m_pSocketPlugIn_Window->m_IndependentItem->m_cType, m_pSocketPlugIn_Window->m_IndependentItem->Get_m_sID() );

							int i = 0 ; for(; i < _XDEF_SOCKETITEM_MAXCOUNT/*g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucSocketCount*/; i++ )
							{
								if( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sSocket(i) )
								{
									bflag = true;
									m_pSocketPlugIn_Window->m_SocketItemList[i].Reset();
									m_pSocketPlugIn_Window->m_SocketItemList[i].m_cType	= _XGI_FC_SOCKET;
					...
					else if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_SOCKET )	// dependent item
					{
						if( m_pSocketPlugIn_Window->m_IndependentItem )
						{							
							int insertindex = -1;
							int i = 0 ; for(; i < m_pSocketPlugIn_Window->m_IndependentItem->Get_m_ucSocketCount(); i++ )
							{
								if( m_pSocketPlugIn_Window->m_SocketItemList[i].Get_m_sID() == 0 )
								{
									insertindex = i;									
							...
							{
								// error 처리 ------------------------------------------------------------------------------------=
								char temp1 = m_pSocketPlugIn_Window->m_IndependentItem->m_cType;
								char temp2 = g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cSecond;
								...
								if( m_pSocketPlugIn_Window->m_IndependentItem->m_cType == _XGI_FC_CLOTHES ||
									m_pSocketPlugIn_Window->m_IndependentItem->m_cType == _XGI_FC_CLOTHES2 ||
									m_pSocketPlugIn_Window->m_IndependentItem->m_cType == _XGI_FC_CLOTHES3 )
								{
									if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cSecond == 1 )
									{
										...
										case _XGI_SC_CLOTHES_MASK :			// 치장 = 8
											{
												if(m_pSocketPlugIn_Window->m_IndependentItem->m_cSecond != g_SocketItemProperty[g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].Get_m_sID()].ucThirdType)
												{
													//맞는 장착물이 아님
									...
									{
										_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_pSocketPlugIn_Window->m_IndependentItem->m_cType);
										if( ClothesItem )
										{
	...
	#ifdef _XDEF_NEW_SOCKET_071031_KUKURI
								if(_XGI_FC_WEAPON == GetEqualItemType(m_pSocketPlugIn_Window->m_IndependentItem->m_cType))
								{
									if(g_SocketItemProperty[g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID()].ucGrade > 0)
									{
										_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_pSocketPlugIn_Window->m_IndependentItem->m_cType);
										if( WeaponItem )
										{
								...
								PlayItemEquipSound( m_pSocketPlugIn_Window->m_SocketItemList[insertindex].m_cType, m_pSocketPlugIn_Window->m_SocketItemList[insertindex].Get_m_sID() );
...
BOOL _XWindow_Inventory::ProcessToSocketDetach(void)
{
	...
	if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0  && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
	{
...
#endif
		{
			...
			if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cSecond == _XGI_SC_CLOTHES_BACKPACK || //행낭
			...
			PlayItemEquipSound( pItem->m_cType, sID );
		...
		if( m_pInchant_Window->GetShowStatus() )
		{
			if( !this->m_bHaveMouseSlot )
			{
				...
				if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0  && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
				{
					...
					else if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_LIFE )
					{
						...
						{							
							...
							else
							{
								...
								if( check == 1 )
								{
									...
									m_pInchant_Window->m_InchantStuff[1].m_ucCount = m_pInchant_Window->m_InchantStuff[0].m_ucCount;
									m_iSocketIndexList[0] = m_LastDraggingSlot;
									m_pInchant_Window->SetItemInchantStep(US_STEP1);
									PlayItemEquipSound( m_pInchant_Window->m_InchantStuff[1].m_cType, m_pInchant_Window->m_InchantStuff[1].Get_m_sID() );
								}
								else if( check == -1 )
								{
...
BOOL _XWindow_Inventory::ProcessToSmeltItem(void)
{
	...
	if( !this->m_bHaveMouseSlot )
	{
		...
		if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0  && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
		{
			if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType) )
			{
				_XGI_WeaponItem_Property* tempWeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType);
		...
		if( m_pGamble_Window->GetShowStatus() && !g_pLocalUser->m_bGambleBoxOpenLock )
		{
			if( !m_pGamble_Window->m_bHaveMouseSlot && !this->m_bHaveMouseSlot )
			{
				...
				if( m_LastDraggingSlot != -1)
				{
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0  && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_BOXKEY )	// dependent item
						{
							if( m_pGamble_Window->m_GambleBoxItem )
							{							
								int insertindex = -1;
								int i = 0 ; for(; i < _XDEF_GAMBLEBOX_MAXCOUNT; i++ )
								{
									if( m_pGamble_Window->m_GambleKeyList[i].Get_m_sID() == 0 )
									{
										insertindex = i;									
								...
								else
								{
									...
									{
										if( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount > 0 )
										{
											m_pGamble_Window->m_GambleKeyList[insertindex].Copy(g_pLocalUser->m_UserItemList[m_LastDraggingSlot]);
											m_pGamble_Window->m_GambleKeyList[insertindex].m_ucCount = 1;
											m_pGamble_Window->m_GambleKeyFromInvenNumber[insertindex] = m_LastDraggingSlot;
#ifdef VN_RB_GAMBLEBOX_070607_KUKURI
											// inven 건드리지 않는다
#else
											g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount--;
#endif
										}
										
										m_iSocketIndexList[i+1] = m_LastDraggingSlot;
										m_pGamble_Window->m_pBoxOpenButton->SetButtonTextID(_XSTRINGDB_SN_EODSTRING_XSR, ID_STRING_INVENTORY_2121 );	
										PlayItemEquipSound( m_pGamble_Window->m_GambleKeyList[insertindex].m_cType, m_pGamble_Window->m_GambleKeyList[insertindex].Get_m_sID() );
										return TRUE;
									}
								}
							}
		...
		if( m_pContribution_Window->GetShowStatus() )
		{
			if( !this->m_bHaveMouseSlot )
			{
				...
				if( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART )
				{
					...
					// 올리기				
					m_pContribution_Window->m_ContributionType = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType;
					m_pContribution_Window->m_ContributionSecondType = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cSecond;
					...
					m_pContribution_Window->m_ContributionItemCount = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount;
...
{
	if(m_MouseSlotItem.m_cType == _XGI_FC_QUEST)
		return FALSE;
...
#endif
					/*else if( g_pLocalUser->m_UserItemList[ m_LastDraggingSlot ].m_cType == _XGI_FC_POTION )
					{
				...
				if( !this->m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						...
						if(nextslot != -1)
						{
							...
							m_pPersonalStore_Window->CopyItemToSlot(nextslot, m_MouseSlotItem.m_InventoryNumber);
							m_pPersonalStore_Window->m_TradeItem[nextslot].m_InventoryNumber = m_MouseSlotItem.m_InventoryNumber;
							m_pPersonalStore_Window->m_TradeItem[nextslot].m_ItemNo = g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_ucCount;
				...
				if(!m_pPCTradeWindow->m_bHaveMouseSlot && !this->m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						...
						if(nextslot != -1)
						{
							m_pPCTradeWindow->CopyItemToSlot(nextslot, m_MouseSlotItem.m_InventoryNumber);
							m_pPCTradeWindow->m_SelfTradeItem[nextslot].m_InventoryNumber = m_MouseSlotItem.m_InventoryNumber;
							g_NetworkKernel.SendPacket(MSG_NO_PVP_TRADE_ADD, nextslot, m_MouseSlotItem.m_ucCount );
			...
			if(CheckMousePosition())
			{
				if( !this->m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						...
						//필요한 아이템인지 검사
						int reason = 0;
						if( m_pEnterBossRoom_Window->CheckNeedItem(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID(), reason) )
						{
#ifdef _XDEF_DUNGEONTICKET_070615_KUKURI
							m_pEnterBossRoom_Window->m_CurrentSlotNumber = -1;

							int i = 0 ; for( ; i < _XDEF_BOSSROOM_NEEDITEMNUMBER ; i++)
							{
								if(!m_pEnterBossRoom_Window->m_ItemInSlot[i].bInsert)
								{
									if(i == 0)
									{
										if(m_MouseSlotItem.m_cType == _XGI_FC_RESOURCE)
										{
							...
							int i = 0 ; for( ; i < _XDEF_BOSSROOM_NEEDITEMNUMBER ; i++)
							{
								if( m_pEnterBossRoom_Window->m_NeedItem[i].m_cType == -1 && m_pEnterBossRoom_Window->m_NeedItem[i].Get_m_sID() == 0 )
								{
									m_pEnterBossRoom_Window->m_CurrentSlotNumber = i;
									break;
								}
...
//											int i = 0 ; for( ; i < _XDEF_MAXPARTYMEMBERCOUNT ; i++)
//											{
			...
			if(CheckMousePosition())
			{
				if( !this->m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						...
						if(m_pMixWeapon_Window->CheckItem(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID(), m_LastDraggingSlot))
						{
							m_pMixWeapon_Window->SetItemSlot(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID(), m_LastDraggingSlot);
							m_LastDraggingSlot = -1;
							rtnValue = TRUE;
						}
					}
			...
			if(CheckMousePosition())
			{
				if(!this->m_bHaveMouseSlot)
				{
					...
					if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0 && m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_POCKETSTART)
					{
						...
						if(m_pNPCResource_New_Window->CheckItem(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID(), m_LastDraggingSlot, index))
						{
...
BOOL _XWindow_Inventory::ProcessItemEquip(void)
{
	...
	if(selectedslot != -1)
	{
		if(g_pLocalUser->m_UserItemList[selectedslot].Get_m_sID() > 0)
		{
			...
			m_MouseSlotItem.m_cType = g_pLocalUser->m_UserItemList[selectedslot].m_cType;
			m_MouseSlotItem.Set_m_sID( g_pLocalUser->m_UserItemList[selectedslot].Get_m_sID() );
			m_MouseSlotItem.m_ucCount = g_pLocalUser->m_UserItemList[selectedslot].m_ucCount;
			
			switch(m_MouseSlotItem.m_cType)
			{
			case _XGI_FC_CLOTHES :
				{
			...
			case _XGI_FC_ACCESSORY :
				{
					...
					int nSetGroup = _XGameItem::GetSetItem(g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cType, g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].Get_m_sID());
					if( nSetGroup )
					{
						int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_1; i <= _XINVENTORY_SLOTTYPE_ACCESSORY_8; i++ )
						{
							if( g_pLocalUser->m_UserItemList[i].m_cType == g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cType &&
					...
					{
						if( g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cSecond == 0 && g_AccessoryItemProperty[acid].cClan == 20 )	// 일반 신물일 경우 중복착용 금지
						{
							int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_ACCESSORY_1; i <= _XINVENTORY_SLOTTYPE_ACCESSORY_8; i++ )
							{								
			...
			case _XGI_FC_ELIXIR :
				{	
					...
					{
						if( g_pLocalUser->m_UserItemList[m_MouseSlotItem.m_InventoryNumber].m_cSecond == 0 )	// 영약 보조제
						{
					...
					case _XGI_SC_BOOK_SKILL2 :
						{
							...
							if(g_BookItemProperty[m_MouseSlotItem.Get_m_sID()].cSecondType == _XGI_SC_BOOK_SKILL)
							{
								...
								int i = 0 ; for( ; i < _XDEF_MAX_LEARNSKILL ; ++i)
								{
			...
			case _XEQUIPERROR_PERIODITEM_ACCOVERLAPPING :
				{
					TCHAR	messagestring[256];
					memset(messagestring, 0, sizeof(TCHAR)*256);					
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3159), _XGameItem::GetItemName(m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID()));	//%s은(는) 중복 장착이 불가능 합니다.
...
void _XWindow_Inventory::ProcessUsingItemTooltip(_XGUIObject*& pfocusobject)
{
	if( !m_UsingItemList.empty() )
	{
		...
		int usingno = -1;
		int i = 0 ; for( ; i < m_UsingItemList.size(); i++)
		{					
...
void _XWindow_Inventory::DrawGrabItem()
{
	...
	{		
		...
		int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_POCKETSTART; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; i++ )
		{
...
void _XWindow_Inventory::DrawSocketItem( void )	// 2004.11.8->hotblood
{
	...
	else if( m_pGamble_Window && m_pGamble_Window->GetShowStatus() )
	{
		int i = 0 ; for(; i < 5; i++ )
		{
			if( m_iSocketIndexList[i] > 0 )
			{
				if( i == 0 || g_pLocalUser->m_UserItemList[m_iSocketIndexList[i]].m_ucCount == 0 )
				{
...
{
	int tempcount = 0;
	int i = 0 ; for(  i = 1; i < _XDEF_SOCKETITEM_MAXCOUNT+1; i++ )
	{
		if( m_iSocketIndexList[i] > 0 )
			tempcount++;
	}
...
#else
		if( m_iSocketIndexList[i] > 0 )
			g_pLocalUser->m_UserItemList[m_iSocketIndexList[i]].m_ucCount++;
#endif
		m_iSocketIndexList[i] = 0;
	}
	ResetMouseSlot();
