	...
	if(mousestate)
	{
					...
					if(g_pInventory_Window)	// 인벤토리에서 드래그 했을 경우
					{
						if(g_pInventory_Window->GetShowStatus())
						{
							if(g_pInventory_Window->m_bHaveMouseSlot && g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)
							{
								...
								if( _XGameItem::CheckNPCTrade( g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType, 
								...
								if( flag )
								{
									...
									if(g_pInventory_Window->m_MouseSlotItem.IsStackable())
									{
										...
										int resultcashcheck = CheckCashItem(g_pInventory_Window->m_MouseSlotItem.m_cType,g_pInventory_Window->m_MouseSlotItem.Get_m_sID());
										...
										{	
											...
											strcpy(namestring, _XGameItem::GetItemName(g_pInventory_Window->m_MouseSlotItem.m_cType, g_pInventory_Window->m_MouseSlotItem.Get_m_sID()));
		...
		if(!mousestate->bButton[1])
		{
			if(m_WindowMouseState == _XW_STATE_RIGHTCLICK)
			{
				...
				m_LastDraggingSlot = GetSlotIndex();
				short sID = 0;
				if(GetItemPropertyFromSlot(m_LastDraggingSlot, m_MouseSlotItem.m_cType, sID, m_MouseSlotItem.m_InventoryNumber))
				{
					m_MouseSlotItem.Set_m_sID( sID );
...
void _XWindow_NPCTrade::DrawTradeItemIcon(void)
{
	...
	if( startnumber < m_TradeItemList.size() )
	{
		...
		int i = 0 ; for(; i < _XDEF_MAX_NPCTRADESLOT; i++ )
		{		
			if( iter_item != m_TradeItemList.end() )
			{
				...
				if( m_BlackMarketMode )
				{
					g_pInventory_Window->DrawIcon_Gamble( TRUE, m_WindowPosition.x+m_SlotPosition[i].x, m_WindowPosition.y+m_SlotPosition[i].y, (_XUserItem*)&tradeitem, FALSE, FALSE, CheckCondition(tradeitem.m_cType, tradeitem.Get_m_sID()) );
				}
				else
				{
					...
					// 조건 안 맞는 아이템 처리
					if( !CheckCondition(tradeitem.m_cType, tradeitem.Get_m_sID()) )
					{
...
void _XWindow_NPCTrade::SetListItem(void)
{
	...
	int i = 0 ; for( ; i < g_ItemTradePackage[m_ItemPackageID].itemCount ; i++)
	{
		_XTradeItem tradeitem;
		tradeitem.m_cType = g_ItemTradePackage[m_ItemPackageID].itemUnit[i].category;
...
{
	if( m_MouseSlotItem.m_cType != _XGI_FC_POTION && m_MouseSlotItem.m_cType != _XGI_FC_CONSUMING &&
		m_MouseSlotItem.m_cType != _XGI_FC_BOXKEY && m_MouseSlotItem.m_cType != _XGI_FC_RESOURCE )
		m_MouseSlotItem.m_ucCount = 1;
	...
	else
	{
		...
		switch(GetEqualItemType(m_MouseSlotItem.m_cType))
		{
		case _XGI_FC_WEAPON :
			{
				_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_MouseSlotItem.m_cType);
				if( WeaponItem )
					needmoney = WeaponItem[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CLOTHES :
			{
				_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_MouseSlotItem.m_cType);
				if( ClothesItem )
					needmoney = ClothesItem[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOOK :
			{
				needmoney = g_BookItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_POTION :
			{
				needmoney = g_PotionItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CONSUMING :
			{
				needmoney = g_ConsumingItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_ELIXIR :
			{
				needmoney = g_ElixirItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_ACCESSORY :
			{
				needmoney = g_AccessoryItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_LIFE :
			{
				needmoney = g_LifeItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_SOCKET :
			{
				needmoney = g_SocketItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_RESOURCE :
			{
				needmoney = g_ResourceItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOX :
			{
				needmoney = g_BoxItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOXKEY :
			{
				needmoney = g_BoxKeyItemProperty[m_MouseSlotItem.Get_m_sID()].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		}
#else
		...
		case _XGI_FC_WEAPON :
			{
				needmoney = g_WeaponItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CLOTHES :
			{
				needmoney = g_ClothesItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_WEAPON2 :
			{
				needmoney = g_WeaponItemProperty2[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CLOTHES2 :
			{
				needmoney = g_ClothesItemProperty2[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOOK :
			{
				needmoney = g_BookItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_POTION :
			{
				needmoney = g_PotionItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CONSUMING :
			{
				needmoney = g_ConsumingItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_ELIXIR :
			{
				needmoney = g_ElixirItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_ACCESSORY :
			{
				needmoney = g_AccessoryItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_LIFE :
			{
				needmoney = g_LifeItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_SOCKET :
			{
				needmoney = g_SocketItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_RESOURCE :
			{
				needmoney = g_ResourceItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOX :
			{
				needmoney = g_BoxItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_BOXKEY :
			{
				needmoney = g_BoxKeyItemProperty[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_WEAPON3 :
			{
				needmoney = g_WeaponItemProperty3[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		case _XGI_FC_CLOTHES3 :
			{
				needmoney = g_ClothesItemProperty3[m_MouseSlotItem.m_sID].uiPrice * m_MouseSlotItem.m_ucCount;
			}
			break;
		default :
			break;
...
void _XWindow_NPCTrade::ProcessSell(void)
{
	...
	if(	!g_pInventory_Window->m_MouseSlotItem.IsStackable() )
	{
		g_pInventory_Window->m_MouseSlotItem.m_ucCount = 1;
	}
	...
	needmoney = PriceCalc(g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber, g_pInventory_Window->m_MouseSlotItem.m_ucCount );
...
int _XWindow_NPCTrade::GetSlotIndex(void)
{
	...
	/*int i = 0 ; for( ; i < _XDEF_MAX_NPCTRADESLOT ; i++)		// 예전 소스
	{
...
BOOL _XWindow_NPCTrade::GetItemPropertyFromSlot(int selectedslot, int& cType, short& sID, int& itemNo)
{
	list <_XTradeItem>::iterator iter_item;
	for(iter_item = m_TradeItemList.begin() ; iter_item != m_TradeItemList.end() ; iter_item++)
	{
...
{
	int reslutPrice = 0;	
	int TempType = g_pLocalUser->m_UserItemList[slotnumber].m_cType;
					...
					{						
						...
						while( !feof(fp) )
						{
							...
							if( _stricmp( buffer, findItemNo ) == 0 )
							{
								...
								_XTradeItem tradeitem;
								tradeitem.m_cType = m_GambleItemInfo[i].Type;
