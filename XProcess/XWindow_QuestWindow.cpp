...
{	
	...
	int i = 0 ; for(; i < _XQUESTTAB_TOTALCOUNT; i++ )
	{			
		m_QuestTab[i] = NULL;
	}
...
BOOL _XWindow_QuestWindow::Initialize( void )
{
	...
	{
		int i = 0 ; for(; i < _XQUESTTAB_TOTALCOUNT-1; i++ ) //무림소사 탭버튼이미지는 다른 이미지이므로 뺀다. 
		{			
				...
				int itemcounterindex = 0 ; for(  itemcounterindex = 0; itemcounterindex < 8; itemcounterindex++ )
				{
					if( currentquest->m_ItemCounterInformation[itemcounterindex].CounterTargetType != -1 )
					{
						...
						{													
							...
							{
								if(g_pLocalUser->m_UserItemList[i].m_cType == cType
...
{	
	...
	if( questgroupnumber != -1)
	{
		if( m_QuestGroupNumber != questgroupnumber )
		{
			...
			_XQuestScriptItemGroup* currentgroup = NULL;
			int i = 0 ; for(; i < questgroupnumber; i++ )
			{ 
