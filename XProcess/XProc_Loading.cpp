	...
	if( m_AcclaimADPhase != _XACCLAIMADPHASE_END )
	{
		if( m_AcclaimADPhase == _XACCLAIMADPHASE_WATCHANDWINBUTTON )
		{
			...
			else
			{
				...
				{
					TCHAR buttontext[64];
					sprintf( buttontext, _T("WATCH & WIN: 15 sec video ad : %d"), 3 - ((g_LocalSystemTime - m_AutoCancelCounter) / 1000) );
...
//		int i = 0 ; for(; i < _XDEF_FLASH_MAX_ZONECOUNT; i++ )
//		{
//			if( minimapindex[i] != -1 )
//			{
...
void XProc_Loading::UpdateProgress( int increase )
{
	...
	int i = 0 ; for(  i = m_Progress; i <= increase; i++ )
	{
