...
{	
	...
	{				
		...
		{
			if(g_pLocalUser->m_UserItemList[hoveredslot].Get_m_sID() > 0) // 슬롯에 아이템이 있는경우만 드래그 가능
			{
	...
	int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_AMOR; i < _XINVENTORY_SLOTTYPE_POCKETSTART; i++ )
	{
		if(g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0)
		{
			...
			g_pInventory_Window->DrawIcon(TRUE,m_WindowPosition.x + g_EquippedItemCoord[i].x, 
				m_WindowPosition.y + g_EquippedItemCoord[i].y, &g_pLocalUser->m_UserItemList[i]);
		}
		else
		{
		#ifdef _XDEF_CASHBACKPACK_080404_MAGVIPER //Author : 양희왕 //breif : 캐시 행낭 추가
...
int	_XWindow_CInfo::SearchSlotRegion(  RECT* pRect  )
{
	...
	// 장착 슬롯 검사
	int iter = 0 ; for(  iter = _XINVENTORY_SLOTTYPE_AMOR; iter < _XINVENTORY_SLOTTYPE_POCKETSTART; iter++ )
	{
			...
			if(m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_AMOR && m_LastDraggingSlot <= _XINVENTORY_SLOTTYPE_BACKPACK2 )
			{
				...
				if( temperrorcode == _XEQUIPERROR_NO )
				{
					...
					if(m_LastDraggingSlot >= _XINVENTORY_SLOTTYPE_ACCESSORY_1 && m_LastDraggingSlot <= _XINVENTORY_SLOTTYPE_ACCESSORY_8)
					{
						...
						int nSetGroup = _XGameItem::GetSetItem(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType, 
						...
						if( nSetGroup )
						{
							...
							{
								if( g_pLocalUser->m_UserItemList[i].m_cType == g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType &&
...
void _XWindow_CInfo::ProcessDblClick(void)
{
	...
	if( CheckMousePosition() )
	{
		...
		int m_LastDraggingSlot = SearchSlotRegion();
		...
		if( m_LastDraggingSlot != -1 )
		{
			if(g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() > 0)
			{
				g_pInventory_Window->m_bHaveMouseSlot = TRUE;
				g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber = m_LastDraggingSlot;
				g_pInventory_Window->m_MouseSlotItem.m_cType = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_cType;
				g_pInventory_Window->m_MouseSlotItem.Set_m_sID( g_pLocalUser->m_UserItemList[m_LastDraggingSlot].Get_m_sID() );
				g_pInventory_Window->m_MouseSlotItem.m_ucCount = g_pLocalUser->m_UserItemList[m_LastDraggingSlot].m_ucCount;
				...
				else if( m_LastDraggingSlot == _XINVENTORY_SLOTTYPE_BACKPACK1 )
				{
					...
					else
					{
						...
						int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART; i < end; i++ )
						{
							if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
							{
								resetmouse = FALSE;
				...
				else if( m_LastDraggingSlot == _XINVENTORY_SLOTTYPE_BACKPACK2 )
				{
					...
					int i = _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART;for( ; i < end; i++ )
					{
						if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
						{
							resetmouse = FALSE;
				...
				else if( m_LastDraggingSlot == _XINVENTORY_SLOTTYPE_CASHBACKPACK1 )
				{
					...
					else
					{
						...
						int i = _XINVENTORY_SLOTTYPE_FIRST_CASHBACKSTART ; for( ; i < end; i++ ) //아이템이 있나 검사
						{
							if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
							{
								resetmouse = FALSE;
				...
				else if( m_LastDraggingSlot == _XINVENTORY_SLOTTYPE_CASHBACKPACK2 )
				{
					...
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_SECOND_CASHBACKSTART; i < end; i++ )
					{
						if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
						{
							resetmouse = FALSE;
				...
				if( resetmouse )
				{
					...
					{
						int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_POCKETSTART; i < _XINVENTORY_SLOTTYPE_SECOND_BACKPACKSTART; i++ )
						{
							if( g_pLocalUser->m_UserItemList[i].Get_m_sID() <= 0 )
							{
								m_LastDraggingSlot = i;
					...
					{
						int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_POCKETSTART; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART; i++ )
						{
							if( g_pLocalUser->m_UserItemList[i].Get_m_sID() <= 0 )
							{
								m_LastDraggingSlot = i;
