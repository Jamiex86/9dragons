...
void _XWindow_Event::SetListItem(void)
{
	...
	for(iter_eventinfo = m_listEventInfo.begin() ; iter_eventinfo != m_listEventInfo.end() ; ++iter_eventinfo)
	{
		...
		if( currenteventinfo )
		{
			...
			/*if( currenteventinfo->startyear <= TimeInfo.wYear && currenteventinfo->endyear >= TimeInfo.wYear )
			{
				if( currenteventinfo->endyear == TimeInfo.wYear )
				{
					if(currenteventinfo->startyear < TimeInfo.wYear)
					{
						// 작년에 시작했음. 끝나는 날짜 비교해야함
						if(currenteventinfo->endmonth >= TimeInfo.wMonth)
						{
							// 끝나는 달이 아직 되지 않음
							checkdate = true;
						}
						else if(currenteventinfo->endmonth == TimeInfo.wMonth)
						{
							// 이번달이 끝나는 달, 날짜 비교
							if( currenteventinfo->endday >= TimeInfo.wDay )
							{
								checkdate = true;
							}
						}
					}
					else if( currenteventinfo->startmonth <= TimeInfo.wMonth && currenteventinfo->endmonth >= TimeInfo.wMonth )
					{
						if( currenteventinfo->endmonth == TimeInfo.wMonth )
						{
							if( currenteventinfo->startmonth == TimeInfo.wMonth ) // 이벤트시작 달과 끝나는 달이 같을 경우
							{
								// 날짜 비교를 시작하는 날과 끝나는 날 둘 다 검사한다.
								if( currenteventinfo->startday <= TimeInfo.wDay && currenteventinfo->endday >= TimeInfo.wDay )
								{
									checkdate = true;
								}
							}
							else // 이벤트시작 달과 끝나는 달이 같을 경우
							{
								// 날짜 비교를 끝나는 날만 한다.
								if( currenteventinfo->endday >= TimeInfo.wDay )
								{
	...
	for(iter_eventinfo = m_listEventInfo.begin() ; iter_eventinfo != m_listEventInfo.end() ; ++iter_eventinfo)
	{
		...
		if( currenteventinfo )
		{
			if( currenteventinfo->eventid == selectedeventid )
			{
				...
				int index = 0 ; for(  index = 0; index < 5; ++index )
				{					
				...
				else if(m_SelectedEventType == 5 || m_SelectedEventType == 1)
				{
					// 아무것도 안나오게
					m_imageIMEBorderSerialNumber->ShowWindow( FALSE );
					m_IMESerialNumber.ShowWindow(FALSE);
