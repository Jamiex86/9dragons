...
bool XProc_FirstLoad::Process( void )
{
	...
	int i = 0 ; for(; i < 256; i++ ) // lump table runtime xor.
	{
		int j = 0 ; for( j = 0; j < 32; j++ )
...
BOOL XProc_FirstLoad::LoadSceneList( void )
{
	...
	int i = 0 ; for(; i < _XDEF_MAXFLASHCOUNT; i++)
	{		
		g_SceneList[i] = NULL;
	}
...
BOOL XProc_FirstLoad::CreateMainInterface( void )
{
	...
	int i = 0 ; for(  i = 1; i < _XDEF_MAXPARTYMEMBERCOUNT; i++ )
	{				
		...
		#ifdef _ACCLAIM_RUBICONADSYSTEM
		pPartyMemberMinibar_Window->MoveWindow( 14, 170 + g_RubiconADWindowSize.cy + partyminiwindowstruct.position.y);
		#else
		pPartyMemberMinibar_Window->MoveWindow( 14, 170 + partyminiwindowstruct.position.y);
		#endif
		pPartyMemberMinibar_Window->ShowWindow( FALSE );
		g_MainWindowManager.InsertWindow( pPartyMemberMinibar_Window );	
		partyminiwindowstruct.position.y += 36;
	}
...
// Create HelpScreen Window=======================================================
	...
	pPartyMasterMinibar_Window->MoveWindow( 6, 46);
	int ypos = 0;
	...
	{				
		...
		pPartyMemberMinibar_Window->MoveWindow( 6, 86 + ypos); 
		ypos += 27;
	}
...
{		
	...
	int i = 0 ; for(; i < g_MaxInterfaceSoundEffectCount; i++ )
	{	
