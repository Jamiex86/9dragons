...
//#ifdef _XDEF_CASHITEM_SHOUT
	int i = 0 ; for(; i < _XCHATMODE_TOTALCOUNT; i++ )
//#else
//	int i = 0 ; for(; i < 6; i++ )
//#endif
	{
...
{	
	...
	int i = 0 ; for(; i < _XDEF_MAXWHISPERTARGETCOUNT; i++ )
	{			
...
//#endif //_XDEF_CASHITEM_SHOUT 
	...
	m_pChatBlackListButton = new _XButton;
...
//#endif
	{
		SAFE_DELETE( m_btnChatFunctionMenuButton[i] );
		SAFE_DELETE( m_btnimageChatFunctionMenuButton[i] );
	}	
	
	SAFE_DELETE( m_EmotionListBox );
}
...
#else
		int i = 0 ; for(; i < m_ChatListScrollPos; i++ ) g_Chatlist.nextItem();
#endif
				...
				if( (g_LocalSystemTime - curstring->Get_Attribute(1)) > 3000)//_XDEF_CHATMESSAGEBLINKTIME ) // 2004.06.07->oneway48 insert
				{
					...
					if( defaultsystemmsgwindow )
					{
						if( defaultsystemmsgwindow->GetShowStatus() )
							...
							g_XBaseFont->PutsAlign( this->m_WindowPosition.x + 16 + offset, (m_WindowPosition.y + 8) + (ypos * 15), _XFONT_ALIGNTYPE_LEFT, curstring->Get_String() );
						else
							g_XBaseFont->PutsAlign( this->m_WindowPosition.x + m_WindowSize.cx - 6 + offset, (m_WindowPosition.y + 8) + (ypos * 15), _XFONT_ALIGNTYPE_RIGHT, curstring->Get_String() );							
					}
				}
				else
				{
				...
				else
				{
					if( pfocusobject == this )
					{
						if( curstring->Get_Attribute(1) != 0x0000000 )	// ID 일 경우
						{
							...
							if( gpInput->CheckMousePos( this->m_WindowPosition.x + 16 + offset, (m_WindowPosition.y + 8) + (ypos * 15), 
								this->m_WindowPosition.x + 16 + offset + width, (m_WindowPosition.y + 8) + (ypos * 15) + g_XBaseFont->GetHeight() ) )
							{
