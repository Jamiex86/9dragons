		...
		case _XINVENTORY_SLOTTYPE_MASK :
			{
				...
				if( index == 0 )
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_AMOR; i < _XINVENTORY_SLOTTYPE_MASK; i++ )
					{
	...
	case _XDEF_INV_WASTBASKET_CASHITEM			:
		{
			...
			if(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].IsStackable())
			{
				...
				{
					pTradeConfirm_Window->ShowWindow(TRUE);
					pTradeConfirm_Window->m_SelectedCategory1 = g_pInventory_Window->m_MouseSlotItem.m_cType;
	...
	case _XDEF_PARTYWINDOW_MASTERMINI :
		{
			...
			if(check)
			{
				int i = 0 ; for( ; i < _XDEF_MAXPARTYMEMBERCOUNT ; i++) // 2004.06.01->oneway48 modify : 15->9
				{
		...
		case _XDEF_PARTYWINDOW_LEAVE :
			{				
				...
				if( g_pLocalUser->m_PartyInfo[g_pLocalUser->m_PartySelfIndex].isLeader )
				{
					...
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1151));	// "동행장을 위임하시고 탈퇴하십시오."
		...
		case _XDEF_PARTYWINDOW_DISSOLUTION :
			{
				...
				else
				{
					...
					TCHAR messagestring[256];
					memset( messagestring, 0, sizeof(TCHAR)*256 );
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_DONOTDESTROYPARTY));		// "동행을 해체할 권한이 없습니다."
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
					pMessageBox->StartTimer(5000, TRUE) ;
				}
			}
		...
		case _XDEF_PARTYWINDOW_USERSECESSION :
			{
				...
				else
				{
					...
					TCHAR messagestring[256];
					memset( messagestring, 0, sizeof(TCHAR)*256 );
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_DONOTTRUSTCAPTAIN));		// "동행장을 위임할 권한이 없습니다."
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
					pMessageBox->StartTimer(5000, TRUE) ;
				}
			}
		...
		case _XDEF_PARTYWINDOW_USERLEAVE :
			{				
				...
				else
				{
					...
					TCHAR messagestring[256];
					memset( messagestring, 0, sizeof(TCHAR)*256 );
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_DONOTDROP));		// "강제로 추방할 권한이 없습니다."
					pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
					pMessageBox->StartTimer(5000, TRUE) ;
				}
			}
	...
	case _XDEF_MINIPARTYWINDOW_CLOSEBUTTON1		:
		{
			...
			TCHAR messagestring[256];
			memset( messagestring, 0, sizeof(TCHAR)*256 );
			sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_DESTROYPARTY));	// "동행을 해산하시겠습니까?"
					...
					int i = 0 ; for( ; i < _XDEF_MAXPARTYMEMBERCOUNT ; i++) 
					{
						if( !g_pLocalUser->m_PartyInfo[i].isLeader )
						{
							if( strcmp( g_pLocalUser->m_PartyInfo[i].playername, pMiniBar_Window->m_PartyMemberName ) == 0 )
							{
								...
								TCHAR messagestring[256];
								memset( messagestring, 0, sizeof(TCHAR)*256 );
								sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_MEMVERDROP));	// "이 동행원을 동행에서 퇴출시키겠습니까?"
				...
				if(pMiniBar_Window)
				{
					if( strcmp(pMiniBar_Window->m_PartyMemberName, g_pLocalUser->m_CharacterInfo.charactername) == 0 )
					{
						...
						if( partyindex != -1 )
						{
							...
							TCHAR messagestring[256];
							memset( messagestring, 0, sizeof(TCHAR)*256 );
							sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_SELFDROP));		// "동행에서 탈퇴하시겠습니까?"
	...
	case _XDEF_MINIPARTYWINDOW_ENTRUSTBUTTON8		:	
		{
					...
					int i = 0 ; for( ; i < _XDEF_MAXPARTYMEMBERCOUNT ; i++) 
					{
						...
						{				
							if( strcmp( g_pLocalUser->m_PartyInfo[i].playername, pMiniBar_Window->m_PartyMemberName ) == 0 )
							{
								...
								TCHAR messagestring[256];
								memset( messagestring, 0, sizeof(TCHAR)*256 );
								sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_TRUSCAPTAIN));		// "이 사람을 행수로 위임하시겠습니까?"
			...
			else
			{
				...
				TCHAR messagestring[256];
				memset( messagestring, 0, sizeof(TCHAR)*256 );
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_DOCAPTAIN));		// "위임기능은 행수만 사용 가능합니다."
				pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK,  _T(""), messagestring, TRUE);
				pMessageBox->StartTimer(5000, TRUE) ;
			}
...
#ifdef _XDEF_INTERFACE_RENEWAL_071002_KUKURI
								#ifdef _ACCLAIM_RUBICONADSYSTEM	
								pMiniBar_Window->MoveWindow( 50, 170 + ypos +((g_ViewRubiconAD)?g_RubiconADWindowSize.cy:0) );
								#else
								pMiniBar_Window->MoveWindow( 50, 170 + ypos );
								#endif
#else
								#ifdef _ACCLAIM_RUBICONADSYSTEM	
								pMiniBar_Window->MoveWindow( 14, 170 + ypos +((g_ViewRubiconAD)?g_RubiconADWindowSize.cy:0) );
								#else
...
#else	
			int i = 0 ; for(; i < g_MaxSoundEffectCount; i++ )
			{	
		...
		case _XDEF_CHATDEFAULT_CHATBLACKLISTUBUTTON		:			
			{
				...
				{
					pBlackList_Window->ShowWindow( !pBlackList_Window->GetShowStatus() );
				}
			}
			break;
					...
					if(pUser)
					{
						...
						else
						{
							...
							else
							{
								...
								TCHAR message[512];
								sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3478), pUser->m_CharacterInfo.charactername );//_T("%s님이 블랙리스트에 추가되었습니다.")
						...
						if( pNickName_Window )
						{
							...
							if( g_pLocalUser->m_CharacterInfo.m_NickNameList[pNickName_Window->m_SelectedNickNameSlotNumber] != 0 )
							{
								...
								else
								{
									...
									sprintf( tempstr, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1861), pNickName_Window->m_RemainChangeTime );
...
//					}
					...
					sprintf( tempStr, "http://9dingame.zing.vn/9d/");
					#else
					if( !g_bTestWebPageAddress )
					{
					...
					case _XDEF_NPCSUBSCRIPTCLASS_SAVEPOINT :
						{
							...
							else
							{
								...
								else if(result == NPC_CONDITION_RESULT_FAIL)
								{
									...
									{
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3306));//장원전이 시작되어야지만 여장을 풀 수 있습니다
									}
									else if(g_CBManager.m_nSavePointFailResult == 2)
									{
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3307));//장원 도전자만 여장을 풀 수 있습니다
									}
									else if(g_CBManager.m_nSavePointFailResult == 3)
									{
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3308));//장원 수비자만 여장을 풀 수 있습니다
									}
									else
									{
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3165), g_pLocalUser->m_CharacterInfo.charactername);//%s님은 이 곳에 여장을 풀 수 없습니다.
									}
#else
									sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_3165), g_pLocalUser->m_CharacterInfo.charactername);//%s님은 이 곳에 여장을 풀 수 없습니다.
#endif
					...
					case _XDEF_NPCSUBSCRIPTCLASS_HEALPLAYER :
						{
							...
							else if( needmoney <= g_pLocalUser->m_Money )
							{
								...
								TCHAR messagestring[256];
								memset( messagestring, 0, sizeof(TCHAR)*256 );
								sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1865), needmoney );
							...
							else
							{
								TCHAR messagestring[256];
								memset( messagestring, 0, sizeof(TCHAR)*256 );
								sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_NOMONEY));		// "소지금이 부족합니다."
	...
	case _XDEF_NPCSCRIPT_MINISTOPOK	:
		{			
			...
			else
			{
				...
				{
					int i = 0 ; for(  i = _XINVENTORY_SLOTTYPE_POCKETSTART ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; i++)
					{
	...
	case _XDEF_TRADECONFIRM_ALL :
		{
			if(pTradeConfirm_Window)
			{
				...
				case _XDROP_WASTEBASKET_ITEM :	
					{
						if(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].IsStackable())
							pTradeConfirm_Window->m_TradeCount = g_pInventory_Window->m_MouseSlotItem.m_ucCount;
					}
					break;
				...
				case _XWAREHOUSE_FROMWAREHOUSE_ITEM :
					{
						...
						if(m_pWarehouseWindowPtr && g_pInventory_Window)
						{
							if(m_pWarehouseWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus())
							{
								pTradeConfirm_Window->m_TradeCount = m_pWarehouseWindowPtr->m_MouseSlotItem.m_ucCount;
							}
						} // if(m_pWarehouseWindowPtr && g_pInventory_Window)
					}
					break;
				...
				case _XPCTRADE_FROMTRADE_ITEM :
					{
						if(m_pPCTradeWindowPtr && g_pInventory_Window)
						{
							...
							{
								pTradeConfirm_Window->m_TradeCount = m_pPCTradeWindowPtr->m_MouseSlotItem.m_ucCount;
							}
						}
					}
					break;
				...
				sprintf( tempstring, _T("%u"), pTradeConfirm_Window->m_TradeCount );
				...
				case _XDROP_ITEM :
					{
						if(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].IsStackable())
						{
							if(g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_ucCount < pTradeConfirm_Window->m_TradeCount)
							{
								_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
...
//										g_MessageBox.SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), _XGETINTERFACETEXT(ID_STRING_MESSAGEHANDLER_EXCESS), TRUE);
									...
									{
										m_pNPCTradeWindowPtr->m_MouseSlotItem.m_ucCount = (unsigned char)pTradeConfirm_Window->m_TradeCount;
										m_pNPCTradeWindowPtr->ProcessBuy();
									}
								}
								else if(pTradeConfirm_Window->m_TradeMode == _XNPCTRADE_SELL)
								{
									...
									{
										g_pInventory_Window->m_MouseSlotItem.m_ucCount = pTradeConfirm_Window->m_TradeCount;
										m_pNPCTradeWindowPtr->ProcessSell();
									}
								}
								pTradeConfirm_Window->ShowWindow(FALSE);
				...
				case _XWAREHOUSE_FROMWAREHOUSE_ITEM :
					{
						...
						if(m_pWarehouseWindowPtr && g_pInventory_Window)
						{
							if(m_pWarehouseWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus())
							{
								if(g_pLocalUser->m_WarehouseItemList[m_pWarehouseWindowPtr->m_MouseSlotItem.m_InventoryNumber].m_ucCount < pTradeConfirm_Window->m_TradeCount)
								{
									_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
									pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
//									g_MessageBox.SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
				...
				case _XPCTRADE_TOTRADE_ITEM :
					{
						if(m_pPCTradeWindowPtr && g_pInventory_Window)
						{
							if(m_pPCTradeWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus())
							{
								...
								if(g_pLocalUser->m_UserItemList[invenindex].m_ucCount < pTradeConfirm_Window->m_TradeCount)
								{
									_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
									pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
//									g_MessageBox.SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
...
//									g_MessageBox.SetMessage(_XDEF_DLGBTNTYPE_OK, _T(""), _XGETINTERFACETEXT(ID_STRING_MESSAGEHANDLER_INPUTNUMBER), TRUE);
								...
								else
								{
									...
									m_pPCTradeWindowPtr->m_SelfTradeItem[index].m_ucCount = pTradeConfirm_Window->m_TradeCount;
									m_pPCTradeWindowPtr->m_SelfTradeItem[index].m_InventoryNumber = invenindex;
									g_pLocalUser->m_UserItemList[invenindex].m_ucCount -= pTradeConfirm_Window->m_TradeCount;
									
									g_pInventory_Window->m_bHaveMouseSlot = FALSE;
									g_pInventory_Window->m_MouseSlotItem.ResetMouseSlotItem();
									if(g_pLocalUser->m_UserItemList[invenindex].m_ucCount <= 0)
									{
				...
				case _XPCTRADE_FROMTRADE_ITEM :
					{
						if(m_pPCTradeWindowPtr && g_pInventory_Window)
						{
							if(m_pPCTradeWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus())
							{
								int index = pTradeConfirm_Window->m_PCTradeSlotIndex;
								int invenindex = m_pPCTradeWindowPtr->m_SelfTradeItem[index].m_InventoryNumber;
								if(m_pPCTradeWindowPtr->m_SelfTradeItem[index].m_ucCount < pTradeConfirm_Window->m_TradeCount)
								{
									_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
									pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
//									g_MessageBox.SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_MANY, TRUE);
				...
				case _XCONTRIBUTION_TOCONTRIBUTION_ITEM :
					{
						if( m_pContributionWindowPtr && g_pInventory_Window )
						{
							if( m_pContributionWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus() )
							{
								if(g_pInventory_Window->m_MouseSlotItem.m_ucCount < pTradeConfirm_Window->m_TradeCount)
								{
									_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
									pMessageBox->SetMessage(_XDEF_DLGBTNTYPE_OK, ID_STRING_NPCTRADE_NOTENOUGHMONEY, TRUE);
								}
								...
								{
									m_pContributionWindowPtr->m_ContributionType			= g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType;
									m_pContributionWindowPtr->m_ContributionSecondType		= g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cSecond;
						...
						if(m_pPersonalStoreWindowPtr && g_pInventory_Window)
						{
							if(m_pPersonalStoreWindowPtr->GetShowStatus() && g_pInventory_Window->GetShowStatus())
							{
								...
								else
								{
									...
									m_pPersonalStoreWindowPtr->m_TradeItem[index].m_ucCount = pTradeConfirm_Window->m_TradeCount;
				...
				if(pRequestParty_Window && m_pPCTradeWindowPtr)
				{
					...
					else
					{
						...
						else
						{
							...
							sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_REQTRADE), pUser->m_CharacterInfo.charactername );	// "%s님에게 거래를 요청하는 중입니다..."
	...
	case _XDEF_REQUESTPARTY_REQUESTFRIEND :
		{
			...
			if( pMessenger_Window->m_TotalCountFriends <= 50) // 최대 50명까지 가능
			{
				...
				else
				{
					...
					sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MESSAGEHANDLER_REGISTALREADY));
					g_NetworkKernel.InsertChatString( messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
				}
			}
			else
			{
				...
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MESSAGEHANDLER_EXCESSPERSON) );
				g_NetworkKernel.InsertChatString( messagestring, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS );
...
#endif
				TCHAR messagestring[256];
				sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1896), pRequestParty_Window->m_PickedCharacterName);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	case _XDEF_REQUESTPARTY_REQPVPGROUP :
		{
			...
			{
				TCHAR messagestring[256];
				sprintf(_XGETINTERFACETEXT(ID_STRING_MAINGAMEMESSAGEHANDLER_1897), pRequestParty_Window->m_PickedCharacterName);
				_XMessageWindow* pMessageBox = _XMessageWindow::CreateMessageBox(&g_MainWindowManager, _XDEF_WTITLE_MESSAGEWINDOW);
	...
	case _XDEF_REQUESTPARTY_REQALIENCE			:		//영웅단 동맹을 권유한다.
		{
			...
			if( g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_iIndex > 0 )
			{
				if( g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_iIndex == pUser->m_CharacterInfo.m_AlienceInfo.m_iIndex )
				{
					...
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3185));			//_T("같은 방회 소속입니다.") 
				...
				if( g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cClass != _XUSERALIENCEINFO::_UNIT_CALSS_ALIENCE_CAPTAIN_UNIT_ )
				{
					...
					TCHAR message[256];
					memset( message, 0, 256 );
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3186));			//_T("회주만 동맹 권유가 가능합니다.") 
					g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
					break;
				...
				else
				{
					if( g_pGroup_Window->m_listAlienceMember.size() > 5 )
					{
						...
						TCHAR message[256];
						memset( message, 0, 256 );
						sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3187) );			//_T("최대 5개의 영웅단까지만 동맹을 맺을 수 있습니다.")
						g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
						break;
					}
				}
			...
			else	//영웅단이면 상대방이 
			{
				if( g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_iIndex > 0 )
				{
					if( g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_iIndex == pUser->m_CharacterInfo.m_OrganizationInfo.m_iIndex )
					{
						...
						TCHAR message[256];
						memset( message, 0, 256 );
						sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3188));			//_T("같은 영웅단에 소속되어 있습니다.") 
						g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
						break;
					}
					...
					if( g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cClass != _XUSERORGANIZATIONINFO::_UNIT_CALSS_CAPTAIN_UNIT_  )
					{
						...
						TCHAR message[256];
						memset( message, 0, 256 );
						sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3189));			//_T("단주만 동맹 권유가 가능합니다.") 
						g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
						break;
					}
				...
				else
				{
					...
					TCHAR message[256];
					memset( message, 0, 256 );
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3183));			//_T("영웅단에 소속되어 있지 않습니다.") 
					g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
					break;
				}
			...
			if( pUser->m_CharacterInfo.m_AlienceInfo.m_iIndex  > 0 )
			{
				...
				TCHAR message[256];
				memset( message, 0, 256 );
				sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3190));			//_T("이미 방회에 속해 있습니다.") 
				g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
				break;
			}
			...
			if( pUser->m_CharacterInfo.m_OrganizationInfo.m_cClass != _XUSERORGANIZATIONINFO::_UNIT_CALSS_CAPTAIN_UNIT_  )
			{
				...
				TCHAR message[256];
				memset( message, 0, 256 );
				sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3191) );			//_T("대상은 영웅단 단주가 아닙니다.")
				g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
				break;
			}
				...
				g_pLocalUser->m_CharacterInfo.groupindex == _XGROUP_MAKYO )
			{
					...
					pUser->m_CharacterInfo.groupindex == _XGROUP_MOODANG )
				{
					TCHAR message[256];
					memset( message, 0, 256 );
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3192) );			//_T("흑도와 백도는 방회를 맺을 수 없습니다.")
					g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
				}
					...
					g_pLocalUser->m_CharacterInfo.groupindex == _XGROUP_MOODANG )
			{
					...
					pUser->m_CharacterInfo.groupindex == _XGROUP_MAKYO )
				{
					TCHAR message[256];
					memset( message, 0, 256 );
					sprintf( message, _XGETINTERFACETEXT(ID_STRING_NEW_3184) );			//_T("흑도와 백도는 방회를 맺을 수 없습니다.")
					g_NetworkKernel.InsertChatString(message, _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS, TRUE);
				}
			}
			...
			case _XSENDMODE_TOWAREHOUSE_ITEM :
				{
					...
					int i = 0 ; for( ; i < m_pWarehouseWindowPtr->m_UseSlotCount ; i++) // 2004.05.24->oneway48 modify
					{
						if(g_pLocalUser->m_WarehouseItemList[i].Get_m_sID() == 0)
							emptycount++;
					}
	...
	case _XDEF_WAREHOUSEWINDOW_ALLSELECT :
		{
			...
			if( !m_pWarehouseWindowPtr->m_bWarehouseCancleMode ) // 창고사용 모드
			{
				...
				int totalcount = 0;
				int i = 0  ; for( i = tempitemcount; i < tempitemcount+25; i++) //현재 선택된 행낭 시작 부터 끝
				{
					if( totalcount > 20 )
						break;
					...
					if( g_pLocalUser->m_UserItemList[i].Get_m_sID() > 0 )
					{
						if(g_pLocalUser->m_UserItemList[i].IsStackable())
							m_pWarehouseWindowPtr->SetTempItemFromInven(i, g_pLocalUser->m_UserItemList[i].m_ucCount);
						else
				...
				int tempitemcount = g_pInventory_Window->m_SelectedBaseTab * 25 + _XINVENTORY_SLOTTYPE_POCKETSTART;
				int totalcount = 0;
				int i = 0 ; for(  i = tempitemcount ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; i++)
				{
					if( totalcount > 20 )
						break;
...
void XProc_MainGame::EmailWindowMessageHandling(WPARAM wparam, LPARAM lparam)
{
	...
	case _XDEF_EMAILWINDOW_EMAILSEND:
		{					
			...
			int i = 0 ; for(; i < pEmail_Window->m_EmailTextLineTotalCount ; i++)
			{
			...
			if( pEmail_Window->m_EmailTextLength != 0 )
			{
				if( pEmail_Window->m_EmailReceiver[0] == '@' )
				{
					...
					memset( tempOrgaName, 0, sizeof(TCHAR)*128);
					sprintf( tempOrgaName, _XGETINTERFACETEXT(ID_STRING_ORGANIZATION_ORGANIZATIONINFO2), g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName);
	...
	case _XDEF_REPAIRITEMWINDOW_ALLBUTTON :
		{
			if(g_pInventory_Window && m_pRepairItemWindowPtr)
			{
				...
				for(  selectedslot = _XINVENTORY_SLOTTYPE_AMOR; selectedslot < _XINVENTORY_SLOTTYPE_POCKETSTART; selectedslot++ )
				{
					...
					{
						if( g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_WEAPON || 
							g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_WEAPON2 ||
							g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_WEAPON3 )
						{
					...
					{
						if( g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_CLOTHES || 
							g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_CLOTHES2 ||
							g_pLocalUser->m_UserItemList[selectedslot].m_cType == _XGI_FC_CLOTHES3 )
						{
							shortdur = g_pLocalUser->m_UserItemList[selectedslot].Get_m_usMaxDur() - g_pLocalUser->m_UserItemList[selectedslot].Get_m_usCurDur();
							if( shortdur > 0)
							{
...
void XProc_MainGame::MatchPRWindowMessageHandling(WPARAM wparam, LPARAM lparam)
{
	...
	{		
		...
		case _XDEF_MATCHPR_CLOSEBUTTON :
			{
				memset(pMarthPR_Window->m_stringPVPPR, 0, sizeof(TCHAR)*81);
				memset(g_pLocalUser->m_stringMatchPR, 0, sizeof(TCHAR)*81);
			...
			if( g_pLocalUser->m_PersonalStoreState != _XUSER_PERSONALSTORE_OWNER_MODIFY )
			{
				...
				if( m_pPersonalStoreWindowPtr->m_ServerPersonalStoreID != -1 )
				{
					if( g_pLocalUser->m_PersonalStoreState == _XUSER_PERSONALSTORE_OWNER_PREPARE )
					{
						...
						int i = 0 ; for( ; i < _XDEF_PERSONALSTORE_TRADEITEM_MAX_SLOTNUMBER ; i++)
						{
							if( m_pPersonalStoreWindowPtr->m_TradeItem[i].Get_m_sID() != 0)
							{
								...
								{
									if( g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount == 0)
									{
										g_pLocalUser->m_UserItemList[inventoryindex].Copy( m_pPersonalStoreWindowPtr->m_TradeItem[i] );
									}
									else
									{
										g_pLocalUser->m_UserItemList[inventoryindex].m_ucCount += m_pPersonalStoreWindowPtr->m_TradeItem[i].m_ucCount;
									}
								}
								else
								{
			...
			case _XUSER_PERSONALSTORE_CLIENT_INTEREST: // 물품 구매 
				{
					if( m_pPersonalStoreWindowPtr->m_SelectedSlotNumber > -1 )
					{
						if( m_pPersonalStoreWindowPtr->m_TradeItemPrice[m_pPersonalStoreWindowPtr->m_SelectedSlotNumber] <= g_pLocalUser->m_Money )
						{
							...
							//기본 행낭 검사
							int i = _XINVENTORY_SLOTTYPE_POCKETSTART ; for(; i < end; i++ )
							{
								if( g_pLocalUser->m_UserItemList[i].Get_m_sID() <= 0 )
									emptyslot++;
							}
							...
							char ctype = m_pPersonalStoreWindowPtr->m_TradeItem[selectedslotnumber].m_cType;
						...
						else
						{
							...
							int type = m_pPersonalStoreWindowPtr->m_TradeItem[ m_pPersonalStoreWindowPtr->m_SelectedSlotNumber ].m_cType;
	...
	case _XDEF_MOVEVILLAGE_DECIDEMOVING :
		{
		        ...
		        invennumber < _XINVENTORY_SLOTTYPE_SECOND_CASHBACKSTART )
	        {
		        if( g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_CASHBACKPACK1].Get_m_sID() <= 0 )
                {
                    g_NetworkKernel.InsertChatString(_T("기간제 행낭의 사용기간이 만료되어 아이템을 사용할 수 없습니다."), _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
