...
_XWindow_QuickSlot::_XWindow_QuickSlot()
{
	...
	m_SelectedSet			= 0;
	m_SelectedSlot			= 0;
	memset(m_QuickSlot, 0, sizeof(_XQSlot_Info)*30);
	memset(m_PositionArray, 0, sizeof(POINT)*10);
			...
			if(!mousestate->bButton[0])
			{
				...
				if(scrnpos->x == m_IconDragStartPos.x && scrnpos->z == m_IconDragStartPos.y)
				{
					...
					if(index >= 0 && index < 30)
					{
						if(m_QuickSlot[index].type == _XQS_SLOTTYPE_SKILL)
						{
							if(m_LastDraggingSlot > -1 && m_LastDraggingSlot < 10)
...
bool _XWindow_QuickSlot::CopyMouseSlot(int draggingslot)
{
	...
	if(m_QuickSlot[index].used)
	{
		...
		else if(m_QuickSlot[index].type == _XQS_SLOTTYPE_ITEM)
		{
			...
			m_MouseSlotItem.m_cType = m_QuickSlot[index].itemCType;
...
void _XWindow_QuickSlot::ProcessIconMove(void)
{
	...
	else if(m_MouseSlotType == _XQS_SLOTTYPE_ITEM)
	{
		...
		m_QuickSlot[index].itemCType = m_MouseSlotItem.m_cType;
...
{
	if(g_pInventory_Window->m_MouseSlotItem.m_cType == _XGI_FC_POTION || 
		/*g_pInventory_Window->m_MouseSlotItem.m_cType == _XGI_FC_BOOK || */
		g_pInventory_Window->m_MouseSlotItem.m_cType == _XGI_FC_LIFE ||
		g_pInventory_Window->m_MouseSlotItem.m_cType == _XGI_FC_CONSUMING )
	{
		if( g_pInventory_Window->m_MouseSlotItem.m_cType == _XGI_FC_POTION && 
			g_pInventory_Window->m_MouseSlotItem.m_cSecond == _XGI_SC_POTION_HWATA )			
			return;
		int index = m_SelectedSet*10 + m_LastDraggingSlot;
		if(index < 0 || index >= 30)
			return;
		...
		m_QuickSlot[index].type = _XQS_SLOTTYPE_ITEM;
		m_QuickSlot[index].itemCType = g_pInventory_Window->m_MouseSlotItem.m_cType;
...
void _XWindow_QuickSlot::CheckQuickSlotItem(void)
{
	int i = 0 ; for( ; i < 30 ; i++)
	{
		if(m_QuickSlot[i].used)
		{
			...
			{
				if(g_pLocalUser->m_UserItemList[m_QuickSlot[i].invenNumber].m_cType != m_QuickSlot[i].itemCType ||
...
void _XWindow_QuickSlot::ProcessCooldown(void)
{
	...
	for(index = 0 ; index < 30 ; index++)
	{
		if(m_QuickSlot[index].used && m_QuickSlot[index].type == _XQS_SLOTTYPE_ITEM)
		{
			...
			if(g_pLocalUser->m_UserItemList[invennumber].m_cType == m_QuickSlot[index].itemCType &&
