...
void _XWindow_DefaultTooltip::DrawItemTooltip()
{
	...
	if( m_CurrentItemSocketCount > -1 )
	{
		...
		int i = 0 ; for(; i < m_CurrentItemSocketCount; i++ )
		{
						...
						case TMODE_NPCRESOURCEITEM :
							{
								...
								if( m_CurrentItemSocketCount > 0 )
								{
									...
									int i = 0 ; for(; i < 4; i ++ )
									{
										...
										if(  socketindex > 0 )
										{
											m_AddSocketItemList[i].m_cType				= _XGI_FC_SOCKET;
											m_AddSocketItemList[i].m_cSecond			= 0;
											m_AddSocketItemList[i].Set_m_sID(socketindex);
											m_AddSocketItemList[i].m_cIdentify			= 0;
											m_AddSocketItemList[i].m_cThird			= 0;
											m_AddSocketItemList[i].m_ucCount			= 0;									
...
//Author : 양희왕 //breif : 수집 가능 메시지 추가
		...
		if(tempWeaponItem[m_ItemId].cCashCheck == 2 || tempWeaponItem[m_ItemId].cCashCheck == 3)
		{
			...
			case TMODE_TARGETINFO :
				{
					param1 = m_pTargetInfo_Window->m_SelectedTargetInfo.getOnItems[m_CurrentSlotNumber].m_ucYear+1900;
					param2 = m_pTargetInfo_Window->m_SelectedTargetInfo.getOnItems[m_CurrentSlotNumber].m_ucMonth;
					param3 = m_pTargetInfo_Window->m_SelectedTargetInfo.getOnItems[m_CurrentSlotNumber].m_ucDay;
					param4 = m_pTargetInfo_Window->m_SelectedTargetInfo.getOnItems[m_CurrentSlotNumber].m_ucHour;
					param5 = m_pTargetInfo_Window->m_SelectedTargetInfo.getOnItems[m_CurrentSlotNumber].m_ucMinute;
			...
			case TMODE_WAREHOUSE_WAREHOUSEITEM :
				{
					param1 = g_pLocalUser->m_WarehouseItemList[m_CurrentSlotNumber].m_ucYear+1900;
					param2 = g_pLocalUser->m_WarehouseItemList[m_CurrentSlotNumber].m_ucMonth;
					param3 = g_pLocalUser->m_WarehouseItemList[m_CurrentSlotNumber].m_ucDay;
					param4 = g_pLocalUser->m_WarehouseItemList[m_CurrentSlotNumber].m_ucHour;
					param5 = g_pLocalUser->m_WarehouseItemList[m_CurrentSlotNumber].m_ucMinute;
			...
			case TMODE_PCTRADE_SELFITEM :
				{
					...
					if(pPCTrade_Window)
					{
						param1 = pPCTrade_Window->m_SelfTradeItem[m_CurrentSlotNumber].m_ucYear+1900;
						param2 = pPCTrade_Window->m_SelfTradeItem[m_CurrentSlotNumber].m_ucMonth;
						param3 = pPCTrade_Window->m_SelfTradeItem[m_CurrentSlotNumber].m_ucDay;
						param4 = pPCTrade_Window->m_SelfTradeItem[m_CurrentSlotNumber].m_ucHour;
						param5 = pPCTrade_Window->m_SelfTradeItem[m_CurrentSlotNumber].m_ucMinute;
			...
			case TMODE_PCTRADE_TARGETITEM :
				{
					...
					if(pPCTrade_Window)
					{
						param1 = pPCTrade_Window->m_TargetTradeItem[m_CurrentSlotNumber].m_ucYear+1900;
						param2 = pPCTrade_Window->m_TargetTradeItem[m_CurrentSlotNumber].m_ucMonth;
						param3 = pPCTrade_Window->m_TargetTradeItem[m_CurrentSlotNumber].m_ucDay;
						param4 = pPCTrade_Window->m_TargetTradeItem[m_CurrentSlotNumber].m_ucHour;
						param5 = pPCTrade_Window->m_TargetTradeItem[m_CurrentSlotNumber].m_ucMinute;
			...
			case TMODE_PERSONALSTORE_TRADEITEM :
				{
					...
					{
						param1 = pPersonalStore_Window->m_TradeItem[m_CurrentSlotNumber].m_ucYear+1900;
						param2 = pPersonalStore_Window->m_TradeItem[m_CurrentSlotNumber].m_ucMonth;
						param3 = pPersonalStore_Window->m_TradeItem[m_CurrentSlotNumber].m_ucDay;
						param4 = pPersonalStore_Window->m_TradeItem[m_CurrentSlotNumber].m_ucHour;
						param5 = pPersonalStore_Window->m_TradeItem[m_CurrentSlotNumber].m_ucMinute;
			...
			default :
				{
					param1 = g_pLocalUser->m_UserItemList[m_CurrentSlotNumber].m_ucYear+1900;
					param2 = g_pLocalUser->m_UserItemList[m_CurrentSlotNumber].m_ucMonth;
					param3 = g_pLocalUser->m_UserItemList[m_CurrentSlotNumber].m_ucDay;
					param4 = g_pLocalUser->m_UserItemList[m_CurrentSlotNumber].m_ucHour;
					param5 = g_pLocalUser->m_UserItemList[m_CurrentSlotNumber].m_ucMinute;
		...
		if( nSetGroup > 0 )
		{
			...
			int nSetItemSkillCount = g_pSetIteminfo->SetGroupSkillString( nSetGroup, nSetItemAccout, m_CurrentTooltipInfo ); //스킬 스트링 저장
...
void _XWindow_DefaultTooltip::DrawTextWeaponItem()
{
	...
	if( tempWeaponitem )
	{
		...
		if( strlen( m_CurrentTooltipInfo.Data_Buf[11] ) )
		{
			...
			int i = 0 ; for(; i < GetSmeltItem_MaxScaleInfo(); i++)
			{
				if( GetSmeltItem_ScaleInfo() > i )
				{
					_XDrawSolidBar( m_WindowPosition.x+ 50 +(i*3), m_WindowPosition.y + m_CurrentDrawPosY,
						m_WindowPosition.x+50+2+(i*3), m_WindowPosition.y + m_CurrentDrawPosY+11, D3DCOLOR_ARGB(255,0,240,100));
				}
				else
				{
					_XDrawSolidBar( m_WindowPosition.x+ 50 +(i*3), m_WindowPosition.y + m_CurrentDrawPosY,
						m_WindowPosition.x+50+2+(i*3), m_WindowPosition.y + m_CurrentDrawPosY+11, D3DCOLOR_ARGB(255,255,255,255));
				}
			...
			g_XSmallFont->SetColor( _XSC_DEFAULT );
			g_XSmallFont->PutsAlign(m_WindowPosition.x + 55 + nSmallFontDraw, m_WindowPosition.y + m_CurrentDrawPosY, _XFONT_ALIGNTYPE_LEFT, m_CurrentTooltipInfo.Strength_Data_Buf );
			g_XSmallFont->Flush();
		}
...
void _XWindow_DefaultTooltip::DrawTextAccessory()
{
	...
	if( strlen( m_CurrentTooltipInfo.Data_Buf[databufindex] ) )
	{
		...
		int i = 0 ; for(  i = databufindex+1; i < databufindex+5; i++ )
		{
			if( strlen( m_CurrentTooltipInfo.Data_Buf[i] ) )
			{
				m_CurrentDrawPosY += DT_ROW_INTERVAL;
