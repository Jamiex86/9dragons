...
//						currentslot->showtime = g_LocalSystemTime;
	...
	for(iter_state = m_BuffStateList.begin() ; iter_state != m_BuffStateList.end() ; iter_state++)
	{
		...
		if(currentslot)
		{
			...
			{
				int effectid = g_SkillProperty[currentslot->slot.slot_skill.id]->detailInfo[0].effect[0].id;
				DWORD lefttime = g_pLocalUser->GetLeftTimeFromStateList(en_from_specialbuff, effectid, currentslot->slot.slot_skill.id, currentslot->slot.slot_skill.level);
				if(lefttime < 6000)	// 남은 시간이 6초이하
				{
					if(!currentslot->bSign)
					{
...
void _XWindow_State::InsertBuffStateList(_XStateType type, short id, char slotindex)
{
	...
	for(iter_state = m_BuffStateList.begin() ; iter_state != m_BuffStateList.end() ; iter_state++)
	{
		...
		if(currentslot)
		{
			...
			else if( type == _XST_SPECIALBUFF )
			{
				...
				if( currentslot->type == type && currentslot->slot.slot_skill.level == slotindex )
				{
	...
	if(type == _XST_SKILL || type == _XST_PASSIVE || type == _XST_EXTRASKILL || type == _XST_SPECIALBUFF)
	{
...
void _XWindow_State::InsertHostileStateList(_XStateType type, short id, char slotindex)
{
	if(type != _XST_SKILL && type != _XST_EFFECT && type != _XST_PASSIVE && type != _XST_EXTRASKILL && type != _XST_SPECIALBUFF)
		return;
...
void _XWindow_State::UpdatePlayerStateList(_XStateType type, short beforevalue, short aftervalue)
{
	...
	{
		FLOAT percentage = (((FLOAT)aftervalue)/(FLOAT)g_pLocalUser->m_CharacterInfo.Get_max_lifepower())*100.0f;
		if(percentage >= 40.0f)
...
//					int index = g_pQuickSlot_Window->GetSelectedSet()*10 + g_pQuickSlot_Window->GetLevelButtonIndex();
				...
				{//Author : 양희왕 //breif : 추가
					...
					if( g_pMiniTooltip_Window->SetTooltipMode( MTMODE_BUFF_SKILL, 0, 0, currentslot->slot.slot_skill.id, 
						g_pLocalUser->GetLeftTimeFromStateList(en_from_specialbuff, effectid, currentslot->slot.slot_skill.id, currentslot->slot.slot_skill.level), pos.x, pos.y ) )
						g_pMiniTooltip_Window->ShowWindow( TRUE );					
					return;
				}
			} // if(currentslot->icon->CheckMousePosition())
		...
		if(currentslot)
		{
			if(currentslot->icon->CheckMousePosition())
			{
				...
				else if(currentslot->type == _XST_SPECIALBUFF)
				{
					...
					if( g_pMiniTooltip_Window->SetTooltipMode( MTMODE_BUFF_SKILL, 0, 0, currentslot->slot.slot_skill.id, 0, pos.x, pos.y ) )
						g_pMiniTooltip_Window->ShowWindow( TRUE );
					return;
				}
			} // if(currentslot->icon->CheckMousePosition())
