...
void _XWindow_MiniTooltip::DrawCharInfoAbilityStep()
{
	...
	int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[m_ItemId].__Grade; i++ )
	{
...
void _XWindow_MiniTooltip::DrawPartyInfo()
{
	...
	if( g_LanguageType == _XLANGUAGE_TYPE_VIETNAMESE || g_LanguageType == _XLANGUAGE_TYPE_ENGLISH || g_LanguageType == _XLANGUAGE_TYPE_RUSSIAN )
	{
		if( strlen( m_CurrentMiniTooltipInfo.Data_Buf[0] ) ) 
		{
			...
			m_CurrentDrawPosY = 5;
			g_XBaseFont->PutsAlign(m_WindowPosition.x+90, m_WindowPosition.y + m_CurrentDrawPosY, _XFONT_ALIGNTYPE_CENTER, m_CurrentMiniTooltipInfo.Data_Buf[0]);
		}
		...
		if( strlen( m_CurrentMiniTooltipInfo.NameBuf ) ) 
		{
			...
			g_XBaseFont->PutsAlign(m_WindowPosition.x+90, m_WindowPosition.y + m_CurrentDrawPosY, _XFONT_ALIGNTYPE_CENTER, m_CurrentMiniTooltipInfo.NameBuf);
		}
	}
	else
	{
	...
	if( strlen( m_CurrentMiniTooltipInfo.LData_Buf ) ) 
	{
		g_XBaseFont->SetColor(_XSC_DEFAULT);
		m_CurrentDrawPosY += 15;
		g_XBaseFont->PutsAlign(m_WindowPosition.x+90, m_WindowPosition.y + m_CurrentDrawPosY, _XFONT_ALIGNTYPE_CENTER, m_CurrentMiniTooltipInfo.LData_Buf);		
	}
...
//				int i = 0 ; for(; i < g_NPCBuffCount; i++ )
//				{
...
//			}
			...
			{
				int tempcount = 0;
				int i = 0 ; for(; i < strlen( g_CharacterStateTable[m_ItemId]->concept ); i++ )
				{
			...
			case 0:		// 근력
				{
					...
					int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[0].__Grade; i++ )
					{
			...
			case 1:		// 진기
				{
					...
					int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[1].__Grade; i++ )
					{
			...
			case 2:		// 지혜
				{
					...
					int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[2].__Grade; i++ )
					{
			...
			case 3:		// 건강
				{
					...
					int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[3].__Grade; i++ )
					{
			...
			case 4:		// 민첩
				{
					...
					int i = 0 ; for(; i < g_pLocalUser->m_ElixirInfo.Data[4].__Grade; i++ )
					{
	...
	case MTMODE_BUFF_NICKNAME:
		{
			...
			int i = 0 ; for(; i < 5; i++ )
			{
				if( g_NickNameInfoTable[m_ItemId]._Effect[i].usEffectIndex > 0 )
				{
					...
					{
						int tempcount = 0;
						int j = 0 ; for( j = 0; j < strlen( g_CharacterStateTable[g_NickNameInfoTable[m_ItemId]._Effect[i].usEffectIndex]->concept ); j++ )
						{
