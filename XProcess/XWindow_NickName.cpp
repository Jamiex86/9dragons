...
void _XWindow_NickName::SetNickNameList( void )
{
	...
	// 고유 별호
	int i = 0 ; for(; i < _XDEF_NICKNAME_TOTALCOUNT; i++)
	{
		if( i >=1 && i < 11 )// // 특수비무별호 
		{
			int tableindex = -1;
			...
			for( i = 0; i < 5; i++ )
			{
				if( g_NickNameInfoTable[tableindex]._Effect[i].usEffectIndex > 0 )
				{
					...
					if(iter_table != g_CharacterStateTable.end())
					{
						...
						{
							int tempcount = 0;
							int ii = 0 ; for(  ii = 0; ii < strlen( g_CharacterStateTable[effindex]->concept ); ii++ )
							{
						...
						TCHAR	splitstring[256];		
						int j = 0 ; for( j = 0; j < splitinfo.splitcount ; j++)
						{
							memset(splitstring, 0, sizeof(TCHAR)*256);
