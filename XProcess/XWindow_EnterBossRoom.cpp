					...
					if(g_pInventory_Window->GetShowStatus())
					{
						...
						{								
							if(g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)	//장착하고 있는 것은 불가능
							{
								//필요한 아이템인지 검사
								int reason = 0;
								if( CheckNeedItem(g_pInventory_Window->m_MouseSlotItem.m_cType, g_pInventory_Window->m_MouseSlotItem.Get_m_sID(), reason) )
								{
#ifdef _XDEF_DUNGEONTICKET_070615_KUKURI
...
#ifdef _XDEF_DUNGEONTICKET_070615_KUKURI
	int i = 0 ; for( ; i < _XDEF_BOSSROOM_NEEDITEMTYPE ; ++i)
	{
...
BOOL _XWindow_EnterBossRoom::CheckRequirement( void )
{
	...
	int i = 1 ; for( ; i < _XDEF_BOSSROOM_NEEDITEMNUMBER ; i++)
	{
...
void _XWindow_EnterBossRoom::CopyItemToSlot(int slotnumber, int invennumber)
{
	...
	m_NeedItem[slotnumber].m_cType		= g_pLocalUser->m_UserItemList[invennumber].m_cType;
	m_NeedItem[slotnumber].m_cSecond	= g_pLocalUser->m_UserItemList[invennumber].m_cSecond;
	...
	m_NeedItem[slotnumber].m_cThird		= g_pLocalUser->m_UserItemList[invennumber].m_cThird;
	m_NeedItem[slotnumber].m_ucCount	= g_pLocalUser->m_UserItemList[invennumber].m_ucCount;
...
void _XWindow_EnterBossRoom::CopySlotToInventory(int slotnumber, int invennumber)
{
	...
	g_pLocalUser->m_UserItemList[invennumber].m_cType     =     m_NeedItem[slotnumber].m_cType		;	
	g_pLocalUser->m_UserItemList[invennumber].m_cSecond	  =		m_NeedItem[slotnumber].m_cSecond	;
	...
	g_pLocalUser->m_UserItemList[invennumber].m_cThird	  =		_XGameItem::GetThird(m_NeedItem[slotnumber].m_cType, m_NeedItem[slotnumber].Get_m_sID());	
	g_pLocalUser->m_UserItemList[invennumber].m_ucCount   =		m_NeedItem[slotnumber].m_ucCount	;	
		
	g_pLocalUser->m_UserItemList[invennumber].Set_m_usCurDur(m_NeedItem[slotnumber].Get_m_usCurDur());
	g_pLocalUser->m_UserItemList[invennumber].Set_m_usMaxDur(_XGameItem::GetMaxDurability(m_NeedItem[slotnumber].m_cType, m_NeedItem[slotnumber].Get_m_sID()) );
...
void _XWindow_EnterBossRoom::SetItemInSlot(int index, int invennumber)
{
	...
	if(index == 0)
	{
		...
		if(g_pLocalUser->m_UserItemList[invennumber].m_cType == _XGI_FC_RESOURCE)
		{
			if(g_ResourceItemProperty[g_pLocalUser->m_UserItemList[invennumber].Get_m_sID()].cCashCheck == 1)
			{
				if(!m_ItemInSlot[index].bInsert)
				{
					...
					m_ItemInSlot[index].cType = static_cast <int>(g_pLocalUser->m_UserItemList[invennumber].m_cType);
...
BOOL _XWindow_EnterBossRoom::CheckAllSlot(int invennumber)
{
	...
	if(g_pLocalUser->m_UserItemList[invennumber].m_ucCount > usecnt)
		return TRUE;
	else
		return FALSE;
}