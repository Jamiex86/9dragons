...
int _XWindow_SocketPlugInExp::GetSocketModelIndex()
{
	...
	char itemtype = GetEqualItemType(m_IndependentItem->m_cType);
	if( itemtype != -1 )
	{
		switch(itemtype)
		{
	...
	case SM_EXPAND :
		{
			...
			case SS_READY :
				{
					int i = 0 ; for(; i < m_CurrentItemMaxSocketCount; i++ )
...
{
	if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[invenslotnumber].m_cType) ||
		_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[invenslotnumber].m_cType) )
	{
		// error 처리 ------------------------------------------------------------------------------------=
		if( g_pLocalUser->m_UserItemList[invenslotnumber].m_cSecond == _XGI_SC_CLOTHES_BACKPACK )
		{
			MessagePrint(7);
			return FALSE;
		}
		...
		{
			char maxSocketCount = g_pInventory_Window->GetMaxSocketCount(g_pLocalUser->m_UserItemList[invenslotnumber].m_cType, g_pLocalUser->m_UserItemList[invenslotnumber].Get_m_sID());
			...
			int i = 0 ; for(; i < g_pLocalUser->m_UserItemList[invenslotnumber].Get_m_ucSocketCount(); i++ )
			{
				if( g_pLocalUser->m_UserItemList[invenslotnumber].Get_m_sSocket(i) )
				{
					...
					m_SocketExpItemList[i].Reset();
					m_SocketExpItemList[i].m_cType	= _XGI_FC_SOCKET;					
	...
	else if( g_pLocalUser->m_UserItemList[invenslotnumber].m_cType == _XGI_FC_SOCKET )
	{
		if( m_IndependentItem )
		{
