		...
		if( pTradeConfirm_Window )
		{
			if( pTradeConfirm_Window->GetShowStatus() )
			{
				...
				else if( pTradeConfirm_Window->m_TradeMode == _XWAREHOUSE_FROMWAREHOUSE_ITEM )
				{
					...
					if( tempcount > g_pLocalUser->m_WarehouseItemList[pWarehouse_Window->m_MouseSlotItem.m_InventoryNumber].m_ucCount )
					{
						tempcount = g_pLocalUser->m_WarehouseItemList[pWarehouse_Window->m_MouseSlotItem.m_InventoryNumber].m_ucCount;
						//itoa(tempcount, tempstring, 10);
						sprintf( tempstring, _T("%u"), tempcount );
						int length = strlen(tempstring);
						pIMEContainer->ClearBuffer();
				...
				else if( pTradeConfirm_Window->m_TradeMode == _XPCTRADE_FROMTRADE_ITEM )
				{
					...
					if( pPCTrade_Window )
					{
						...
						{
							if( tempcount > pPCTrade_Window->m_MouseSlotItem.m_ucCount )
							{
								tempcount = pPCTrade_Window->m_MouseSlotItem.m_ucCount;
								//itoa(tempcount, tempstring, 10);
								sprintf( tempstring, _T("%u"), tempcount );
								int length = strlen(tempstring);
								pIMEContainer->ClearBuffer();
				...
				else if( pTradeConfirm_Window->m_TradeMode > 10 )
				{
					if( g_pInventory_Window )
					{
						...
						{
							if( tempcount > g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_ucCount )				
							{
								tempcount = g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_ucCount;
								//itoa(tempcount, tempstring, 10);
								sprintf( tempstring, _T("%u"), tempcount );
								int length = strlen(tempstring);
								pIMEContainer->ClearBuffer();
