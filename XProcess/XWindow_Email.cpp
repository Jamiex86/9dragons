...
void __stdcall _XIMECallback_EmailEditReturn( _XIMEContainer* pIMEContainer )
{
	...
	else
	{
		...
		if( cursorx == length )
		{
			int i = 0 ; for(  i = pEmail_Window->m_EmailTextLineTotalCount-1; i > pEmail_Window->m_EmailTextCurrentLinePos; i--)
			{
				TCHAR buffer[128];
...
//						}
				...
				else
				{
					...
					int i = 0 ; for(  i = pEmail_Window->m_EmailTextCurrentLinePos; i < pEmail_Window->m_EmailTextLineTotalCount - 1; i++)
					{
						TCHAR buffer[128];
						memset( buffer, 0, sizeof(TCHAR)*128 );
...
void _XWindow_Email::Draw(_XGUIObject*& pfocusobject)
{
	...
	case _XEMAILTAB_RECEIVE:
		{			
			...
			g_XBaseFont->SetColor( m_EmailColor );
			int linecount = 0;
			int i = 0 ; for(  i = m_EmailTextScrollPos ; i < m_EmailTextScrollPos + 8; i++)
			{
...
{
	m_EmailTextLength = 0;
	int i = 0 ; for(; i < m_EmailTextLineTotalCount ; i++)
	{
...
{
	int i = 0  ; for( i = emailindex; i < m_EmailReceiveTextTotalCount; i++)
	{
...
{
	int i = 0  ; for( i = emailindex; i < m_EmailTextTotalCount; i++)
	{
