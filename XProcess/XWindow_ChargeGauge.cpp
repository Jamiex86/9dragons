				...
				if( pSmeltItemWindow )
				{
					...
					case SCALES :
						if( m_bReadySandMsg )
						{
							...
							int nTempCount = 0;
							int nSlot = 0;
							if( g_pLocalUser->m_UserItemList[pSmeltItemWindow->GetSmeltItemindex()].m_ucCount < pSmeltItemWindow->GetCount() ) //슬롯에 수량보다 바를 수량이 많은경우
							{
								while( nSlot != -1 )
								{
									//바를 용린 개수 저장
									...
									//현재 바를 수량 저장//인벤 수량
									nTempCount = g_pLocalUser->m_UserItemList[pSmeltItemWindow->GetSmeltItemindex()].m_ucCount;
									
									if( g_pLocalUser->m_UserItemList[pSmeltItemWindow->GetSmeltItemindex()].m_ucCount >= pSmeltItemWindow->GetCount() )
									{
									...
									{
										//슬롯에 있는 수량을 저장
										pSmeltItemWindow->SetCount( g_pLocalUser->m_UserItemList[pSmeltItemWindow->GetSmeltItemindex()].m_ucCount );
										//패킷을 보냄
										g_NetworkKernel.SendPacket( MSG_NO_INCHANT_CTRL, inchant_mode_protect_add_count_req );
