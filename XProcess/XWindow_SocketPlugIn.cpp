...
{	
	...
	{
		int i = 0 ; for(; i < m_IndependentItem->Get_m_ucSocketCount(); i++ )
		{
	...
	if( m_IndependentItem )
	{
		...
		int tempLength = strlen( _XGameItem::GetItemName(m_IndependentItem->m_cType, m_IndependentItem->Get_m_sID() ));
		if( tempLength > 16 )
		{
		...
		{
			g_XBaseFont->PutsAlign(m_WindowPosition.x+91, m_WindowPosition.y+40, _XFONT_ALIGNTYPE_CENTER, _XGameItem::GetItemName(m_IndependentItem->m_cType, m_IndependentItem->Get_m_sID() ) );		
		}	
...
{	
					...
					if(g_pInventory_Window)	// 인벤토리에서 드래그 했을 경우
					{
						if(g_pInventory_Window->GetShowStatus() && !g_pLocalUser->m_bSocketPlugInLock )
						{
							...
							if(g_pInventory_Window->m_bHaveMouseSlot && ivindex >= _XINVENTORY_SLOTTYPE_POCKETSTART)
							{
								...
								if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[ivindex].m_cType) ||
									_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[ivindex].m_cType) )
#else
								if( g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_WEAPON || 
									g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_CLOTHES ||
									g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_WEAPON2 || 
									g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_CLOTHES2 ||
									g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_WEAPON3 ||
									g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_CLOTHES3 )
#endif
								{
									// error 처리 ------------------------------------------------------------------------------------=
									if( g_pLocalUser->m_UserItemList[ivindex].m_cSecond == _XGI_SC_CLOTHES_BACKPACK )
									{
										MessagePrint(7);
										return FALSE;
									}
									...
									if(_XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[ivindex].m_cType))
									{
										_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[ivindex].m_cType);
										if( WeaponItem )
										{
											socketcheck = WeaponItem[g_pLocalUser->m_UserItemList[ivindex].Get_m_sID()].ucSocketCheck;
										}
									}
									else if(_XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[ivindex].m_cType))
									{
										_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[ivindex].m_cType);
										if( ClothesItem )
										{
									...
									else
									{
										...
										PlayItemEquipSound( m_IndependentItem->m_cType, m_IndependentItem->Get_m_sID() );
										bool bflag = false;
										int i = 0 ; for(; i < _XDEF_SOCKETITEM_MAXCOUNT/*g_pLocalUser->m_UserItemList[ivindex].m_ucSocketCount*/; i++ )
										{
											if( g_pLocalUser->m_UserItemList[ivindex].Get_m_sSocket(i) )
											{
												bflag = true;
												m_SocketItemList[i].Reset();
												m_SocketItemList[i].m_cType	= _XGI_FC_SOCKET;
								...
								else if( g_pLocalUser->m_UserItemList[ivindex].m_cType == _XGI_FC_SOCKET )
								{
									if( m_IndependentItem )
									{
										int insertindex = -1;
										...
										{
											// error 처리 ------------------------------------------------------------------------------------=
											char temp1 = m_IndependentItem->m_cType;
											char temp2 = g_pLocalUser->m_UserItemList[ ivindex ].m_cSecond;
											...
											if( m_IndependentItem->m_cType == _XGI_FC_CLOTHES ||
												m_IndependentItem->m_cType == _XGI_FC_CLOTHES2 ||
												m_IndependentItem->m_cType == _XGI_FC_CLOTHES3 )
											{
												if( g_pLocalUser->m_UserItemList[ ivindex ].m_cSecond == 1 )
												{
													...
													case _XGI_SC_CLOTHES_MASK :			// 치장 = 8
														{
															if(m_IndependentItem->m_cSecond != 
												...
												{
													_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_IndependentItem->m_cType);
													if( ClothesItem )
													{
	...
	#ifdef _XDEF_NEW_SOCKET_071031_KUKURI
											if(_XGI_FC_WEAPON == GetEqualItemType(m_IndependentItem->m_cType))
											{
												if(g_SocketItemProperty[g_pLocalUser->m_UserItemList[ ivindex ].Get_m_sID()].ucGrade > 0)
												{
													_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_IndependentItem->m_cType);
													if( WeaponItem )
													{
											...
											PlayItemEquipSound( m_SocketItemList[insertindex].m_cType, m_SocketItemList[insertindex].Get_m_sID() );
										}
									}
									else
									{
			...
			if(m_WindowMouseState == _XW_STATE_RIGHTCLICK)
			{
		...
		
				{
					...
					if( m_LastDraggingSlot != -1)
					{
						...
						{
							int i = 0 ; for(; i < m_IndependentItem->m_ucSocketCount; i++ )
							{								
								m_SocketItemList[i].Reset();
						...
						
						{
							...
							else
							{
								...
								// 소켓 중간이 빠졌을 경우
								int i = 0 ; for(  i = m_LastDraggingSlot+1; i < m_IndependentItem->Get_m_ucSocketCount(); i++ )
								{
#ifdef _XDEF_SOCKET_DETACHSYSTEM_070704_MAGVIPER //Author : 양희왕 //breif : 치장의상 소켓 장착 방식 변경
									if( m_SocketItemList[i].Get_m_sID() > 0 && m_IndependentItem->Get_m_sSocket(i) < 1)
									{
										int j = 0 ; for( j = 0; j < m_IndependentItem->Get_m_ucSocketCount(); j++ )
										{
								...
								if( tempflag )
								{
									...
									m_SocketPlugInButton->SetButtonTextID(_XSTRINGDB_SN_EODSTRING_XSR, ID_STRING_SOCKETPLUGIN_DROPITEM); // 물품 내리기
									BOOL tempflag2 = TRUE;
									int i = 0 ; for(; i < m_IndependentItem->Get_m_ucSocketCount()+1; i++ )
									{
										if( m_SocketItemList[i].Get_m_sID() > 0 )
											tempflag2 = FALSE;
									}
	...
	case _BOXTEXTMODE_STEP2:
		{
			int i = 0 ; for(; i < _XDEF_SOCKETITEM_MAXCOUNT; i++ )
			{
				if( m_SocketItemList[i].Get_m_sID() > 0 )
				{
					...
					if( _XGI_FC_WEAPON == GetEqualItemType(m_IndependentItem->m_cType) )
#else
					if( m_IndependentItem->m_cType == _XGI_FC_WEAPON || m_IndependentItem->m_cType == _XGI_FC_WEAPON2 || m_IndependentItem->m_cType == _XGI_FC_WEAPON3 )
#endif
					{
					...
					else if( m_IndependentItem->m_cType == _XGI_FC_CLOTHES || m_IndependentItem->m_cType == _XGI_FC_CLOTHES2 || m_IndependentItem->m_cType == _XGI_FC_CLOTHES3 )
					{
...
#ifdef _XTS_ITEM_OPTIMIZATION
	char cType = GetEqualItemType(m_IndependentItem->m_cType);
...
#endif
		{
			switch( m_IndependentItem->m_cSecond ) 
			{
...
{
	int index=0;
	switch( GetEqualItemType(m_IndependentItem->m_cType) ) 
	{
	case _XGI_FC_WEAPON:
		{
			_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_IndependentItem->m_cType);
	...
	case _XGI_FC_CLOTHES:
		{
			if( m_IndependentItem->m_cSecond == _XGI_SC_CLOTHES_MASK )
			{
				_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_IndependentItem->m_cType);
				if( ClothesItem )
				{
	...
	case _XGI_FC_POTION:
		{
			_XGI_PotionItem_Property* PotionItem = (_XGI_PotionItem_Property*)GetFirstTypeItemPorperty(m_IndependentItem->m_cType);
			if( PotionItem )
