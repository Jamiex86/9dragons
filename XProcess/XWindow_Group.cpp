...
void __stdcall _XIMECallback_NoticeEditReturn( _XIMEContainer* pIMEContainer )
{
	...
	else
	{
		...
		if( cursorx == length )
		{
			int i = 0 ; for(  i = g_pGroup_Window->m_NoticeTextTotalCount-1; i > g_pGroup_Window->m_CurrentNoticeLinePos; i--)
			{
				TCHAR buffer[128];
		...
		if( g_pGroup_Window->m_CurrentNoticeLinePos != 0 )
		{
			...
			if( strlen( g_pGroup_Window->m_NoticeIMEControl.GetText() ) == 0 )
			{
				...
				else
				{
					...
					int i = 0 ; for(  i = g_pGroup_Window->m_CurrentNoticeLinePos; i < g_pGroup_Window->m_NoticeTextTotalCount - 1; i++)
					{
						TCHAR buffer[64];
						memset( buffer, 0, sizeof(TCHAR)*64 );
...
{	
	int index = 0 ; for(  index = 0; index < _XGROUPTAB_TOTALCOUNT; index++ )
	{		
...
void _XWindow_Group::Draw( _XGUIObject*& pfocusobject )
{
	...
	{	
		...
		{	
			int i = 0 ; for(  i = m_NoticeTextScrollPos ; i < m_NoticeTextScrollPos+4; i++)
			{
...
void _XWindow_Group::LineUpLevel()
{
	...
	int i = 0 ; for(  i = 1; i < m_OrganizationMemberList.size(); ++i )
...
{
	m_NoticeTextLength = 0;
	int i = 0 ; for(; i < m_NoticeTextTotalCount ; i++)
	{
...
void _XWindow_Group::ChangeTab(_XGroupWindowTab selectedtab)
{
	...
	int i = 0 ; for(; i < _XGROUPTAB_TOTALCOUNT; i++ )
	{			
		m_BaseTabButton[i]->SetCheck( FALSE );
...
void _XWindow_Group::DrawCBOrgTotalRecord(void)
{
	...
	int i = 0 ; for( ; i < m_lstboxOrgRecord->GetItemCount() ; ++i)
	{
