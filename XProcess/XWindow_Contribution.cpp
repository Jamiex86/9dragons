						...
						if(g_pInventory_Window->GetShowStatus())
						{
							if(g_pInventory_Window->m_bHaveMouseSlot && g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber >= _XINVENTORY_SLOTTYPE_POCKETSTART)
							{
								...
								if( flag )
								{
									...
									{
										m_ContributionType			= g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cType;
										m_ContributionSecondType	= g_pLocalUser->m_UserItemList[g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber].m_cSecond;
										...
										m_ContributionItemCount		= g_pInventory_Window->m_MouseSlotItem.m_ucCount;
...
int	_XWindow_Contribution::CalcContribution(int invennumber)
{
	...
	return static_cast<int>( TempRet * static_cast<float>(CheckClanPoint(g_pLocalUser->m_UserItemList[invennumber].m_cType, g_pLocalUser->m_UserItemList[invennumber].Get_m_sID()) * g_pLocalUser->m_UserItemList[invennumber].m_ucCount) );
}
