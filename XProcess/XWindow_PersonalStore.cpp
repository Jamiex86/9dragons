...
{
	int type = GetEqualItemType( m_TradeItem[m_SelectedSlotNumber].m_cType );
	...
	case _XGI_FC_WEAPON:
		{
			_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_TradeItem[m_SelectedSlotNumber].m_cType);
			if( WeaponItem )
			{
	...
	case _XGI_FC_CLOTHES:
		{
			_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(m_TradeItem[m_SelectedSlotNumber].m_cType);
			if( ClothesItem )
			{
							...
							if(g_pInventory_Window->GetShowStatus())
							{
								...
								else // 개인노점 창에서 그냥 클릭 했을 경우 
								{
									...
									if( selectedindex > -1 )
									{
										...
										{
											char cType = m_TradeItem[m_SelectedSlotNumber].m_cType;
				...
				if(m_WindowMouseState == _XW_STATE_RIGHTCLICK)
				{
					...
					{	
						...
						if( m_TradeItem[m_LastDraggingSlot].IsStackable() )
						{
							...
							{
								g_pLocalUser->m_UserItemList[ inventoryindex ].m_ucCount += m_TradeItem[m_LastDraggingSlot].m_ucCount;
							}
						}
						else
						{
...
{
	m_TradeItem[slotnumber].m_cType = g_pLocalUser->m_UserItemList[invennumber].m_cType;
	m_TradeItem[slotnumber].m_cSecond= g_pLocalUser->m_UserItemList[invennumber].m_cSecond;
	...
	m_TradeItem[slotnumber].m_cThird = g_pLocalUser->m_UserItemList[invennumber].m_cThird;
	m_TradeItem[slotnumber].m_ucCount = g_pLocalUser->m_UserItemList[invennumber].m_ucCount;
