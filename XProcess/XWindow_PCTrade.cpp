						...
						if(g_pInventory_Window->GetShowStatus())
						{
							if(g_pInventory_Window->m_bHaveMouseSlot)
							{
								...
								if(m_LastDraggingSlot >= 10)
								{
									...
									{
										if( _XGameItem::CheckUserTrade( g_pLocalUser->m_UserItemList[ g_pInventory_Window->m_MouseSlotItem.m_InventoryNumber ].m_cType, 
					...
					if(scrnpos->x == m_IconDragStartPos.x && scrnpos->z == m_IconDragStartPos.y)
					{
						...
						if(m_LastDraggingSlot >= 10)
						{
							...
							if( g_pDefaultTooltip_Window )
							{
								...
								if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_PCTRADE_SELFITEM, m_LastDraggingSlot, m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID() ) )
								{
						...
						else if(m_LastDraggingSlot >= 0)
						{
							...
							if( g_pDefaultTooltip_Window )
							{
								...
								if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_PCTRADE_TARGETITEM, m_LastDraggingSlot, m_MouseSlotItem.m_cType, m_MouseSlotItem.Get_m_sID() ) )
								{
									g_pDefaultTooltip_Window->ShowWindow( TRUE );
								}
							}
	...
	int i = 0 ; for( ; i < _XDEF_PCTRADE_MAX_SLOTNUMBER ; i++)
	{
		if(m_SelfTradeItem[i].Get_m_sID() > 0)
		{
			if(m_SelfTradeItem[i].IsStackable())
			{
				...
				{
					g_pLocalUser->m_UserItemList[m_SelfTradeItem[i].m_InventoryNumber].m_ucCount += m_SelfTradeItem[i].m_ucCount;
				}
				else
				{
...
{
	m_SelfTradeItem[slotnumber].m_cType = g_pLocalUser->m_UserItemList[invennumber].m_cType;
	m_SelfTradeItem[slotnumber].m_cSecond= g_pLocalUser->m_UserItemList[invennumber].m_cSecond;
	...
	m_SelfTradeItem[slotnumber].m_cThird = g_pLocalUser->m_UserItemList[invennumber].m_cThird;
	m_SelfTradeItem[slotnumber].m_ucCount = g_pLocalUser->m_UserItemList[invennumber].m_ucCount;
	...
	m_SelfTradeItem[slotnumber].m_ucYear		= g_pLocalUser->m_UserItemList[invennumber].m_ucYear;
	m_SelfTradeItem[slotnumber].m_ucMonth		= g_pLocalUser->m_UserItemList[invennumber].m_ucMonth;
	m_SelfTradeItem[slotnumber].m_ucDay			= g_pLocalUser->m_UserItemList[invennumber].m_ucDay;
	m_SelfTradeItem[slotnumber].m_ucHour		= g_pLocalUser->m_UserItemList[invennumber].m_ucHour;
	m_SelfTradeItem[slotnumber].m_ucMinute		= g_pLocalUser->m_UserItemList[invennumber].m_ucMinute;
}
