...
BOOL _XWindow_RepairItem::Initialize( void )
{
	...
	int i = 0 ; for(; i < _XDEF_REPAIR_MAX_SENDCOUNT; i++ )
	{
							...
							if( m_LastDraggingSlot < m_SelectedSlotNoList.size() )
							{
								...
								if( g_pDefaultTooltip_Window )
								{
									...
									if( g_pDefaultTooltip_Window->SetTooltipMode( TMODE_REPAIRITEM, invennumber,
										g_pLocalUser->m_UserItemList[invennumber].m_cType, g_pLocalUser->m_UserItemList[invennumber].Get_m_sID() ) )
									{
...
void _XWindow_RepairItem::SetRepairListItem(void)
{
	...
	for( ; iter != m_SelectedSlotNoList.end(); iter++ )
	{
		...
		g_pInventory_Window->GetIconRect( g_pLocalUser->m_UserItemList[*iter].m_cType, 
		...
		FLOAT temp3 = (FLOAT)(_XGameItem::GetItemPrice( g_pLocalUser->m_UserItemList[*iter].m_cType, g_pLocalUser->m_UserItemList[*iter].Get_m_sID()) ) / 5.0f;
