...
//	g_XBaseFont->Puts(m_WindowPosition.x+174, m_WindowPosition.y+351, g_pLocalUser->m_myMatchMemberInfo.cName, 1.0f);
	...
	if( temp != 0)
	{
		FLOAT winrate = (FLOAT)g_pLocalUser->m_CharacterInfo.winscore /	(FLOAT)(temp) * 100.f;
	}
	...
	g_XBaseFont->Print(m_WindowPosition.x+57, m_WindowPosition.y+103, 1.0f, _XGETINTERFACETEXT(ID_STRING_MATCH_2521), winrate);

	g_XBaseFont->SetColor(0xFFFC3410);
	winrate = (FLOAT)g_pLocalUser->m_myMatchMemberInfo.usWinRate / 10.0f;
	g_XBaseFont->Print(m_WindowPosition.x+163, m_WindowPosition.y+103, 1.0f, _XGETINTERFACETEXT(ID_STRING_MATCH_2521), winrate);
