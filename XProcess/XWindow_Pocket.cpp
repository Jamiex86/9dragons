...
{
	int i = 0 ; for(; i < _XDEF_POCKET_MAX_SLOTNUMBER; i++ )
...
{
	m_AccessoryItemList[index].m_cType		= _XGI_FC_ACCESSORY;
	m_AccessoryItemList[index].m_cSecond	= second;
	m_AccessoryItemList[index].Set_m_sID(sId);
	m_AccessoryItemList[index].m_ucCount	= 1;
