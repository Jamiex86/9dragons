...
//	Create a new text block and add it to the current docuemnt-paragraph
{
	if( m_strToken.GetSize() )
	{
		...
		CHTMLTextBlock *pText = new CHTMLTextBlock( str, m_pDocument->GetFontDefIndex( def ), m_pCurrentStyle->m_crFore, m_pCurrentStyle->m_bBackground, m_pCurrentStyle->m_crBack, IsPreformatted() );
		if( m_pLastAnchor )
...
void CHTMLParse::OnGotFont( const CParameters &pList )
{
	...
	for( UINT n = 0; n < uParamSize; n++ )
	{
		...
		case pBColor:
			m_pCurrentStyle->m_bBackground = true;
...
void CHTMLParse::OnGotEndAnchor()
{
	...
	if( m_pLastAnchor )
		CreateNewTextObject();
	m_pLastAnchor = NULL;
	m_pCurrentStyle->bUnderline = false;
}
...
void CHTMLParse::CreateList( bool bOrdered, const CParameters & paramList )
{
	...
	const UINT uParamSize = paramList.GetSize();
	for( UINT n = 0; n < uParamSize; n++ )
	{
...
void CHTMLParse::OnGotListItem( const CParameters & paramList )
{
...

	{
		...
		for( UINT n = 0; n < uParamSize; n++ )
		{
			...
			{
			case pValue:
				uValue = GetNumberParameter( strParam, m_stkList.GetSize() + 1 );
				bGotValue = true;
				break;
			}
		}
...
void CHTMLParse::OnGetInlineStyle( const CStaticString &strStyle )
{
	...
	const UINT uStyleCount = styles.GetSize();
	for( UINT n = 0; n < uStyleCount; n++ )
	{
