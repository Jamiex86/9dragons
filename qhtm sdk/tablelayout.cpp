...
CHTMLTableLayout::CHTMLTableLayout( CHTMLTable* ptab, GS::CDrawContext& dc, int nMaxWidth, int nZoomLevel)
	: m_pTab(ptab)
...
WinHelper::CSize CHTMLTableLayout::MeasureDocument(CHTMLDocument* pdoc, int nWidth)
{
	...
	CHTMLSectionCreator htCreate( &TempSection, m_dc, 0, 0, nWidth, COLORREF(0), true, m_nZoomLevel, NULL );
...
void CHTMLTableLayout::MeasureCell(CHTMLTableCell* pCell, int nCol)
{
 	...
 	if (!m_pTab->m_bCellsMeasured)
	{
		...
		const WinHelper::CSize size( MeasureDocument( pCell, testWidth ));
