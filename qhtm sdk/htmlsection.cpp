...
void CHTMLSection::AddHTML( LPCTSTR pcsz )
{
	...
	CHTMLParse html( pcsz, _tcslen( pcsz ), m_hDefaultInstance, NULL, m_pDefaults );
...
WinHelper::CSize CHTMLSection::LayoutDocument( GS::CDrawContext &dc, CHTMLDocument *pDocument, int nYPos, int nLeft, int nRight )
{
	...
	CHTMLSectionCreator htCreate( this, dc, nYPos, nLeft, nRight, m_pDocument->m_back.m_crBack, false, m_uZoomLevel, NULL );
...
bool CHTMLSection::TabToNextControl()
{
	...
	CFocusControlABC *pFocus = NULL;

	const UINT uSize = m_arrFocus.GetSize();
	if( m_pFocus )
	{
		for( UINT u = 0; u < uSize; u++ )
		{
			if( m_pFocus == m_arrFocus[ u ] )
			{
				if( u == uSize - 1 )
				{
					pFocus = m_arrFocus[ 0 ];
...
void CHTMLSection::SubmitForm( CHTMLForm *pForm, LPCTSTR pcszSubmitValue )
{
	...
	else
	{
		...
		if( m_pDefaults->m_funcQHTMFORMCallback )
		{
			...
			form.uFieldCount = arrField.GetSize();
...
{
	const UINT uLinks = m_arrLinks.GetSize();
	for( UINT uLink = 0; uLink < uLinks; uLink++ )
	{
		const CHTMLSectionLink *pSectionLink = m_arrLinks[ uLink ];
		const UINT uSections = pSectionLink->m_arrSections.GetSize();
		for( UINT uSection = 0; uSection < uSections; uSection++ )
		{
...
void CHTMLSection::SelectNearestLink( POINT pt )
{
	...
	const UINT uLinkCount = m_arrLinks.GetSize();
	for( UINT u = 0; u < uLinkCount; u++ )
	{
