...
typedef struct _arrCharacters
{	
	TCHAR pcszName[10];	
	TCHAR ch;
} _arrCharactersType;

_arrCharactersType	g_arrCharacters[101];
...
int CHTMLParseBase::Initialise()
{
	TCHAR _CDATA[][8] = 
	{
			_T("nbsp"),		_T("apos"),		_T("quot"),		_T("amp"),		_T("lt"),		_T("gt"),		_T("iexcl"),	_T("cent"),		_T("pound"),
			_T("curren"),	_T("yen"),		_T("brvbar"),	_T("sect"),		_T("uml"),		_T("copy"),		_T("ordf"),		_T("laquo"),	_T("not"),		
			_T("shy"),		_T("reg"),		_T("macr"),		_T("deg"),		_T("plusmn"),   _T("sup2"),		_T("sup3"),		_T("acute"),	_T("micro"),	
			_T("para"),		_T("middot"),	_T("cedil"),	_T("sup1"),		_T("ordm"),		_T("raquo"),	_T("frac14"),	_T("frac12"),	_T("frac34"),	
			_T("iquest"),	_T("Agrave"),	_T("Aacute"),	_T("Acirc"),	_T("Atilde"),	_T("Auml"),		_T("Aring"),	_T("AElig"),	_T("Ccedil"),	
			_T("Egrave"),	_T("Eacute"),	_T("Ecirc"),	_T("Euml"),		_T("Igrave"),	_T("Iacute"),	_T("Icirc"),	_T("Iuml"),		_T("ETH"),		
			_T("Ntilde"),	_T("Ograve"),	_T("Oacute"),	_T("Ocirc"),	_T("Otilde"),	_T("Ouml"),		_T("times"),	_T("Oslash"),	_T("Ugrave"),	
			_T("Uacute"),	_T("Ucirc"),	_T("Uuml"),		_T("Yacute"),	_T("THORN"),	_T("szlig"),	_T("agrave"),	_T("aacute"),	_T("acirc"),	
			_T("atilde"),	_T("auml"),		_T("aring"),	_T("aelig"),	_T("ccedil"),	_T("egrave"),	_T("eacute"),	_T("ecirc"),	_T("euml"),		
			_T("igrave"),	_T("iacute"),	_T("icirc"),	_T("iuml"),		_T("eth"),		_T("ntilde"),	_T("ograve"),	_T("oacute"),	_T("ocirc"),	
			_T("otilde"),	_T("ouml"),		_T("divide"),	_T("oslash"),	_T("ugrave"),	_T("uacute"),	_T("ucirc"),	_T("uuml"),		_T("yacute"),	
			_T("thorn"),	_T("yuml")
	};
	
	for( int i = 0; i < 101; i++ )
	{
		memset( g_arrCharacters[i].pcszName, 0, sizeof(TCHAR) * 10 );
		
#if	_UNICODE
		wcscpy( g_arrCharacters[i].pcszName, _CDATA[i] );
#else
		strcpy( g_arrCharacters[i].pcszName, _CDATA[i] );
#endif
		
		if( i == 0 )
		{			
			BYTE	_char = 160;
			g_arrCharacters[i].ch = _char;
		}
		else if( i >= 6 )
		{
			g_arrCharacters[i].ch = (TCHAR)(161 + (i-6));
		}			
		else
		{
			if( i == 1 )
				g_arrCharacters[i].ch = _T('\'');
			else if( i == 2 )
				g_arrCharacters[i].ch = _T('\"');
			else if( i == 3 )
				g_arrCharacters[i].ch = _T('&');
...
void CHTMLParseBase::OnGotComment( LPCTSTR pcszTagText, UINT uBytes)
//
//	Send when a comment is found
{
	...
	LPTSTR pcszVirtual = _tcsstr( str, _T("virtual") );
	if( pcszVirtual )
	{
		pcszVirtual += 7;
		pcszVirtual = SkipWhiteSpace( pcszVirtual );
		if( *pcszVirtual == _T('=') )
		{
			pcszVirtual++;
			pcszVirtual = SkipWhiteSpace( pcszVirtual );
