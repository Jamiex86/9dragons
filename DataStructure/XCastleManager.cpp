	...
	if(m_bDrawOwnerMessage)
	{
		...
		if(elapsedtime <= 10000)	// 10초 동안 보여줌
		{
			...
			if(pCastleInfo)
			{
				...
				{
					// 몬스터 소유
					sprintf(messagestr1, _XGETINTERFACETEXT(ID_STRING_NEW_3768), 
				...
				else
				{
					...
					{
						// 단
						sprintf(messagestr1, _XGETINTERFACETEXT(ID_STRING_NEW_3769),
							g_ZoneInfoTable[g_CurrentZoneInfoIndex-1].zonename, 
						...
						sprintf(messagestr2, _XGETINTERFACETEXT(ID_STRING_NEW_3771), pCastleInfo->m_tOwner.szOrgName);//%s단 세력은 경험치 증가 혜택을 받을 수 있습니다.
					}
					else
					{
						// 회
						sprintf(messagestr1, _XGETINTERFACETEXT(ID_STRING_NEW_3770),
							g_ZoneInfoTable[g_CurrentZoneInfoIndex-1].zonename, 
							_XGETINTERFACETEXT(g_CBManager.GetCastleNameTextId(g_CurrentZoneInfoIndex)),
							pCastleInfo->m_tOwner.szOrgName);//"%s지역의 %s는 %s회가 점령하고 있습니다."

						sprintf(messagestr2, _XGETINTERFACETEXT(ID_STRING_NEW_3772), pCastleInfo->m_tOwner.szOrgName);//%s회 세력은 경험치 증가 혜택을 받을 수 있습니다.
					}
				}
			}
