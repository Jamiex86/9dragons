...
BOOL _XGameItem::LoadItemProperty(void)
{
	...
	do 
	{
		...
		if(_XDEF_ITEMTYPE_COUNT <= maxindex){//BM_add
...
BOOL _XGameItem::LoadItemTradePackage(void)
{
	...
	int i = 0 ; for( ; i < packagecount ; i++)
	{
		...
		int j = 0 ; for( j = 0 ; j < g_ItemTradePackage[i].itemCount ; j++)
		{
			...
			if( WeaponItem )
			{
				...
				{					
					if( WeaponItem[sID].usAddto )
						sprintf( g_ItemnameString, "%s %s %s", WeaponItem[sID].cNick, g_StringDataBase.GetTextString( _XSTRINGDB_SN_ITEMTABLE_ADDTO, WeaponItem[sID].usAddto ), WeaponItem[sID].cName );
					else
						sprintf( g_ItemnameString, "%s %s", WeaponItem[sID].cNick, WeaponItem[sID].cName );
				}
				else
				{
					if( WeaponItem[sID].usAddto )
						sprintf( g_ItemnameString, "%s %s", g_StringDataBase.GetTextString( _XSTRINGDB_SN_ITEMTABLE_ADDTO, WeaponItem[sID].usAddto ), WeaponItem[sID].cName );
					else
					{
						if(WeaponItem[sID].cName)
							strcpy( g_ItemnameString, WeaponItem[sID].cName );
						else
							sprintf(g_ItemnameString, _XGETINTERFACETEXT(ID_STRING_NETWORK_UNKNOWNITEM));
					}
				}
		...
		{			
			...
			if( ClothesItem )
			{
				...
				{	
					if(ClothesItem[sID].cName)
					{
						if( ClothesItem[sID].usAddto )
							sprintf( g_ItemnameString, "%s %s %s", ClothesItem[sID].cNick, g_StringDataBase.GetTextString( _XSTRINGDB_SN_ITEMTABLE_ADDTO, ClothesItem[sID].usAddto ), ClothesItem[sID].cName );
						else
							sprintf( g_ItemnameString, "%s %s", ClothesItem[sID].cNick, ClothesItem[sID].cName );
					}
					else
					{
						sprintf(g_ItemnameString, _XGETINTERFACETEXT(ID_STRING_NETWORK_UNKNOWNITEM));
				...
				else
				{
					...
					{
						if( ClothesItem[sID].usAddto )
							sprintf( g_ItemnameString, "%s %s", g_StringDataBase.GetTextString( _XSTRINGDB_SN_ITEMTABLE_ADDTO, ClothesItem[sID].usAddto ), ClothesItem[sID].cName );
						else
							sprintf( g_ItemnameString, "%s",ClothesItem[sID].cName );
					}
					else
					{
						sprintf(g_ItemnameString, _XGETINTERFACETEXT(ID_STRING_NETWORK_UNKNOWNITEM));
		...
		{			
			...
			if( ClothesItem )
			{
				if( ClothesItem[sID].usAddto > 0 && ClothesItem[sID].usAddto < 7 )
					...
					strcpy( g_ItemnameString, ClothesItem[sID].cName );
			}
			return g_ItemnameString;
		}
		break;					
...
{
	int i1 = 0 ; for(  i1 = 0; i1 < g_maxItemMixTableCount; i1++ )
	{
		if( index1 == g_ItemMixTable[i1].ucIndex[0] )
		{
			int i2 = 0 ; for(  i2 = i1; i2 < g_maxItemMixTableCount; i2++ )
			{
				if( index2 == g_ItemMixTable[i2].ucIndex[1] )
				{
					int i3 = 0 ; for(  i3 = i2; i3 < g_maxItemMixTableCount; i3++ )
					{
						if( index3 == g_ItemMixTable[i3].ucIndex[2] )
						{
							int i4 = 0 ; for(  i4 = i3; i4 < g_maxItemMixTableCount; i4++ )
							{
			...
			if( curItem->m_InsideFrustum && curItem->m_pMeshContainer )
			{
				...
				if(g_ViewItemNameTooltip)
				{
					...
					if( !g_pLocalUser->m_InParty )
					{
						...
						{
							sprintf(str, _XGETINTERFACETEXT(ID_STRING_NPCSKILL_MONEY), curItem->m_uiCount); // "금 %d"
						}
						else
						{
							if( curItem->m_ucCount > 1)
							{
								...
								// "%s %d개"
								strcpy( str, g_StringDataBase.CompositeString( _XSTRINGDB_SN_EODSTRING_XSR,ID_STRING_GAMEITEM_1570, param1, &param2 ) );
							}
							else
							{
#ifdef _XTS_ITEM_V20
								strcpy( str, _XGameItem::GetItemFullName(curItem->m_cType, curItem->m_sID) );
#else
								...
								{	
									sprintf( str, "%s %s", _XGameItem::GetItemNick(curItem->m_cType, curItem->m_sID), _XGameItem::GetItemName(curItem->m_cType, curItem->m_sID) );
								}
								else
								{
									strcpy( str, _XGameItem::GetItemName(curItem->m_cType, curItem->m_sID) );
								}
#endif
							}
							...
							TCHAR tempView[64];						
							sprintf( tempView, "Owner ID[%d] / My ID[%d] : distance [%.2f]", curItem->m_usOwnerID, g_pLocalUser->m_Uniqueid, distance );
#endif
						}
...
//							g_ItemTooltip.SetToolTipStringEx( rvec.x, rvec.y+20, tempView, 0xffffff00, 32, TRUE, FALSE );
					...
					else // 파티일때는 소유자를 표시한다.
					{
						...
						if(curItem->m_cType == _XGI_FC_MONEY)
						{
							...
							sprintf(str, "%s %d", _XGETINTERFACETEXT(ID_STRING_NETWORK_MONEY), curItem->m_uiCount);
						...
						else
						{
							if( str[0] != 0 )
							{
								...
								if( g_pLocalUser->m_PartyInfo[partyindex].playername[0] != 0 )
								{
									...
									TCHAR playername[64];
									sprintf(playername, "[%s]", g_pLocalUser->m_PartyInfo[partyindex].playername);	
...
BOOL _XUserItem::IsStackable(void)
{
	...
	case _XGI_FC_WEAPON3 :
		{
			if(m_cSecond == _XGI_SC_WEAPON_HIDDEN)
				return TRUE;
			else
				return FALSE;
		}
...
{
	m_cType			= useritem.m_cType;
	m_cSecond		= useritem.m_cSecond;
	Set_m_sID( useritem.Get_m_sID() );
	m_cIdentify		= useritem.m_cIdentify;
	
	m_cThird		= useritem.m_cThird;
	m_ucCount		= useritem.m_ucCount;
	...
	m_ucYear		= useritem.m_ucYear;
	m_ucMonth		= useritem.m_ucMonth;
	m_ucDay			= useritem.m_ucDay;
	m_ucHour		= useritem.m_ucHour;
	m_ucMinute		= useritem.m_ucMinute;
...
//				);
//			
//			int j = 0 ; for( j = 0; j < ITEM_NAME_LENGTH; j++)
//			{
//				if( newcashitempackage->m_PackageName[j] == '_' )
//					newcashitempackage->m_PackageName[j] = ' ';			
//			}						  
