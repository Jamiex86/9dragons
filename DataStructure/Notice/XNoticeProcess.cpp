...
bool _XNoticeProcess::SetNoticeProfile( LPTSTR szFileName )
{
	...
	//메시지 총 갯수 만큼 저장
	int i = 0 ; for(; i < m_XNoticeParser.GetMaxInfo(); i++)
	{
...
void _XNoticeProcess::PrintMessgae( vector<sMsgInfo*>::iterator iter )
{
	...
	sprintf(msgbuffer, pString, nHour);
...
bool _XNoticeProcess::SearchSlotFlagItem()
{
	...
	for( int count = 5; count < _XINVENTORY_SLOTTYPE_TOTALCOUNT; count++ )
	{
		...
		if( g_pLocalUser->m_UserItemList[count].Get_m_sID() > 0 &&
				g_pLocalUser->m_UserItemList[count].m_ucYear > 0 )
		{
			//만료 3일후를 계산 해야 함..
			int DueDay = GetTotalDays( (int)(g_pLocalUser->m_UserItemList[count].m_ucYear+1900), (int)g_pLocalUser->m_UserItemList[count].m_ucMonth, (int)g_pLocalUser->m_UserItemList[count].m_ucDay) 
