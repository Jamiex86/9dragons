...
//	_XSetDefaultPath();
	...
	sprintf(filename, _T("\\NDBattle_%s_%d_%d_%d_%d_%d.log"), charactername, systemtime.wYear, systemtime.wMonth, systemtime.wDay, systemtime.wHour, systemtime.wMinute);

	strcpy(m_FileName, gModulePath);
...
void _XBattleLog::Print(char* argv, ...)
{
	...
	retValue = vsprintf(szBuffer, argv, argptr);
	va_end(argptr);
...
void _XBattleLog::SetEffect(int id, int level)
{
	...
	{
		int i = 0 ; for( ; i < g_SkillProperty[id]->detailInfo[level].effectCount ; ++i)
		{
