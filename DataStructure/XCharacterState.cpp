...
BOOL _XCharacterStateList::GetCommand(char* cmd, FILE* fileptr, bool bnumber)
{
	while(1)
	{
		...
		else
		{
			...
			while(0 <cmd[i] && (!isalnum(cmd[i]) && cmd[i] != '{' && cmd[i] != '}') && i < len)
			{
				i++;
			}
			if(i > 0 && i < len)
			...
			if(0 <cmd[0] && cmd[0] == '{' || cmd[0] == '}' || bnumber || isalpha(cmd[0]))
				return true;
			else 
				return false;
		}
		...
		if(pState)
		{
			if(pState->m_FromType == en_from_skill || pState->m_FromType == en_from_specialbuff)		// 스킬에서 들어온 이펙트만 req packet을 보낸다. 
			{
				if(((g_LocalSystemTime - pState->m_dwStateStartTime) > pState->m_usLeftTimeSec*1000) && !pState->m_bDeleteReqPacket)
				{
					...
					_XDWINPRINT("[CharacterStateList] Spell Delete Request Send ==== [%d/%d]", g_LocalSystemTime - pState->m_dwStateStartTime, pState->m_usLeftTimeSec*1000);
				}
			}
		}
	}
			...
			case 'M' :
				{
					if(buffer[5] == 'O')
					{
						if(buffer[6] == 'V')
						{
							// move lock
							int movelock = 0;
							sscanf(buffer, "%s %d", arg, &movelock);
							g_CharacterConditionTable[index-1].movelock = movelock;
						}
						else if(buffer[6] == 'D')
						{
							// mode lock
							int modelock = 0;
							sscanf(buffer, "%s %d", arg, &modelock);
							g_CharacterConditionTable[index-1].modelock = modelock;
						}
					}
					else if(buffer[5] == 'E')
					{
						// meditation lock
						int medlock = 0;
						sscanf(buffer, "%s %d", arg, &medlock);
						g_CharacterConditionTable[index-1].medlock = medlock;
					}
				}
				break;
			case 'T' :
				{
					// trade lock
					int tradelock = 0;
					sscanf(buffer, "%s %d", arg, &tradelock);
					g_CharacterConditionTable[index-1].tradelock = tradelock;
				}
				break;
			case 'B' :
				{
					// battle lock
					int battlelock = 0;
					sscanf(buffer, "%s %d", arg, &battlelock);
					g_CharacterConditionTable[index-1].battlelock = battlelock;
				}
				break;
			case 'S' :
				{
					if(buffer[5] == 'P')
					{
						if(buffer[7] == 'L')
						{
							// spell lock
							int spelllock = 0;
							sscanf(buffer, "%s %d", arg, &spelllock);
							g_CharacterConditionTable[index-1].spelllock = spelllock;
						}
						else if(buffer[7] == 'E')
						{
							// speed lock
							int speedlock = 0;
							sscanf(buffer, "%s %d", arg, &speedlock);
							g_CharacterConditionTable[index-1].speedlock = speedlock;
						}
					}
					else if(buffer[5] == 'K')
					{
						// skill lock
						int skilllock = 0;
						sscanf(buffer, "%s %d", arg, &skilllock);
						g_CharacterConditionTable[index-1].skilllock = skilllock;
					}
					else if(buffer[5] == 'O')
					{
						// sound
			...
			case 'C' :
				{
					...
					else if(buffer[5] == 'H')
					{
						// chatting lock
						int chattinglock = 0;
						sscanf(buffer, "%s %d", arg, &chattinglock);
						g_CharacterConditionTable[index-1].chattinglock = chattinglock;
					}
				}
				break;
			case 'P' :
				{
					// party lock
					int partylock = 0;
					sscanf(buffer, "%s d", arg, &partylock);
					g_CharacterConditionTable[index-1].partylock = partylock;
				}
				break;
			}
#endif
...
void _XCharacterStateList::ApplyFinalValue(void)
{
	...
	{
		if( g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cSecond != _XGI_SC_WEAPON_SPECIAL )
		{
			if( _XGI_FC_WEAPON == GetEqualItemType(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType) )
			{
				...
				if( WeaponItem )
				{
					...
					inchantminaddpoint = _XGameItem::GetInchantPlusPoint( g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cSecond, 
						item_min_damage, g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].Get_m_ucStrength() );				
					inchantmaxaddpoint = _XGameItem::GetInchantPlusPoint( g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cSecond, 
	...
	{
		if( _XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_ADAE].m_cType) )
		{
			_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_ADAE].m_cType);
			if( ClothesItem )
	...
	int pos = _XINVENTORY_SLOTTYPE_AMOR ; for( ; pos <= _XINVENTORY_SLOTTYPE_GAP_LOWER; ++pos )
	{
		if(g_pLocalUser->m_UserItemList[pos].Get_m_sID() > 0)
		{
			if( _XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[pos].m_cType) )
			{
				_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[pos].m_cType);
				if( ClothesItem )
	...
	{
		if( _XGI_FC_CLOTHES == GetEqualItemType(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_CAP].m_cType) )
		{
			_XGI_ClothesItem_Property* ClothesItem = (_XGI_ClothesItem_Property*)GetFirstTypeItemPorperty(g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_CAP].m_cType);
			if( ClothesItem )
...
void _XCharacterStateList::GetBaseBonusFromItem(void)
{
	...
	_XGI_FirstCategory type = (_XGI_FirstCategory)g_pLocalUser->m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType;
	...
	int pos = _XINVENTORY_SLOTTYPE_AMOR ; for( ; pos <= _XINVENTORY_SLOTTYPE_MASK; ++pos )
	{
		...
		type = (_XGI_FirstCategory)g_pLocalUser->m_UserItemList[pos].m_cType;
