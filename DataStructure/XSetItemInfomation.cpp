...
int XSetItemInfomation::SearchGroupItem(int nGroup)
{
	...
	{
		int i = 0 ; for(; i < _XINVENTORY_SLOTTYPE_POCKETSTART; i++)
		{
			if( i == 11 ) // i 가 11번일때.. W 무기 밑에 있을 때.. 이땐 입고있는 걸로 생각 안한다
				continue;

			if( g_pLocalUser->m_UserItemList[i].m_cType == (*vtiter)->nType &&
...
int XSetItemInfomation::SetGroupSkillString( int nGroup, int nAccouter, _ITEMTOOLTIP_INFO& TooltipInfo )
{
	map<int, SetInfo*>::iterator iter;
	iter = m_mpData.find( nGroup );
	...
	for(; Detailiter != pSetInfo->ltItemInfo.end(); ++Detailiter)
	{
		...
		for( Skilliter = (*Detailiter)->ltSkillInfo.begin(); Skilliter != (*Detailiter)->ltSkillInfo.end(); ++Skilliter )
		{
			...
			{
				sprintf( TooltipInfo.SetItemSkill_Data_Buf[m_nSkillStringCount], 
...
int XSetItemInfomation::SetGroupItemString( int nGroup, _ITEMTOOLTIP_INFO& TooltipInfo )
{
	...
	for(; vtiter != pSetInfo->vtItem.end(); ++vtiter)
	{
		...
		case _XGI_FC_WEAPON3:
			{
				...
				{
					sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s %s", _XGameItem::GetItemNameAddtoString((*vtiter)->nType,(*vtiter)->nID), tempWeaponItem[(*vtiter)->nID].cName );
				}
				else
				{
					if((*vtiter)->nType == _XGI_FC_WEAPON2)
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", g_WeaponItemProperty2[(*vtiter)->nID].cName );
					else if((*vtiter)->nType == _XGI_FC_WEAPON3)
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", g_WeaponItemProperty3[(*vtiter)->nID].cName );
					else
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", tempWeaponItem[(*vtiter)->nID].cName );
				}
		...
		case _XGI_FC_CLOTHES3:
			{
				...
				{
					sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s %s", _XGameItem::GetItemNameAddtoString((*vtiter)->nType,(*vtiter)->nID), tempClothesItem[(*vtiter)->nID].cName);
				}
				else
				{
					if((*vtiter)->nType == _XGI_FC_CLOTHES2)
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", g_ClothesItemProperty2[(*vtiter)->nID].cName  );
					else if((*vtiter)->nType == _XGI_FC_CLOTHES3)
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", g_ClothesItemProperty3[(*vtiter)->nID].cName  );
					else
						sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", tempClothesItem[(*vtiter)->nID].cName );
				}		
				m_nSetItemStringCount++;
			}
			break;
		case _XGI_FC_ACCESSORY :
			{
				sprintf( TooltipInfo.SetItem_Data_Buf[m_nSetItemStringCount], "%s", g_AccessoryItemProperty[(*vtiter)->nID].cName );
				m_nSetItemStringCount++;
			}
			break;
		default :
...
void XSetItemInfomation::DrawToolTip( int nGroup, int DrawX, int DrawY, _ITEMTOOLTIP_INFO& TooltipInfo )
{
	...
	//이중 iter -_-ㅋㅋㅋㅋ
	int i = 0 ; for(; vtiter != pSetInfo->vtItem.end(); i++, ++vtiter)
	{
...
void XSetItemInfomation::GroupItemSkillUpdate()
{
	...
	int i = 0 ; for(; i < _XINVENTORY_SLOTTYPE_POCKETSTART; i++)
	{
		...
		int nGroup = _XGameItem::GetSetItem(g_pLocalUser->m_UserItemList[i].m_cType, g_pLocalUser->m_UserItemList[i].Get_m_sID());
		if( nGroup == 0 )
			continue; 
