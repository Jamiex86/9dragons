...
void _XLocalUser::ResetInventoryInfo( void )
{
	...
	int i = 0 ; for(; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; ++i )
	{
...
_XUserItem*	_XLocalUser::GetUserItem( int firstcategory, int id )
{
	int i = 0 ; for(; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; ++i )
	{
		if( m_UserItemList[i].m_cType == firstcategory && 
...
_XUserItem* _XLocalUser::GetUserItemFromSecondType(int cType, int sType)
{
	int i = 0 ; for(; i < _XINVENTORY_SLOTTYPE_TOTALCOUNT; ++i )
	{
		if(m_UserItemList[i].m_cType == cType)
		{
			switch(cType)
			{
			case _XGI_FC_POTION :
...
{
	int i = 0 ; for( ; i < _XDEF_MAX_LEARNMASTERYSKILL ; ++i)
	{
...
void _XLocalUser::RequestBattleMode(_XBATTLEMODETYPE mode)
{
		...
		|| m_PersonalStoreState == _XUSER_PERSONALSTORE_OWNER_MODIFY)
	{
		...
		sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_LOCALUSER_1571));// "노점이 열린 채로는 상태변경이 불가능 합니다."
...
#ifdef _XTS_ITEM_OPTIMIZATION
	if( _XGI_FC_WEAPON == GetEqualItemType(m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType) )
	{
		_XGI_WeaponItem_Property* WeaponItem = (_XGI_WeaponItem_Property*)GetFirstTypeItemPorperty(m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType);
		if( WeaponItem )
		{
...
#else
	if(m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType == _XGI_FC_WEAPON)
		reqLevel = g_WeaponItemProperty[m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_sID].sReqLevel;
	else if(m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType == _XGI_FC_WEAPON2)
		reqLevel = g_WeaponItemProperty2[m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_sID].sReqLevel;
	else if(m_UserItemList[_XINVENTORY_SLOTTYPE_WEAPON_1].m_cType == _XGI_FC_WEAPON3)
...
//	FLOAT separatedmovelength = (_XDEF_CHARACTERMOVESPEED_RUN * 30.0f);
	...
	for( int repeat = 0; repeat < repeatcollidechecktime; ++repeat )
	{
		...
		{
			int i = 0 ; for(; i < g_LodTerrain.m_FunctionalObjectManager.m_countFunctionalObject; ++i )
			{
...
void _XLocalUser::DrawMovePath( void )
{
	...
	int i = 0 ; for(; i < m_PathNodeCount -1; ++i )
	{						
		line[0].v = curpos;
	...
	case _XGI_FC_WEAPON :
		{
			if(pItem->m_cSecond == _XGI_SC_WEAPON_HIDDEN)
			{
				...
				int i = _XINVENTORY_SLOTTYPE_POCKETSTART ; for( ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; ++i)
				{
					if((m_UserItemList[i].m_cType == pItem->m_cType) && (m_UserItemList[i].Get_m_sID() == pItem->m_sID))
					{
						if(m_UserItemList[i].m_ucCount < 255)
							return i;
					}
					else if( m_UserItemList[i].Get_m_sID() == 0 )
						return i;
				}
	...
	case _XGI_FC_WEAPON3 :
		{
			if(pItem->m_cSecond == _XGI_SC_WEAPON_HIDDEN)
			{
				...
				int i = _XINVENTORY_SLOTTYPE_POCKETSTART ; for( ; i < _XINVENTORY_SLOTTYPE_FIRST_BACKPACKSTART+g_pLocalUser->m_cBackPackExtend ; ++i)
				{
					if((m_UserItemList[i].m_cType == pItem->m_cType) && (m_UserItemList[i].m_sID == pItem->m_sID))
					{
						if(m_UserItemList[i].m_ucCount < 255)
							return i;
					}
					else if( m_UserItemList[i].m_sID == 0 )
						return i;
				}
...
void _XLocalUser::UseItem(int invennumber)
{
	...
	switch(m_UserItemList[invennumber].m_cType)
	{
	case _XGI_FC_POTION :
		{
			// 예외 사항 처리 -----------------------------------------------┓
			...
			case _XGI_SC_POTION_EVENT :
				{
					if(g_pState_Window->FindUsingItemFromSecondType(_XST_ITEM, m_UserItemList[invennumber].m_cType, _XGI_SC_POTION_EVENT))
						errcode = 1;
				}
				break;
			...
			case _XGI_SC_POTION_COOLDOWN :
				{
					if(g_pState_Window->FindUsingItemFromSecondType(_XST_ITEM, m_UserItemList[invennumber].m_cType, _XGI_SC_POTION_COOLDOWN))
						errcode = 1;
				}
				break;
			case _XGI_SC_POTION_ADDEXP :
				{
					...
					{
						// 증가 아이템 사용중
						sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2959));	//아이템 사용시, 기존 사용중이던 증가 아이템 서비스는 사라집니다. 사용하시겠습니까?
					}
					else
					{
						if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 1)
						{
							sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2960));	//경험치 증가 아이템을 사용하시겠습니까? 선택한 아이템은 사용 시 삭제됩니다.
						}
						else if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 2)
						{
							sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2961));	//숙련도 증가 아이템을 사용하시겠습니까? 선택한 아이템은 사용 시 삭제됩니다.
						}
						else if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 3)
						{
							sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2962));	//경험치/숙련도 증가 아이템을 사용하시겠습니까? 선택한 아이템은 사용 시 삭제됩니다.
						}
						else if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 4)
						{
							sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2971));	//구룡천하를 사용하시겠습니까? 선택한 아이템은 사용 시 삭제됩니다.
						}
						else if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 5)
						{
							sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_3415));	//구룡천하(가칭)를 사용하시겠습니까? 선택한 아이템은 사용 시 삭제됩니다.
						}
			...
			case _XGI_SC_POTION_RESETPOINT :
				{
					...
					if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 1)
					{
						sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2963));	//오기 초기화 아이템을 사용하시겠습니까? 사용시 모든 오기가 초기화 됩니다.
					}
					else if(g_PotionItemProperty[m_UserItemList[invennumber].Get_m_sID()].sSimgi == 2)
					{
						sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2964)); //오의 초기화 아이템을 사용하시겠습니까? 사용시 모든 오의가 초기화 됩니다.
					}
					else
					{
						errcode = 1;
			...
			case _XGI_SC_POTION_SHOUT :
				{
					...
					if(g_pState_Window->FindUsingItemFromSecondType(_XST_SHOUT, m_UserItemList[invennumber].m_cType, _XGI_SC_POTION_SHOUT))
					{
					...
					if(errcode == 0)
					{
						...
						sprintf(msgbuffer, _XGETINTERFACETEXT(ID_STRING_NEW_2958), time); // 사자후를 사용하시겠습니까? 사용시 %d분간 외치기가 가능합니다.
	...
	case 82:// Key LPS
		{
			if(GetMotionClass() == _XACTION_LPS)
			{
				if(m_ModelDescriptor.m_FrameResult & _XDEF_ACI_LASTFRAME)
				{
					...
					{
						if(g_pLocalUser->m_UserItemList[i].m_cType == g_MobAttribTable[pMob->m_MobType].key_type )
						{
					...
					{												
						TCHAR messagestring[256];
						sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_LOCALUSER_2326), 
