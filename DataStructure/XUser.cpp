...
{	
	...
	memset(m_PersonalStoreName, 0, sizeof(TCHAR)*37);
	
	// 비무 홍보 관련
	m_bMatchPRMode					= FALSE;
	memset(m_stringMatchPR, 0, sizeof(TCHAR)*81);	
...
void _XUser::DestroyData()
{
	...
	memset(&m_CharacterInfo, 0, sizeof(_XCHARACTER_INFO));
...
{
	memset( g_UserIntersectionPoly, 0, sizeof( D3DXVECTOR3 ) * 8 );
...
//			m_BattleMode = _XBATTLE_BATTLE;
	...
	if( g_EnableUserPacketFiltering )
	{
		if( strcmpi( m_CharacterInfo.charactername, g_PacketFilteringTargetName ) == 0 )
		{
			...
			sprintf( _messagestring, "%s User change motion : AnimIndex:%d Action:%d X:%4.2f Y:%4.2f", 
...
//								g_pLocalUser->SetBlowMotion();
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[i] == 0)
										sprintf(messagestring, "Active HIT damage[%d]", m_AttackResult.sDamage[i]);
									else
										sprintf(messagestring, "Active 상극일반 damage[%d]", m_AttackResult.sDamage[i]);	// add string
...
//								g_pLocalUser->SetBlowMotion();
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[i] == 1)
										sprintf(messagestring, "Active CRITICAL damage[%d]", m_AttackResult.sDamage[i]);
									else
										sprintf(messagestring, "Active 상극크리 damage [%d]", m_AttackResult.sDamage[i]);	// add string
						...
						case 2 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Active MISS");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 3 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Active BLOCK");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 4 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Active SHOCK damage [%d]", m_AttackResult.sDamage[i]);
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						...
						case 5 :
							{
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 0)
										sprintf(messagestring, "Charge HIT damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Charge 상극일반 damage[%d]", m_AttackResult.sDamage[0]);	// add string
...
//								g_pLocalUser->SetBlowMotion();
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 1)
										sprintf(messagestring, "Charge CRITICAL damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Charge 상극크리 damage[%d]", m_AttackResult.sDamage[0]);	// add string
						...
						case 2 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Charge MISS");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 3 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Charge BLOCK");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 4 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Charge SHOCK damage [%d]", m_AttackResult.sDamage[0]);
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
							...
							case 5 :
								{
									...
									if(g_ViewBattleDetail)
									{
										...
										if(m_AttackResult.usAtkInfo[0] == 0)
											sprintf(messagestring, "Rounding HIT damage[%d]", m_AttackResult.sDamage[0]);
										else
											sprintf(messagestring, "Rounding 상극일반 damage[%d]", m_AttackResult.sDamage[0]);	// add string
							...
							case 6 :
								{
									...
									if(g_ViewBattleDetail)
									{
										...
										if(m_AttackResult.usAtkInfo[0] == 1)
											sprintf(messagestring, "Rounding CRITICAL damage[%d]", m_AttackResult.sDamage[0]);
										else
											sprintf(messagestring, "Rounding 상극크리 damage[%d]", m_AttackResult.sDamage[0]);	// add string
							...
							case 2 :
								{
									...
									{
										TCHAR messagestring[128];
										sprintf(messagestring, "Rounding MISS");
										g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
									}
								}
								break;
							case 3 :
								{
									...
									{
										TCHAR messagestring[128];
										sprintf(messagestring, "Rounding BLOCK");
										g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
									}
								}
								break;
							case 4 :
								{
									...
									{
										TCHAR messagestring[128];
										sprintf(messagestring, "Rounding SHOCK damage [%d]", m_AttackResult.sDamage[0]);
										g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
									}
								}
								break;
						...
						case 5 :
							{
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 0)
										sprintf(messagestring, "Finish HIT damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Finish 상극일반 damage[%d]", m_AttackResult.sDamage[0]);	// add string
...
//								g_pLocalUser->SetBlowMotion();
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 1)
										sprintf(messagestring, "Finish Critical damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Finish 상극크리 damage[%d]", m_AttackResult.sDamage[0]);	// add string
						...
						case 2 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Finish MISS");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 3 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Finish BLOCK");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 4 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Finish SHOCK damage [%d]", m_AttackResult.sDamage[0]);
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						...
						case 5 :
							{
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 0)
										sprintf(messagestring, "Smashing HIT damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Smashing 상극일반 damage[%d]", m_AttackResult.sDamage[0]);	// add string
						...
						case 6 :
							{
								...
								if(g_ViewBattleDetail)
								{
									...
									if(m_AttackResult.usAtkInfo[0] == 1)
										sprintf(messagestring, "Smashing CRITICAL damage[%d]", m_AttackResult.sDamage[0]);
									else
										sprintf(messagestring, "Smashing 상극크리 damage[%d]", m_AttackResult.sDamage[0]);	// add string
						...
						case 2 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Smashing MISS");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 3 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Smashing BLOCK");
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
						case 4 :
							{
								...
								{
									TCHAR messagestring[128];
									sprintf(messagestring, "Smashing SHOCK damage [%d]", m_AttackResult.sDamage[0]);
									g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGECOLOR_WHISPER);
								}
							}
							break;
		...
		if(this->m_AttackResult.display[i])
		{
			if(m_AttackResult.cBattleResult == en_battle_result_binsa_or_die || m_AttackResult.cBattleResult == en_battle_result_honsu_or_dead)
			{
				...
				if(pMob)
				{
					...
					memset(m_AttackResult.display, 0, sizeof(BOOL)*3);
					return;
				}
			}
...
void _XUserList::RemoveSortBuffer( _XUser* pUser )
{
	...
	int i = 0 ; for(; i < m_RenderUserZSortListCurrentLength; ++i )
	{
...
#define _XDEF_USER_KILLTIME				15000
				...
				{	
					if( g_pLocalUser->m_PersonalStoreState == _XUSER_PERSONALSTORE_OWNER_OPEN )
					{
						...
						{	
							...
							{											
								...
								sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2893), g_pLocalUser->m_CharacterInfo.charactername );//"[%s님의 노점]"
								
								TCHAR storenamestring[64];
								memset( storenamestring, 0, sizeof(TCHAR)*64 );	
								...
								{
									strcpy( storenamestring, g_pLocalUser->m_PersonalStoreName );
								}
					...
					else if( g_pLocalUser->m_bMatchPRMode ) // 비무 홍보일때 
					{
						...
						{	
							...
							{																			
								TCHAR messagestring[64];
								memset( messagestring, 0, sizeof(TCHAR)*64 );	
								sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2892), g_pLocalUser->m_CharacterInfo.charactername );//"[%s님의 비무홍보]"
					...
					else
					{
						...
						if(g_pLocalUser->m_Lock.notdrawusername == 0)
						{
							...
							{
								TCHAR namebuffer[64];
								sprintf( namebuffer, "GM : %s", g_pLocalUser->m_CharacterInfo.charactername );
							...
							else
							{
...
#endif
									{
										int j = 0 ; for( j = 0; j < g_MaxNickNameCount; ++j)
										{
								...
								if( g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_iIndex > 0 )
								{
									if( strlen(g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName) )	// 영웅단 표시
									{
										...
										TCHAR orgName[256];
										memset(orgName, 0, sizeof(TCHAR)*256 );
										if( g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cClass == _XUSERALIENCEINFO::_UNIT_CALSS_ALIENCE_CAPTAIN_UNIT_ )
										{
											
											sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3172), g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s회 회주]"
										...
										{
											sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3173), g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s회 부회주]"
											g_XBaseFont->SetColor(0xFF8BC7F6);
											g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
										}
										else
										{
											sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3174), g_pLocalUser->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s]"
											g_XBaseFont->SetColor(0xFF8BC7F6);
											g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
										}
										g_XBaseFont->Flush();
								...
								else //영웅단 동맹에 가입되지 않았으므로 영웅단을 표시한다.
								{
									if( g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_iIndex > 0 )
									{
										if( strlen(g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName) )	// 영웅단 표시
										{
											...
											{
												sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_2894), g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName ); //"[%s단 단주]"
												g_XBaseFont->SetColor(0xFF8BC7F6);
												g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
											}
											else
											{
												sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_2895), g_pLocalUser->m_CharacterInfo.m_OrganizationInfo.m_cName );//"[%s단]"
												g_XBaseFont->SetColor(0xFF8BC7F6);
												g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
											}
											g_XBaseFont->Flush();
									...
									if( _user->m_PersonalStoreState == _XUSER_PERSONALSTORE_OWNER_OPEN )
									{
										...
										{	
											...
											{											
												...
												TCHAR messagestring[128];
												memset( messagestring, 0, sizeof(TCHAR)*128 );	
												sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2893), _user->m_CharacterInfo.charactername );//"[%s님의 노점]"
												
												TCHAR storenamestring[128];
												memset( storenamestring, 0, sizeof(TCHAR)*128 );	
												...
												{
													strcpy( storenamestring, _user->m_PersonalStoreName );
												}
									...
									else if( _user->m_bMatchPRMode ) // 비무 홍보일때 
									{
										...
										{	
											...
											{																			
												TCHAR messagestring[128];
												memset( messagestring, 0, sizeof(TCHAR)*128 );	
												sprintf( messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2892), _user->m_CharacterInfo.charactername );//"[%s님의 비무홍보]"
									...
									else
									{
										...
										if(g_pLocalUser->m_Lock.notdrawusername == 0)
										{
											...
											if( _user->m_CharacterInfo.Get_gm_level() > 2 ) 
											{
												...
												sprintf( namebuffer, "GM : %s", _user->m_CharacterInfo.charactername );
												g_OperaterTooltip.SetToolTipStringEx_Noresize( tooltipxpos, tooltipypos, namebuffer, 0xFFFFE500 );
												g_OperaterTooltip.Draw();
											}
											else
											{
												...
												// 별호 보여주기
#ifdef _XTS_MASKSYSTEM
												if( !_user->m_bFaceMaskMode && _user->m_CharacterInfo.m_NickName[0] != 0 )
#else
												if( _user->m_CharacterInfo.m_NickName[0] != 0 )
#endif
												{													
													DrawPos -= 20;
...
#ifdef _XTS_MASKSYSTEM
												if( !_user->m_bFaceMaskMode && _user->m_CharacterInfo.m_AlienceInfo.m_iIndex > 0 )
#else
												if( _user->m_CharacterInfo.m_AlienceInfo.m_iIndex > 0 )
#endif//#ifdef _XTS_MASKSYSTEM
												{
													if( _user->m_CharacterInfo.m_AlienceInfo.m_cName[0] != 0 )	//영웅회 이름을 가지고 있는 경우
													{
														...
														{
															sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3172), _user->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s회 회주]"
															g_XBaseFont->SetColor(0xFF8BC7F6);
															g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
														}
														else if( _user->m_CharacterInfo.m_AlienceInfo.m_cClass == _XUSERALIENCEINFO::_UNIT_CALSS_ALIENCE_SUBCAPTAIN_UNIT_ )
														{
															sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3173), _user->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s회 부회주]"
															g_XBaseFont->SetColor(0xFF8BC7F6);
															g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
														}
														else
														{
															sprintf( orgName, _XGETINTERFACETEXT(ID_STRING_NEW_3174), _user->m_CharacterInfo.m_AlienceInfo.m_cName ); //"[%s]"
															g_XBaseFont->SetColor(0xFF8BC7F6);
															g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
														}
														g_XBaseFont->Flush();
												...
												{
#ifdef _XTS_MASKSYSTEM
													if( !_user->m_bFaceMaskMode && _user->m_CharacterInfo.m_OrganizationInfo.m_iIndex > 0 )
#else
														if( _user->m_CharacterInfo.m_OrganizationInfo.m_iIndex > 0 )
#endif//#ifdef _XTS_MASKSYSTEM
														{
															if( strlen(_user->m_CharacterInfo.m_OrganizationInfo.m_cName) )	// 영웅단 표시
															{
																...
																if( _user->m_CharacterInfo.m_OrganizationInfo.m_cClass == _XUSERORGANIZATIONINFO::_UNIT_CALSS_CAPTAIN_UNIT_ )
																{
																	sprintf( orgName,  _XGETINTERFACETEXT(ID_STRING_NEW_2894), _user->m_CharacterInfo.m_OrganizationInfo.m_cName );//"[%s단 단주]"
																	g_XBaseFont->SetColor(0xFF8BC7F6);
																	g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
																}
																else
																{
																	sprintf( orgName,  _XGETINTERFACETEXT(ID_STRING_NEW_2895), _user->m_CharacterInfo.m_OrganizationInfo.m_cName ); //"[%s단]"
																	g_XBaseFont->SetColor(0xFF8BC7F6);
																	g_XBaseFont->PutsAlign(tooltipxpos, tooltipypos+DrawPos, _XFONT_ALIGNTYPE_CENTER, orgName );
																}
																g_XBaseFont->Flush();
