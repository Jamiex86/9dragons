							...
							{							
								...
								case ID_OBJECT_CONDITION_CHECKCLAN :				// 플레이어_문파비교
									{							
										...
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2909) , g_JoinGroupName[ currentcondition->m_TDC.m_compareclass1 ] );//"[%s]이(가) 아닙니다."
								...
								case ID_OBJECT_CONDITION_CHECKITEM :				// 플레이어_아이템검사
									{
										TCHAR messagestring[256];
										sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2910), _XGameItem::GetItemName(currentcondition->m_TDC.m_compareclass1, currentcondition->m_TDC.m_numericvalue1));//[%s]이(가) 없습니다.
										g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);	
									}
									break;
								...
								case ID_OBJECT_CONDITION_CHECKGENDER :				// 플레이어_성별검사
									{
										...
										{
											TCHAR messagestring[256];
											sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2911));//"남자가 아닙니다."
											g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
										}
										else
										{
											TCHAR messagestring[256];
											sprintf(messagestring, _XGETINTERFACETEXT(ID_STRING_NEW_2912));//"여자가 아닙니다."
											g_NetworkKernel.InsertChatString(messagestring, _XDEF_CHATMESSAGEMODE_GLOBALNOTICE + _XDEF_CHATMESSAGECOLOR_SYSTEMLOSS);
										}	
									}
									break;
...
BOOL _XFunctionalObjectManager::LoadFunctionalObjectList( FILE* Fileptr, LPSTR Filename )
{
	...
	if( m_countFunctionalObject != 0 )
	{
		...
		int i = 0 ; for(; i < m_countFunctionalObject; ++i )
		{
