...
void _XMatchEventManager::Draw()
{
	if(m_bDrawReadyMessage)
	{
		...
		{	
			TCHAR messagestr1[256];
			memset(messagestr1, 0, sizeof(messagestr1));
			sprintf(messagestr1, _XGETINTERFACETEXT(ID_STRING_NEW_3729), m_nTextArg);//%d분 후에 비무대회가 시작됩니다.

			TCHAR messagestr2[256];
			...
			{
				sprintf(messagestr2, _XGETINTERFACETEXT(ID_STRING_NEW_3740), g_LevelName[fromlevel]);//%s 성취단계  분들은 합비 비무장으로 모여주시길  바랍니다.
			}
			else
			{
				sprintf(messagestr2, _XGETINTERFACETEXT(ID_STRING_NEW_3741), g_LevelName[fromlevel], g_LevelName[tolevel]);	//%s-%s 성취단계 분들은 합비 비무장으로 모여주시길 바랍니다.
			}
...
{
	if(g_pLocalUser->m_UserItemList[invennumber].m_cType != _XGI_FC_POTION)
		return TRUE;
	...
	if(iter_match != m_mapMatchTypeList.end())
	{
		...
		if(pMatchStr)
		{
			...
			{
				_s_Item_Str* pItem = *iter_item;
				if(g_pLocalUser->m_UserItemList[invennumber].m_cType == pItem->cType && 
