...
BOOL _XSkillItem::LoadSkillProperty( void )
{
	...
	short		skillid = 0;
	int i = 0 ; for( ; i < _XDEF_MAXSKILLCOUNT ; ++i)
	{
		// index
...
BOOL _XSkillItem::LoadSkillDetailProperty(FILE* fileptr, _XSKILLINFOSTRUCTURE* skillinfo)
{
	int i = 0 ; for( ; i < 12 ; ++i)
	{
		...
		if(skillinfo->detailInfo[i].effectCount > 0)
		{
			...
			int j = 0 ; for( j = 0 ; j < skillinfo->detailInfo[i].effectCount ; ++j)
			{
		...
		if(skillinfo->detailInfo[i].additiveCount > 0)
		{
			...
			int j = 0 ; for( j = 0 ; j < skillinfo->detailInfo[i].additiveCount ; ++j)
			{
...
BOOL _XSkillItem::LoadSkillPackage(void)
{
	...
	int i = 0 ; for( ; i < packagecount ; ++i)
	{
		...
		int j = 0 ; for( j = 0 ; j < g_SkillPackage[i].nSkillCount ; ++j)
		{
