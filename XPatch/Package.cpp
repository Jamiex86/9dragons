...
{	
	...
	CString tMessagestring = szBuff;
	tMessagestring.Replace( _T('\n'), _T('\0') );
	tMessagestring.Replace( _T('\r'), _T('\0') );
	
	((CXPatchDlg*)AfxGetMainWnd())->SetProgressString(tMessagestring);
}
...
BOOL CPackage::Open( LPCTSTR filename )
{
	...
	_XLog( "Package Message : Package opened [%s]\n", filename );
...
{	
	...
	_XLog( "Package Message : Export file [%d:%s]\n", file_id, filename );
	...
	_XLog( "Package Message : Check folder and create [%d:%s]\n", file_id, filename );
	...
	_XLog( "Package Message : Exporting [%d:%s]\n", file_id, filename );
	...
	_XLog( "Package Message : Check old file [%d:%s]\n", file_id, filename );
	...
	_XLog( "Package Message : Rename new file [%s->%s]\n", newfilename, filename );
...
BOOL CPackage::ExportFile( int file_id, FILE* pExportFile )
{
	...
	_XLog( "Package Message : Search Export file [%d]\n", file_id );
	...
	_XLog( "Package Message : Exporting [%d]\n", file_id );
	...
	_XLog( "Package Message : Read org file [%d]\n", file_id );
	...
	_XLog( "Package Message : write new file [%d]\n", file_id );
...
BOOL CPackage::Patch( LPCTSTR packagefilename, LPCTSTR patchinfofilename )
{
	...
	_XLog( "Package Message : Open patch information file [%s:%s]\n", packagefilename, patchinfofilename );
	...
	CString currentPathString = registrybuffer;
	...
	while( !feof( pPatchinfoFile ) )
	{
		...
		if( strlen(parsingString) && parsingString[0] != '\n' && parsingString[0] != ';' )
		{
			...
			{				
				...
				_XLog( tMessagestring.GetBuffer(256) );
...
BOOL CPackage::PatchXPToXP( LPCTSTR packagefilename )
{
	...
	CString tMessagestring;
	tMessagestring.Format( "팩키지파일을 재구성합니다.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  tMessagestring.GetBuffer(256) );
	...
	tMessagestring.Format( "패키지 재구성정보를 계산중입니다.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  tMessagestring.GetBuffer(256) );
	...
	tMessagestring.Format( "패키지를 재구성합니다.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  tMessagestring.GetBuffer(256) );
	...
	tMessagestring.Format( "패키지 재구성에 사용된 임시파일을 삭제합니다.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  tMessagestring.GetBuffer(256) );
...
//#define _TEST_EXPORT_
	...
	tMessagestring.Format( "패키지 파일이 재구성 되었습니다.\n" );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog(  tMessagestring.GetBuffer(256) );
