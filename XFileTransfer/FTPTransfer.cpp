...
#ifdef TEST_VERSION
	m_sServer = _T("221.147.34.23");		// test
#else
	_XRegReadString(ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_LOGFTPSERVERADDRESS, _T("221.147.34.23"), registrybuffer, _REGSTRING_MAXLENGTH, TRUE);
	
	m_sServer = registrybuffer;	
#endif

	m_bUsePassive = _XRegReadInt(ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_PASV, 0, TRUE);
	m_bUseProxy = _XRegReadInt(ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_USEPROXY, 0, TRUE);
	m_bUsePreconfig = _XRegReadInt(ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_PRECONFIGUREDSETTINGS, 1, TRUE);

	memset(registrybuffer, 0, _REGSTRING_MAXLENGTH);
	if(_XRegReadString(ID_REGSTR_MASTERKEY, ID_REGSTR_INDY21KEY, ID_REGSTR_PROXYSERVERADDRESS, "", registrybuffer, _REGSTRING_MAXLENGTH, TRUE))
...
void CFTPTransfer::SetServerPath( CString senderid )
{
	...
	if( senderid.GetLength() )
		sprintf(directoryname, "%s_%d", senderid, timeGetTime());
	else
		sprintf(directoryname, "%s_%d", "UNKNOWNUSER", timeGetTime());
